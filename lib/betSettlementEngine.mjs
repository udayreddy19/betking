...
export class BetSettlementEngine {
  ...
  async settleSingleBet({ betId, matchState, authorization = null }, correlationId = null) {
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (isBonusBet) {
        const { recordBonusWageringInTx, forfeitLockedBonusWinningsIfBonusExhausted } = await import('./promotionsEngine.mjs');
        ...
        // Last bonus stake lost → locked profit cannot finish rollover; forfeit with the bonus.
        if (outcome === 'LOST') {
          const forfeit = await forfeitLockedBonusWinningsIfBonusExhausted(client, {
            userId,
            betId,
            reason: 'bonus_principal_lost',
          });
          if (forfeit?.forfeited && walletSnapshot) {
            walletSnapshot.lockedBonusWinnings = 0;
          }
        }
      }
    ...
    // Referral play commission: 5% of cash stake → referrer freebet (idempotent per bet).
    const settledFund = String(bet.fund_source || 'cash').toLowerCase();
    if (
      result?.betId
      ...
      try {
        const { creditReferrerPlayCommission } = await import('./referralLoyaltyEngine.mjs');
        await creditReferrerPlayCommission({
          betId,
          referredUserId: userId,
          stake,
          fundSource: settledFund,
          outcome,
        });
      } catch (err) {
        logger.error('referral_play_commission_hook_failed', {
          betId,
          userId,
          error: err?.message || err,
        });
