...
export function adaptV3SnapshotToPublicContract(v3Snapshot, matchObj = {}) {
  ...
  const adaptedMarkets = (v3Snapshot.markets || [])
    .filter((m) => {
      const st = String(m?.status || 'OPEN').toUpperCase();
      // Drop suspended clutter from the public board (clients hide these anyway).
      return st === 'OPEN' || st === 'DETERMINED' || st === 'SETTLED';
    })
    .map((m) => {
  ...
  // Match Winner first so it never scrolls under locked clutter.
  adaptedMarkets.sort((a, b) => {
    const rank = (m) => (m.marketId === 'match_winner' || m.key === 'winner' ? 0 : 1);
