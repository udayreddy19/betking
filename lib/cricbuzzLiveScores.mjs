...
function buildLiveDetails(info, matchScore, matchState = 'pre', match = null) {
  ...
  const matchTeam = (innName, targetName, targetSName) => {
    if (!innName) return false;
    ...
    return a === b || (s && a === s) || (b && (a.includes(b) || b.includes(a)));
  };

  const team1Latest = latestTeamInnings(allInnings.filter((inn) => matchTeam(inn.teamName, team1Name, team1SName) || matchTeam(inn.teamSName, team1Name, team1SName)));
  const team2Latest = latestTeamInnings(allInnings.filter((inn) => matchTeam(inn.teamName, team2Name, team2SName) || matchTeam(inn.teamSName, team2Name, team2SName)));
  const firstInn = allInnings.find((inn) => inn.inningsId === 1) || allInnings[0];
  const chaseInn = allInnings.find((inn) => inn.inningsId === 2);

  const result = {
    commentary,
    runs: team1Latest?.runs ?? (matchTeam(firstInn?.teamName, team1Name, team1SName) ? firstInn?.runs : 0) ?? 0,
    wickets: team1Latest?.wickets ?? 0,
    overs: normalizeOvers(team1Latest?.overs ?? firstInn?.overs ?? 0, match),
    score2: team2Latest?.runs ?? (matchTeam(firstInn?.teamName, team2Name, team2SName) ? firstInn?.runs : 0) ?? 0,
...
function mapMatch(raw, matchType = 'League') {
  ...
  const matchFormat = info.matchFormat || info.matchType || matchType;
  const matchMeta = { league, seriesName: info.seriesName, matchFormat };
