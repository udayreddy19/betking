...
function envDefaults() {
  ...
  return {
    ...
    minDeposit: Math.max(0, Number(process.env.REFERRAL_MIN_DEPOSIT || '100') || 100),
    // Default off — deposit (and optional first bet) qualify invites; KYC stays for withdrawals.
...
export function referralLinkFromCode(code) {
  ...
  // Land on /invite so guests see the reward pitch; register still accepts ?ref=
  return `${referralBaseUrl()}/invite?ref=${encodeURIComponent(normalized)}`;
}
