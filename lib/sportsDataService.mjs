...
export async function getAllApiMatches() {
  ...
  const combined = [...liveMatches, ...srlMatches];
...
export async function getMatchesFiltered(filters = {}) {
  ...
  if (filters.status) {
    ...
    if (st === 'live') {
      result = result.filter((m) => getMatchState(m) === 'in');
    } else if (st === 'upcoming' || st === 'scheduled') {
      result = result.filter(m => getMatchState(m) === 'pre');
    } else if (st === 'completed') {
      result = result.filter(m => getMatchState(m) === 'post');
    }
  }
...
export async function getSingleMatchDetails(matchId) {
  ...
  if (!match) {
    ...
    match = {
      ...
      status: 'LIVE',
      isLive: true,
      team1: { id: `team_${parts[0] || '1'}`, name: team1Name, runs: 0, wickets: 0, balls: 0 },
      team2: { id: `team_${parts[1] || '2'}`, name: team2Name, runs: 0, wickets: 0, balls: 0 },
      liveDetails: { runs: 0, wickets: 0, overs: '0.0', score2: 0, wickets2: 0, overs2: '0.0' },
    };
  }
  ...
  return {
    ...
    score: {
      ...
      status: match.time || match.status || 'LIVE',
    },
    events: detailData?.events || [],
    commentary: detailData?.commentary || [],
    lineups: {
      team1: { name: team1Name, players: t1Players },
      team2: { name: team2Name, players: t2Players },
    },
    headToHead: detailData?.headToHead || 'NOT_AVAILABLE',
  };
}
