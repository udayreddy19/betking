...
export function generateExtendedOverMarkets(state, validation = {}, marginConfig = {}) {
  ...
  for (const targetOvers of milestones) {
    ...
    const targetBalls = targetOvers * ballsPerOver;
    markets.push(createMarketDefinition({
      marketId: `overs_0_${targetOvers}_total`,
      marketType: `OVERS_0_${targetOvers}_TOTAL`,
      category: 'overs',
      name: `${innLabel} Overs 0 to ${targetOvers} - ${battingTeam.name} Total`,
      status: currentBalls < targetBalls ? 'OPEN' : 'SUSPENDED',
      line,
      selections: [
        priceSelection({ selectionId: `sel_0${targetOvers}_over`, name: `Over ${line}`, probability: pOver, overround }),
        priceSelection({ selectionId: `sel_0${targetOvers}_under`, name: `Under ${line}`, probability: pUnder, overround }),
      ],
    }));
  }
