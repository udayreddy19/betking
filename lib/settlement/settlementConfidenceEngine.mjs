...
export function evaluateSettlementConfidence({
  ...
  const candidateTimestamp = match.providerTimestamp || match.cachedAt || match.lastUpdatedAt || match.fetchedAt || match.updatedAt;
  let ageSeconds = 0;
  let freshestTimestamp = null;

  if (candidateTimestamp) {
    const parsedTime = new Date(candidateTimestamp).getTime();
    if (Number.isFinite(parsedTime) && !Number.isNaN(parsedTime) && parsedTime > 0) {
      freshestTimestamp = new Date(parsedTime).toISOString();
      ageSeconds = Math.max(0, (Date.now() - parsedTime) / 1000);
    }
  }
  if (!freshestTimestamp && match.time) {
    const parsedTime = new Date(match.time).getTime();
    if (Number.isFinite(parsedTime) && !Number.isNaN(parsedTime) && parsedTime > 0) {
      freshestTimestamp = new Date(parsedTime).toISOString();
      ageSeconds = Math.max(0, (Date.now() - parsedTime) / 1000);
    }
