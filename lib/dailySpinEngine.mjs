...
export async function claimDailySpin(userId) {
  ...
  return withTransaction(async (client) => {
    ...
    const spinMethod = prize.type === 'freebet'
      ? 'DAILY_SPIN_FREEBET'
      : prize.type === 'bonus'
        ? 'DAILY_SPIN_BONUS'
        : 'DAILY_SPIN_XP';
    await client.query(
      `INSERT INTO transactions (transaction_id, user_id, type, method, amount, status)
       VALUES ($1, $2, 'BONUS_CLAIM', $3, $4, 'COMPLETED')`,
      [txId, userId, spinMethod, scaledValue],
    );
    ...
    if (prize.type === 'bonus') {
      ...
      // Register active 5x rollover record in user_bonuses so wagering engine tracks turnover
      const promoId = `promo_spin_${spinDate}`;
      await client.query(
        `INSERT INTO promotions (id, name, code, type, status, budget, used_budget, max_reward, min_odds, min_stake, wagering_multiplier, expires_at)
         VALUES ($1, 'Daily Spin Bonus', $2, 'BONUS', 'ACTIVE', 10000000.00, 0.00, 10000.00, 1.75, 10.00, 5.0, NOW() + INTERVAL '1 year')
         ON CONFLICT (id) DO NOTHING`,
        [promoId, `SPIN_${spinDate}`],
      );
      const ubonusId = `ubonus_spin_${userId}_${spinDate}`;
      const wageringRequired = parseFloat((scaledValue * 5.0).toFixed(2));
      await client.query(
        `INSERT INTO user_bonuses (
           id, user_id, promotion_id, bonus_amount, wagering_required, wagering_completed, status, expires_at
         )
         VALUES ($1, $2, $3, $4, $5, 0.00, 'ACTIVE', $6)
         ON CONFLICT (id) DO NOTHING`,
        [ubonusId, userId, promoId, scaledValue, wageringRequired, prizeExpiresAt],
      );
    ...
    if (prize.type === 'bonus' || prize.type === 'freebet') {
      ...
      try {
        ...
        await issueDiscreteReward({
          ...
          expiresAt: prizeExpiresAt,
          metadata: { walletMirrored: true, spinId },
          creditWallet: false,
          client,
        });
      } catch (rewardErr) {
        throw Object.assign(
          new Error(`SPIN_REWARD_SYNC_FAILED: ${rewardErr.message || 'Could not create spin voucher'}`),
          { code: 'SPIN_REWARD_SYNC_FAILED', cause: rewardErr },
        );
      }
    }
