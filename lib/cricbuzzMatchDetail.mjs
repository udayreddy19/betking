...
function mapBatter(obj) {
  if (!obj?.playerName) return null;
  const stats = parsePlayerScore(obj.playerScore);
  return {
    name: obj.playerName,
    runs: stats.runs,
    balls: stats.balls,
...
export function parseCricbuzzMatchHtml(html, matchId) {
  ...
  const batStriker = extractJsonObject(html, 'batStrikerObj');
  const batNonStriker = extractJsonObject(html, 'batNonStrikerObj');
  const bowler = extractJsonObject(html, 'bowlerObj');
  ...
  if (battingInnings) {
    ...
    liveDetails.chaseOvers = battingInnings.overs;
  ...
  if (bowlingInnings) {
    ...
    liveDetails.firstOvers = bowlingInnings.overs;
  ...
  if (bowler?.playerName) {
    liveDetails.bowler = { name: bowler.playerName };
  }
...
export function parseMetaScores(html) {
  ...
  const scoreMatch = title.match(
    /([A-Z]{2,4}W?)\s+(\d+)\/(\d+)\s*\(([\d.]+)\)\s+vs\s+([A-Z]{2,4}W?)\s+(\d+)\/(\d+)/i,
  );
  if (!scoreMatch) return null;

  const desc = html.match(/"description" content="([^"]+)"/);
  const commentary = desc?.[1]?.match(/need \d+ runs? in \d+ balls?/i)?.[0]
    || desc?.[1]?.slice(0, 120)
    || '';

  return {
    chaseTeamName: scoreMatch[1],
    chaseRuns: parseInt(scoreMatch[2], 10),
    chaseWickets: parseInt(scoreMatch[3], 10),
    chaseOvers: normalizeCricbuzzOvers(scoreMatch[4]),
    firstTeamName: scoreMatch[5],
    firstRuns: parseInt(scoreMatch[6], 10),
    firstWickets: parseInt(scoreMatch[7], 10),
...
export function parseCricbuzzMatchHtmlFast(html, matchId) {
  ...
  const bowler = html.match(/bowlerObj\\":\{[^}]*\\"playerName\\":\\"([^"\\]+)\\"/);
  ...
  if (bowler) liveDetails.bowler = { name: bowler[1] };
...
export async function fetchCricbuzzMatchDetailFast(matchId) {
  ...
  const html = await readPartialText(response, 12000, 'og:title');
  const meta = parseMetaScores(html);
  if (!meta) {
    throw new Error('Could not parse fast scores from Cricbuzz page');
  }

  return { matchId, liveDetails: meta, fetchedAt: new Date().toISOString() };
}
