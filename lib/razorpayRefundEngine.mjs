...
export async function requestDepositRefund({
  ...
  if (String(deposit.status).toUpperCase() !== 'CAPTURED'
    && String(deposit.status).toUpperCase() !== 'PARTIALLY_REFUNDED') {
    throw Object.assign(
...
export async function applyProcessedRefund({
  ...
  return withTransaction(async (client) => {
    ...
    if (balance + 0.001 < amount) {
      // Explicit ops policy: never invent wallet funds. Queue for manual finance review.
