...
export function parseCricbuzzComm(data) {
  ...
  const liveInningsId = Number(currentInnings?.inningsId || liveDetails.inningsId || 1);

  for (const entry of Object.values(commentary)) {
    const sep = entry?.overSeparator;
    if (!sep?.overNumber) continue;
    const inningsId = Number(sep.inningsId || sep.innId || sep.innings || liveInningsId || 1);
    const overNum = Number(sep.overNumber);
    const key = `${inningsId}:${overNum}`;
    const balls = parseOverSummary(sep.overSummary || '');
    const runs = sep.overRuns ?? sep.runs;
    const row = {
      overNum,
      inningsId,
      balls,
      runs: runs != null ? Number(runs) : undefined,
      isCurrent: false,
    };
    const existingIdx = overHistory.findIndex((o) => `${o.inningsId}:${o.overNum}` === key);
    if (existingIdx >= 0) {
      const prev = overHistory[existingIdx];
      if ((balls.length || 0) >= (prev.balls?.length || 0)) overHistory[existingIdx] = row;
    } else {
      seen.add(key);
      overHistory.push(row);
    }
  }

  overHistory.sort((a, b) => (Number(a.inningsId) - Number(b.inningsId)) || (a.overNum - b.overNum));
  ...
  if (liveDetails.currentOverBalls?.length) {
    ...
    const existing = overHistory.find((o) => o.overNum === currentOverNum && Number(o.inningsId || liveInningsId) === liveInningsId)
      || overHistory.find((o) => o.overNum === currentOverNum);
    if (existing) {
      if ((liveDetails.currentOverBalls?.length || 0) >= (existing.balls?.length || 0)) {
