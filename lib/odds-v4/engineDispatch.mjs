 ...
 * OddsYra / other SRL fixtures always price on V4 (toss family + resource book).
 */
...
function priceWithMode(baseMatch, mode, { isCricket = true } = {}) {
  if (!isCricket) {
    // V4 mode: other-sports book + house-protect wrap inside OddsEngineV4.
    if (mode === 'v4' || mode === 'shadow') {
      if (mode === 'shadow') {
        try { runShadowCompare(baseMatch); } catch { /* ignore */ }
      }
      const snap = generateV4({
        ...baseMatch,
...
export function priceMatchWinnerForAggregator(baseMatch, { isCricket = true } = {}) {
  return priceWithMode(baseMatch, forceV4ForSrl(baseMatch, resolveOddsEngineMode()), { isCricket });
}

export async function priceMatchWinnerForAggregatorAsync(baseMatch, opts = {}) {
  const mode = forceV4ForSrl(baseMatch, await resolveOddsEngineModeAsync());
...
function snapshotWithMode(matchObj, mode, v3Config = {}) {
  ...
  const effectiveMode = forceV4ForSrl(matchObj, mode);

  if (mode === 'shadow' || effectiveMode === 'shadow') {
    try { runShadowCompare(matchObj); } catch { /* ignore */ }
  }

  // SRL always V4; global shadow still dual-runs but serves V4 for SRL.
  const useV4 = effectiveMode === 'v4' || (mode === 'shadow' && isMatchSRL(matchObj));

  if (useV4) {
    const raw = cricket
      ? generateV4(buildCanonicalFromMatch(matchObj), v3Config)
      : generateV4({
        ...matchObj,
    ...
    if (publicSnapshot) {
      ...
      publicSnapshot.engineVersion = '4.5.0';
