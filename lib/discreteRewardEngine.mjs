...
let rewardSchemaReady = null;
export async function ensureDiscreteRewardSchema(clientOrQuery) {
  if (!rewardSchemaReady) {
    rewardSchemaReady = (async () => {
      try {
        const q = clientOrQuery?.query ? clientOrQuery.query.bind(clientOrQuery) : clientOrQuery || await dbQuery;
        await q(`
...
export async function issueDiscreteReward({
  ...
  await ensureDiscreteRewardSchema(client);
