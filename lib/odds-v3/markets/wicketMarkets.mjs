...
export function generateExtendedWicketMarkets(state, validation = {}, marginConfig = {}) {
  ...
  // 4. Team Score at Next Dismissal
  const currentRuns = battingTeam.runs || 0;
  const lineAtDismissal = Math.floor(currentRuns + 22.5) + 0.5;
  markets.push(createMarketDefinition({
    ...
    selections: [
      priceSelection({ selectionId: 'sel_dis_over', name: `Over ${lineAtDismissal}`, probability: 0.52, overround }),
      priceSelection({ selectionId: 'sel_dis_under', name: `Under ${lineAtDismissal}`, probability: 0.48, overround }),
    ],
  }));
