 ...
 */
export function applyLiveTotalOverOddsCap(overSel, underSel, overround = 0.08) {
  if (!overSel || !underSel) return { overSel, underSel };
  const maxOver = MAX_LIVE_TOTAL_OVER_ODDS;
...
export function generateTeamTotalMarket(state, validation = {}, marginConfig = DEFAULT_MARGIN_CONFIG) {
  ...
  if (state.currentInnings === 2 && state.target != null) {
    ...
    // Never offer a line the chase has already cleared
    if (currentScore >= effectiveLine) {
      return createMarketDefinition({
        marketId: 'team_total',
        marketType: 'TEAM_TOTAL',
