...
export async function lookupEventForSettlement({
  ...
  // Tier 4: PostgreSQL `matches` Table
  try {
    const dbRes = await queryRead(
      `SELECT match_id, name, home_team, away_team, status, score_home, score_away, overs_completed, created_at, updated_at
    ...
    if (dbRes.rows.length > 0) {
      ...
      if (validateMatchIdentity(dbMatch)) {
        return {
          success: true,
          match: dbMatch,
          lookupSource: 'DATABASE_MATCHES_TABLE',
          lookupResult: isFinal ? LOOKUP_RESULT_CODES.EVENT_FOUND_COMPLETED : LOOKUP_RESULT_CODES.EVENT_FOUND_LIVE,
