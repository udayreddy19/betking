...
class SupportEngine {
  ...
  async startConversation({
    ...
    } else if (['KYC', 'Betting', 'Bet Settlement', 'VIP'].includes(category)) {
      calculatedPriority = 'HIGH';
    }
    ...
    const normalizedCategory = SUPPORT_CATEGORIES.includes(category) ? category : 'General';
    let assignedTeam = 'SUPPORT_AGENT';
    if (['Withdrawal', 'Deposit', 'Payment'].includes(normalizedCategory)) {
      assignedTeam = 'PAYMENTS';
    } else if (['KYC', 'Responsible Gaming'].includes(normalizedCategory)) {
      assignedTeam = 'KYC_FRAUD';
    } else if (['Betting', 'Bet Settlement'].includes(normalizedCategory)) {
      assignedTeam = 'RISK';
    }
    ...
    try {
      ...
      await notifyUserSupportEvent({
        ...
        subject: `Ticket ${ticketNum} created`,
  ...
  async addMessage(
    ...
    if (messageType !== 'INTERNAL_NOTE') {
      ...
      try {
        const { notifyAdminSupportEvent, notifyUserSupportEvent } = await import('./supportNotify.mjs');
        const preview = String(text || '').slice(0, 160);
        const ticketLabel = conv.ticketNumber || conv.conversationNumber || conversationId;
        if (senderType === 'user') {
          await notifyAdminSupportEvent({
          ...
          const { emailUserOnAdminReply } = await import('./supportNotify.mjs');
  ...
  async ensureConversationInMemory(conversationId) {
    ...
    const status = String(loaded.status || '').toUpperCase();
    const conv = {
      ...loaded,
      messages: loaded.messages || [],
      internalNotes: loaded.internalNotes || [],
      resolutionStatus: loaded.resolutionStatus
        || (status === 'RESOLVED' || status === 'CLOSED' ? 'PROVIDED' : 'NOT_PROVIDED'),
    };
    this.conversations.set(conversationId, conv);
    return conv;
  }
  ...
  async closeTicket(conversationId, { closedBy = 'admin', resolutionCode = null, resolutionStatus = null } = {}) {
    ...
    try {
      const { notifyUserSupportEvent, emailUserOnTicketClosed } = await import('./supportNotify.mjs');
      const ticketLabel = conv.ticketNumber || conv.conversationNumber || conversationId;
      await notifyUserSupportEvent({
        userId: conv.userId,
        eventType: 'support.ticket.closed',
        subject: `Ticket ${ticketLabel} closed`,
        message: conv.resolutionSummary || 'Your support ticket was closed.',
        conversationId,
        eventId: `ticket_closed_${conversationId}_${now}`,
      });
      void emailUserOnTicketClosed({
        userId: conv.userId,
        ticketNumber: ticketLabel,
        resolutionSummary: conv.resolutionSummary,
      });
    } catch (err) {
      console.error('[SupportEngine close notify]', err.message);
    }
  ...
  /** Admin one-step close: record a resolution if needed, then mark CLOSED. */
  async adminCloseTicket(conversationId, {
    ...
    if (status === 'CLOSED') return conv;

    const alreadyResolved = String(conv.resolutionStatus || '').toUpperCase() === 'PROVIDED'
      || status === 'RESOLVED';
    const terminal = TERMINAL_NOT_REQUIRED_CODES.includes(resolutionCode);

    if (!alreadyResolved && !terminal) {
      await this.provideResolution(conversationId, {
        resolutionCode: APPROVED_RESOLUTION_CODES.includes(resolutionCode)
          ? resolutionCode
          : 'INFORMATION_PROVIDED',
        resolutionSummary,
        resolvedBy: closedBy,
  ...
  async getConversationById(conversationId, userRole = 'user') {
    try {
      const cRes = await safePgQuery(
        `SELECT c.conversation_id, c.conversation_number, c.ticket_number, c.user_id, c.subject, c.category, c.priority, c.status,
                c.assigned_agent_id, c.assigned_agent_name, c.assigned_team, c.sla_due_at, c.unread_user_count, c.unread_admin_count,
                c.resolution_status, c.resolution_code, c.resolution_summary, c.resolved_by, c.resolved_at, c.closed_at,
                c.created_at, c.updated_at
         FROM support_conversations c
