...
function paceModel(match, isNfl, score1, score2, minute, live) {
  if (isNfl) {
    return calculateAmericanFootballProbabilities({
      currentHomeScore: score1,
      currentAwayScore: score2,
      minute: Math.min(60, minute || (live ? 30 : 0)),
      totalGameMinutes: 60,
      expectedDrives: 22,
    });
  }
...
export function generateBasketballMatchWinner(match, team1Name, team2Name, margins = OSV4_MARGIN_CONFIG) {
  ...
  const { score1, score2, minute, live } = readLiveScoreState(match);
  const bb = paceModel(match, isNfl, score1, score2, minute, live);
...
export function generateBasketballExtras(match, team1Name, team2Name, winnerMarket, margins = OSV4_MARGIN_CONFIG) {
  ...
  const isNfl = sport === 'american-football';
  const { score1, score2, minute, live } = readLiveScoreState(match);
  const bb = winnerMarket?._pace || paceModel(match, isNfl, score1, score2, minute, live);
  ...
  const spread = twoWayMarket({
    ...
    pLeft: homeGives ? cover.pHomeCover : cover.pAwayCover,
    overround: totalsOo,
    houseBias: false,
    margins,
  });
