import { query, withTransaction } from '../db/pg.js';
import { BONUS_MIN_BET_ODDS, BONUS_WAGERING_MULTIPLIER } from './promoRules.mjs';
import { getVerifiedIdentity, assertIdentityHasNotClaimedPromo } from './userIdentity.mjs';

/**
 * Server-Authoritative Enterprise Promotions & Bonus Engine
 * Budget row-level locking, server-side eligibility, wagering multiplier calculations,
 * atomic turnover capping, and Phase 6 wallet/ledger bonus release.
 */

/** Create a new Promotion */
export async function createPromotion({
  name,
  code,
  type = 'DEPOSIT_BONUS',
  budget = 100000.00,
  maxReward = 5000.00,
  perUserLimit = 1,
  minOdds = BONUS_MIN_BET_ODDS,
  minStake = 100.00,
  wageringMultiplier = BONUS_WAGERING_MULTIPLIER,
  durationDays = 30,
}) {
  const promoId = `promo_${code.toLowerCase()}_${Date.now()}`;
  const expiresAt = new Date(Date.now() + durationDays * 24 * 60 * 60 * 1000).toISOString();

  await query(`
    INSERT INTO promotions (id, name, code, type, status, budget, used_budget, max_reward, per_user_limit, min_odds, min_stake, wagering_multiplier, expires_at)
    VALUES ($1, $2, $3, $4, 'ACTIVE', $5, 0.00, $6, $7, $8, $9, $10, $11);
  `, [promoId, name, code, type, budget, maxReward, perUserLimit, minOdds, minStake, wageringMultiplier, expiresAt]);

  return { success: true, promoId, code, type, budget, maxReward, expiresAt };
}

/** Claim Promotion Bonus with Row Locking & Idempotency */
export async function claimPromotionBonus({
  userId,
  promoCode,
  depositAmount: _clientDepositAmount = 1000.00,
}) {
  return await withTransaction(async (client) => {
    // 1. Verify User Profile & KYC/Account Status (Phase 9 Integration)
    const userRes = await client.query(`
      SELECT account_status, kyc_status
      FROM user_profiles
      WHERE user_id = $1;
    `, [userId]);

    if (userRes.rows.length > 0 && userRes.rows[0].account_status !== 'ACTIVE') {
      throw new Error('PROMOTION_ERROR: Account is restricted or suspended');
    }

    // 2. Lock Promotion row for Budget Safety
    const promoRes = await client.query(`
      SELECT id, name, code, type, status, budget, used_budget, max_reward, per_user_limit,
             min_odds, min_stake, wagering_multiplier, match_percent, starts_at, expires_at
      FROM promotions
      WHERE code = $1 AND status = 'ACTIVE'
      FOR UPDATE;
    `, [promoCode]);

    if (promoRes.rows.length === 0) {
      throw new Error('PROMOTION_ERROR: Promotion not found or inactive');
    }

    const promo = promoRes.rows[0];
    if (promo.starts_at && new Date(promo.starts_at).getTime() > Date.now()) {
      throw new Error('PROMOTION_ERROR: Promotion has not started yet');
    }
    if (promo.expires_at && new Date(promo.expires_at).getTime() < Date.now()) {
      throw new Error('PROMOTION_ERROR: Promotion has expired');
    }
    const identity = await getVerifiedIdentity(userId, client);
    if (identity) {
      await assertIdentityHasNotClaimedPromo({
        exec: client,
        promotionId: promo.id,
        panHash: identity.panHash,
        aadhaarHash: identity.aadhaarHash,
        excludeUserId: userId,
      });
    }

    // Idempotency Check: Existing active bonus for this user and promo
    const activeBonusRes = await client.query(`
      SELECT id, bonus_amount, wagering_required, wagering_completed, status
      FROM user_bonuses
      WHERE user_id = $1 AND promotion_id = $2 AND status IN ('ACTIVE', 'COMPLETED', 'RELEASED');
    `, [userId, promo.id]);

    if (activeBonusRes.rows.length > 0) {
      const existing = activeBonusRes.rows[0];
      return {
        success: true,
        alreadyClaimed: true,
        bonusId: existing.id,
        rewardAmount: parseFloat(existing.bonus_amount),
        wageringRequired: parseFloat(existing.wagering_required),
        status: existing.status,
      };
    }

    // Lifetime claim guard — include forfeited / cancelled so withdraw-forfeit cannot reclaim.
    const claimCountRes = await client.query(`
      SELECT COUNT(*)::int AS cnt
      FROM user_bonuses
      WHERE user_id = $1 AND promotion_id = $2
    `, [userId, promo.id]);
    const claimCount = Number(claimCountRes.rows[0]?.cnt || 0);
    const perUserLimit = Math.max(1, Number(promo.per_user_limit) || 1);
    if (claimCount >= perUserLimit) {
      throw new Error('PROMOTION_ERROR: Promotion already claimed for this account');
    }

    // Match against a real completed deposit — never trust a client-supplied amount.
    const minStake = parseFloat(promo.min_stake) || 0;
    const depositRes = await client.query(
      `SELECT amount FROM transactions
       WHERE user_id = $1
         AND type = 'DEPOSIT'
         AND status IN ('SUCCESS', 'COMPLETED')
       ORDER BY created_at DESC
       LIMIT 1`,
      [userId],
    );
    if (depositRes.rows.length === 0) {
      throw new Error('PROMOTION_ERROR: Deposit first to claim this bonus');
    }
    const deposit = parseFloat(depositRes.rows[0].amount) || 0;
    if (minStake > 0 && deposit < minStake) {
      throw new Error(`PROMOTION_ERROR: Deposit at least ₹${minStake.toFixed(0)} to claim this bonus`);
    }
    if (String(promo.code || promoCode).toUpperCase() === 'WELCOME150') {
      const depositCount = await client.query(
        `SELECT COUNT(*)::int AS n FROM transactions
         WHERE user_id = $1 AND type = 'DEPOSIT' AND status IN ('SUCCESS', 'COMPLETED')`,
        [userId],
      );
      if (Number(depositCount.rows[0]?.n || 0) !== 1) {
        throw new Error('PROMOTION_ERROR: WELCOME150 applies to your first deposit only');
      }
    }

    const maxReward = parseFloat(promo.max_reward);
    const matchPercent = promo.match_percent == null ? null : parseFloat(promo.match_percent);
    const rewardAmount = parseFloat(
      (matchPercent != null
        ? Math.min(deposit * (matchPercent / 100), maxReward)
        : Math.min(deposit, maxReward)
      ).toFixed(2),
    );
    const usedBudget = parseFloat(promo.used_budget);
    const totalBudget = parseFloat(promo.budget);

    if (usedBudget + rewardAmount > totalBudget) {
      throw new Error('PROMOTION_ERROR: Promotion budget exhausted');
    }

    // Update used budget
    await client.query(`
      UPDATE promotions SET used_budget = used_budget + $1 WHERE id = $2;
    `, [rewardAmount, promo.id]);

    // Create User Bonus Record with Wagering Requirement
    const bonusId = `ubonus_${userId}_${Date.now()}`;
    const wageringRequired = parseFloat((rewardAmount * parseFloat(promo.wagering_multiplier)).toFixed(2));
    const bonusExpiresAt = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString();

    await client.query(`
      INSERT INTO user_bonuses (
        id, user_id, promotion_id, bonus_amount, wagering_required, wagering_completed, status, expires_at, pan_hash, aadhaar_hash
      )
      VALUES ($1, $2, $3, $4, $5, 0.00, 'ACTIVE', $6, $7, $8);
    `, [
      bonusId,
      userId,
      promo.id,
      rewardAmount,
      wageringRequired,
      bonusExpiresAt,
      identity?.panHash || null,
      identity?.aadhaarHash || null,
    ]);

    // Update Wallet Bonus Balance
    const walletRes = await client.query(`
      UPDATE wallets
      SET bonus_balance = bonus_balance + $1, updated_at = CURRENT_TIMESTAMP
      WHERE user_id = $2
      RETURNING wallet_id, balance, bonus_balance;
    `, [rewardAmount, userId]);

    const wallet = walletRes.rows.length > 0 ? walletRes.rows[0] : { wallet_id: `wal_${userId}`, balance: 0.00, bonus_balance: rewardAmount };

    // Record Double-Entry Ledger Entry
    await client.query(`
      INSERT INTO ledger_entries (wallet_id, type, amount, balance_after, description)
      VALUES ($1, 'CREDIT', $2, $3, $4);
    `, [wallet.wallet_id, rewardAmount, wallet.balance, `Bonus Claimed: ${promo.name}`]);

    return {
      success: true,
      bonusId,
      promoCode,
      rewardAmount,
      wageringRequired,
      bonusBalance: parseFloat(wallet.bonus_balance || rewardAmount),
      status: 'ACTIVE',
    };
  });
}

export async function hasIncompleteBonusWagering(userId, exec = query) {
  const run = typeof exec === 'function' ? exec : exec.query.bind(exec);
  const res = await run(
    `SELECT id FROM user_bonuses
     WHERE user_id = $1 AND status = 'ACTIVE'
     LIMIT 1`,
    [userId],
  );
  return res.rows.length > 0;
}

import { isQualifyingBonusOdds } from './wageringRules.mjs';

/**
 * Authoritative Transactional Bonus Wagering Progress & Rollover Engine.
 * Executed INSIDE the same PostgreSQL client transaction that commits bet settlement.
 */
export async function recordBonusWageringInTx(client, {
  userId,
  bonusId = null,
  betId,
  stake,
  bonusStake = 0,
  odds,
  outcome, // 'WON' | 'LOST' | 'VOID' | 'CANCELLED'
  profit = 0,
}) {
  const effectiveBonusStake = Number(bonusStake) > 0 ? Number(bonusStake) : Number(stake || 0);
  const actualOdds = Number(odds || 1.0);
  const isOddsQualifying = isQualifyingBonusOdds(actualOdds);

  const activeBonusRes = await client.query(
    `SELECT id, bonus_amount, wagering_required, wagering_completed, COALESCE(locked_winnings, 0) AS locked_winnings, status
     FROM user_bonuses
     WHERE user_id = $1 AND status = 'ACTIVE'
       AND ($2::varchar IS NULL OR id = $2)
     ORDER BY created_at ASC
     FOR UPDATE`,
    [userId, bonusId || null]
  );

  if (activeBonusRes.rows.length === 0) {
    return { applied: false, reason: 'no_active_bonus' };
  }

  const bonus = activeBonusRes.rows[0];
  const required = parseFloat(bonus.wagering_required);
  const currentCompleted = parseFloat(bonus.wagering_completed);

  // Idempotency check: has this bet settlement already recorded a ledger entry?
  const existingLedger = await client.query(
    `SELECT id, qualifying_amount, status FROM bonus_wagering_ledger
     WHERE bonus_id = $1 AND bet_id = $2`,
    [bonus.id, betId]
  );

  if (existingLedger.rows.length > 0) {
    return { applied: false, alreadyProcessed: true, ledgerId: existingLedger.rows[0].id };
  }

  let qualifyingAmount = 0;
  let ledgerStatus = 'APPLIED';

  if (outcome === 'VOID' || outcome === 'CANCELLED') {
    qualifyingAmount = 0;
    ledgerStatus = 'VOID_NO_TURNOVER';
  } else if (!isOddsQualifying) {
    qualifyingAmount = 0;
    ledgerStatus = 'REJECTED_ODDS';
  } else {
    // Both WON and LOST qualifying bets advance rollover turnover by the bonus stake
    qualifyingAmount = parseFloat(effectiveBonusStake.toFixed(2));
    ledgerStatus = 'APPLIED';
  }

  const remainingBefore = parseFloat(Math.max(0, required - currentCompleted).toFixed(2));
  const newCompleted = parseFloat(Math.min(required, currentCompleted + qualifyingAmount).toFixed(2));
  const remainingAfter = parseFloat(Math.max(0, required - newCompleted).toFixed(2));
  const isCompleted = newCompleted >= required;

  const ledgerId = `bwl_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  await client.query(
    `INSERT INTO bonus_wagering_ledger (
       id, user_id, bonus_id, bet_id, stake_amount, qualifying_amount,
       minimum_odds, actual_odds, result, turnover_before, turnover_after,
       remaining_before, remaining_after, status, metadata, created_at
     )
     VALUES ($1, $2, $3, $4, $5, $6, 1.75, $7, $8, $9, $10, $11, $12, $13, $14, NOW())
     ON CONFLICT (bonus_id, bet_id, result) DO NOTHING`,
    [
      ledgerId,
      userId,
      bonus.id,
      betId,
      effectiveBonusStake,
      qualifyingAmount,
      actualOdds,
      outcome,
      currentCompleted,
      newCompleted,
      remainingBefore,
      remainingAfter,
      ledgerStatus,
      JSON.stringify({ profit, odds: actualOdds, isOddsQualifying }),
    ]
  );

  const addedProfit = (outcome === 'WON' && profit > 0) ? parseFloat(profit.toFixed(2)) : 0;
  const currentLockedWinnings = parseFloat(bonus.locked_winnings || 0);
  const newLockedWinnings = parseFloat((currentLockedWinnings + addedProfit).toFixed(2));

  if (isCompleted) {
    // 5x ROLLOVER COMPLETED! Atomically release accumulated locked bonus winnings to cash
    const walletRes = await client.query(
      `SELECT wallet_id, balance, bonus_balance, COALESCE(locked_bonus_winnings, 0) AS locked_bonus_winnings
       FROM wallets WHERE user_id = $1 FOR UPDATE`,
      [userId]
    );
    const wallet = walletRes.rows[0];
    const totalLockedWinningsToRelease = parseFloat(
      Number(wallet?.locked_bonus_winnings || 0).toFixed(2)
    );
    const newCash = parseFloat((Number(wallet?.balance || 0) + totalLockedWinningsToRelease).toFixed(2));

    await client.query(
      `UPDATE wallets
       SET balance = $1,
           locked_bonus_winnings = 0.00,
           updated_at = NOW()
       WHERE wallet_id = $2`,
      [newCash, wallet.wallet_id]
    );

    await client.query(
      `UPDATE user_bonuses
       SET wagering_completed = $1,
           status = 'COMPLETED',
           completed_at = NOW(),
           released_winnings = released_winnings + $2,
           locked_winnings = 0.00,
           released_at = NOW()
       WHERE id = $3`,
      [newCompleted, totalLockedWinningsToRelease, bonus.id]
    );

    if (totalLockedWinningsToRelease > 0) {
      const releaseTxId = `tx_bn_rel_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
      await client.query(
        `INSERT INTO transactions (transaction_id, user_id, type, amount, status, created_at)
         VALUES ($1, $2, 'BONUS_RELEASE', $3, 'SUCCESS', NOW())
         ON CONFLICT (transaction_id) DO NOTHING`,
        [releaseTxId, userId, totalLockedWinningsToRelease]
      );
      await client.query(
        `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
         VALUES ($1, $2, 'CREDIT', $3, $4, $5, NOW())`,
        [
          wallet.wallet_id,
          releaseTxId,
          totalLockedWinningsToRelease,
          newCash,
          `Rollover 5x completed: Released ₹${totalLockedWinningsToRelease.toFixed(2)} bonus profit to cash`,
        ]
      );
    }

    return {
      applied: true,
      bonusId: bonus.id,
      qualifyingAmount,
      wageringCompleted: newCompleted,
      wageringRequired: required,
      remainingWagering: 0,
      isCompleted: true,
      releasedAmount: totalLockedWinningsToRelease,
    };
  } else {
    await client.query(
      `UPDATE user_bonuses
       SET wagering_completed = $1,
           locked_winnings = $2
       WHERE id = $3`,
      [newCompleted, newLockedWinnings, bonus.id]
    );

    return {
      applied: true,
      bonusId: bonus.id,
      qualifyingAmount,
      wageringCompleted: newCompleted,
      wageringRequired: required,
      remainingWagering: remainingAfter,
      isCompleted: false,
      lockedWinnings: newLockedWinnings,
    };
  }
}

/** Process Bonus Wagering Progress on Settled Qualifying Bet */
export async function processBonusWageringProgress({
  userId,
  bonusId = null,
  betStake,
  betOdds,
  betId = `sim_bet_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
  outcome = 'WON',
  fundSource = 'bonus',
} = {}) {
  if (fundSource === 'freebet') {
    return { updated: false, skipped: 'freebet' };
  }
  if (Number(betOdds) < 1.75) {
    return { updated: false, skipped: 'odds' };
  }
  return await withTransaction(async (client) => {
    const rec = await recordBonusWageringInTx(client, {
      userId,
      bonusId,
      betId,
      stake: betStake,
      bonusStake: betStake,
      odds: betOdds,
      outcome,
      profit: 0,
    });
    return {
      ...rec,
      updated: rec.applied ?? true,
    };
  });
}

/** Release Completed Bonus Funds to Cash Balance (Atomic Phase 6 Transaction) */
export async function releaseCompletedBonus({ userId, bonusId }) {
  return await withTransaction(async (client) => {
    const bonusRes = await client.query(`
      SELECT id, bonus_amount, wagering_required, wagering_completed, status
      FROM user_bonuses
      WHERE id = $1 AND user_id = $2 FOR UPDATE;
    `, [bonusId, userId]);

    if (bonusRes.rows.length === 0) {
      throw new Error(`Bonus ${bonusId} not found`);
    }

    const bonus = bonusRes.rows[0];

    if (bonus.status === 'RELEASED') {
      return { success: true, status: 'ALREADY_RELEASED', bonusId, amount: parseFloat(bonus.bonus_amount) };
    }

    const completed = parseFloat(bonus.wagering_completed);
    const required = parseFloat(bonus.wagering_required);

    if (completed < required && bonus.status !== 'COMPLETED') {
      throw new Error(`INCOMPLETE_WAGERING: Required: ₹${required}, Completed: ₹${completed}. Bonus cannot be released.`);
    }

    const bonusAmount = parseFloat(bonus.bonus_amount);

    // Playthrough complete: mark status as RELEASED.
    await client.query(`UPDATE user_bonuses SET status = 'RELEASED' WHERE id = $1;`, [bonusId]);

    const walletRes = await client.query(
      `SELECT wallet_id, balance, bonus_balance FROM wallets WHERE user_id = $1`,
      [userId],
    );
    const wallet = walletRes.rows[0] || { balance: 0, bonus_balance: 0 };

    return {
      success: true,
      status: 'RELEASED',
      bonusId,
      releaseAmount: 0,
      bonusAmount,
      newCashBalance: parseFloat(wallet.balance || 0),
      bonusBalance: parseFloat(wallet.bonus_balance || 0),
    };
  });
}

/** Expire Stale Bonuses Background Worker */
export async function expireStaleBonuses() {
  const expiredRes = await query(`
    SELECT ub.id, ub.user_id, ub.bonus_amount
    FROM user_bonuses ub
    WHERE ub.status = 'ACTIVE' AND ub.expires_at < CURRENT_TIMESTAMP;
  `);

  let count = 0;
  for (const row of expiredRes.rows) {
    await withTransaction(async (client) => {
      const ubLock = await client.query(
        `SELECT id, status, bonus_amount FROM user_bonuses WHERE id = $1 FOR UPDATE`,
        [row.id],
      );
      if (ubLock.rows.length === 0 || ubLock.rows[0].status !== 'ACTIVE') return;

      const walletRes = await client.query(
        `SELECT wallet_id, balance, bonus_balance FROM wallets WHERE user_id = $1 FOR UPDATE`,
        [row.user_id],
      );
      if (walletRes.rows.length === 0) return;

      const wallet = walletRes.rows[0];
      const bonusDeduct = Math.min(Number(row.bonus_amount || 0), Number(wallet.bonus_balance || 0));
      const nextBonus = Number(Math.max(0.00, Number(wallet.bonus_balance || 0) - bonusDeduct).toFixed(2));

      await client.query(`UPDATE user_bonuses SET status = 'EXPIRED' WHERE id = $1`, [row.id]);
      await client.query(
        `UPDATE wallets SET bonus_balance = $1, updated_at = NOW() WHERE wallet_id = $2`,
        [nextBonus, wallet.wallet_id],
      );

      if (bonusDeduct > 0) {
        const txId = `tx_bn_exp_${row.id}`;
        await client.query(
          `INSERT INTO transactions (transaction_id, user_id, type, method, amount, status, created_at)
           VALUES ($1, $2, 'BONUS_EXPIRED', 'PROMOTION', $3, 'SUCCESS', NOW())
           ON CONFLICT (transaction_id) DO NOTHING`,
          [txId, row.user_id, bonusDeduct],
        );
        await client.query(
          `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
           VALUES ($1, $2, 'DEBIT', $3, $4, $5, NOW())`,
          [wallet.wallet_id, txId, bonusDeduct, nextBonus, `Expired bonus #${row.id}`],
        );
      }
      count++;
    });
  }

  return { success: true, countExpired: count };
}

/**
 * When the last bonus principal is lost (bonus_balance = 0, no open bonus bets),
 * locked bonus winnings can never complete rollover — forfeit them with the bonus.
 */
export async function forfeitLockedBonusWinningsIfBonusExhausted(client, {
  userId,
  betId = null,
  reason = 'bonus_exhausted',
} = {}) {
  const walletRes = await client.query(
    `SELECT wallet_id, balance,
            COALESCE(bonus_balance, 0) AS bonus_balance,
            COALESCE(locked_bonus_winnings, 0) AS locked_bonus_winnings
     FROM wallets WHERE user_id = $1 FOR UPDATE`,
    [userId],
  );
  const wallet = walletRes.rows[0];
  if (!wallet) return { forfeited: false, reason: 'no_wallet' };

  const bonusBalance = Number(wallet.bonus_balance) || 0;
  const locked = Number(wallet.locked_bonus_winnings) || 0;
  if (locked <= 0) return { forfeited: false, amount: 0, reason: 'no_locked_winnings' };
  if (bonusBalance > 0) return { forfeited: false, amount: locked, reason: 'bonus_principal_remaining' };

  const openBonus = await client.query(
    `SELECT 1 FROM bets
     WHERE user_id = $1
       AND LOWER(COALESCE(fund_source, '')) = 'bonus'
       AND UPPER(status) IN ('ACCEPTED', 'PENDING', 'OPEN')
     LIMIT 1`,
    [userId],
  );
  if (openBonus.rows.length > 0) {
    return { forfeited: false, amount: locked, reason: 'open_bonus_bets' };
  }

  await client.query(
    `UPDATE wallets
     SET locked_bonus_winnings = 0.00,
         updated_at = NOW()
     WHERE wallet_id = $1`,
    [wallet.wallet_id],
  );

  await client.query(
    `UPDATE user_bonuses
     SET status = 'FORFEITED',
         locked_winnings = 0.00
     WHERE user_id = $1 AND status IN ('ACTIVE', 'COMPLETED')`,
    [userId],
  );

  const forfeitTxId = `tx_bn_win_forfeit_${betId || Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  await client.query(
    `INSERT INTO transactions (transaction_id, user_id, type, amount, status, created_at)
     VALUES ($1, $2, 'BONUS_WINNINGS_FORFEIT', $3, 'SUCCESS', NOW())
     ON CONFLICT (transaction_id) DO NOTHING`,
    [forfeitTxId, userId, locked],
  );
  await client.query(
    `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
     VALUES ($1, $2, 'DEBIT', $3, $4, $5, NOW())`,
    [
      wallet.wallet_id,
      forfeitTxId,
      locked,
      Number(wallet.balance) || 0,
      `Locked bonus winnings forfeited (${reason})${betId ? ` after ${betId}` : ''}`,
    ],
  );

  return { forfeited: true, amount: locked, reason, transactionId: forfeitTxId };
}

