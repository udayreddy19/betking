...
export function isMarketEligible(marketKey, state) {
  ...
  // Past / far-future dismissal markets must not stay bettable — only next (or next+1) wicket.
  ...
  if (dismissalHit) {
    ...
    if (n > liveWkts + 2) return false; // never sell "10th wicket" when 4 down
  }
  const methodHit = keyUpper.match(/METHOD_OF_NEXT_WICKET_(\d+)/);
  if (methodHit) {
    const n = Number(methodHit[1]);
    if (liveWkts >= n) return false;
    if (n !== liveWkts + 1) return false; // only the true next dismissal
  }
