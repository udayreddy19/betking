...
export function generateExtendedMatchTotals(state, validation = {}, marginConfig = {}) {
  ...
  let expectedMatchTotal;
  if (state.currentInnings === 2) {
    const firstInningsRuns = fieldingTeam.runs || 0;
    const maxMatchTotal = state.target != null ? (firstInningsRuns + state.target + 2) : (firstInningsRuns + calc.expectedTotal);
    expectedMatchTotal = Math.min(maxMatchTotal, firstInningsRuns + calc.expectedTotal);
  } else {
    // 1st Innings: Team 1 expected runs + 2nd Innings baseline runs
    const secondInningsBaseline = rules.ballsPerInnings * rules.historicalRunsPerBall;
    expectedMatchTotal = calc.expectedTotal + secondInningsBaseline;
  }
  ...
  markets.push(createMarketDefinition({
    ...
    status: isAltDetermined ? 'SETTLED' : 'OPEN',
    line: altLine,
    selections: [
