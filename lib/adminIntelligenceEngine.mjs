...
export async function getUser360View(userId, opts = {}) {
  ...
  try {
    ...
    const declared = details.accountHolderName || details.account_holder_name || details.name || null;
    const method = details.method || details.type || null;
    const upiRaw = details.upiId || details.upi_id || details.vpa || null;
    const acctRaw = details.account || details.accountNumber || details.account_number || null;
    const {
      evaluateBeneficiaryKycMatch,
      resolveVerifiedBeneficiaryName,
      resolveVerifiedKycName,
    } = await import('./beneficiaryKycNameMatch.mjs');
    const evaluation = await evaluateBeneficiaryKycMatch(userId, details, query);
    const verifiedBeneficiaryName = await resolveVerifiedBeneficiaryName(userId, details, query);
