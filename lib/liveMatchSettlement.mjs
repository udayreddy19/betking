...
export async function evaluateDeliveryMarketBetAsync(bet, match, matchId) {
  ...
  if (confirmed?.isConfirmed) {
    const settled = await trySettlePassedDeliveryBetAsync(bet, match, overNum, ballNum, slot, confirmed);
    if (settled) return settled;
  }

  // OddsYra SRL: rehydrate from sim + grade from timeline. Never park as awaitingEvidence.
  if (isSrlMatchId(match || id)) {
    try {
      ...
      if (srlFresh) {
        ...
        const fromFresh = evaluateDeliveryMarketBet(bet, merged);
        if (fromFresh) return fromFresh;
      }
    } catch { /* fall through to direct timeline */ }

    const srlSettled = await trySettlePassedDeliveryBetAsync(bet, match, overNum, ballNum, slot);
    if (srlSettled) return srlSettled;
    return null;
  }
