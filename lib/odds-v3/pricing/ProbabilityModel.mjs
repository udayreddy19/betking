...
export function calculateMatchWinnerProbability({
  ...
  // For Test matches where ballsRemaining is null, evaluate with realistic remaining delivery horizon.
  const effectiveBalls = (ballsRemaining != null && Number.isFinite(ballsRemaining) && ballsRemaining > 0)
    ? ballsRemaining
    : (format === 'TEST' ? Math.max(180, runsRequired * 2) : 0);
  const requiredRunRate = effectiveBalls > 0 ? runsRequired / effectiveBalls : Infinity;
