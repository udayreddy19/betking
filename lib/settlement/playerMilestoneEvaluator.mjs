...
import { isMultiDayCricket } from './wallClockFinality.mjs';
...
function battingFinishedForPlayer(match, batter) {
  ...
  // If not on crease, and match is limited overs (non-multi-day):
  // When the match is in innings 2 or later, any batter not on crease who already batted
  // (with notOut/dismissal not out) finished their innings in the previous innings (innings 1).
  const currentInn = bat.innings || Number(match?.inningsId) || 1;
  if (!isMultiDayCricket(match) && currentInn >= 2 && isInningsComplete(match, 1)) {
    return true;
  }

  // Not on crease: dismissed OR innings/match complete — never treat "faced a ball"
  // as terminal (Test / multi-innings batters return later).
  if (isBatterFinished(batter)) return true;
