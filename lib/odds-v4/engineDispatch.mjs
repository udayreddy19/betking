/**
 * Engine dispatch with reason-coded V4 → V3 fallbacks.
 * Cricket defaults V4; other sports default OtherSportsEngineV4.
 * Every downgrade is logged with a reason code for observability.
 */

import { createLogger } from '../logger.mjs';
const log = createLogger({ engine: 'engineDispatch' });
...
import { generate as generateV4, V4_ENGINE_VERSION } from './OddsEngineV4.mjs';
...
/** Reason codes for auditable engine downgrades / routing decisions. */
export const ENGINE_FALLBACK_REASONS = Object.freeze({
  V4_HEALTH_FAILURE: 'V4_HEALTH_FAILURE',
  V4_CONFIGURATION_ERROR: 'V4_CONFIGURATION_ERROR',
  V4_MARKET_UNSUPPORTED: 'V4_MARKET_UNSUPPORTED',
  V4_PROVIDER_STATE_INVALID: 'V4_PROVIDER_STATE_INVALID',
  MANUAL_ROLLBACK: 'MANUAL_ROLLBACK',
  CANARY_GATE_FAILURE: 'CANARY_GATE_FAILURE',
  V4_EXCEPTION: 'V4_EXCEPTION',
  NON_OSV4_SPORT: 'NON_OSV4_SPORT',
  MODE_V3: 'MODE_V3',
  MODE_SHADOW_PUBLISH_V3: 'MODE_SHADOW_PUBLISH_V3',
});
...
function recordFallback(entry) {
  fallbackRing.push({ ...entry, at: Date.now() });
  if (fallbackRing.length > FALLBACK_MAX) fallbackRing.shift();
  log.warn('engine_fallback', entry);
}

export function getEngineFallbackMetrics() {
  return {
    samples: fallbackRing.length,
    recent: fallbackRing.slice(-25),
    byReason: fallbackRing.reduce((acc, row) => {
...
function v3OtherSportsResult(payload, otherMode, v3Config, reason, matchId) {
  if (reason) {
    recordFallback({
      sport: payload.sport,
      matchId: matchId || payload.matchId,
      from: 'OtherSportsEngineV4',
      to: 'OddsEngineV3',
      reason,
      mode: otherMode,
    });
...
function generateOtherSportsRaw(matchObj, otherMode, v3Config = {}) {
  ...
  if (otherMode === 'shadow') {
    try { runOtherSportsShadowCompare(payload); } catch (err) { log.warn('other-sports shadow compare failed', { matchId, err }); }
  }

  if (otherMode === 'v3') {
    return v3OtherSportsResult(payload, otherMode, v3Config, ENGINE_FALLBACK_REASONS.MODE_V3, matchId);
  }

  if (otherMode === 'shadow') {
    return v3OtherSportsResult(payload, otherMode, v3Config, ENGINE_FALLBACK_REASONS.MODE_SHADOW_PUBLISH_V3, matchId);
  }

  // otherMode === 'v4' (default)
  if (!osv4Eligible) {
    return v3OtherSportsResult(payload, otherMode, v3Config, ENGINE_FALLBACK_REASONS.NON_OSV4_SPORT, matchId);
  }

  try {
    const raw = generateOtherSportsV4(payload, v3Config);
    if (!raw || raw.error) {
      return v3OtherSportsResult(
        payload,
        otherMode,
        v3Config,
        ENGINE_FALLBACK_REASONS.V4_PROVIDER_STATE_INVALID,
        matchId,
      );
    }
    return {
    ...
    log.error('OtherSportsEngineV4 exception — falling back to V3', { matchId, err: err?.message });
    return v3OtherSportsResult(payload, otherMode, v3Config, ENGINE_FALLBACK_REASONS.V4_EXCEPTION, matchId);
  }
}

function priceWithMode(baseMatch, cricketMode, { isCricket = true } = {}, otherMode = 'v4') {
  if (!isCricket) {
    if (otherMode === 'shadow') {
      try { runOtherSportsShadowCompare(otherSportsMatchPayload(baseMatch)); } catch (err) { log.warn('other-sports shadow compare failed', { err }); }
    }
    ...
    if (useOsv4) {
      try {
        ...
        recordFallback({
          sport: baseMatch.sport,
          matchId: baseMatch.id || baseMatch.matchId,
          from: 'OtherSportsEngineV4',
          to: 'OddsEngineV3',
          reason: ENGINE_FALLBACK_REASONS.V4_EXCEPTION,
          mode: otherMode,
          error: err?.message,
        });
      }
    } else if (otherMode === 'v4' && !isOtherSportsV4Sport(baseMatch.sport)) {
      recordFallback({
        sport: baseMatch.sport,
        matchId: baseMatch.id || baseMatch.matchId,
        from: 'OtherSportsEngineV4',
        to: 'OddsEngineV3',
        reason: ENGINE_FALLBACK_REASONS.NON_OSV4_SPORT,
        mode: otherMode,
      });
    }
  ...
  if (cricketMode === 'shadow') {
    try { runShadowCompare(baseMatch); } catch (err) { log.warn('cricket shadow compare failed', { err }); }
  }
...
function snapshotWithMode(matchObj, cricketMode, otherMode, v3Config = {}) {
  ...
  if (!cricket) {
    const { raw, engine, engineVersion, mode, fallbackReason } = generateOtherSportsRaw(matchObj, otherMode, v3Config);
    ...
    if (publicSnapshot) {
      ...
      if (fallbackReason) publicSnapshot.engineFallbackReason = fallbackReason;
    }
    return { rawSnapshot: raw, publicSnapshot, mode, fallbackReason: fallbackReason || null };
  }

  if (cricketMode === 'shadow' || effectiveCricketMode === 'shadow') {
    try { runShadowCompare(matchObj); } catch (err) { log.warn('cricket shadow compare failed', { err }); }
  }
  ...
  if (useV4) {
    try {
      ...
      if (publicSnapshot) {
        ...
        publicSnapshot.engineVersion = V4_ENGINE_VERSION;
        publicSnapshot.source = 'ODDS_ENGINE_V4';
      }
      return { rawSnapshot: raw, publicSnapshot, mode: 'v4', fallbackReason: null };
    } catch (err) {
      recordFallback({
        sport: 'cricket',
  ...
  return {
    rawSnapshot,
    publicSnapshot,
    mode: cricketMode === 'shadow' ? 'shadow' : 'v3',
    fallbackReason: useV4 ? ENGINE_FALLBACK_REASONS.V4_EXCEPTION : (cricketMode === 'v3' ? ENGINE_FALLBACK_REASONS.MODE_V3 : null),
  };
}
