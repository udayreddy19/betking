...
export async function getSystemHealthStatus() {
  ...
  // 2. Check Redis — require a successful PING; never report HEALTHY while disconnected.
  const t1 = Date.now();
  const multiInstance = String(process.env.MULTI_INSTANCE || '').toLowerCase() === 'true'
    || String(process.env.MULTI_INSTANCE || '') === '1'
    || process.env.NODE_ENV === 'production';
  try {
    if (!redis) {
      checks.redis.status = 'DOWN';
      if (multiInstance) checks.overall = 'DOWN';
      else if (checks.overall === 'HEALTHY') checks.overall = 'DEGRADED';
    } else {
      await redis.ping();
    ...
    checks.redis.status = 'DOWN';
    // Multi-instance / production: Redis is security-critical (MFA consume, WS fanout)
    if (multiInstance) checks.overall = 'DOWN';
    else if (checks.overall === 'HEALTHY') checks.overall = 'DEGRADED';
  }

  // 3. Outbox reachability only — never COUNT(*) here (this runs on /health).
  try {
    await query(`SELECT 1 FROM outbox_events LIMIT 1`);
    checks.outboxQueue.status = 'HEALTHY';
    checks.outboxQueue.pending = null;
  } catch (err) {
...
/** Public /health — overall status only, no queue depths or dependency maps. */
export function getPublicHealthStatus(full) {
  return {
    status: full?.status || 'DOWN',
...
export function validateProductionEnvironment(env = process.env) {
  ...
  if (isProd && (env.E2E_HARNESS === '1' || env.E2E_HARNESS === 'true')) {
    throw new Error(
      'PRODUCTION_STARTUP_ERROR: E2E_HARNESS must not be enabled in production',
    );
  }
  ...
  const multiInstance = String(env.MULTI_INSTANCE || '').toLowerCase() === 'true'
    || String(env.MULTI_INSTANCE || '') === '1'
    || isProd;
  if (multiInstance && !env.REDIS_URL) {
    throw new Error('PRODUCTION_STARTUP_ERROR: MULTI_INSTANCE/production requires REDIS_URL (MFA consume + WS fanout)');
  }
