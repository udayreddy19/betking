...
/**
 * Validate proposed deposit against user's deposit / self-exclusion / cooling-off limits.
 * Delegates to the authoritative responsibleGaming engine (not an in-memory stub).
 */
export async function validateDepositAttempt(userId, depositAmount = 0) {
  try {
    const { responsibleGamingEngine } = await import('./responsibleGaming.mjs');
    return await responsibleGamingEngine.validateDepositAttempt(userId, depositAmount);
  } catch (err) {
    return { allowed: false, reason: err.message || 'RG_CHECK_FAILED' };
  }
}

/** @deprecated Prefer validateDepositAttempt — kept for older callers */
export async function validateDepositLimit(userId, depositAmount = 0) {
  const result = await validateDepositAttempt(userId, depositAmount);
  if (result?.allowed === false || result?.isAllowed === false) {
    return {
      isAllowed: false,
      reason: result.reason || result.code || 'DEPOSIT_LIMIT_EXCEEDED',
      ...result,
    };
  }
