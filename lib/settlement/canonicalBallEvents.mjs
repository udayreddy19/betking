...
export async function getConfirmedBallEvent(matchId, innings, overNumber, ballNumber) {
  ...
  return {
    ...
    isConfirmed: Boolean(row.is_confirmed ?? row.isConfirmed),
    sequenceNumber: (row.sequence_number ?? row.sequenceNumber) != null ? Number(row.sequence_number ?? row.sequenceNumber) : null,
  };
}
