...
export async function completeAdminMfa(mfaToken, code, { enroll = false } = {}) {
  ...
  const { isAdminMfaPendingConsumed, consumeAdminMfaPendingToken } = await import('./adminMfaPendingOnce.mjs');
  if (isAdminMfaPendingConsumed(mfaToken)) {
  ...
  consumeAdminMfaPendingToken(mfaToken);
  return issueAdminSessionJson(userId, role);
}
