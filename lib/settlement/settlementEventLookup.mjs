    ...
    for (const alias of aliases) {
      if (byId.has(alias)) {
        ...
        if (validateMatchIdentity(hydrated)) {
          ...
          const now = Date.now();
          const isStaleLive = (startTime > 0 && now - startTime > 3.5 * 3600 * 1000)
          ...
          if (isStaleLive) {
            ...
            hydrated.matchState = 'post';
            hydrated.isCompleted = true;
            hydrated.isLive = false;
          }
          return {
            success: true,
            match: hydrated,
  ...
  try {
    ...
    if (cachedState && validateMatchIdentity(cachedState)) {
      ...
      if (isStaleLive) {
        ...
        cachedState.matchState = 'post';
        cachedState.isCompleted = true;
        cachedState.isLive = false;
      }
      return {
        success: true,
        match: cachedState,
