...
import { getUserPreferences } from './notificationPreferencesEngine.mjs';
...
/** SUPPORT emails respect transactional_email; SECURITY/ACCOUNT stay mandatory elsewhere. */
async function canSendSupportTransactionalEmail(userId) {
  if (!userId) return true;
  try {
    const prefs = await getUserPreferences(userId);
    return prefs.transactionalEmail !== false;
  } catch {
    return true;
  }
}
...
export async function notifyUserSupportEvent({
  ...
  try {
    ...
    const ticketRef = ticketReference || ticketNumber || conversationId || null;
    const metadata = {
      conversationId: conversationId || null,
      ticketId: ticketRef,
      ticketReference: ticketRef,
      ticketNumber: ticketNumber || ticketRef,
    };
    ...
    try {
      await query(
        `INSERT INTO notifications
           (id, user_id, event_type, category, channel, recipient, subject, body, status, event_id, is_read, attempts, metadata)
         VALUES ($1, $2, $3, 'SUPPORT', 'IN_APP', $2, $4, $5, 'DELIVERED', $6, FALSE, 0, $7::jsonb)
         ON CONFLICT (id) DO NOTHING`,
        [id, userId, eventType, subject, message, eid, metaJson],
      );
    } catch {
      try {
        await query(
          `INSERT INTO notifications
             (id, user_id, event_type, category, channel, recipient, subject, body, status, event_id, is_read, attempts, tenant_id, metadata)
           VALUES ($1, $2, $3, 'SUPPORT', 'IN_APP', $2, $4, $5, 'DELIVERED', $6, FALSE, 0, 'oddsyra_in', $7::jsonb)
           ON CONFLICT (id) DO NOTHING`,
          [id, userId, eventType, subject, message, eid, metaJson],
        );
      } catch {
        // metadata column may be missing pre-migration — insert without it
        await query(
          `INSERT INTO notifications
...
export async function emailUserOnTicketCreated(ticket) {
  try {
    if (!(await canSendSupportTransactionalEmail(ticket?.userId))) {
      return { success: false, skipped: true, reason: 'TRANSACTIONAL_EMAIL_OPTED_OUT' };
    }
    const contact = await lookupUserContact(ticket?.userId);
...
export async function emailUserOnAdminReply({
  ...
  try {
    if (!(await canSendSupportTransactionalEmail(userId))) {
      return { success: false, skipped: true, reason: 'TRANSACTIONAL_EMAIL_OPTED_OUT' };
    }
    ...
    const linkId = conversationId || ticketId || ticketNumber || '';
    ...
    return await sendSupportAdminReplyEmail({
      ...
      replyUrl: `${process.env.FRONTEND_URL || 'https://oddsyra.com'}/support/tickets/${encodeURIComponent(linkId)}`,
    });
  } catch (err) {
