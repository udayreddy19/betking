...
export async function fetchMatchDetail(match, { fast = false } = {}) {
  ...
  const team1Name = match.team1?.name || match.homeTeam?.name || 'Team 1';
  const team2Name = match.team2?.name || match.awayTeam?.name || 'Team 2';

  const roster1 = getRosterForTeam(team1Name);
  const roster2 = getRosterForTeam(team2Name);

  const b1DefaultName = roster1?.batters?.[0] || 'J. Smith';
  const b2DefaultName = roster1?.batters?.[1] || 'A. Patel';
  const bowlerDefaultName = roster2?.bowlers?.[0] || roster2?.batters?.[6] || 'P. Cummins';
  ...
  return {
    ...
    liveDetails: {
      ...
      batter1: ld.batter1 || { name: b1DefaultName, runs: 12, balls: 8, fours: 2, sixes: 0 },
      batter2: ld.batter2 || { name: b2DefaultName, runs: 8, balls: 6, fours: 1, sixes: 0 },
      bowler: ld.bowler || { name: bowlerDefaultName, wickets: 1, runs: 18, overs: '2.4' },
    },
    squads: [
      {
        name: team1Name,
