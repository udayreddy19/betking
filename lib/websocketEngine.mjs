...
async function canSubscribeToChannel(session, channel) {
  if (!channel || typeof channel !== 'string') return false;

  if (channel.startsWith('odds:match:')) {
    return true;
  }
...
export function initWebSocketServer(httpServer) {
  ...
  wssInstance.on('connection', async (ws, req) => {
    ...
    if (!userId && process.env.NODE_ENV === 'production') {
      // Anonymous connections may only subscribe to public odds channels.
    } else if (!userId) {
      userId = url.searchParams.get('userId') || 'demo@oddsyra.com';
    ...
    const session = {
      ...
      userId,
      role,
      anonymousOddsOnly: !userId && process.env.NODE_ENV === 'production',
      token: tokenParam,
      channels: new Set(),
