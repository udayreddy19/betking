...
export async function performGlobalAdminSearch(searchQuery = '', _tenantId = 'tenant_default') {
  ...
  if (pg) {
    try {
      // 1. Search Users (email / mobile / name / id)
      const { buildUserContactSearchClause } = await import('./adminDomainData.mjs');
      const contact = buildUserContactSearchClause(q, { startIdx: 1, searchBy: 'all' });
      if (contact.sql) {
        const userRes = await pg(
          `SELECT u.user_id, u.email, u.phone, u.first_name, u.last_name, u.created_at
           FROM users u
