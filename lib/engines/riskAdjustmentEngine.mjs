...
export class RiskAdjustmentEngine {
  ...
  applyTwoWayShift(p0, p1, marketId = '', selId0 = '', selId1 = '') {
    ...
    if (Math.abs(diff) < 250) {
      return { p0, p1, shifted: false, liabilityDiff: diff };
    }
    const shift = Math.max(-0.08, Math.min(0.08, diff * 0.00004));
    const np0 = Math.max(0.05, Math.min(0.95, p0 - shift));
    return { p0: np0, p1: 1 - np0, shifted: true, liabilityDiff: diff, shift };
  }
