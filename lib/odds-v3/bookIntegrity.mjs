...
import {
  acceptedLineStillOpen,
  isOverSelection,
  isUnderSelection,
  lineScopedSelectionId,
  parseOuLine,
} from './lineIdentity.mjs';
...
function firstInningsRunsFromState(matchState) {
  if (!matchState || Number(matchState.currentInnings) !== 2) return null;
  const bowling = matchState.bowlingTeamId === matchState.team1?.id
    ? matchState.team1
    : matchState.team2;
  const runs = Number(bowling?.runs);
  return Number.isFinite(runs) ? runs : null;
}

function overUnderPair(market) {
  const sels = market?.selections || [];
  const over = sels.find((s) => isOverSelection(s.selectionId, s.name));
  const under = sels.find((s) => isUnderSelection(s.selectionId, s.name));
  return over && under ? { over, under } : null;
}

/**
 * In a chase, match total = first-innings runs + batting-team total.
 * Copy team_total prices onto match_total so liability/model drift cannot
 * print two different books on the same event.
 */
export function alignChaseTotalMarkets(markets = [], matchState = null) {
  const firstRuns = firstInningsRunsFromState(matchState);
  if (firstRuns == null) return markets;

  const teamTotal = markets.find((m) => m.marketId === 'team_total' && m.status === 'OPEN');
  const pair = overUnderPair(teamTotal);
  const teamLine = Number(teamTotal?.line);
  if (!pair || !Number.isFinite(teamLine)) return markets;

  const lockedLine = firstRuns + teamLine;

  return markets.map((market) => {
    if (market.marketId !== 'match_total' || market.status !== 'OPEN') return market;
    return cloneMarket(market, {
      line: lockedLine,
      chaseLockedTo: 'team_total',
      selections: [
        {
          ...pair.over,
          selectionId: lineScopedSelectionId('over', lockedLine),
          name: `Over ${lockedLine}`,
        },
        {
          ...pair.under,
          selectionId: lineScopedSelectionId('under', lockedLine),
          name: `Under ${lockedLine}`,
        },
      ],
    });
  });
...
export function hideUnpriceableMarkets(markets = []) {
  return markets.filter((market) => {
    ...
    // Keep match winner visible even when locked so the grid does not look empty.
    if (market.marketId === 'match_winner') {
      return market.status !== 'SETTLED' && market.status !== 'DETERMINED';
    }
...
export function applyBookIntegrity(markets = [], matchState = null) {
  const aligned = alignWinnerMarkets(markets);
  const risked = applyLiabilityToTwoWayMarkets(aligned);
  const chaseLocked = alignChaseTotalMarkets(risked, matchState);
  return hideUnpriceableMarkets(suspendLockMarkets(chaseLocked));
}
