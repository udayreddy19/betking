...
function mapSport(name = '') {
  ...
  if (n.includes('snooker')) return 'snooker';
  if (n.includes('handball') || n.includes('andebol')) return 'handball';
  if (n.includes('esport') || n.startsWith('e')) return 'esports';
  return n.replace(/\s+/g, '-') || 'cricket';
}

/** Finished / resulted book events — used so settlement can declare match_winner. */
export function isTencricEventFinished(event) {
  const status = String(event?.eventStatus || '');
  const phase = String(event?.eventPhase?.description || event?.eventPhase?.showPhase || '');
  const blob = `${status} ${phase}`.trim();
  if (/not.?start|scheduled|pre.?match|upcoming|to.?start|\bns\b/i.test(blob)) return false;
  if (/finish|completed|ended|closed|settled|resulted|\bft\b|full.?time|\bfinal\b/i.test(blob)) {
    return true;
  }
  // Never treat "not live + scores" alone as finished — innings breaks / book glitches
  // left isLive=false mid-match and falsely COMPLETED fixtures (settlement risk).
  if (event?.isLive === true) return false;
  if (/in.?play|live|half|break|interval|innings|paused|suspended|timeout/i.test(blob)) {
    return false;
  }
  return false;
}
...
export function mapTencricEvent(event) {
...
function mapEvent(event) {
  ...
  const finished = isTencricEventFinished(event);
  ...
  const mapped = {
    ...
    time: finished ? 'Completed' : (isLive ? 'Live' : 'Scheduled'),
    isLive,
    isCompleted: finished,
    matchState: finished ? 'post' : (isLive ? 'in' : 'pre'),
    status: finished ? 'COMPLETED' : (isLive ? 'LIVE' : 'SCHEDULED'),
  ...
  if (finished || isCricketMatchCompleted(mapped)) {
    mapped.isLive = false;
    mapped.isCompleted = true;
    mapped.matchState = 'post';
    mapped.time = 'Completed';
    mapped.status = 'COMPLETED';
    if (homeRuns > awayRuns) mapped.winnerSide = '1';
    else if (awayRuns > homeRuns) mapped.winnerSide = '2';
    else if (!isCricketEvent) mapped.winnerSide = 'X';
  }
...
async function findLiveListEvent(eventId) {
  ...
  const chunks = await Promise.allSettled(
    sports.map((sport) => listEvents('WIDGET_TYPE_LIVE_EVENTS', sport.id)),
  );
