...
function mapEvent(event) {
  ...
  const isSecond = /second/i.test(phase) && !/first/i.test(phase);
  let homeRuns = num(event.totalHomeScore);
  let awayRuns = num(event.totalAwayScore);
  ...
  const battingHome = event.home?.isBatting === true
    || (event.away?.isBatting === true
      ? false
      : ((isSecond && event.away?.isBatting !== true) || (!isSecond && event.away?.isBatting !== true)));

  if (!isSecond && homeRuns > 0 && homeRuns === awayRuns) {
    if (battingHome) {
      awayRuns = 0;
      awayWkts = 0;
    } else {
