...
/**
 * Process-local HTTP counter breakdown (not individual request bodies — aggregates by method/route/status).
 * Used by Production Health tile drill-down.
 */
export function getHttpCounterBreakdown({
  statusMin = 0,
  statusMax = 599,
  limit = 100,
  sortBy = 'count',
  ...
  const rows = [];
  for (const [k, v] of counters.entries()) {
    const [method, route, statusStr] = k.split('|');
    const status = Number(statusStr);
    if (status < statusMin || status > statusMax) continue;
    const count = Number(v.count) || 0;
    const totalMs = Number(v.totalMs) || 0;
    rows.push({
      id: k,
      method,
      route,
  ...
  const sorter = sortBy === 'avgMs'
    ? (a, b) => b.avgMs - a.avgMs || b.count - a.count
    : (a, b) => b.count - a.count || b.avgMs - a.avgMs;
  rows.sort(sorter);
  return {
    success: true,
    source: 'process_local_http_counters',
    note: 'Aggregated since process start on this node. Not a full access log of every request.',
    totalKeys: rows.length,
    rows: rows.slice(0, Math.min(500, Math.max(1, Number(limit) || 100))),
  };
}
