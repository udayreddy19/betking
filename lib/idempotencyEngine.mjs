...
export class IdempotencyEngine {
  ...
  async checkOrLock(idempotencyKey, operationType = 'generic', requestHash = '', userId = null) {
    ...
    // Fast memory cache check — if hash missing but caller provided one, verify via DB
    if (this.memoryCache.has(idempotencyKey)) {
      const cached = this.memoryCache.get(idempotencyKey);
      if (
        requestHash
        && cached.requestHash
        && String(cached.requestHash) !== String(requestHash)
      ) {
        const err = new Error('IDEMPOTENCY_KEY_REUSE: Idempotency key already used with a different request payload');
        err.code = 'IDEMPOTENCY_KEY_REUSE';
        err.statusCode = 409;
        throw err;
      }
      if (requestHash && !cached.requestHash) {
        // Fall through to DB to enforce payload hash when memory entry lacks it
      } else {
        return {
          isDuplicate: true,
    ...
    try {
      ...
      const dbCheck = await query(`
        SELECT key, operation_type, status, result, error, request_hash, created_at, completed_at
      ...
      if (dbCheck.rows.length > 0) {
        ...
        const storedHash = row.request_hash != null
          ...
          : null;
        // Same key + different payload must never replay as success
        if (
          requestHash
          && storedHash
          && storedHash.length > 0
          && storedHash !== String(requestHash)
        ) {
          const err = new Error('IDEMPOTENCY_KEY_REUSE: Idempotency key already used with a different request payload');
          err.code = 'IDEMPOTENCY_KEY_REUSE';
          err.statusCode = 409;
      ...
      if (insertRes.rows.length === 0) {
        ...
        if (existing.rows.length > 0) {
          ...
          const storedHash = row.request_hash != null ? String(row.request_hash) : null;
          if (
            requestHash
            && storedHash
            && storedHash.length > 0
  ...
  async complete(idempotencyKey, result = {}) {
    ...
    try {
      ...
      const prior = this.memoryCache.get(idempotencyKey) || {};
      const updated = {
        idempotencyKey,
        status: 'COMPLETED',
        result,
        requestHash: prior.requestHash || null,
        completedAt: now,
      };
