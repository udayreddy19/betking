...
function mapSrlDeskMatch(match) {
  ...
  const row = {
    ...
    squads: Array.isArray(match.squads) && match.squads.length
      ? match.squads
      : [
        getSrlTeamRoster(match.team1?.key) || { name: match.team1?.name, players: [] },
        getSrlTeamRoster(match.team2?.key) || { name: match.team2?.name, players: [] },
      ].filter(Boolean),
    // Always resolve by competing team keys so every fixture (all 10 clubs + custom) auto-fills.
    defaultHomeXI: (() => {
      const fromKey = getSrlPlayingXINames(match.team1?.key || match.team1?.shortName);
      if (fromKey.length) return fromKey;
      return (match.squads?.[0]?.players || []).slice(0, 11).map((p) => p.name).filter(Boolean);
    })(),
    defaultAwayXI: (() => {
      const fromKey = getSrlPlayingXINames(match.team2?.key || match.team2?.shortName);
      if (fromKey.length) return fromKey;
      return (match.squads?.[1]?.players || []).slice(0, 11).map((p) => p.name).filter(Boolean);
    })(),
    ...
    msToStart: Number.isFinite(Number(match.startTime)) ? Number(match.startTime) - Date.now() : null,
  };
  ...
  row.needsSettlement = (controlStatus === 'COMPLETED' || match.matchState === 'post') && !mwSettled;
