...
export async function expireSpinGrants(exec, userId) {
  ...
  const forfeitTotal = Number((bonusDeduct + freebetDeduct).toFixed(2));
  if (forfeitTotal > 0) {
    const txId = `tx_spin_exp_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`;
    await q(
      `INSERT INTO transactions (transaction_id, user_id, type, method, amount, status)
       VALUES ($1, $2, 'BONUS_FORFEIT', 'SPIN_EXPIRED', $3, 'COMPLETED')
       ON CONFLICT DO NOTHING`,
      [txId, userId, forfeitTotal],
    );
    const afterBal = Number(
      (bonusDeduct > 0
        ? Number(wallet.bonus_balance || 0) - bonusDeduct
        : Number(wallet.freebet_balance || 0) - freebetDeduct).toFixed(2),
    );
    await q(
      ...
      [
        ...
        forfeitTotal,
        Math.max(0, afterBal),
        `Spin prize expired · bonus ₹${bonusDeduct.toFixed(2)} · freebet ₹${freebetDeduct.toFixed(2)}`,
      ],
    );
  }

  // Expire mirrored daily-spin vouchers that can no longer be funded.
  try {
    await q(
      `UPDATE user_rewards
       ...
       WHERE user_id = $1
         ...
         AND UPPER(COALESCE(source, '')) = 'DAILY_SPIN'
         AND (
           ($2::numeric > 0 AND reward_type = 'bonus')
           OR ($3::numeric > 0 AND reward_type = 'freebet')
         )`,
      [userId, bonusDeduct, freebetDeduct],
    );
  } catch {
    // user_rewards may be missing on older envs
  }
