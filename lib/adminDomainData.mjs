...
export async function countBetsBySettlement() {
  const snap = await liveSnapshot();
  const liveIds = collectLiveMatchIdAliases(snap?.matches || []);
  const marketRe = LIVE_SETTLE_MARKET_RE.source;

  const pendingRes = await q(
    `SELECT COUNT(*)::int AS c FROM bets WHERE UPPER(status) = ANY($1::text[])`,
    [OPEN_BET_STATUSES],
  );
  const completedRes = await q(
    `SELECT COUNT(*)::int AS c FROM bets
     WHERE UPPER(status) = ANY($1::text[]) OR settled_at IS NOT NULL`,
  ...
  if (liveIds.length) {
    liveRes = await q(
      `SELECT COUNT(*)::int AS c FROM bets b
       WHERE UPPER(b.status) = ANY($1::text[])
         AND (
  ...
  const allRes = await q(`SELECT COUNT(*)::int AS c FROM bets`);
  ...
  return {
    ...
    source: (pendingRes.error || completedRes.error || liveRes.error || allRes.error) ? 'error' : 'database',
    note: schemaNote(pendingRes.error || completedRes.error || liveRes.error || allRes.error),
  };
}
