...
export function isSeoIndexingEnabled() {
  const envVal = process.env.SEO_INDEXING_ENABLED;
  if (envVal === undefined || envVal === null || envVal === '') return true;
  return envVal === 'true' || envVal === '1';
}

/** When false (default), sportsbook paths stay noindex/out of sitemap even if private mode is off. */
export function isSportsbookSeoEnabled() {
  const envVal = process.env.SPORTSBOOK_SEO_ENABLED;
  return envVal === 'true' || envVal === '1';
}
