...
export async function listPendingWithdrawals({ limit = 100 } = {}) {
  ...
  try {
    ({ evaluateBeneficiaryKycMatch } = await import('./beneficiaryKycNameMatch.mjs'));
  } catch {
...
export async function lookupWithdrawalBeneficiaryName(withdrawalId) {
  ...
  const bankDetails = row.bankDetails || {};
  const declaredName = extractDeclaredAccountHolderFromBankDetails(bankDetails);
  const detailsRaw = typeof bankDetails === 'object'
    ? (bankDetails.details || '')
    : '';
  const method = (typeof bankDetails === 'object' && bankDetails.method)
    || 'BANK';
