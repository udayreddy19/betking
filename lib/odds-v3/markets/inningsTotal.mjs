...
export function generateExtendedInningsTotals(state, validation = {}, marginConfig = {}) {
  ...
  const altLineLow = Math.max(currentScore + 0.5, mainLine - 10.0);

  const markets = [];

  // 1. Team Innings High Alternate Line
  const { pOver: pHighOver, pUnder: pHighUnder } = calculateOverUnderProbability(expectedTeamTotal, altLineHigh);
  ...
  // 2. Team Innings Low Alternate Line
  const { pOver: pLowOver, pUnder: pLowUnder } = calculateOverUnderProbability(expectedTeamTotal, altLineLow);
  markets.push(createMarketDefinition({
