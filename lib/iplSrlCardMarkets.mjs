...
function oddsForTeam(selections, team) {
  const tid = team?.id;
  const name = String(team?.name || '').toLowerCase();
  const hit = (selections || []).find((s) => {
    if (tid && String(s.selectionId) === `sel_${tid}`) return true;
    if (name && String(s.name || '').toLowerCase() === name) return true;
    return false;
  });
...
export function enrichIplSrlMatchCard(match) {
  ...
  try {
    ...
    if (mwOpen?.selections?.length >= 2) {
      const o1 = oddsForTeam(mwOpen.selections, match.team1 || state.team1);
      const o2 = oddsForTeam(mwOpen.selections, match.team2 || state.team2);
      if (o1 > 1 && o2 > 1) {
        next.odds = {
          ...(match.odds || {}),
          team1: o1,
