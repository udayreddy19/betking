...
export function evaluateTotalsMarketBet(bet, match) {
  ...
  if (inningsScoped) {
    ...
    const firstInningsComplete = inn === 1 && isInningsComplete(match, 1, bat, ld);
    const chaseInningsComplete = inn === 2 && isInningsComplete(match, 2, bat, ld);
    ...
    if (inn === 2 && !chaseInningsComplete && bat.innings === 2) {
  ...
  if (market === 'team_total' || market.startsWith('team_total_alt_')) {
    ...
    const firstDone = isInningsComplete(match, 1, bat, ld);
    if (!firstDone) {
