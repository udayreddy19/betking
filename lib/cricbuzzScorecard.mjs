...
export function enrichLivePlayersFromScorecard(liveDetails = {}, scorecardInnings = []) {
  ...
  if (matchBatter1) {
    next.batter1 = {
      ...
      runs: Math.max(matchBatter1.runs ?? 0, next.batter1?.runs ?? 0),
      balls: Math.max(matchBatter1.balls ?? 0, next.batter1?.balls ?? 0),
      fours: Math.max(matchBatter1.fours ?? 0, next.batter1?.fours ?? 0),
      sixes: Math.max(matchBatter1.sixes ?? 0, next.batter1?.sixes ?? 0),
    };
  }
  ...
  if (matchBatter2) {
    next.batter2 = {
      ...
      runs: Math.max(matchBatter2.runs ?? 0, next.batter2?.runs ?? 0),
      balls: Math.max(matchBatter2.balls ?? 0, next.batter2?.balls ?? 0),
      fours: Math.max(matchBatter2.fours ?? 0, next.batter2?.fours ?? 0),
      sixes: Math.max(matchBatter2.sixes ?? 0, next.batter2?.sixes ?? 0),
    };
  }
