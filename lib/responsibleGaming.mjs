/**
 * Responsible gaming: cooling-off / self-exclusion, plus hard deposit / stake /
 * loss limit enforcement on money paths.
 * validateDepositAttempt and validateBetPlacementAttempt reject when used+amount
 * would exceed a configured (finite, >0) limit. RG_UNAVAILABLE fails closed
 * (thrown) so deposit/bet engines cannot proceed without a successful check.
 */
...
class ResponsibleGamingEngine {
  ...
  async validateDepositAttempt(userId, depositAmount) {
    ...
    const periods = [
      { key: 'day', limit: limits.depositLimitDaily, reason: 'DEPOSIT_LIMIT_DAILY' },
      { key: 'week', limit: limits.depositLimitWeekly, reason: 'DEPOSIT_LIMIT_WEEKLY' },
      { key: 'month', limit: limits.depositLimitMonthly, reason: 'DEPOSIT_LIMIT_MONTHLY' },
    ];
    for (const { key, limit, reason } of periods) {
      const lim = Number(limit);
      if (!Number.isFinite(lim) || lim <= 0) continue;
      const used = await this.sumPaidDepositsSince(userId, istPeriodStart(key));
      if (used + amount > lim) {
        const details = { reason, limit: lim, used, amount };
        this.auditLimitViolation(userId, reason, details);
  ...
  async validateBetPlacementAttempt(userId, stakeAmount, _opts = {}) {
    ...
    const stakeCap = Number(limits.stakeLimitPerBet);
    if (Number.isFinite(stakeCap) && stakeCap > 0 && stake > stakeCap) {
      const details = { reason: 'STAKE_LIMIT_PER_BET', limit: stakeCap, amount: stake };
      this.auditLimitViolation(userId, details.reason, details);
      return { allowed: false, ...details };
    }

    const lossPeriods = [
      { key: 'day', limit: limits.lossLimitDaily, reason: 'LOSS_LIMIT_DAILY' },
      { key: 'week', limit: limits.lossLimitWeekly, reason: 'LOSS_LIMIT_WEEKLY' },
    ];
    for (const { key, limit, reason } of lossPeriods) {
      const lim = Number(limit);
      if (!Number.isFinite(lim) || lim <= 0) continue;
      const used = await this.getNetCashLoss(userId, istPeriodStart(key));
      // Worst-case: treat this stake as additional cash loss.
      if (used + stake > lim) {
        const details = { reason, limit: lim, used, amount: stake };
        this.auditLimitViolation(userId, reason, details);
        return { allowed: false, ...details };
      }
    }
