...
import { getIplSrlDeskMatches, isIplSrlMatch } from './iplSrlSimulator.mjs';
import {
...
function mapSrlDeskMatch(match) {
  ...
  return {
    ...
    venue: match.league || 'BetKing SRL',
...
export function triggerDelivery(matchId, { admin } = {}) {
  if (isUserFacingSrlMatch(matchId)) {
    throw new Error('This match runs on the live clock. Pause, resume, or declare a winner instead of triggering deliveries.');
  }
  const m = getMatchOrThrow(matchId);
