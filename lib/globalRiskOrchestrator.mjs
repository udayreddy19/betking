...
class GlobalRiskOrchestrator {
  ...
  async evaluateBetRequest({
    ...
    // 4. Market Liability — authoritative Postgres open-bets only (unless already checked upstream)
    if (!skipMemExposure) {
      const { assertPersistedMatchLiabilityCapacity } = await import('./persistedMarketLiability.mjs');
      const { liabilityLimitForMarket } = await import('./houseProtectionEngine.mjs');
      try {
        ...
        return this.recordDecision({
          domain: 'BET',
          decision: 'REJECT',
          reason: err.message || 'Stake exceeds maximum liability capacity for market',
          reasonCode: err.reasonCode || err.code || 'EVENT_EXPOSURE_LIMIT',
          maxAllowedStake: Math.max(10, Math.floor((err.remainingCapacity || 0) / (serverOdds || clientOdds || 1))),
          userId, matchId, marketId, selectionId, stake, clientOdds, serverOdds, timestamp,
        });
      }
    }

    // Do NOT mutate mem exposure here — that would create a second financial path.
    // Exposure caches are updated only after bet commit via recordAcceptedBetLiability.
