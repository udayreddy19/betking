  ...
  try {
    const ledgerRes = await query(`
      SELECT 
        ...
        COALESCE(SUM(CASE WHEN direction = 'CREDIT' OR entry_type = 'CREDIT' THEN amount ELSE 0 END), 0) AS sum_credits,
        COALESCE(SUM(CASE WHEN direction = 'DEBIT' OR entry_type = 'DEBIT' THEN amount ELSE 0 END), 0) AS sum_debits
      FROM ledger_entries
    `);
  ...
  try {
    const userDriftRes = await query(`
      SELECT 
        ...
        COALESCE(SUM(CASE WHEN l.direction = 'DEBIT' OR l.entry_type = 'DEBIT' THEN l.amount ELSE 0 END), 0) AS ledger_computed_balance
      FROM wallets w
