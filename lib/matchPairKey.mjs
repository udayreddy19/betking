...
function isCodeLikeTeamToken(token = '') {
  const t = String(token || '').trim();
  return t.length >= 2 && t.length <= 4 && !/\s/.test(t);
}
...
/**
 * Pair-key candidates so CREX short codes (MUT/IAI) merge onto FanCode/10Cric
 * full names that carry the same shortName.
 */
export function getMatchPairKeyCandidates(match) {
  const primary = getCanonicalMatchPairKey(match);
  const keys = [primary];
  const shortA = normalizeTeamNameForPair(match?.team1?.shortName || '');
  const shortB = normalizeTeamNameForPair(match?.team2?.shortName || '');
  const nameA = normalizeTeamNameForPair(
    match?.team1?.name || match?.homeTeam?.teamName || match?.homeTeam?.name || '',
  );
  const nameB = normalizeTeamNameForPair(
    match?.team2?.name || match?.awayTeam?.teamName || match?.awayTeam?.name || '',
  );

  if (shortA && shortB && shortA !== shortB) {
    keys.push(`m|short|${[shortA, shortB].sort().join('|')}`);
  }
  if (isCodeLikeTeamToken(nameA) && isCodeLikeTeamToken(nameB) && nameA !== nameB) {
    keys.push(`m|short|${[nameA, nameB].sort().join('|')}`);
  }
