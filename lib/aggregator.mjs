...
import { fetchCricbuzzMatchDetailCached, enrichMatchWithDetail } from './cricbuzzMatchDetail.mjs';
...
export async function aggregateLiveScores(opts = {}) {
  ...
  // Enrich live cricket matches with real-time detail (scores + player names)
  const liveCricket = matches.filter(
    (m) => m.isLive && m.sport === 'cricket' && m.cricbuzzMatchId,
  );

  if (liveCricket.length > 0) {
    await Promise.allSettled(
      liveCricket.map(async (m) => {
        try {
          const detail = await fetchCricbuzzMatchDetailCached(m.cricbuzzMatchId);
          const idx = matches.indexOf(m);
          if (idx >= 0) matches[idx] = enrichMatchWithDetail(m, detail);
        } catch (err) {
          console.warn(`[Aggregator] Detail enrich failed for ${m.cricbuzzMatchId}:`, err.message);
        }
      }),
    );
  }
