...
export function extractProviderOdds(match) {
  // Never treat our own V3 attach as an external book.
  if (String(match?.oddsSource || '') === 'OddsEngineV3') return null;
  const o = match?.providerOdds || match?.preOdds || match?.marketReferenceData?.providerOdds || match?.odds || {};
