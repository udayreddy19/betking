...
function buildScorecardInnings(teamName, inningsData, inningsId) {
  ...
  return {
    ...
    // Snapshot / normalizers read `.runs` — keep in sync with `.score`.
...
function buildOverHistory(timeline, startBall = 0) {
  ...
  timeline.forEach((frame, idx) => {
    ...
    const endOfOver = current.length === 6;
    const lastFrame = idx === timeline.length - 1;
    if (endOfOver || lastFrame) {
      const striker = frame.batters?.[frame.strikerIdx];
      const nonStriker = frame.batters?.[frame.nonStrikerIdx];
      const batters = [striker?.name, nonStriker?.name].filter(Boolean);
      rows.push({
        overNum,
        balls: [...current],
        isCurrent: lastFrame && !endOfOver,
        runs: Number(frame.runs) || 0,
        wickets: Number(frame.wickets) || 0,
        scoreAtEnd: `${Number(frame.runs) || 0}-${Number(frame.wickets) || 0}`,
        bowler: frame.bowler?.name || null,
        batters,
        commentary: frame.bowler?.name
          ? `${frame.bowler.name} to ${batters.join(' & ') || 'batters'}`
          : null,
        ballIndexStart: overStartIdx,
        ballIndexEnd: idx,
      });
      current = [];
      if (endOfOver) {
  ...
  if (rows.length) {
    ...
    // Completed final over is confirmed history; only a partial over is "current".
    last.isCurrent = Array.isArray(last.balls) && last.balls.length < 6;
  }
  return rows;
}

function currentOverBallDisplays(frame) {
  const displays = Array.isArray(frame?.ballDisplays) ? frame.ballDisplays : [];
  if (!displays.length) return [];
  const ballsBowled = displays.length;
  const intoOver = ballsBowled % 6;
  const take = intoOver === 0 ? 6 : intoOver;
  return displays.slice(-take);
}
...
export function syncSrlWicketsToFallOfWicket(live) {
  ...
  // Only sync chase wickets once the chase has really started — wickets2:0 from a
  // first-innings stub must not invent chaseWickets / a second innings.
  const chaseOvers = out.chaseOvers ?? out.overs2;
  const chaseOversEmpty = chaseOvers == null || chaseOvers === '' || chaseOvers === 0
    || chaseOvers === '0' || chaseOvers === '0.0';
  const chaseLive = !!inns?.[1]
    || out.phase === 'chase'
    || out.phase === 'chase-complete'
    || Number(out.inningsId) === 2
    || Number(out.chaseRuns) > 0
    || (Number(out.chaseWickets) > 0 && !chaseOversEmpty);
  if (chaseLive) {
...
function stateAtElapsed(sim, elapsedMs, team1, team2) {
  ...
  if (elapsedMs < firstInningsEndMs) {
    ...
    return {
      ...
      // Do NOT emit score2/wickets2/overs2 during 1st inns — zeros look like a started chase
      // and UIs mirror 4/1 : 4/1 or show "need N runs" / false COMPLETED.
      ...
      currentOverBalls: inningsComplete ? [] : currentOverBallDisplays(frame),
      ...
      overHistory: buildOverHistory(first.timeline.slice(0, ballIdx + 1)).map((r) => ({ ...r, inningsId: 1 })),
      inningsPlayMs,
    };
  }
  ...
  if (elapsedMs < breakEndMs) {
    return {
      ...
      overHistory: buildOverHistory(first.timeline).map((r) => ({ ...r, inningsId: 1, isCurrent: false })),
    };
  }
  ...
  return {
    ...
    // Keep 1st-innings ball history so i1_next_delivery_* still settles during the chase.
    overHistory: [
      ...buildOverHistory(first.timeline || []).map((r) => ({ ...r, inningsId: 1, isCurrent: false })),
      ...buildOverHistory(second.timeline.slice(0, ballIdx + 1)).map((r) => ({ ...r, inningsId: 2 })),
    ],
  };
}
...
/**
 * Who wins from the live chase board (ignores scripted/declared overrides).
 * Uses batting order — not fixture home/away — so tossed batting first is correct.
 */
export function resolveChaseResultWinnerKey(live, batFirst, chase, dlsTarget = null) {
  if (!live || !batFirst?.key || !chase?.key) return null;
  const inChase = Number(live.inningsId) === 2
    || live.phase === 'chase'
    || live.phase === 'chase-complete';
  if (!inChase && live.phase !== 'chase-complete') return null;

  const firstRuns = Number(live.firstRuns ?? live.runs);
  const chaseRuns = Number(live.chaseRuns ?? live.score2);
  const chaseWickets = Number(live.chaseWickets ?? live.wickets2 ?? 0);
  const target = Number(dlsTarget ?? live.target);
  const effectiveTarget = Number.isFinite(target) && target > 0
    ? target
    : (Number.isFinite(firstRuns) ? firstRuns + 1 : null);

  if (Number.isFinite(effectiveTarget) && Number.isFinite(chaseRuns) && chaseRuns >= effectiveTarget) {
    return chase.key;
  }
  if (chaseWickets >= 10 || live.phase === 'chase-complete') {
    return batFirst.key;
  }
  return null;
}

/** True when the chase is over (target reached, all out, or natural chase timeline done). */
export function isSrlChaseFinished(live, dlsTarget = null) {
  if (!live) return false;
  if (live.phase === 'chase-complete') return true;
  const inChase = Number(live.inningsId) === 2 || live.phase === 'chase';
  ...
  return chaseWickets >= 10;
}

/**
 * Freeze a chase board once the target is met so overs cannot keep advancing
 * via clock seek / score-anchor rebase while the match is still "live".
 */
export function freezeChaseAtTarget(live, dlsTarget = null) {
  if (!live || !isSrlChaseFinished(live, dlsTarget)) return live;
  const firstRuns = Number(live.firstRuns ?? live.runs);
  const chaseRuns = Number(live.chaseRuns ?? live.score2);
  const chaseWickets = Number(live.chaseWickets ?? live.wickets2 ?? 0);
  const target = Number(dlsTarget ?? live.target);
  const effectiveTarget = Number.isFinite(target) && target > 0
    ? target
    : (Number.isFinite(firstRuns) ? firstRuns + 1 : null);
  const reached = Number.isFinite(effectiveTarget) && chaseRuns >= effectiveTarget;
  const name = live.chaseTeamName || 'Chase';
  return {
    ...live,
    ...
    commentary: reached
      ? `${name} ${chaseRuns}/${chaseWickets} — target reached`
      : `${name} ${chaseRuns}/${chaseWickets} — innings complete`,
  };
}
...
function forceCompletedBoard(sim, template, fixtureTeam1, fixtureTeam2, winnerKey, live = null, batOrder = null) {
  ...
  let chaseRuns = live?.chaseRuns ?? live?.score2;
  let chaseWickets = live?.chaseWickets ?? live?.wickets2;
  let chaseOvers = live?.chaseOvers ?? live?.overs2;
  // First-innings stubs used to emit score2:0 — treat that as "chase not started".
  const chaseNotStarted = live
    && (live.phase === 'first' || live.phase === 'first-complete' || live.phase === 'break' || Number(live.inningsId) === 1)
    && (chaseRuns == null || (Number(chaseRuns) === 0 && (chaseWickets == null || Number(chaseWickets) === 0)));
  if (chaseNotStarted || chaseRuns == null || !Number.isFinite(Number(chaseRuns))) {
    chaseRuns = sim.second.runs;
    chaseWickets = sim.second.wickets;
    chaseOvers = sim.second.overs;
  } else {
    chaseRuns = Number(chaseRuns);
    chaseWickets = Number(chaseWickets ?? sim.second.wickets) || 0;
    chaseOvers = chaseOvers ?? sim.second.overs;
  }

  // Winner must follow batting order: defending side wins by runs, chasing side by wickets.
  // (Fixture home/away alone is wrong when toss flips who bats first.)
  const winnerIsBatFirst = winnerKey === batFirst.key
    || (winnerKey !== chase.key && winnerKey === template.team1Key && batFirst.key === fixtureTeam1.key);
  const winnerIsChase = winnerKey === chase.key
    || (!winnerIsBatFirst && winnerKey === template.team2Key && chase.key === fixtureTeam2.key);

  if (winnerIsBatFirst && chaseRuns >= firstRuns) {
    chaseRuns = Math.max(0, firstRuns - 1);
  } else if (winnerIsChase && chaseRuns < firstRuns + 1) {
    chaseRuns = firstRuns + 1;
    chaseWickets = Math.min(9, Number(chaseWickets) || 0);
  }

  const winnerTeam = winnerIsChase ? chase.name : batFirst.name;
  const margin = winnerIsChase
    ? `won by ${10 - chaseWickets} wickets`
    : `won by ${Math.max(1, firstRuns - chaseRuns)} runs`;
...
function buildMatchFromSchedule(globalSlot, now) {
  ...
  const startTime = EPOCH_MS + (cycleNumber * SCHEDULE.cycleMs) + entry.offsetMs;
  // Toss resolves at start: winner bats if team1, bowls if team2 (seed).
  ...
  const { batFirst, chase, effectiveToss } = resolveSrlBatFirstTeams(team1, team2, toss, op.toss);
  const boardToss = effectiveToss || toss;
  // Template `sim` always bats fixture team1 first. When toss sends the other side
  // in first, re-sim with bat-first/chase keys so scorecard players and totals match.
  const boardSim = (teamsLocked && batFirst?.key && batFirst.key !== team1.key)
    ? simulateFullMatch(entry.seed, batFirst.key, chase.key)
    : sim;
  const naturalWinner = teamsLocked ? boardSim.winner : null;
  ...
  const base = {
    ...
    matchFormat: 'T20',
    oversPerInnings: MAX_OVERS,
    expectedDuration: formatDuration(boardSim.timing.totalDurationMs),
    expectedDurationMs: boardSim.timing.totalDurationMs,
    sport: 'cricket',
    sportColor: '#f97316',
    ...
    startTime,
    endTime: startTime + boardSim.totalDuration,
    scheduleLabel: formatScheduleTime(startTime),
    sim: { ...boardSim, winner: naturalWinner || boardSim.winner },
    operator: null,
    ...stageMeta,
  };
  ...
  // Operator sessions advance on wall clock; `now` here is season-jumped sim time.
  const elapsed = op.startedAt || op.pausedAt
    ? getSrlOperatorElapsedMs(op, Date.now())
    : clockElapsed;
  ...
  const completed = (liveSnapshot = null) => {
    const scoreboardWinner = resolveChaseResultWinnerKey(
      liveSnapshot,
      batFirst,
      chase,
      op.dlsTarget,
    );
    const winnerKey = op.declaredWinnerKey
      || op.forcedWinnerKey
      || scoreboardWinner
      || naturalWinner;
    const board = forceCompletedBoard(boardSim, template, team1, team2, winnerKey, liveSnapshot, batOrder);
    // Prefer frozen chase overs from the live snap so we don't invent full-20 history after an early chase end.
    const firstHist = buildOverHistory(boardSim.first?.timeline || []);
    const liveChaseHist = Array.isArray(liveSnapshot?.overHistory)
      ? liveSnapshot.overHistory.filter((r) => Number(r?.inningsId) === 2)
      : [];
    const secondHist = liveChaseHist.length
      ? liveChaseHist.map((r) => ({ ...r, inningsId: 2, isCurrent: false }))
      : buildOverHistory(boardSim.second?.timeline || []).map((r) => ({ ...r, inningsId: 2, isCurrent: false }));
    const overHistory = [
      ...firstHist.map((r) => ({ ...r, inningsId: 1, isCurrent: false })),
      ...secondHist,
    ];
    return {
      ...base,
      ...
      overHistory,
      scorecardInnings: [
        buildScorecardInnings(batFirst.name, { ...boardSim.first, runs: board.firstRuns, wickets: board.firstWickets, overs: board.firstOvers }, 1),
        buildScorecardInnings(chase.name, { ...boardSim.second, runs: board.chaseRuns, wickets: board.chaseWickets, overs: board.chaseOvers }, 2),
      ],
    };
  };

  if (op.declaredWinnerKey) {
    const snapElapsed = Math.min(elapsed, boardSim.totalDuration - 1);
    const liveSnap = snapElapsed > 0
      ? freezeChaseAtTarget(
        applySrlScoreAnchors(stateAtElapsed(boardSim, snapElapsed, batFirst, chase), boardSim, op.scoreAnchors),
        op.dlsTarget,
      )
      : null;
  ...
  if (elapsed >= boardSim.totalDuration) {
    const endSnap = freezeChaseAtTarget(
      applySrlScoreAnchors(stateAtElapsed(boardSim, boardSim.totalDuration - 1, batFirst, chase), boardSim, op.scoreAnchors),
      op.dlsTarget,
    );
    return completed(endSnap);
  }

  let live = applySrlScoreAnchors(stateAtElapsed(boardSim, elapsed, batFirst, chase), boardSim, op.scoreAnchors);
  if (op.dlsTarget) {
    live.target = op.dlsTarget;
  }
  live = freezeChaseAtTarget(live, op.dlsTarget);

  // Chase over (target / all out / timeline done) → end match immediately so
  // scripted winners apply and overs stop advancing on the desk.
  if (isSrlChaseFinished(live, op.dlsTarget)) {
 ...
 *
 * @param {number} [now]
 * @param {{ publicBoard?: boolean, forPublic?: boolean, simClock?: boolean, forDesk?: boolean, admin?: boolean }} [opts]
 *   Default is wall clock (ignore admin season-jump) so Sports/SRL always show
 *   real-calendar fixtures. Pass simClock/forDesk for admin desk / operator tools.
 */
export function getIplSrlMatches(now = Date.now(), opts = {}) {
  const useSimClock = opts.simClock === true
    || opts.forDesk === true
    || opts.admin === true
    || opts.publicBoard === false;
  const wallNow = Number(now) || Date.now();
  now = useSimClock ? getSrlSimNow(wallNow) : wallNow;
...
export function getIplSrlSeasonMatches(now = Date.now(), opts = {}) {
  const useSimClock = opts.simClock === true
    || opts.forDesk === true
    || opts.admin === true
    || opts.publicBoard === false;
...
export function getIplSrlPointsTable(now = Date.now(), opts = {}) {
  const useSimClock = opts.simClock === true
    || opts.forDesk === true
    || opts.admin === true
    || opts.publicBoard === false;
...
/**
 * Resolve a specific legal delivery label from the OddsYra SRL sim timeline.
 * Authoritative for settlement — does not depend on Cricbuzz / match_ball_events.
 */
export function getSrlDeliveryBallLabel(matchId, innings, overNum, ballNum, now = Date.now()) {
  const over = Number(overNum);
  const ball = Number(ballNum);
  const inn = Number(innings) >= 2 ? 2 : 1;
  if (!matchId || !Number.isFinite(over) || over < 1 || !Number.isFinite(ball) || ball < 1) {
    return null;
  }

  const match = getIplSrlMatchById(matchId, now, { forPublic: false });
  if (!match) return null;

  const history = match.overHistory || match.liveDetails?.overHistory || [];
  for (const row of history) {
    if (Number(row.overNum || row.over) !== over) continue;
    const rowInn = Number(row.inningsId || row.innings || 0);
    // If innings is tagged, require a match; untagged rows are allowed for legacy boards.
    if (rowInn > 0 && rowInn !== inn) continue;
    let legal = 0;
    for (const raw of (row.balls || [])) {
      const label = String(raw || '').trim();
      if (!label) continue;
      if (/wd|nb/i.test(label)) continue;
      legal += 1;
      if (legal === ball) return label;
    }
  }

  const sim = match.sim;
  const timeline = inn === 2 ? sim?.second?.timeline : sim?.first?.timeline;
  if (!Array.isArray(timeline) || !timeline.length) return null;
  const ballIndex = ((over - 1) * 6) + (ball - 1);
  if (ballIndex < 0 || ballIndex >= timeline.length) return null;
  const frame = timeline[ballIndex];
  const displays = Array.isArray(frame?.ballDisplays) ? frame.ballDisplays : [];
  return displays[ballIndex] ?? displays[displays.length - 1] ?? null;
}
...
export function getIplSrlMatchById(matchId, now = Date.now(), opts = {}) {
  ...
  const wallNow = Number(now) || Date.now();
  // Public detail follows wall calendar; admin/desk keep season-jump sim clock.
  const clock = forPublic ? wallNow : getSrlSimNow(wallNow);
  const custom = getCustomSrlMatch(matchId);
  const built = custom || (() => {
    const globalSlot = extractSrlGlobalSlot(matchId);
    if (globalSlot === null) return null;
    return buildMatchFromSchedule(globalSlot, clock);
  })();
  if (!built) return null;
  return forPublic ? redactSrlTossForPublic(built, clock) : built;
}
...
export function getIplSrlDeskMatches(now = Date.now()) {
  const season = getIplSrlSeasonMatches(now, { forDesk: true });
