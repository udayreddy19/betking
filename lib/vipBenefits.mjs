...
/**
 * Resolve VIP tier progress from lifetime vip_points (canonical thresholds).
 */
export function resolveVipTierProgress(lifetimePoints = 0) {
  ...
  const nextTier = idx >= 0 && idx < VIP_TIER_ORDER.length - 1 ? VIP_TIER_ORDER[idx + 1] : null;
  const nextFloor = nextTier ? VIP_TIER_POINTS[nextTier] : null;
  const currentFloor = VIP_TIER_POINTS[tier] || 0;
  const pointsToNextTier = nextFloor != null ? Math.max(0, nextFloor - pts) : 0;
  const span = nextFloor != null ? Math.max(1, nextFloor - currentFloor) : 1;
  const progressPct = nextFloor == null
    ? 100
    : Number((Math.min(100, ((pts - currentFloor) / span) * 100)).toFixed(1));

  return {
    tier,
    points: pts,
    cashbackPct: benefits.cashbackPct,
    cashoutBoostPct: Number(((benefits.cashoutPayoutPct - 0.85) * 100).toFixed(1)),
    nextTier,
    pointsToNextTier,
    progressPct,
  };
...
/**
 * Pure daily-streak calculator (calendar day in UTC date string).
 */
