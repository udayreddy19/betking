...
function normalizeOvers(value) {
  ...
  if (num >= 100) return (num / 5).toFixed(1);
...
function buildLiveDetails(info, matchScore) {
  const commentary = info.status || info.stateTitle || '';
  const team1Id = info.team1?.teamId;
  const team2Id = info.team2?.teamId;
  const currBatTeamId = info.currBatTeamId;

  const team1Innings = extractInnings(matchScore?.team1Score);
  const team2Innings = extractInnings(matchScore?.team2Score);

  let first = team1Innings;
  let second = team2Innings;

  if (!second && team1Innings && currBatTeamId && currBatTeamId !== team1Id) {
    second = team1Innings;
    first = null;
  }

  if (first && second && currBatTeamId === team2Id) {
    // team1 batted first, team2 batting now — default order is fine
  } else if (first && second && currBatTeamId === team1Id && team1Innings && team2Innings) {
  } else if (team1Innings && !team2Innings && currBatTeamId === team1Id) {
    second = team1Innings;
    first = team2Innings || { runs: 0, wickets: 0, overs: 0 };
  }

  const runs = first?.runs ?? 0;
  const wickets = first?.wickets ?? 0;
  const overs = normalizeOvers(first?.overs ?? 0);
  const score2 = second?.runs ?? 0;
  const wickets2 = second?.wickets ?? 0;
  const overs2 = normalizeOvers(second?.overs ?? 0);
...
function mapMatch(raw, matchType = 'League') {
  ...
  return {
    ...
    liveDetails: buildLiveDetails(info, raw.matchScore),
