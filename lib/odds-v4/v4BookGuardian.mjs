...
export function guardV4Book(markets = [], state, marginConfig = V4_MARGIN_CONFIG) {
  ...
  for (const market of markets || []) {
    ...
    // V4.8.5 soft-leak suspend — never publish fav / Over above house caps.
    ...
    const leakSels = openSels(next);
    const softFav = leakSels.find((s) => Number(s.probability) >= 0.48 && Number(s.odds) > maxFav + 0.001);
    const softOver = leakSels.find((s) => String(s.name).toLowerCase().startsWith('over')
    ...
    if (softFav || softOver || longLeak) {
      issues.push(`soft_leak:${market.marketId}`);
      continue;
    }
...
export function scoreV4Book({
  ...
  let stability = 7;
  if (compareEngineVersions(engineVersion, '4.2.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.3.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.4.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.5.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.6.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.7.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.8.0') >= 0) stability += 1;
  if (compareEngineVersions(engineVersion, '4.8.5') >= 0) stability += 1;
