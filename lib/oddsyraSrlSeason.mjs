...
export function getSrlHomeBanner(now = Date.now()) {
  if (!isSrlSeasonLive(now)) {
    return {
      ...
      subtitle: `Simulated cricket on OddsYra from ${SRL_LAUNCH_LABEL} — live matches, the full schedule, and in-play markets. No external feed.`,
      cta: 'View schedule',
    };
  }
  return {
    ...
    subtitle: `Season opened ${SRL_LAUNCH_LABEL}. Simulated cricket around the clock — matches, schedule, and betting.`,
    cta: 'Open SRL',
  };
}
