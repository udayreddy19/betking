...
function rgAutomatedTestBypass() {
  return (process.env.NODE_ENV === 'test' || process.env.VITEST === 'true')
    && process.env.ENFORCE_RG_CHECKS !== '1';
}
...
class ResponsibleGamingEngine {
  ...
  async validateDepositAttempt(userId, depositAmount) {
    ...
    const reality = await this.getRealityCheckState(userId);
    if (!rgAutomatedTestBypass() && reality.due) {
      return {
        allowed: false,
        reason: 'REALITY_CHECK_REQUIRED',
        message: 'Confirm the reality check before depositing.',
        intervalMins: reality.intervalMins,
      };
    }
  ...
  async validateBetPlacementAttempt(userId, stakeAmount, { fundSource = 'cash' } = {}) {
    ...
    if (!rgAutomatedTestBypass() && reality.due) {
      return {
        allowed: false,
        reason: 'REALITY_CHECK_REQUIRED',
        message: 'Confirm the reality check before placing another bet.',
        intervalMins: reality.intervalMins,
      };
    }
