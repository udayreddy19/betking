...
export function guardOsV4Book(markets = [], margins = OSV4_MARGIN_CONFIG) {
  const issues = [];
  const out = [];
  const maxOver = Number(margins.maxOverOdds) || 1.35;
  const maxYes = Number(margins.maxYesOdds) || 1.85;
  const maxFav = Number(margins.maxFavoriteOdds) || 1.55;
  const maxSel = Number(margins.maxSelectionOdds) || 3.5;
  ...
  for (const market of markets || []) {
    ...
    const longLeak = sels.some((s) => Number(s.odds) > maxSel + 0.001);
    if (longLeak) {
