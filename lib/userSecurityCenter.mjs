...
class UserSecurityCenter {
  ...
  async setAccountControlState(userId, {
    ...
    // Claw back referral rewards when account is hard-restricted / banned.
    if (['SUSPENDED', 'FROZEN', 'PERMANENT_CLOSED', 'BANNED'].includes(String(accountState || '').toUpperCase())) {
      void import('./referralLoyaltyEngine.mjs')
        .then(({ clawbackReferralRewards }) => clawbackReferralRewards({
          referredUserId: userId,
          reason: `account_${String(accountState).toLowerCase()}`,
