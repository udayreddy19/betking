...
export class DepositEngine {
  ...
  async createOrder({ userId, amount, currency = 'INR' }, correlationId = null) {
    ...
    let razorpayOrderId = `order_sim_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    ...
    if (key_id && key_secret) {
      try {
        ...
        console.warn('[DepositEngine Warning] Razorpay SDK fallback:', err.message);
      }
    }
  ...
  async processWebhook({ rawBody, signature, payload, event }, correlationId = null) {
    ...
    // 2. Idempotency Check Against Existing Payments & UTR
    ...
    // 3. Atomic Financial Commit inside PostgreSQL Transaction Boundary
    const result = await withTransaction(async (client) => {
      // Lock or Create User Wallet Row
      ...
      await client.query(
        ...
        [paymentId, userId, amountInINR, utr, method, paymentId, orderId || null]
      );
      ...
      await client.query(
        ...
        [`evt_${paymentId}`, paymentId, JSON.stringify({ userId, amount: amountInINR, paymentId, utr }), correlationId || null]
      );
