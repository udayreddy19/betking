...
/**
 * Deposit / bet require date of birth proving 18+ (KYC not required).
 * @param {string} userId
 * @param {{ action?: string }} [opts]
 */
export async function assertAdultDob(userId, { action = 'continue' } = {}) {
  ...
  try {
    const res = await query(
      `SELECT date_of_birth FROM user_profiles WHERE user_id = $1`,
      [userId],
    );
    dob = res.rows[0]?.date_of_birth || null;
  } catch {
    throw kycAgeGateError('Could not verify age. Try again.');
  }

  const age = ageFromDob(dob, new Date());
  if (age == null) {
    throw kycAgeGateError(`Add your date of birth before you can ${action}.`);
  }
  if (age < MIN_LEGAL_AGE) {
    throw kycAgeGateError(`You must be 18 or older to ${action}.`);
  }
