import { sumRunsFromBallLabels } from './cricketBallOutcome.mjs';
...
export async function recordMatchOverSnapshots(match) {
  ...
  if (parts.balls === 0 && parts.completed > 0) {
    let runsNote = `|runs:${bat.score}`;
    if (parts.completed > 1) {
      const prevScore = await getScoreAtOverEnd(matchId, parts.completed - 1, bat.innings);
      if (prevScore != null) runsNote = `|runs:${Math.max(0, bat.score - prevScore)}`;
    }
    await dbQuery(
      `INSERT INTO match_over_snapshots
       ...
       DO UPDATE SET
         ...
         recorded_at = NOW()`,
      [matchId, bat.innings, parts.completed, bat.score, bat.wickets, `${parts.raw}${runsNote}`],
    );
  }
  ...
  if (Array.isArray(history) && history.length) {
    for (const row of history) {
      ...
      // Never persist hist rows past the live overs clock (stale/cross-innings hist caused false VOIDs)
      if (overNum > parts.completed) continue;
      const rowInn = Number(row.inningsId || row.innings);
      if (Number.isFinite(rowInn) && rowInn > 0 && rowInn !== bat.innings) continue;
      ...
      await dbQuery(
        `INSERT INTO match_over_snapshots
         ...
         DO UPDATE SET
           score_at_end = EXCLUDED.score_at_end,
           -- Prefer live wickets; never keep a stale all-out (10) over a live 0
           wickets_at_end = EXCLUDED.wickets_at_end,
           overs_raw = EXCLUDED.overs_raw,
           recorded_at = NOW()`,
...
export function resolveOverHistoryRow(hist, overNum, settleInnings, currentBatInnings, completedOvers = null) {
  if (!Array.isArray(hist) || !hist.length) return null;
  const target = Number(overNum);
  const rows = hist.filter((h) => Number(h.overNum || h.over) === target);
  if (!rows.length) return null;

  const innMatch = rows.filter((h) => Number(h.inningsId || h.innings) === Number(settleInnings));
  if (innMatch.length) {
    return innMatch.find((h) => !h.isCurrent) || innMatch[innMatch.length - 1];
  }

  const unlabeled = rows.filter((h) => h.inningsId == null && h.innings == null);
  if (!unlabeled.length || Number(currentBatInnings) !== Number(settleInnings)) return null;

  const maxOver = Math.max(...hist.map((h) => Number(h.overNum || h.over) || 0));
  if (completedOvers != null && Number(settleInnings) >= 2 && maxOver > Number(completedOvers) + 2) {
    return null;
  }
  return unlabeled.find((h) => !h.isCurrent) || unlabeled[unlabeled.length - 1];
}

export function runsFromOverRow(row) {
  if (!row) return null;
  if (row.runs != null && Number.isFinite(Number(row.runs))) return Number(row.runs);
  if (Array.isArray(row.balls) && row.balls.length) {
    return sumRunsFromBallLabels(row.balls);
  }
  return null;
}
...
export async function getRunsInOver(matchId, overNum, innings = null) {
  ...
  const tagged = String(end.overs_raw || '').match(/\|runs:(\d+)/);
...
/**
 * Wickets fallen during over N (end of N minus end of N-1).
 * Returns null until over N snapshot exists.
 */
export async function getWicketsInOver(matchId, overNum, innings = null) {
  await ensureMatchOverSnapshotsTable();
  const id = String(matchId);
  const over = Number(overNum);
  if (!id || !Number.isFinite(over) || over <= 0) return null;
  ...
  const res = innings != null
    ? await dbQuery(
      `SELECT innings, over_num, wickets_at_end, overs_raw
       FROM match_over_snapshots
       WHERE match_id = $1 AND innings = $2 AND over_num IN ($3, $4)
       ORDER BY over_num`,
      [id, innings, over - 1, over],
  ...
  const endWkts = Number(end.wickets_at_end) || 0;
  if (over === 1) return Math.max(0, endWkts);

  const start = scoped.find((r) => Number(r.over_num) === over - 1);
  if (!start) {
    // No prior snapshot — still settle from absolute end if over is done
    return null;
  }
  const startWkts = Number(start.wickets_at_end) || 0;
  return Math.max(0, endWkts - startWkts);
}
