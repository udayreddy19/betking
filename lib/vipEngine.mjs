...
export async function getVipAdminDashboard() {
  ...
  const spendAgg = await query(
    `SELECT
       ...
       COALESCE(SUM(CASE WHEN UPPER(t.type) = 'BET_STAKE' AND UPPER(t.status) IN ('SUCCESS', 'COMPLETED') THEN t.amount ELSE 0 END), 0)::float AS total_stake_tx,
       COALESCE(SUM(CASE WHEN UPPER(t.type) = 'DEPOSIT' AND UPPER(t.status) IN ('SUCCESS', 'COMPLETED') THEN t.amount ELSE 0 END), 0)::float AS total_deposits_tx
...
export async function adminOverrideVipTier({
  ...
  return withTransaction(async (client) => {
    ...
    if (nextIdx > previousIdx) {
      tierUpRewards = await grantCrossedTierRewards(client, userId, previous, tier);
    }
    ...
    try {
      ...
      enterpriseAuditEngine.recordEvent?.({
        ...
        referenceId: userId,
        reason: auditReason,
        after: { previousTier: previous, newTier: tier, vipPoints: nextVip, tierUpRewards },
      });
    } catch {
      // optional audit sink
    }
