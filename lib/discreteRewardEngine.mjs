...
import { BONUS_MIN_BET_ODDS } from './promoRules.mjs';
...
function parseRewardMetadata(raw) {
  if (!raw) return {};
...
/** Sources that credit wallets.freebet/bonus AND insert a mirror row in user_rewards. */
export const WALLET_MIRRORED_REWARD_SOURCES = Object.freeze([
  'DAILY_SPIN',
  'DEPOSIT_FREEBET',
  'SIGNUP_PROMO',
]);

export function isWalletMirroredRewardSource(source) {
  return WALLET_MIRRORED_REWARD_SOURCES.includes(String(source || '').toUpperCase());
}

/**
 * Debit bonus/freebet bucket without creating a second BONUS_CLAIM transaction.
 * Used when the bet-stake ledger already records the money movement.
 */
export async function debitPromoWalletBucketSilent(client, {
  userId,
  rewardType,
  amount,
}) {
  if (!client) {
    throw new Error('DATABASE_TRANSACTION_REQUIRED: debitPromoWalletBucketSilent requires a transaction client');
  }
  const take = parseFloat(Number(amount || 0).toFixed(2));
  if (!(take > 0) || !['freebet', 'bonus'].includes(rewardType)) {
    return { debited: 0, nextBalance: null };
  }
  ...
  const isFreebet = rewardType === 'freebet';
  const current = Number(isFreebet ? wallet.freebet_balance : wallet.bonus_balance);
  const debited = Math.min(take, current);
  const next = parseFloat(Math.max(0, current - debited).toFixed(2));
  if (isFreebet) {
    await client.query(
  ...
  return { debited, nextBalance: next, walletId: wallet.wallet_id };
}

/**
 * When a freebet/bonus stake is taken from the wallet bucket (no rewardId),
 * mark matching AVAILABLE discrete reward rows CONSUMED so the slip/admin
 * cannot keep offering a voucher the wallet no longer funds.
 */
export async function consumeMatchingDiscreteRewardsForWalletSpend({
  userId,
  rewardType,
  amount,
  betId,
  ...
  if (!client) {
    throw new Error('DATABASE_TRANSACTION_REQUIRED: consumeMatchingDiscreteRewardsForWalletSpend requires a transaction client');
  }
  if (!['freebet', 'bonus'].includes(rewardType)) return [];
  let left = parseFloat(Number(amount || 0).toFixed(2));
  if (!(left > 0)) return [];

  const res = await client.query(
    `SELECT reward_id, amount, metadata, source
     FROM user_rewards
     WHERE user_id = $1
       AND reward_type = $2
  ...
  for (const row of res.rows) {
    if (left <= 0) break;
    const rewardAmount = Number(row.amount) || 0;
    if (!(rewardAmount > 0)) continue;
    // Exact-stake instruments: only consume when the remaining stake covers this voucher.
    if (rewardAmount - left > 1e-9) continue;
    ...
    const upd = await client.query(
      `UPDATE user_rewards
       SET status = 'CONSUMED',
           ...
           used_at = NOW(),
           updated_at = NOW()
       WHERE reward_id = $2 AND status = 'AVAILABLE'
       RETURNING reward_id, amount`,
      [betId || null, row.reward_id],
    );
    ...
    await client.query(
      ...
      [
        ...
        rewardAmount,
        betId
          ? `Synced consume after wallet ${rewardType} stake on bet #${betId}`
          : `Synced consume after wallet ${rewardType} stake`,
      ],
    );
    left = parseFloat((left - rewardAmount).toFixed(2));
    consumed.push({ rewardId: row.reward_id, amount: rewardAmount });
  }
  return consumed;
}

/**
 * Resolve admin input (user id, email, or phone) to the canonical users.user_id.
 */
export async function resolveRewardRecipient(identifier, run) {
  const exec = run || dbQuery;
  const raw = String(identifier || '').trim();
  if (!raw) {
    throw Object.assign(new Error('USER_REQUIRED: Enter a user ID, email, or phone number'), { code: 'USER_REQUIRED' });
  }
  ...
  const res = await exec(
    `SELECT u.user_id, u.email, u.phone,
     ...
     WHERE u.user_id = $1
        ...
        OR ($3 <> '' AND RIGHT(regexp_replace(COALESCE(u.phone, ''), '[^0-9]', '', 'g'), 10) = $3)
     LIMIT 8`,
    [raw, digits, last10],
  );
  const rows = res.rows || [];
  if (rows.length === 0) {
    throw Object.assign(
      new Error('USER_NOT_FOUND: No account matches that user ID, email, or phone'),
      { code: 'USER_NOT_FOUND' },
    );
  }
  const exactId = rows.find((r) => r.user_id === raw);
  const emailMatches = rows.filter((r) => String(r.email || '').toLowerCase() === raw.toLowerCase());
  if (rows.length > 1 && !exactId && emailMatches.length !== 1) {
    throw Object.assign(
      new Error('USER_AMBIGUOUS: Multiple accounts match. Paste the exact user ID from Customers.'),
      { code: 'USER_AMBIGUOUS' },
    );
  }
  const row = exactId || emailMatches[0] || rows[0];
  return {
    userId: row.user_id,
    email: row.email,
    phone: row.phone,
...
async function applyPromoWalletDelta(run, {
  userId,
  rewardType,
  delta,
  description,
}) {
  const numericDelta = parseFloat(Number(delta).toFixed(2));
  ...
  if (walletRes.rows.length === 0) {
    if (numericDelta > 0) {
      throw Object.assign(new Error('WALLET_NOT_FOUND: User has no wallet to receive this reward'), {
  ...
  const isFreebet = rewardType === 'freebet';
  const current = Number(isFreebet ? wallet.freebet_balance : wallet.bonus_balance);
  const next = parseFloat(Math.max(0, current + numericDelta).toFixed(2));
  if (isFreebet) {
    await run(
      `UPDATE wallets SET freebet_balance = $1, updated_at = NOW() WHERE wallet_id = $2`,
      [next, wallet.wallet_id],
  ...
  const txId = `tx_rwd_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  await run(
    `INSERT INTO transactions (transaction_id, user_id, type, method, amount, status)
     VALUES ($1, $2, 'BONUS_CLAIM', 'ADMIN_REWARD', $3, 'COMPLETED')`,
    [txId, userId, Math.abs(numericDelta)],
  );
  await run(
    ...
    [
      wallet.wallet_id,
      txId,
      numericDelta >= 0 ? 'CREDIT' : 'DEBIT',
      Math.abs(numericDelta),
      next,
      description || 'Admin discrete reward',
    ],
  );
  return { walletId: wallet.wallet_id, nextBalance: next };
}
...
export async function issueDiscreteReward({
  ...
  const shouldCreditWallet = creditWallet === true
    || (creditWallet !== false && !client && String(source || '').toUpperCase() === 'ADMIN_GRANT');

  const execute = async (run) => {
    let resolvedUserId = String(userId || '').trim();
    let recipient = null;
    if (!client) {
      recipient = await resolveRewardRecipient(resolvedUserId, run);
      resolvedUserId = recipient.userId;
    }
    ...
    const meta = {
      ...
      walletCredited: Boolean(shouldCreditWallet),
    };
  ...
  `;
    ...
    if (shouldCreditWallet) {
      await applyPromoWalletDelta(run, {
        userId: resolvedUserId,
        rewardType,
        delta: numericAmount,
        description: `${rewardTitle} (${rewardId})`,
      });
    }
  ...
  if (client) {
    return execute(client.query.bind(client));
  }
  return withTransaction(async (c) => execute(c.query.bind(c)));
}
...
/**
 * Heal stale AVAILABLE mirrored vouchers when the matching wallet bucket is empty.
 * Safe because freebet/bonus stakes require full-bucket use; an empty bucket means
 * every outstanding voucher of that type is already spent or forfeited.
 */
export async function reconcileUnfundedMirroredRewards(userId, client = null) {
  if (!userId) return { freebet: 0, bonus: 0 };
  const run = client ? client.query.bind(client) : dbQuery;
  const walletRes = await run(
  ...
  if (walletRes.rows.length === 0) return { freebet: 0, bonus: 0 };

  const bonus = Number(walletRes.rows[0].bonus_balance || 0);
  const freebet = Number(walletRes.rows[0].freebet_balance || 0);
  const healed = { freebet: 0, bonus: 0 };

  for (const [rewardType, bucket] of [['freebet', freebet], ['bonus', bonus]]) {
    if (bucket > 1e-9) continue;
    const stale = await run(
      `SELECT reward_id, amount, source, metadata
       FROM user_rewards
       WHERE user_id = $1
         AND reward_type = $2
         AND status = 'AVAILABLE'
         AND (
           UPPER(COALESCE(source, '')) = ANY($3::text[])
           OR COALESCE(metadata->>'walletMirrored', 'false') = 'true'
         )
       FOR UPDATE`,
      [userId, rewardType, [...WALLET_MIRRORED_REWARD_SOURCES]],
    );
    for (const row of stale.rows) {
      const upd = await run(
        `UPDATE user_rewards
         SET status = 'CONSUMED', used_at = COALESCE(used_at, NOW()), updated_at = NOW()
         WHERE reward_id = $1 AND status = 'AVAILABLE'
         RETURNING reward_id`,
        [row.reward_id],
      );
      if (upd.rows.length === 0) continue;
      ...
      await run(
        `INSERT INTO reward_ledger (
           event_id, reward_id, user_id, amount, event_type,
           previous_status, new_status, notes, created_at
         ) VALUES ($1, $2, $3, $4, 'REWARD_CONSUMED', 'AVAILABLE', 'CONSUMED', $5, NOW())`,
        [
          eventId,
          row.reward_id,
          userId,
          Number(row.amount),
          `Reconciled: ${rewardType} wallet empty after stake/expiry`,
        ],
      );
      healed[rewardType] += 1;
    }
...
export async function listUserAvailableRewards(userId, { client = null } = {}) {
  ...
  await reconcileUnfundedMirroredRewards(userId, client);
  const run = client ? client.query.bind(client) : dbQuery;
  ...
  return filterRewardsByWallet(mapped, {
    bonus: Number(walletRes.rows[0]?.bonus_balance || 0),
    freebets: Number(walletRes.rows[0]?.freebet_balance || 0),
  });
}
...
export async function listUserAllRewards(userId, { status = null, limit = 50, offset = 0, client = null } = {}) {
  ...
  const openAvailable = formatted.filter((r) => r.status === 'AVAILABLE' && new Date(r.expiresAt) > new Date());
  // Hide vouchers the wallet can no longer fund (stale AVAILABLE after wallet-path stake).
  const available = filterRewardsByWallet(openAvailable, {
    bonus: Number(walletRes.rows[0]?.bonus_balance || 0),
    freebets: Number(walletRes.rows[0]?.freebet_balance || 0),
  });
...
export async function lockAndValidateRewardForBet({
  ...
  // Odds Eligibility — freebet/bonus never below house promo floor (1.75)
  const rewardType = String(reward.reward_type || reward.rewardType || '').toUpperCase();
  const configuredMin = Number(reward.min_odds || 1.00);
  const minOdds =
    rewardType === 'FREEBET' || rewardType === 'BONUS'
      ? Math.max(configuredMin, BONUS_MIN_BET_ODDS)
      : configuredMin;
...
export async function consumeRewardForBet({
  ...
  const meta = parseRewardMetadata(consumed.metadata);
  const mirrored = Boolean(meta.walletMirrored) || isWalletMirroredRewardSource(consumed.source);

  // Admin grants that credited the wallet via issueDiscreteReward use applyPromoWalletDelta
  // (separate ledger tx). Wallet-mirrored promos (daily spin / deposit freebet) are debited
  // silently by the bet placement engine so the BET_STAKE ledger owns the money movement.
  if (meta.walletCredited && !mirrored) {
    await applyPromoWalletDelta(client.query.bind(client), {
      userId,
      rewardType: consumed.reward_type,
      delta: -Number(consumed.amount),
      description: `Consumed ${rewardId} on bet ${betId}`,
    });
  }
  ...
  return {
    ...consumed,
    walletMirrored: mirrored,
    walletAlreadyDebited: Boolean(meta.walletCredited) && !mirrored,
  };
}
...
export async function adminListRewards({
  ...
  if (search) {
    ...
    whereClauses.push(`(
      ur.reward_id ILIKE $${params.length}
      OR ur.title ILIKE $${params.length}
      OR ur.user_id ILIKE $${params.length}
...
export async function adminCancelReward({ rewardId, adminId, reason = 'Cancelled by administrator', client = null }) {
  const runner = async (c) => {
    ...
    if (parseRewardMetadata(reward.metadata).walletCredited) {
      await applyPromoWalletDelta(c.query.bind(c), {
        userId: reward.user_id,
        rewardType: reward.reward_type,
        delta: -Number(reward.amount),
        description: `Cancelled ${rewardId}`,
      });
    }
...
export async function adminGetRewardLedger(rewardId, client = null) {
  ...
  const res = await run(
    `SELECT rl.*,
     ...
     LEFT JOIN user_profiles p ON p.user_id = rl.user_id
