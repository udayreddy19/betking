...
export function resolveServerOddsFromSnapshot(liveSnap, {
  ...
  const matchObj = liveSnap?.match || liveSnap?.canonicalState || null;
  const ld = matchObj?.liveDetails || liveSnap?.liveDetails || {};
  const cs = liveSnap?.canonicalState || {};
  const innings = Number(
    ld.inningsId
    ?? cs.currentInnings
    ?? matchObj?.currentInnings
    ?? liveSnap?.currentInnings,
  );
  const battingRuns = Number(
    ld.runs ?? ld.firstRuns ?? matchObj?.team1?.runs ?? cs.team1?.runs,
  );
  const chaseRuns = Number(
    ld.chaseRuns ?? ld.score2 ?? matchObj?.team2?.runs ?? cs.team2?.runs,
  );
  const stateKey = liveSnap?.stateKey
    || (matchObj ? matchOddsStateKey(matchObj) : null)
    || liveSnap?.matchStateKey
    || null;
  ...
  return {
    ...
    stateKey,
    matchStateKey: stateKey,
    innings: Number.isFinite(innings) ? innings : null,
    currentInnings: Number.isFinite(innings) ? innings : null,
    overs: ld.overs ?? ld.firstOvers ?? cs.oversStr ?? null,
    oversStr: ld.overs ?? ld.firstOvers ?? null,
    scoreAtPlacement: {
      innings: Number.isFinite(innings) ? innings : null,
      battingRuns: Number.isFinite(battingRuns) ? battingRuns : null,
      chaseRuns: Number.isFinite(chaseRuns) ? chaseRuns : null,
      firstRuns: Number.isFinite(Number(ld.firstRuns)) ? Number(ld.firstRuns) : null,
      overs: ld.overs ?? ld.firstOvers ?? null,
    },
    isLive: Boolean(
      liveSnap?.isLive
      || liveSnap?.matchState === 'in'
      || liveSnap?.live
      || liveSnap?.canonicalState?.isLive
      || String(matchObj?.matchState || '').toLowerCase() === 'in',
    ),
  };
}
...
export async function resolveServerOdds({
  ...
  const snapshot = liveSnap ?? await loadLiveOddsSnapshot(matchId, {
    force: Boolean(forceFresh || force),
  });
