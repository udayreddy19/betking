 ...
 * @param {number} [currentScore=0]
 * @param {number} [minStdDev=5] - Floor on std-dev; shrink for short remaining windows
 * @returns {{ pOver: number, pUnder: number }}
 */
export function calculateOverUnderProbability(expectedMean, line, varianceRatio = 1.5, currentScore = 0, minStdDev = 5) {
  ...
  const floor = Number.isFinite(minStdDev) && minStdDev > 0 ? minStdDev : 5;
  const stdDev = Math.max(floor, Math.sqrt(expectedRemaining * varianceRatio));
 ...
 */
export function calculateRangeProbability(expectedMean, minRuns, maxRuns, currentScore = 0) {
  if (maxRuns < currentScore) return 0;
  const adjMin = Math.max(minRuns, currentScore);
  const stdDev = Math.max(8, Math.sqrt(Math.max(expectedMean, 1) * 1.5));
  const z1 = (adjMin - 0.5 - expectedMean) / stdDev;
  const z2 = (maxRuns + 0.5 - expectedMean) / stdDev;

  const cdf1 = normalCDF(z1);
  const cdf2 = normalCDF(z2);

  return Math.max(0, cdf2 - cdf1);
}
