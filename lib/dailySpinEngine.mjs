  ...
  return withTransaction(async (client) => {
    ...
    if (prize.type === 'bonus') {
      ...
      await client.query(
        ...
        [wallet.wallet_id, txId, scaledValue, nextBonus, `Daily spin bonus · ${prize.amount}`],
      );
      ...
      await client.query(
        `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description)
         VALUES ($1, $2, 'CREDIT', $3, $4, $5)`,
        [wallet.wallet_id, txId, scaledValue, nextFreebet, `Daily spin freebet · ${prize.amount}`],
      );
    } else {
      const nextPoints = Number(loyaltyNow.rows[0]?.points || 0) + scaledValue;
      ...
      await grantCrossedTierRewards(client, userId, previousTier, tier);
    }

    await client.query(
      `INSERT INTO daily_spins (spin_id, user_id, spin_date, prize_type, prize_value, prize_index)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [spinId, userId, spinDate, prize.type, scaledValue, prize.index],
    );
    ...
    return {
      success: true,
      alreadySpun: false,
      prize: prizePayload({ ...prize, prize_value: scaledValue, value: scaledValue }),
      wallet: snapshot,
    };
  });
}
