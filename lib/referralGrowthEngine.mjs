...
export function buildReferralShareAssets(code, {
  ...
  if (!link) return null;
  const text = `Join me on OddsYra — get ₹${referredReward} ${rewardLabel} with my link, and I earn ${playCommissionRatePct}% when you play: ${link}`;
  ...
  return {
    ...
    telegram: `https://t.me/share/url?url=${encodedUrl}&text=${encodeURIComponent(`Join OddsYra — ₹${referredReward} ${rewardLabel}`)}`,
    qrUrl: `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodedUrl}`,
    ogTitle: 'Invite friends to OddsYra',
    ogDescription: `Share your code ${code}. Friends get ₹${referredReward} ${rewardLabel}; you earn when they play.`,
  };
}
