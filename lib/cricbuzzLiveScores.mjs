...
function normalizeOvers(value, match = null) {
  ...
  if (/hundred/i.test(league)) {
    ...
    if (ball === 0 && whole > 20 && whole <= 100) {
      const overs = Math.floor(whole / 5);
      const rem = whole % 5;
      return `${overs}.${rem}`;
    }
    if (ball > 5) {
...
function parseOversBallCount(overs, match = null) {
  ...
  if (/hundred/i.test(league)) {
    ...
    const whole = parseInt(parts[0], 10) || 0;
    const ball = parseInt(parts[1], 10) || 0;
    if (ball === 0 && whole > 20 && whole <= 100) return whole;
    return whole * 5 + ball;
  }
  const str = String(overs ?? '0');
  const parts = str.split('.');
...
function buildLiveDetails(info, matchScore, matchState = 'pre', match = null) {
  ...
  const hasPlay = inningsHasPlay(team1Innings, match) || inningsHasPlay(team2Innings, match);
  ...
  return {
    ...
    overs: normalizeOvers(team1Innings?.overs ?? 0, match),
    score2: team2Innings?.runs ?? 0,
    wickets2: team2Innings?.wickets ?? 0,
    overs2: normalizeOvers(team2Innings?.overs ?? 0, match),
    commentary,
  };
}
