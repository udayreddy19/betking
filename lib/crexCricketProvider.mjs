...
export function parseCrexLiveCards(html = '') {
  ...
  for (const part of String(html).matchAll(re)) {
    ...
    const id = extractCrexId(href);
    let teams = parseCrexTeamScoreBlocks(block);
    if (teams.length < 2) {
      const teamSpans = [...block.matchAll(/<(?:span|div)[^>]*class="[^"]*team-name[^"]*"[^>]*>([\s\S]*?)<\/(?:span|div)>/gi)];
      if (teamSpans.length >= 2) {
        const idx1 = teamSpans[0].index;
        const idx2 = teamSpans[1].index;
        const part1 = block.slice(idx1, idx2);
        const part2 = block.slice(idx2);

        const parseTeamChunk = (chunk, defaultName) => {
          const name = stripTags((chunk.match(/class="[^"]*team-name[^"]*"[^>]*>([\s\S]*?)<\//i) || [])[1] || defaultName);
          const batting = /inning-active/i.test(chunk);
          let scoreText = stripTags((chunk.match(/class="[^"]*inning-active[^"]*"[^>]*>([\s\S]*?)<\//i) || [])[1] || '');
          if (!scoreText) {
            const bits = [...chunk.matchAll(/<(?:span|div)[^>]*class="([^"]*)"[^>]*>([\s\S]*?)<\//gi)]
              .filter((row) => !/team-name|match-over|liveTag|comment/i.test(row[1] || ''))
              .map((row) => stripTags(row[2]))
              .filter((text) => /\d/.test(text));
            scoreText = bits[0] || '';
          }
          const overs = parseOversText(
            stripTags((chunk.match(/class="[^"]*match-over[^"]*"[^>]*>([\s\S]*?)<\//i) || [])[1] || ''),
          ) || '0.0';
          const score = parseCricketScoreText(scoreText);
          return {
            name,
        ...
        teams = [
          parseTeamChunk(part1, stripTags(teamSpans[0][1])),
          parseTeamChunk(part2, stripTags(teamSpans[1][1])),
        ];
      }
    }
