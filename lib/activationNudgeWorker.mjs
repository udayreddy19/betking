...
async function alreadyNudged(userId, cooldownHours) {
 ...
 */
export async function processUnconvertedDepositorNudges({ limit } = {}) {
  const conf = await getGrowthAutomationConfig();
  const act = conf.activation;
  if (!act.enabled) {
    return { checked: 0, sent: 0, skipped: 0, disabled: true };
  }

  const lookbackHours = act.lookbackHours;
  const cooldownHours = act.cooldownHours;
  const lim = Math.min(100, Math.max(1, Number(limit) || act.batchLimit || 25));
  ...
  for (const row of usersRes.rows || []) {
    try {
      if (await alreadyNudged(row.user_id, cooldownHours)) {
        skipped += 1;
        continue;
      }
  ...
  const result = { checked: usersRes.rows.length, sent, skipped };
  void recordGrowthAutomationRun('activation', result);
  return result;
}
