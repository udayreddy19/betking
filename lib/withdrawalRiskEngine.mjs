...
export async function evaluateWithdrawalRisk({
  ...
  // Fail closed: open-bets / velocity query errors must not score as "zero risk"
  let openBets;
  try {
    openBets = await exec(
    ...
    throw Object.assign(
      new Error(`Withdrawal risk check failed (open bets): ${err.message || err}`),
      { code: 'RISK_QUERY_FAILED', status: 503, cause: err },
    );
  }
  ...
  try {
    ...
    throw Object.assign(
      new Error(`Withdrawal risk check failed (velocity): ${err.message || err}`),
      { code: 'RISK_QUERY_FAILED', status: 503, cause: err },
    );
  }
