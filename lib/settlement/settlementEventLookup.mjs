...
function isMultiDayCricket(match) {
  const fmt = detectCricketMatchFormat(match);
  return fmt === 'TEST' || fmt === 'FIRST_CLASS';
}

/**
 * Wall-clock "stale live ⇒ completed" is for short formats only.
 * County/Test matches run for days; mutating them to COMPLETED voids open overs.
 */
function inferStaleLiveAsFinal(match, { startTime, updatedAt, s1, s2, now }) {
  if (isMultiDayCricket(match)) return false;
  const state = String(match?.matchState || '').toLowerCase();
  if (match?.isLive === true || state === 'in' || state === 'live') return false;
  if (String(match?.time || '').toLowerCase() === 'live') return false;
  return (startTime > 0 && now - startTime > 3.5 * 3600 * 1000)
    || (s1 > 0 && s2 > 0 && updatedAt > 0 && now - updatedAt > 15 * 60 * 1000);
}

function markInferredFinal(match) {
...
export async function lookupEventForSettlement({
    ...
    for (const alias of aliases) {
      if (byId.has(alias)) {
        ...
        if (validateMatchIdentity(hydrated)) {
          ...
          const now = Date.now();
          const isStaleLive = inferStaleLiveAsFinal(hydrated, { startTime, updatedAt, s1, s2, now });
  ...
  try {
    ...
    if (cachedState && validateMatchIdentity(cachedState)) {
      ...
      const now = Date.now();
      const isStaleLive = inferStaleLiveAsFinal(cachedState, { startTime, updatedAt, s1, s2, now });
