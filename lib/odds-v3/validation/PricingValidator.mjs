...
export function validateSelectionPrice(selection) {
  ...
  else if (selection.odds <= 1.0) errors.push(`odds must be > 1.0: ${selection.odds}`);
 ...
 */
export function validateOverround(finalOdds, configuredOverround, tolerance = 0.08) {
  const impliedSum = finalOdds.reduce((sum, o) => sum + (1 / o), 0);
  const expectedSum = 1 + configuredOverround;
  const isBoundary = finalOdds.some(o => o <= 1.02);
  const effectiveTolerance = isBoundary ? 0.10 : tolerance;
  if (Math.abs(impliedSum - expectedSum) > effectiveTolerance) {
    return {
      valid: false,
      impliedSum,
      expectedSum,
