...
export async function authenticateApiKey(rawKeyInput, requiredScope = null) {
  ...
  // Sliding window (Redis-first, 100 req/min per API key)
  const limit = 100;
  ...
  const resetSeconds = slot.retryAfterSeconds;
...
export async function retryWebhookDelivery(deliveryId) {
  ...
  if (!id) throw new Error('deliveryId required');
  const res = await query(
    `UPDATE webhook_deliveries
     SET status = 'QUEUED', attempts = 0, response_code = NULL
     WHERE id = $1 AND UPPER(status) IN ('DEAD_LETTER', 'FAILED', 'RETRYING', 'ERROR', 'REJECTED')
     RETURNING id, status`,
    [id],
  );
  if (!res.rows.length) {
  ...
  return { success: true, id: res.rows[0].id, status: res.rows[0].status };
}
