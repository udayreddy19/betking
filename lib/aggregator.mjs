...
import { normalizeMatchLiveFlags, getMatchState, hasCricketPlayStarted } from './matchState.mjs';
import { filterMatchesForPublicBoard } from './matchQualityGate.mjs';
...
import { priceMatchWinnerForAggregator } from './odds-v4/engineDispatch.mjs';
...
function normalizeMatchOdds(match) {
  ...
  try {
    const priced = priceMatchWinnerForAggregator(baseMatch, { isCricket });
    if (priced?.odds?.team1 != null && priced?.odds?.team2 != null) return priced;
  } catch {
...
async function refreshAggregatedLiveScores() {
  ...
  const [tencricSettled, crexSettled, cbSettled, fcSettled, espnSettled, flashscoreSettled, guruSettled, crixSettled] = isTest
    ? [
        { status: 'fulfilled', value: [] },
        { status: 'fulfilled', value: { matches: [], counts: {} } },
        { status: 'fulfilled', value: { matches: [], series: [], counts: {} } },
        { status: 'fulfilled', value: { matches: [], counts: {} } },
        { status: 'fulfilled', value: { matches: [], counts: {} } },
        { status: 'fulfilled', value: { matches: [], counts: {} } },
        { status: 'fulfilled', value: { matches: [], counts: {} } },
  ...
  if (tencricSettled.status === 'fulfilled') {
    ...
    sourceStatus.tencric = (Array.isArray(tencricResult) ? tencricResult.length : 0) > 0 ? 'ok' : 'empty';
  ...
  if (crexSettled.status === 'fulfilled') {
    ...
    sourceStatus.crex = (crexResult?.matches?.length || 0) > 0 ? 'ok' : 'empty';
  ...
  if (cbSettled.status === 'fulfilled') {
    ...
    sourceStatus.cricbuzz = (cricbuzzResult?.matches?.length || 0) > 0 ? 'ok' : 'empty';
  ...
  const srlMatches = getIplSrlMatches(Date.now(), { publicBoard: true }) || [];
  ...
  // Drop stub live cricket (no play / placeholder teams) from the public board payload.
  matches = filterMatchesForPublicBoard(matches, { hasCricketPlayStarted });

  // V4.4: promote frozen score-sport boards into COMPLETED on the public feed
  // (not only on DB upsert), so WS/odds and settlement see the same finality.
  try {
    const { inferScoreSportStaleLiveFinal, markInferredFinal } = await import('./settlement/wallClockFinality.mjs');
    matches = matches.map((m) => {
      if (!inferScoreSportStaleLiveFinal(m)) return m;
      return markInferredFinal({ ...m });
    });
  } catch { /* keep original */ }
  ...
  if (feedError?.code === 'ALL_PROVIDERS_FAILED') {
    ...
    // Recovery — clear sticky PROVIDER_FAILURE once any real feed is healthy again.
    try {
      const { marketSuspensionEngine } = await import('./marketSuspensionEngine.mjs');
      const liveForClear = matches
        .filter((m) => getMatchState(m) === 'in')
        .filter((m) => m?.source !== 'srl' && !String(m?.id || '').startsWith('srl_'));
      await Promise.all(
        liveForClear.map(async (m) => {
          const matchId = String(m?.id || m?.matchId || '');
          if (!matchId) return;
          const markets = ['match_winner', `${matchId}:match_winner`, `${matchId}|match_winner`];
          await Promise.all(markets.map((marketId) =>
            marketSuspensionEngine.clearSuspensionCause(marketId, 'PROVIDER_FAILURE')));
        }),
      );
    } catch (err) {
      console.warn('[Aggregator] clear PROVIDER_FAILURE failed:', err?.message || err);
    }
  }
    ...
    await Promise.all(
      [...liveMatches, ...completedMatches].map(async (m) => {
        ...
        const completed = row?.isCompleted === true
          ...
          || String(row?.matchState || '').toLowerCase() === 'post'
          || /^(completed|final|finished)$/i.test(String(row?.time || ''));
        await cacheCanonicalMatchState(id, row, { completed }).catch(() => {});
        await upsertPersistentMatch(row).catch(() => {});
      }),
    );
  } catch (err) {
