...
function mapCricketEvent(evt) {
  ...
  const homeScoreStr = homeLinescores[0]?.displayValue || home.score || '';
  const awayScoreStr = awayLinescores[0]?.displayValue || (awayLinescores.length > 0 ? away.score : '');

  const parseScoreStr = (str) => {
    if (!str || typeof str !== 'string') return [0, 0];
    const parts = str.split('/');
    const r = parseInt(parts[0], 10) || 0;
    const w = parts.length > 1 ? parseInt(parts[1], 10) || 0 : 0;
    return [r, w];
  };

  const [hRuns, hWickets] = parseScoreStr(homeScoreStr);
  const isSecondInnings = awayLinescores.length > 0 && awayScoreStr !== '';
  const [aRuns, aWickets] = isSecondInnings ? parseScoreStr(awayScoreStr) : [0, 0];
  ...
  const overs2 = isSecondInnings ? (awayLinescores[0]?.overs || '0.0') : '0.0';
  ...
  return {
    ...
    liveDetails: {
      ...
      wickets: hWickets,
      overs,
      score2: isSecondInnings ? aRuns : 0,
      wickets2: isSecondInnings ? aWickets : 0,
      overs2,
