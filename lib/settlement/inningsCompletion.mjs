...
export function isExplicitMatchFinal(match) {
  ...
  // Stale wall-clock mutations set COMPLETED/post while the feed is still Live —
  // never treat those flags as authoritative in that case.
...
export function isOverNeverCompleted(match, settleInnings, overNum) {
  ...
  const bat = getBattingOversAndScore(match);
  const inn = Number(settleInnings) || 1;
  const liveParts = parseOversParts(bat.oversStr);
  const liveWkts = Number(bat.wickets);

  // Hard guard: still batting this innings short of the over — never VOID, even if a
  // stale wall-clock heuristic flipped status to COMPLETED (County/Test false-final).
  if (
    bat.innings === inn
    && liveParts
    && liveParts.completed < target
    && Number.isFinite(liveWkts)
    && liveWkts < 10
  ) {
    return false;
  ...
  const parts = parseOversParts(oversStr);
  if (!parts) {
    // Innings done but no overs clock — await evidence; do not invent "never bowled" VOID.
    return false;
  }
