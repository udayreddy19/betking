...
import { fetchFlashscoreMatchById, lookupFlashscoreMatch } from './providers/flashscoreProvider.mjs';
...
import { fetchCrexCricketMatches, fetchCrexMatchById } from './crexCricketProvider.mjs';
import { fetchCricketGuruMatchById } from './providers/cricketGuruProvider.mjs';
import { fetchCricketLivelineMatchById } from './providers/cricketLivelineProvider.mjs';
import {
...
export function canFetchMatchDetail(match) {
  ...
  if (match.source === 'flashscore' || match.id?.startsWith('fs_') || match.flashscoreEventId) return true;
  ...
  if (match.source === 'cricketguru' || match.id?.startsWith('guru_') || match.guruMatchId) return true;
  if (match.source === 'cricketliveline' || match.id?.startsWith('crix_') || match.crixMatchId) return true;
  return true;
}
...
export async function fetchMatchDetail(rawMatch, { fast = false } = {}) {
  ...
  const wantsFlashscore = match.source === 'flashscore'
    || String(match.id || '').startsWith('fs_')
    || match.flashscoreEventId
    || (!isCricket && !detail?.isCompleted && !detail?.winnerSide);
  if (wantsFlashscore) {
    const fs = await (match.flashscoreEventId || String(match.id || '').startsWith('fs_')
      ? fetchFlashscoreMatchById(match.flashscoreEventId || match.id)
      : lookupFlashscoreMatch(match)
    ).catch(() => null);
    if (fs) {
      detail = mergeRosterDetail(detail || match, {
        ...fs,
        fetchedAt: new Date().toISOString(),
        matchId: match.id || fs.id,
      }, match);
    }
  }

  if (match.source === 'crex' || match.id?.startsWith('crex_') || match.crexEventId) {
    const crex = await fetchCrexMatchById(match.crexEventId || match.id).catch(() => null);
    if (crex) {
      detail = mergeRosterDetail(detail || match, {
        ...crex,
        fetchedAt: new Date().toISOString(),
        matchId: match.id || crex.id,
      }, match);
    }
  }

  if (match.source === 'cricketguru' || match.id?.startsWith('guru_') || match.guruMatchId) {
    const guru = await fetchCricketGuruMatchById(match.guruMatchId || match.id).catch(() => null);
    if (guru) {
      detail = mergeRosterDetail(detail || match, {
        ...guru,
        fetchedAt: new Date().toISOString(),
        matchId: match.id || guru.id,
      }, match);
    }
  }

  if (match.source === 'cricketliveline' || match.id?.startsWith('crix_') || match.crixMatchId) {
    const crix = await fetchCricketLivelineMatchById(match.crixMatchId || match.id).catch(() => null);
    if (crix) {
      detail = mergeRosterDetail(detail || match, {
        ...crix,
        fetchedAt: new Date().toISOString(),
        matchId: match.id || crix.id,
      }, match);
    }
  }

  if (match.source === 'srl' || String(match.id || '').startsWith('srl_ipl_')) {
    const srlMatch = getIplSrlMatches(Date.now(), { publicBoard: true }).find((item) => item.id === match.id)
      || getIplSrlMatchById(match.id, Date.now(), { forPublic: true });
    if (srlMatch) {
      detail = mergeRosterDetail(detail, {
        ...srlMatch,
        fetchedAt: new Date().toISOString(),
