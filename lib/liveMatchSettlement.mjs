...
import { getMatchMaxOvers, oversToBallsForMatch, isTestMatch } from '../src/utils/cricketFormat.js';
import { teamNameMatches } from '../src/utils/cricketScores.js';
import { resolveTossWinnerLabel, resolveTossDecision, resolveBatFirstTeam, teamMatchesLabel } from './odds-v4/markets/TossWinnerMarketV4.mjs';
import {
...
import { inferWallClockMatchFinal, isFeedStillLive, inferScoreSportStaleLiveFinal, isMultiDayCricket } from './settlement/wallClockFinality.mjs';
...
function hasExplicitWinnerSide(match) {
  const explicit = String(match?.winnerSide || match?.winner || '').trim();
  return explicit === '1' || explicit === '2' || explicit === 'X'
    || /^home$/i.test(explicit)
    || /^away$/i.test(explicit);
}

function isMatchFinal(match) {
  if (!match) return false;
  const cricket = isCricketSettlementSport(match);
  // Cricket: never override an actively live feed unless the card itself is completed.
  if (cricket && isFeedStillLive(match) && !isCricketMatchCompleted(match)) {
    return false;
  }
  // Multi-day / FC / Test: never finalize from stale "post"/COMPLETED flags alone.
  // Require an explicit result phrase (isCricketMatchCompleted) or provider winnerSide.
  if (cricket && isMultiDayCricket(match) && !isCricketMatchCompleted(match)) {
    return hasExplicitWinnerSide(match);
  }
  // Score sports: stale LIVE boards (frozen mid-match) can still be final.
  // Do not mutate the caller object here — aggregator/persistence own markInferredFinal.
  if (!cricket && inferScoreSportStaleLiveFinal(match)) {
    return true;
  }
  ...
  return inferWallClockMatchFinal(match);
}

/** @returns {'1'|'2'|'X'|null} */
function isCricketSettlementSport(match) {
...
export function resolveLiveMatchWinner(match) {
  ...
  const explicit = String(match.winnerSide || match.winner || '').trim();
  if (explicit === '1' || explicit === '2' || explicit === 'X') return explicit;
  if (/^home$/i.test(explicit)) return '1';
  if (/^away$/i.test(explicit)) return '2';
  ...
  const multiDay = isMultiDayCricket(match) || isTestMatch(match);

  // Limited-overs chase result only — never invent a match winner from 1st vs 2nd
  // innings totals on Test / First Class / County boards (first-innings lead ≠ result).
  if (
    !multiDay
    && isCricketSettlementSport(match)
    && firstRuns > 0
    && (chaseRuns > 0 || ld.chaseWickets != null)
  ) {
    // Tie — never invent a defending-side win (super over / NR handled elsewhere).
    if (chaseRuns === firstRuns) return 'X';
    ...
    if (chaseTeam && (t1 || t2)) {
      ...
      if (chaseWon) {
        ...
        // defending side won (chase short of target)
  ...
  // Score comparison is fine for soccer/handball; unsafe for multi-day cricket innings boards.
  if (!multiDay) {
    const s1 = parseRuns(match.team1?.runs ?? match.team1?.score ?? match.score1 ?? ld.score1 ?? ld.runs ?? ld.firstRuns ?? match.live_score1);
    const s2 = parseRuns(match.team2?.runs ?? match.team2?.score ?? match.score2 ?? ld.score2 ?? ld.chaseRuns ?? match.live_score2);
  ...
  // Score sports (handball/snooker/tennis/etc): draw only when soccer-like.
  return null;
}
...
export async function evaluateOverMarketBet(bet, match) {
  ...
  if (nextOver) {
    ...
    // OddsYra SRL: rebuild overHistory from operator-fresh sim when snapshots lag.
    if ((runs == null || !Number.isFinite(runs)) && isSrlMatchId(match)) {
      try {
        ...
        await refreshSrlOperatorSession(id);
        const fresh = getIplSrlMatchById(id, Date.now(), { forPublic: false });
        const freshHist = fresh?.overHistory || fresh?.liveDetails?.overHistory || [];
        const freshRow = resolveOverHistoryRow(
          freshHist,
          overNum,
          settleInnings,
          getBattingOversAndScore(fresh || match).innings,
          parseOversParts(getBattingOversAndScore(fresh || match).oversStr)?.completed,
        );
        const freshRuns = runsFromOverRow(freshRow);
        if (freshRuns != null) runs = freshRuns;
      } catch { /* keep null */ }
    }
...
export function evaluateTotalsMarketBet(bet, match) {
  ...
  if (isTeamTotal) {
    ...
    if (inn === 1) {
      ...
      if (isUnderSelection(selectionId, selectionName)) {
        if (final1 > line) {
          return { outcome: 'LOST', reason: `team_total_i1_final=${final1}_line=${line}`, hasImmutableSnapshotEvidence: true };
        }
        if (!inningsScoped && !leg?.innings && snap?.inningsAtPlacement == null && bat.innings === 2 && !chaseInningsComplete) {
          return null;
        }
        return { outcome: final1 < line ? 'WON' : 'LOST', reason: `team_total_i1_final=${final1}_line=${line}`, hasImmutableSnapshotEvidence: true };
      }
  ...
  if (market === 'match_total' || market.startsWith('match_total_alt')) {
    ...
    if (!isMatchFinal(match)) {
      const combined = Math.max(
        ...
        resolveInningsRuns(match, 1) + resolveInningsRuns(match, 2),
      );
...
/** Settle Toss Winner once toss evidence is available (does not wait for match end). */
export function evaluateTossWinnerBet(bet, match) {
  const abandoned = /abandon|cancel|no result|washed out/i.test(
    `${match?.status || ''} ${match?.result || ''} ${match?.liveDetails?.commentary || ''}`,
  );
  if (abandoned) {
    return { outcome: 'VOID', reason: 'Match abandoned before/without toss settlement' };
  }

  const winnerLabel = resolveTossWinnerLabel(match);
  if (!winnerLabel) return null;

  const t1 = match?.team1 || {};
  const t2 = match?.team2 || {};

  const winnerIsT1 = teamMatchesLabel(t1, winnerLabel);
  const winnerIsT2 = teamMatchesLabel(t2, winnerLabel);

  if (!winnerIsT1 && !winnerIsT2) return null;

  const selectedT1 = selectionLooksLikeTeam(bet.selection_id, bet.selection_name, t1)
    || /^(?:sel[_-]?)?(?:t1|team1|home)$/i.test(String(bet.selection_id || ''));
  const selectedT2 = selectionLooksLikeTeam(bet.selection_id, bet.selection_name, t2)
    || /^(?:sel[_-]?)?(?:t2|team2|away)$/i.test(String(bet.selection_id || ''));

  if (selectedT1 === selectedT2) return null;

  if (selectedT1) {
    return {
      outcome: winnerIsT1 ? 'WON' : 'LOST',
      reason: `toss_winner=${winnerLabel}`,
    };
  }
  return {
    outcome: winnerIsT2 ? 'WON' : 'LOST',
    reason: `toss_winner=${winnerLabel}`,
  };
}

/** Settle Toss & Bat / Toss & Bowl Yes-No markets. */
export function evaluateTossElectionBet(bet, match) {
  const abandoned = /abandon|cancel|no result|washed out/i.test(
    `${match?.status || ''} ${match?.result || ''} ${match?.liveDetails?.commentary || ''}`,
  );
  if (abandoned) {
  ...
  const decision = resolveTossDecision(match);
  if (!decision) return null;

  const market = String(bet.market_id || '').toLowerCase();
  const expect = market.includes('bowl') ? 'bowl' : 'bat';
  const yesWon = decision === expect;

  const selId = String(bet.selection_id || '').toLowerCase();
  const selName = String(bet.selection_name || '').toLowerCase();
  const isYes = selId.endsWith('_yes') || selId === 'yes' || selName === 'yes';
  const isNo = selId.endsWith('_no') || selId === 'no' || selName === 'no';
  if (isYes === isNo) return null;

  return {
    outcome: isYes ? (yesWon ? 'WON' : 'LOST') : (yesWon ? 'LOST' : 'WON'),
    reason: `toss_decision=${decision}`,
  };
}

/** Settle Team to Bat First. */
export function evaluateTeamBatFirstBet(bet, match) {
  const abandoned = /abandon|cancel|no result|washed out/i.test(
    `${match?.status || ''} ${match?.result || ''} ${match?.liveDetails?.commentary || ''}`,
  );
  if (abandoned) {
  ...
  const batFirst = resolveBatFirstTeam(match);
  if (!batFirst) return null;

  const t1 = match?.team1 || {};
  const t2 = match?.team2 || {};
  const batIsT1 = teamMatchesLabel(t1, batFirst.name) || batFirst.id === t1.id;
  const batIsT2 = teamMatchesLabel(t2, batFirst.name) || batFirst.id === t2.id;
  if (!batIsT1 && !batIsT2) return null;
  ...
  if (selectedT1) {
    return { outcome: batIsT1 ? 'WON' : 'LOST', reason: `bat_first=${batFirst.name}` };
  }
  return { outcome: batIsT2 ? 'WON' : 'LOST', reason: `bat_first=${batFirst.name}` };
}
...
export function evaluateMostBoundariesBet(bet, match) {
  ...
  const raw1 = isSixes ? (t1.sixes ?? ld.team1?.sixes) : (t1.fours ?? ld.team1?.fours);
  const raw2 = isSixes ? (t2.sixes ?? ld.team2?.sixes) : (t2.fours ?? ld.team2?.fours);
  // Missing boundary stats must not settle as 0–0 push/VOID.
  if (raw1 == null && raw2 == null) return null;
  const c1 = Number(raw1 ?? 0);
  const c2 = Number(raw2 ?? 0);
...
export async function evaluateOverOddEvenBet(bet, match) {
  ...
  // OddsYra SRL / any overHistory feed — sum ball labels for the over.
  if (runsInOver == null) {
    const hist = match?.overHistory || match?.liveDetails?.overHistory || [];
    const row = hist.find((r) => Number(r.overNum || r.over) === targetOver);
    if (row) {
      const fromRow = runsFromOverRow(row);
      if (fromRow != null) runsInOver = fromRow;
    }
  }
  ...
  // SRL: rebuild from sim if still missing.
  if (runsInOver == null && isSrlMatchId(match)) {
    try {
      const { refreshSrlOperatorSession } = await import('./iplSrlOperatorState.mjs');
      const { getIplSrlMatchById } = await import('./iplSrlSimulator.mjs');
      const id = String(match.id || match.matchId);
      await refreshSrlOperatorSession(id);
      const fresh = getIplSrlMatchById(id, Date.now(), { forPublic: false });
      const hist = fresh?.overHistory || fresh?.liveDetails?.overHistory || [];
      const row = hist.find((r) => Number(r.overNum || r.over) === targetOver);
      const fromRow = runsFromOverRow(row);
      if (fromRow != null) runsInOver = fromRow;
    } catch { /* keep null */ }
  ...
  if (runsInOver == null) {
    ...
    if (['CANCELLED', 'ABANDONED', 'POSTPONED', 'NO_RESULT'].includes(status) || /abandon|cancel|no result|washed out/i.test(String(match?.result || ''))) {
      return { outcome: 'VOID', reason: 'match_abandoned' };
    }
    const bat = getBattingOversAndScore(match);
    const settleInn = bat.innings || 1;
    if (isOverNeverCompleted(match, settleInn, targetOver)) {
...
export function evaluateSoccerResultBet(bet, match) {
  ...
  if (market === 'double_chance' || market.includes('double_chance')) {
    ...
    // Binary Yes/No legs use ids like DC:1X:No — must not match the Yes branch via includes('1X').
    const isNoLeg = /:NO(?:\b|$)/i.test(selectionId) || /\bNO$/i.test(id);
    let yesOutcome = null;
    let reasonTag = null;
    if (id.includes('1X') || /dc:1x/i.test(selectionId)) {
      yesOutcome = result === '1' || result === 'X' ? 'WON' : 'LOST';
      reasonTag = '1x';
    } else if (id.includes('X2') || id.includes('2X') || /dc:x2|dc:2x/i.test(selectionId)) {
      yesOutcome = result === '2' || result === 'X' ? 'WON' : 'LOST';
      reasonTag = 'x2';
    } else if (id.includes('12') || /dc:12/i.test(selectionId)) {
      yesOutcome = result !== 'X' ? 'WON' : 'LOST';
      reasonTag = '12';
    }
    if (!yesOutcome) return null;
    const outcome = isNoLeg
      ? (yesOutcome === 'WON' ? 'LOST' : 'WON')
      : yesOutcome;
    return {
      outcome,
      reason: `soccer_dc_${reasonTag}${isNoLeg ? '_no' : ''}_${score1}-${score2}`,
    };
  }
...
export function evaluateScoreSpreadBet(bet, match) {
  ...
  // Basketball Spread:, baseball RunLine:, ice hockey PuckLine:
  const parsed = text.match(/(?:Spread|RunLine|PuckLine):([12])\s*([+-]\d+(?:\.\d+)?)/i);
...
function readHalfTimeScores(match) {
  const ld = match?.liveDetails || {};
  const candidates = [
    [ld.htScore1, ld.htScore2],
    [ld.score1Ht, ld.score2Ht],
    [ld.halfTimeScore1, ld.halfTimeScore2],
    [ld.ht?.score1, ld.ht?.score2],
    [match?.htScore1, match?.htScore2],
  ];
  for (const [a, b] of candidates) {
    const s1 = Number(a);
    const s2 = Number(b);
    if (Number.isFinite(s1) && Number.isFinite(s2)) return { score1: s1, score2: s2, present: true };
  }
  return { score1: null, score2: null, present: false };
}

/** Soccer HT 1X2 — requires explicit HT evidence; never grades from FT alone. */
export function evaluateSoccerHalfTimeBet(bet, match) {
  if (!isMatchFinal(match) && Number(match?.liveDetails?.minute ?? match?.minute ?? 0) < 45) {
    return null;
  }
  const ht = readHalfTimeScores(match);
  if (!ht.present) {
    if (!isMatchFinal(match)) return null;
    return { outcome: 'VOID', reason: 'ht_scores_missing' };
  }
  const result = soccerFtResult(ht.score1, ht.score2);
  const selectionId = String(bet.selection_id || '');
  const want = /HT:X/i.test(selectionId) ? 'X' : /HT:2|:2$/i.test(selectionId) ? '2' : /HT:1|:1$/i.test(selectionId) ? '1' : null;
  if (!want) return null;
  return {
    outcome: want === result ? 'WON' : 'LOST',
    reason: `soccer_ht_${ht.score1}-${ht.score2}`,
  };
}

/** Soccer correct score CS:h-a */
export function evaluateSoccerCorrectScoreBet(bet, match) {
  if (!isMatchFinal(match)) return null;
  const { score1, score2 } = readFtScores(match);
  const text = `${bet.selection_id || ''} ${bet.selection_name || ''}`;
  const parsed = text.match(/CS:(\d+)\s*-\s*(\d+)/i) || text.match(/(\d+)\s*-\s*(\d+)/);
  if (!parsed) return null;
  const h = Number(parsed[1]);
  const a = Number(parsed[2]);
  if (!Number.isFinite(h) || !Number.isFinite(a)) return null;
  return {
    outcome: score1 === h && score2 === a ? 'WON' : 'LOST',
    reason: `soccer_cs_final_${score1}-${score2}_pick_${h}-${a}`,
  };
}

/**
 * First team to score — needs firstGoalTeam evidence.
 * 0-0 final → neither scored → both lose (or VOID if selection requires a scorer).
 */
export function evaluateFirstToScoreBet(bet, match) {
  if (!isMatchFinal(match)) return null;
  const { score1, score2, ld } = readFtScores(match);
  const selectionId = String(bet.selection_id || '');
  const wantHome = /FTS:1|:1$|^1$/i.test(selectionId);
  const wantAway = /FTS:2|:2$|^2$/i.test(selectionId);
  const wantNoGoal = /FTS:NoGoal|no_?goal|none/i.test(selectionId);
  if (!wantHome && !wantAway && !wantNoGoal) return null;

  if (score1 === 0 && score2 === 0) {
    return {
      outcome: wantNoGoal ? 'WON' : 'LOST',
      reason: 'first_to_score_no_goal',
    };
  }

  const first = String(ld.firstGoalTeam || ld.firstScorerTeam || match?.firstGoalTeam || '').toLowerCase();
  if (first === '1' || first === 'home' || first === 'team1') {
    return { outcome: wantHome ? 'WON' : 'LOST', reason: 'first_goal_home' };
  }
  if (first === '2' || first === 'away' || first === 'team2') {
    return { outcome: wantAway ? 'WON' : 'LOST', reason: 'first_goal_away' };
  }
  if (wantNoGoal) {
    // If goals were scored, No Goal is definitely lost
    return { outcome: 'LOST', reason: 'goals_were_scored' };
  }
  // FT known but first-goal identity missing — do not invent from FT score.
  return { outcome: 'VOID', reason: 'first_goal_evidence_missing' };
}

/** Winning margin: handles both banded (WM:1-5, WM:6-10...) and team-specific (WM:H1, WM:H2, WM:H3+, WM:A1, WM:A2, WM:A3+, WM:draw) */
export function evaluateWinningMarginBet(bet, match) {
  if (!isMatchFinal(match)) return null;
  const { score1, score2 } = readFtScores(match);
  ...
  // 1. Team-specific soccer winning margin
  if (/WM:H1/i.test(selectionId)) {
    return { outcome: diff === 1 ? 'WON' : 'LOST', reason: `wm_home_1_actual_${diff}` };
  }
  if (/WM:H2/i.test(selectionId)) {
    return { outcome: diff === 2 ? 'WON' : 'LOST', reason: `wm_home_2_actual_${diff}` };
  }
  if (/WM:H3\+/i.test(selectionId)) {
    return { outcome: diff >= 3 ? 'WON' : 'LOST', reason: `wm_home_3plus_actual_${diff}` };
  }
  if (/WM:A1/i.test(selectionId)) {
    return { outcome: diff === -1 ? 'WON' : 'LOST', reason: `wm_away_1_actual_${diff}` };
  }
  if (/WM:A2/i.test(selectionId)) {
    return { outcome: diff === -2 ? 'WON' : 'LOST', reason: `wm_away_2_actual_${diff}` };
  }
  if (/WM:A3\+/i.test(selectionId)) {
    return { outcome: diff <= -3 ? 'WON' : 'LOST', reason: `wm_away_3plus_actual_${diff}` };
  }
  if (/WM:draw/i.test(selectionId)) {
    return { outcome: diff === 0 ? 'WON' : 'LOST', reason: `wm_draw_actual_${diff}` };
  }

  // 2. Generic absolute differential bands (Basketball / AF)
  let band = null;
  if (/WM:16\+/i.test(selectionId)) band = '16+';
  else if (/WM:11-15/i.test(selectionId)) band = '11-15';
  else if (/WM:6-10/i.test(selectionId)) band = '6-10';
  else if (/WM:1-5/i.test(selectionId)) band = '1-5';
  if (!band) return null;

  let actual = 'draw';
  if (absDiff >= 16) actual = '16+';
  else if (absDiff >= 11) actual = '11-15';
  else if (absDiff >= 6) actual = '6-10';
  else if (absDiff >= 1) actual = '1-5';

  return {
    outcome: band === actual ? 'WON' : 'LOST',
    reason: `winning_margin_${score1}-${score2}_band_${actual}`,
  };
}

/** Tennis correct set score SS:2-0 / SS:2-1 / SS:0-2 / SS:1-2 */
export function evaluateCorrectSetScoreBet(bet, match) {
  if (!isMatchFinal(match)) return null;
  const { ld } = readFtScores(match);
  const a = asSetList(ld.sets1);
  const b = asSetList(ld.sets2);
  ...
  for (let i = 0; i < n; i += 1) {
    if (a[i] === b[i]) continue;
    // Completed set: winner must reach ≥6 games (or TB 7) with 2-game margin, or TB at 6-6.
    const maxG = Math.max(a[i], b[i]);
    const minG = Math.min(a[i], b[i]);
    const completed = (maxG >= 6 && maxG - minG >= 2) || (maxG === 7 && minG === 6) || (maxG === 7 && minG <= 5);
    if (!completed) continue;
    if (a[i] > b[i]) w1 += 1;
    else w2 += 1;
  }
  const text = `${bet.selection_id || ''} ${bet.selection_name || ''}`;
  const parsed = text.match(/SS:(\d+)\s*-\s*(\d+)/i) || text.match(/(\d+)\s*-\s*(\d+)/);
  if (!parsed) return null;
  const h = Number(parsed[1]);
  const aw = Number(parsed[2]);
  return {
    outcome: w1 === h && w2 === aw ? 'WON' : 'LOST',
    reason: `correct_set_score_${w1}-${w2}`,
  };
}

/** Basketball / AF 1H winner — requires half-time (or period-1) scores. */
export function evaluateFirstHalfWinnerBet(bet, match) {
  const ht = readHalfTimeScores(match);
  if (!ht.present) {
    if (!isMatchFinal(match)) return null;
    return { outcome: 'VOID', reason: 'first_half_scores_missing' };
  }
  if (ht.score1 === ht.score2) {
    return { outcome: 'VOID', reason: `first_half_tie_${ht.score1}-${ht.score2}` };
  }
  const winner = ht.score1 > ht.score2 ? '1' : '2';
  const selectionId = String(bet.selection_id || '');
  const wantHome = /H1:1|:1$|^1$/i.test(selectionId);
  const wantAway = /H1:2|:2$|^2$/i.test(selectionId);
  if (wantHome && !wantAway) {
    return { outcome: winner === '1' ? 'WON' : 'LOST', reason: `h1_winner_${ht.score1}-${ht.score2}` };
  }
  if (wantAway) {
    return { outcome: winner === '2' ? 'WON' : 'LOST', reason: `h1_winner_${ht.score1}-${ht.score2}` };
  }
  return null;
}

/** Basketball / AF 1H total — grades on HT points only. */
export function evaluateFirstHalfTotalBet(bet, match) {
  const ht = readHalfTimeScores(match);
  if (!ht.present) {
    if (!isMatchFinal(match)) return null;
    return { outcome: 'VOID', reason: 'first_half_scores_missing' };
  ...
  const line = parseOuLine(selectionName, selectionId);
  if (line == null) return null;
  const total = ht.score1 + ht.score2;
  if (isOverSelection(selectionId, selectionName)) {
    return { outcome: total > line ? 'WON' : 'LOST', reason: `h1_total=${total}_line=${line}` };
  }
  if (isUnderSelection(selectionId, selectionName)) {
    return { outcome: total < line ? 'WON' : 'LOST', reason: `h1_total=${total}_line=${line}` };
  }
...
export function resolveDeliveryBallFromMatch(match, overNum, ballNum, inningsHint = null) {
  ...
  for (const row of rows) {
    ...
    if (inningsHint != null) {
      const rowInn = Number(row.inningsId || row.innings || 0);
      if (rowInn > 0 && rowInn !== Number(inningsHint)) continue;
    }
  ...
  if (parts) {
    ...
    const currentInn = bat.innings || 1;
    if (currentOverNum === targetOver && (inningsHint == null || Number(inningsHint) === currentInn)) {
...
function isSrlMatchId(matchOrId) {
  if (!matchOrId) return false;
  if (typeof matchOrId === 'string') {
    return matchOrId.startsWith('srl_') || /srl/i.test(matchOrId);
  }
  const id = String(matchOrId.id || matchOrId.matchId || '');
  return matchOrId.source === 'srl' || id.startsWith('srl_');
}

/** Grade an SRL delivery from the in-house sim timeline (never wait on Cricbuzz). */
export async function resolveSrlDeliveryOutcome(matchId, innings, overNum, ballNum) {
  try {
    const { getSrlDeliveryBallLabel } = await import('./iplSrlSimulator.mjs');
    const label = getSrlDeliveryBallLabel(matchId, innings, overNum, ballNum);
    if (!label) return null;
    return parseDeliveryBallOutcome(label);
  } catch {
    return null;
  }
}
...
function trySettlePassedDeliveryBet(bet, match, overNum, ballNum, slot, confirmedEvent = null) {
  ...
  const inningsHint = hit?.[1] != null ? Number(hit[1]) : null;
  const ballOutcome = confirmedEvent?.parsed
    ?? resolveDeliveryBallFromMatch(match, overNum, ballNum, inningsHint);
...
async function trySettlePassedDeliveryBetAsync(bet, match, overNum, ballNum, slot, confirmedEvent = null) {
  const sync = trySettlePassedDeliveryBet(bet, match, overNum, ballNum, slot, confirmedEvent);
  if (sync) return sync;

  const matchId = String(bet.match_id || match?.id || match?.matchId || '');
  if (!isSrlMatchId(match || matchId)) return null;

  const hit = String(bet.market_id || '').match(/^(?:i(\d+)_)?next_delivery_[a-z]+_(\d+)_(\d+)$/i);
  const innings = hit?.[1] != null ? Number(hit[1]) : (getBattingOversAndScore(match).innings || 1);
  const outcome = await resolveSrlDeliveryOutcome(matchId, innings, overNum, ballNum);
  return gradeDeliveryMarketBet(bet, outcome);
}

/** In-play / ungradeable markets once the fixture is over or gone from the book. */
export function evaluateBetAfterMatchOver(bet, match = null) {
  const market = String(bet.market_id || '');
  ...
  if (/wicket_in_(?:next_)?over_/i.test(market)) {
    const hit = market.match(/^(?:i(\d+)_)?wicket_in_(?:next_)?over_(\d+)$/i);
    if (match && hit) {
      const settleInnings = hit[1] != null ? Number(hit[1]) : 1;
      const overNum = Number(hit[2]);
      if (isOverNeverCompleted(match, settleInnings, overNum)) {
  ...
  // next_over totals: only VOID when we can prove the over never completed — never blind-void
  // (County/Test matches were wrongly treated as final via wall-clock heuristics).
  if (/next_over_\d+_total/i.test(market)) {
    const nextOver = market.match(/^(?:i(\d+)_)?next_over_(\d+)_total$/i);
    if (!match || !nextOver) return null;
    const settleInnings = nextOver[1] != null ? Number(nextOver[1]) : 1;
    const overNum = Number(nextOver[2]);
    if (isOverNeverCompleted(match, settleInnings, overNum)) {
...
export async function evaluateDeliveryMarketBetAsync(bet, match, matchId) {
  ...
  // OddsYra SRL: always rehydrate operator clock + timeline BEFORE ballPassed gates.
  // Aggregator cache can lag the desk seek by one ball and would otherwise return null.
  if (isSrlMatchId(match || id) && hit) {
    try {
      const { refreshSrlOperatorSession } = await import('./iplSrlOperatorState.mjs');
      await refreshSrlOperatorSession(id);
      const { getIplSrlMatchById, getSrlDeliveryBallLabel } = await import('./iplSrlSimulator.mjs');
      const srlFresh = getIplSrlMatchById(id, Date.now(), { forPublic: false });
      if (srlFresh) {
        board = {
          ...(match || {}),
          ...srlFresh,
          ...
          matchId: id,
          source: 'srl',
          overHistory: srlFresh.overHistory?.length
            ? srlFresh.overHistory
            : (match?.overHistory || match?.liveDetails?.overHistory || []),
          liveDetails: {
            ...(match?.liveDetails || {}),
            ...(srlFresh.liveDetails || {}),
          },
        };
      }
      ...
      const overNum = Number(hit[2]);
      const ballNum = Number(hit[3]);
      const queryInn = marketInn ?? getBattingOversAndScore(board || {}).innings ?? 1;

      const first = evaluateDeliveryMarketBet(bet, board);
      // never_bowled VOID is often a false negative when chase overHistory hid 1st-innings balls.
      if (first && !(first.outcome === 'VOID' && /never_bowled/i.test(String(first.reason || '')))) {
        return first;
      }

      const bat = getBattingOversAndScore(board || {});
      const oversStr = bat.oversStr || board?.liveDetails?.overs;
      let ballPassed = false;
      let slot = { overNum: overNum + 1, ballNum: 1 };
      if (oversStr != null && String(oversStr).trim() !== '') {
        const format = resolveCricketFormat(board);
        const rules = getFormatRules(format) || getFormatRules('T20');
        const ballsCompleted = oversToBallsForMatch(oversStr, board);
        slot = nextBallSlot(ballsCompleted, rules.ballsPerOver || 6);
        ballPassed = slot.overNum > overNum
          || (slot.overNum === overNum && slot.ballNum > ballNum);
      }

      // Timeline presence is authoritative even if overs string is momentarily stale.
      const timelineLabel = getSrlDeliveryBallLabel(id, queryInn, overNum, ballNum);
      if (timelineLabel) ballPassed = true;

      if (!ballPassed && (board?.matchState === 'post' || Number(board?.liveDetails?.inningsId) > queryInn)) {
        ballPassed = true;
      }
      if (!ballPassed) return null;

      const fromBoard = await trySettlePassedDeliveryBetAsync(bet, board, overNum, ballNum, slot);
      if (fromBoard) return fromBoard;

      const outcome = await resolveSrlDeliveryOutcome(id, queryInn, overNum, ballNum);
      const graded = gradeDeliveryMarketBet(bet, outcome);
      if (graded) return graded;
      // Only VOID when the sim timeline also lacks the delivery.
      if (first?.outcome === 'VOID') return first;
      return null;
    } catch (err) {
      console.error('[deliverySettlement] srl', id, err.message);
  ...
  if (confirmed?.isConfirmed) {
    const settled = await trySettlePassedDeliveryBetAsync(bet, board, overNum, ballNum, slot, confirmed);
    if (settled) return settled;
  }

  const enriched = await enrichMatchForDeliverySettlement(board, matchId);
  const second = evaluateDeliveryMarketBet(bet, enriched);
  if (second) return second;

  // Provider confirmed scorecard-only — keep open until ops / recovery (do not VOID).
  if (
    (enriched?.scorecardOnly || board?.scorecardOnly)
    && (overSnapDone || isTargetOverComplete(enriched || board, overNum))
  ) {
    return {
      outcome: null,
      awaitingEvidence: true,
      reason: 'delivery_scorecard_only_awaiting_ball_feed',
    };
  }

  // Over done but no ball-by-ball — await evidence; never invent VOID refunds.
  if (overSnapDone || isTargetOverComplete(enriched || board, overNum)) {
    return {
      outcome: null,
      awaitingEvidence: true,
      reason: 'delivery_no_ball_evidence_awaiting',
    };
  }
  return null;
}
...
async function evaluateLegForSettlement(betRow, match, matchId) {
  ...
  if (grader === 'tossWinnerMarket') {
    return evaluateTossWinnerBet(betRow, match);
  }
  if (grader === 'tossElectionMarket') {
    return evaluateTossElectionBet(betRow, match);
  }
  if (grader === 'teamBatFirstMarket') {
    return evaluateTeamBatFirstBet(betRow, match);
  }
  ...
  if (grader === 'soccerHalfTimeMarket') {
    return evaluateSoccerHalfTimeBet(betRow, match);
  }
  if (grader === 'soccerCorrectScoreMarket') {
    return evaluateSoccerCorrectScoreBet(betRow, match);
  }
  if (grader === 'firstToScoreMarket') {
    return evaluateFirstToScoreBet(betRow, match);
  }
  if (grader === 'winningMarginMarket') {
    return evaluateWinningMarginBet(betRow, match);
  }
  if (grader === 'correctSetScoreMarket') {
    return evaluateCorrectSetScoreBet(betRow, match);
  }
  if (grader === 'halfWinnerMarket') {
    return evaluateFirstHalfWinnerBet(betRow, match);
  }
  if (grader === 'halfTotalMarket') {
    return evaluateFirstHalfTotalBet(betRow, match);
  }
...
export async function settleOpenBetsFromLiveScores({ limit = 200, matchId = null, seedMatches = [] } = {}) {
  clearDeliverySettlementCache();

  const matchIdFilter = matchId
    ? [...new Set(matchIdAliases(String(matchId)).map(String).filter(Boolean))]
    : null;

  const betsRes = await dbQuery(
    matchIdFilter?.length
      ...
      : `SELECT b.bet_id, b.match_id, b.market_id, b.selection_id, b.status, b.stake, b.bet_type,
         ...
         ORDER BY b.created_at ASC
         LIMIT $1`,
    matchIdFilter?.length ? [limit, matchIdFilter] : [limit],
  );
  ...
  // Prefer desk-fresh SRL boards over aggregator cache (cache lags seeks by one ball).
  for (const seeded of (Array.isArray(seedMatches) ? seedMatches : [])) {
    if (seeded) {
      indexMatch(liveById, seeded);
      indexMatch(byId, seeded);
    }
  }
  ...
  // Re-seed after live book so stale aggregator rows cannot overwrite fresh SRL.
  for (const seeded of (Array.isArray(seedMatches) ? seedMatches : [])) {
    if (seeded) {
      indexMatch(liveById, seeded);
      indexMatch(byId, seeded);
  ...
  // Any open SRL bets: force-reload operator session and index a fresh sim board.
  try {
    const { refreshSrlOperatorSession } = await import('./iplSrlOperatorState.mjs');
    const { getIplSrlMatchById } = await import('./iplSrlSimulator.mjs');
    const srlIds = new Set();
    for (const bet of betsRes.rows) {
      const mid = String(bet.match_id || '');
      if (mid.startsWith('srl_')) srlIds.add(mid);
      for (const leg of normalizeBetLegs(bet)) {
        const lid = String(leg.match_id || '');
        if (lid.startsWith('srl_')) srlIds.add(lid);
      }
    }
    if (matchIdFilter) {
      for (const mid of matchIdFilter) {
        if (String(mid).startsWith('srl_')) srlIds.add(String(mid));
      }
    }
    for (const srlId of srlIds) {
      await refreshSrlOperatorSession(srlId);
      const fresh = getIplSrlMatchById(srlId, Date.now(), { forPublic: false });
      if (fresh) {
        indexMatch(liveById, fresh);
        indexMatch(byId, fresh);
      }
    }
  } catch (err) {
    console.error('[liveMatchSettlement] srl refresh', err.message);
  }
    ...
    try {
      ...
      if (!evaluated && finalSource) {
        ...
        if (!evaluated && hasFinalResultWithoutBallFeed(canonicalFinal)) {
          evaluated = evaluateOpenBetOutcome(betRow, matchState)
            || evaluateBetAfterMatchOver(betRow, canonicalFinal);
        } else if (!evaluated && isAuthoritativeMatchFinal(canonicalFinal)) {
          evaluated = evaluateBetAfterMatchOver(betRow, canonicalFinal);
        }
      }
      ...
      if (!evaluated || evaluated.awaitingEvidence || !evaluated.outcome) {
        skipped++;
        continue;
      }
      ...
      try {
        const { recordSettledCalibrationObservation } = await import('./odds-v4/calibration/calibrationBridge.mjs');
        recordSettledCalibrationObservation({
          ...
          acceptedOdds: bet.accepted_odds || bet.odds,
          sport: candidateMatch?.sport || 'cricket',
          settledAt: new Date().toISOString(),
        });
      } catch {
        // Non-fatal V4 calibration bridge
      }
