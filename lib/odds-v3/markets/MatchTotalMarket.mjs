...
export function generateMatchTotalMarket(state, validation = {}, marginConfig = DEFAULT_MARGIN_CONFIG) {
  const overround = marginConfig.liveMatchTotalOverround ?? DEFAULT_MARGIN_CONFIG.liveMatchTotalOverround;
  ...
  const lead = state.status === 'LIVE'
    ...
    : 0.5;
  let effectiveLine = Math.max(rawLine, currentCombined + lead);
  ...
  // Don't keep selling once combined score already clears the line
  if (state.status === 'LIVE' && currentCombined >= effectiveLine) {
    return createMarketDefinition({
      marketId: 'match_total',
      marketType: 'MATCH_TOTAL',
      name: 'Total Match Runs',
  ...
  const spread = state.status === 'LIVE'
    ...
    : 8;
  const { pOver, pUnder } = calculateLineProbability(expectedMatchTotal, effectiveLine, spread);
