...
export function evaluateFeedCircuitBreaker(matchId, tickMetadata = {}, customConfig = {}) {
  ...
  // 1. Check for excessive latency (when timestamp present)
  if (tickMetadata.timestamp && latencyMs > config.maxFeedLatencyMs) {
  ...
  // 1b. Stale tick age vs last *recorded* healthy tick (wall clock)
  const ageSinceLastTick = Math.max(0, now - (history.lastHealthyTickTime || history.lastTickTime || now));
  if (history.lastHealthyTickTime && ageSinceLastTick > config.maxStaleTickAgeMs) {
    history.isSuspended = true;
    history.consecutiveHealthyTicks = 0;
    history.trippedReason = `FEED_STALE (${ageSinceLastTick}ms > ${config.maxStaleTickAgeMs}ms)`;
    MATCH_FEED_HEALTH.set(matchId, history);
    return { isTripped: true, reason: history.trippedReason, latencyMs };
  }
