...
let srlDeskOpsInterval = null;
let srlLaunchBroadcastInterval = null;
let activationNudgeInterval = null;
let kycAutoReminderInterval = null;
...
export function startBackgroundWorkers() {
  ...
  // Scheduled full SQL dumps (admin-configured interval; default disabled)
  const tickBackup = async () => {
    try {
      const { tickScheduledBackup } = await import('./backupEngine.mjs');
      const res = await tickScheduledBackup();
      if (res.ran) {
        console.log(`[Backup] scheduled dump ok id=${res.backupId} size=${res.sizeBytes}`);
      }
    } catch (err) {
      console.error('[Backup Loop Error]', err.message);
    }
  };
  if (!backupInterval) {
    setTimeout(tickBackup, 90_000);
    backupInterval = setInterval(tickBackup, 5 * 60 * 1000);
  }

  // SRL Match Control desk sweep: liability pause, velocity breaker, offline pagers, auto-settle
  const tickSrlDeskOps = async () => {
    try {
      const { runSrlDeskOpsSweep } = await import('./iplSrlAdminControl.mjs');
      const res = await runSrlDeskOpsSweep();
      if (res.actions?.length) {
        console.log(`[SrlDeskOps] actions=${res.actions.length} online=${res.coverage?.onlineCount || 0}`);
      }
    } catch (err) {
      console.error('[SrlDeskOps Loop Error]', err.message);
    }
  };
  if (!srlDeskOpsInterval) {
    setTimeout(tickSrlDeskOps, 8_000);
    srlDeskOpsInterval = setInterval(tickSrlDeskOps, 15_000);
  }

  // Continue OddsYra SRL launch email/in-app broadcast while job is running
  const tickSrlLaunchBroadcast = async () => {
    try {
      const {
        getSrlLaunchBroadcastJob,
        processSrlLaunchBroadcastBatch,
      } = await import('./srlLaunchBroadcast.mjs');
      const job = await getSrlLaunchBroadcastJob();
      if (job.status !== 'running') return;
      const res = await processSrlLaunchBroadcastBatch({ batchSize: 25 });
      if (res.processed) {
        console.log(
          `[SrlLaunch] batch=${res.processed} emailSent=${res.job?.totals?.emailSent || 0} status=${res.job?.status}`,
        );
      }
    } catch (err) {
      console.error('[SrlLaunch Loop Error]', err.message);
    }
  };
  if (!srlLaunchBroadcastInterval) {
    setTimeout(tickSrlLaunchBroadcast, 20_000);
    srlLaunchBroadcastInterval = setInterval(tickSrlLaunchBroadcast, 20_000);
  }

  // Activation nudge: deposited but no first bet (hourly)
  const tickActivationNudge = async () => {
    try {
      const { processUnconvertedDepositorNudges } = await import('./activationNudgeWorker.mjs');
      const res = await processUnconvertedDepositorNudges({ limit: 25 });
      if (res.sent > 0 || res.skipped > 0) {
        console.log(`[ActivationNudge] checked=${res.checked} sent=${res.sent} skipped=${res.skipped}`);
      }
    } catch (err) {
      console.error('[ActivationNudge Loop Error]', err.message);
    }
  };
  if (!activationNudgeInterval) {
    setTimeout(tickActivationNudge, 180_000);
    activationNudgeInterval = setInterval(tickActivationNudge, 60 * 60 * 1000);
  }

  // Auto-queue KYC reminders for depositors missing KYC (every 6h)
  const tickKycAuto = async () => {
    try {
      const { enqueueAutoKycReminders } = await import('./kycAutoReminderWorker.mjs');
      const res = await enqueueAutoKycReminders({ limit: 20 });
      if (res.queued > 0) {
        console.log(`[KycAutoReminder] queued=${res.queued} skipped=${res.skipped}`);
      }
    } catch (err) {
      console.error('[KycAutoReminder Loop Error]', err.message);
    }
  };
  if (!kycAutoReminderInterval) {
    setTimeout(tickKycAuto, 240_000);
    kycAutoReminderInterval = setInterval(tickKycAuto, 6 * 60 * 60 * 1000);
  }

  console.log('✅ Background Workers Active: Outbox (2s), Live aggregator (15s), Settlement Recovery (30s), Spin grant expiry (30s), Support SLA (300s), KYC Reminder (120s), Activation nudge (1h), KYC auto-queue (6h), Reconciliation (300s), Ops Alerts (60s), Notifications (15s), API Explorer health (60s), Resettle scan (6h), Payments digest, Backups (5m), SRL Desk Ops (15s), SRL Launch Broadcast (20s)');
}
...
export function stopBackgroundWorkers() {
  ...
  if (srlDeskOpsInterval) clearInterval(srlDeskOpsInterval);
  if (srlLaunchBroadcastInterval) clearInterval(srlLaunchBroadcastInterval);
  if (activationNudgeInterval) clearInterval(activationNudgeInterval);
  if (kycAutoReminderInterval) clearInterval(kycAutoReminderInterval);
  outboxInterval = null;
  reconInterval = null;
  ...
  backupInterval = null;
  srlDeskOpsInterval = null;
  srlLaunchBroadcastInterval = null;
  activationNudgeInterval = null;
  kycAutoReminderInterval = null;
