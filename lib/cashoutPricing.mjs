/**
 * Live cashout pricing from current OddsEngineV3 snapshot.
 * Fair value = stake × Π(accepted_leg / current_leg), then VIP cashout %.
 */

import { resolveServerOdds, unwrapServerOddsQuote } from './oddsQuoteService.mjs';
import { getBenefitsForTier } from './vipBenefits.mjs';
import { isTotalsMarket, isOverSelection } from './houseProtectionEngine.mjs';

const HOUSE_CASHOUT_MARGIN = 0.04; // extra 4% house take on fair value before VIP pct
/** Soft Over totals near lock — block cashout (settle instead / wait). */
const SOFT_OVER_LOCK_ODDS = 1.12;
/** Soft Over totals collapsing — heavy haircut. */
const SOFT_OVER_HAIRCUT_ODDS = 1.28;
const SOFT_OVER_HAIRCUT_FACTOR = 0.45;

/**
 * @param {{
 *   stake: number,
 *   acceptedOdds: number,
 *   legs?: Array<{ matchId: string, marketId: string, selectionId: string, odds: number }>,
 *   matchId?: string,
 *   marketId?: string,
 *   selectionId?: string,
 *   vipTier?: string,
 * }} bet
 */
export async function priceCashoutFromV3Snapshot(bet) {
  const stake = Number(bet.stake) || 0;
  const acceptedOdds = Number(bet.acceptedOdds || bet.odds) || 0;
  if (stake <= 0 || acceptedOdds < 1.01) {
    return { available: false, cashoutValue: 0, reason: 'INVALID_BET' };
  }

  let legs = Array.isArray(bet.legs) ? bet.legs.filter((l) => l?.matchId && l?.marketId && l?.selectionId) : [];
  if (legs.length === 0 && bet.matchId && bet.marketId && bet.selectionId) {
    legs = [{
      matchId: bet.matchId,
      marketId: bet.marketId,
      selectionId: bet.selectionId,
      odds: acceptedOdds,
      selectionName: bet.selectionName || bet.selection_name || null,
    }];
  }
  if (legs.length === 0) {
    return { available: false, cashoutValue: 0, reason: 'NO_SELECTIONS' };
  }

  const currentLegs = [];
  let oddsRatio = 1;

  for (const leg of legs) {
    const acceptedLeg = Number(leg.odds) || acceptedOdds;
    let currentOdds;
    try {
      currentOdds = unwrapServerOddsQuote(await resolveServerOdds({
        matchId: leg.matchId,
        marketId: leg.marketId,
        selectionId: leg.selectionId,
        clientOdds: null,
        selectionName: leg.selectionName || leg.selection_name || null,
      }));
    } catch (err) {
      const msg = err?.message || '';
      if (msg.startsWith('MARKET_ALREADY_DETERMINED') || msg.startsWith('ODDS_LOCKED') || msg.startsWith('MARKET_SUSPENDED')) {
        return { available: false, cashoutValue: 0, reason: msg.split(':')[0], detail: msg };
      }
      return { available: false, cashoutValue: 0, reason: 'ODDS_UNAVAILABLE', detail: msg };
    }

    if (!(currentOdds >= 1.01)) {
      return { available: false, cashoutValue: 0, reason: 'ODDS_UNAVAILABLE' };
    }

    oddsRatio *= acceptedLeg / currentOdds;
    currentLegs.push({
      matchId: leg.matchId,
      marketId: leg.marketId,
      selectionId: leg.selectionId,
      acceptedOdds: acceptedLeg,
      currentOdds,
      softOver: isTotalsMarket(leg.marketId)
        && isOverSelection(leg.selectionId, leg.selectionName || leg.selection_name),
    });
  }

  // Soft Over protection: near-certain Overs cannot lock value via cashout
  for (const leg of currentLegs) {
    if (!leg.softOver) continue;
    if (leg.currentOdds <= SOFT_OVER_LOCK_ODDS) {
      return {
        available: false,
        cashoutValue: 0,
        reason: 'SOFT_OVER_LOCKED',
        detail: 'Cashout unavailable — Over is effectively locked; wait for settlement',
        currentLegs,
      };
    }
  }

  const potentialPayout = Number((stake * acceptedOdds).toFixed(2));
  const fairCashout = Number((stake * oddsRatio).toFixed(4));
  const afterHouseMargin = fairCashout * (1 - HOUSE_CASHOUT_MARGIN);
  const vipPct = getBenefitsForTier(bet.vipTier || 'BRONZE').cashoutPayoutPct || 0.85;
  let cashoutValue = afterHouseMargin * vipPct;
  let softOverHaircut = false;

  const softOverLegs = currentLegs.filter((l) => l.softOver);
  if (softOverLegs.length) {
    const minCurrent = Math.min(...softOverLegs.map((l) => l.currentOdds));
    const maxAccepted = Math.max(...softOverLegs.map((l) => l.acceptedOdds));
    if (minCurrent <= SOFT_OVER_HAIRCUT_ODDS || minCurrent < maxAccepted * 0.72) {
      softOverHaircut = true;
      cashoutValue = Math.min(cashoutValue * SOFT_OVER_HAIRCUT_FACTOR, stake * 0.55);
    }
  }

  // Cap only — never inflate dead bets with a 10% stake floor
  const maxCashout = potentialPayout * 0.98;
  cashoutValue = Math.max(0, Math.min(maxCashout, cashoutValue));
  cashoutValue = Number(cashoutValue.toFixed(2));

  if (cashoutValue < 0.01) {
    return { available: false, cashoutValue: 0, reason: 'CASHOUT_TOO_LOW', currentLegs };
  }

  return {
    available: true,
    cashoutValue,
    fairCashout: Number(fairCashout.toFixed(2)),
    potentialPayout,
    stake,
    acceptedOdds,
    vipCashoutPct: vipPct,
    houseMarginPct: HOUSE_CASHOUT_MARGIN * 100,
    softOverHaircut,
    currentLegs,
    pricedAt: new Date().toISOString(),
  };
}

/**
 * Calculate Partial Cashout values
 * @param {object} fullCashoutQuote Full cashout result from priceCashoutFromV3Snapshot
 * @param {number} percentage Cashout proportion (e.g. 0.25, 0.50, 0.75)
 */
export function calculatePartialCashout(fullCashoutQuote, percentage = 0.5) {
  const pct = Math.max(0.1, Math.min(0.9, Number(percentage) || 0.5));
  if (!fullCashoutQuote || !fullCashoutQuote.available) {
    return { available: false, reason: 'FULL_CASHOUT_UNAVAILABLE' };
  }

  const partialCashoutValue = Number((fullCashoutQuote.cashoutValue * pct).toFixed(2));
  const remainingStake = Number((fullCashoutQuote.stake * (1 - pct)).toFixed(2));
  const remainingPotentialPayout = Number((remainingStake * fullCashoutQuote.acceptedOdds).toFixed(2));

  return {
    available: true,
    percentage: pct,
    partialCashoutValue,
    remainingStake,
    remainingPotentialPayout,
    originalStake: fullCashoutQuote.stake,
    fullCashoutValue: fullCashoutQuote.cashoutValue,
  };
}
