/**
 * Probability Engine — Pure ES Module Implementation for OddsYra Sportsbook.
...
// ---------------------------------------------------------------------------
// 8. IPLSRL Live Match Probability Model
// ---------------------------------------------------------------------------
export function calculateIPLSRLMatchProbabilities(matchState = {}) {
  const {
    currentInnings = 1,
    targetScore = 175,
    innings1 = { runs: 174, wickets: 5, overs: 20 },
    innings2 = { runs: 85, wickets: 3, overs: 10, balls: 2 },
    homeTeamRating = 85,
    awayTeamRating = 82,
  } = matchState;

  // Pre-match / Innings 1 ELO base
  const eloBase = calculateEloProbability(homeTeamRating * 18, awayTeamRating * 18, 25);
  let homeWinProb = eloBase.homeWin;

  if (currentInnings === 1) {
    const oversDone = innings1.overs + (innings1.balls || 0) / 6;
    const crr = oversDone > 0 ? innings1.runs / oversDone : 8.5;
    const projectedRuns = Math.round(innings1.runs + (20 - oversDone) * Math.max(7.0, crr));
    const winDelta = ((projectedRuns - 165) / 100) * 0.3;
    homeWinProb = Math.max(0.05, Math.min(0.95, homeWinProb + winDelta));

    return {
      homeWinProbability: Number(homeWinProb.toFixed(3)),
      awayWinProbability: Number((1.0 - homeWinProb).toFixed(3)),
      projectedScore: projectedRuns,
      projectedWickets: Math.min(10, innings1.wickets + Math.round((20 - oversDone) * 0.25)),
      nextOverProbability: { overRunsUnder7: 0.35, overRuns8to11: 0.45, overRuns12Plus: 0.20 },
      nextBallProbability: { dot: 0.35, run1: 0.38, boundary: 0.18, wicket: 0.05, extra: 0.04 },
    };
  }

  // Innings 2 Live Chase Model
  const oversLeft = Math.max(0.1, 20 - innings2.overs - (innings2.balls || 0) / 6);
  const runsNeeded = Math.max(0, targetScore - innings2.runs);
  const rrr = runsNeeded / oversLeft;
  const wicketsLeft = Math.max(1, 10 - innings2.wickets);

  // Required Run Rate & Wickets Factor
  const difficulty = (rrr / (wicketsLeft * 1.5));
  let chaseWinProb = Math.max(0.02, Math.min(0.98, 1.0 - (difficulty * 0.35)));

  // If batting second team is home vs away
  const isHomeBattingSecond = matchState.innings2?.battingTeamId === matchState.homeTeam?.teamId;
  homeWinProb = isHomeBattingSecond ? chaseWinProb : 1.0 - chaseWinProb;

  return {
    homeWinProbability: Number(homeWinProb.toFixed(3)),
    awayWinProbability: Number((1.0 - homeWinProb).toFixed(3)),
    projectedScore: innings2.runs + Math.round(oversLeft * (innings2.runs / Math.max(1, 20 - oversLeft))),
    requiredRunRate: Number(rrr.toFixed(2)),
    nextOverProbability: { overRunsUnder7: rrr > 10 ? 0.25 : 0.40, overRuns8to11: 0.45, overRuns12Plus: 0.30 },
    nextBallProbability: { dot: 0.32, run1: 0.40, boundary: 0.19, wicket: 0.05, extra: 0.04 },
  };
}
