...
export function generateExtendedPlayerMarkets(state, validation = {}, marginConfig = {}) {
  ...
  const rawAltOver = calculatePlayerMilestoneProbability(b1Runs, altLine, ballsRemaining);
  const pAltOver = Math.max(0.0001, Math.min(0.9999, rawAltOver));
  const isAltDetermined = b1Runs > altLine;
  const isAltSuspended = !isAltDetermined && pAltOver < 0.02;
  ...
  markets.push(createMarketDefinition({
    ...
    status: isAltDetermined ? 'SETTLED' : (isAltSuspended ? 'SUSPENDED' : 'OPEN'),
    line: altLine,
    selections: [
      priceSelection({ selectionId: 'sel_palt_over', name: `Over ${altLine}`, probability: isAltDetermined ? 0.99 : pAltOver, overround }),
      priceSelection({ selectionId: 'sel_palt_under', name: `Under ${altLine}`, probability: isAltDetermined ? 0.01 : Math.max(0.0001, 1.0 - pAltOver), overround }),
    ],
  }));
  ...
  const addMilestoneMarket = (target, marketId, marketType, name) => {
    ...
    const isSuspended = !isPassed && pReach < 0.02;
    ...
    markets.push(createMarketDefinition({
      ...
      status: isPassed ? 'SETTLED' : (isSuspended ? 'SUSPENDED' : 'OPEN'),
      selections: [
        priceSelection({ selectionId: `sel_${target}_yes`, name: 'Yes', probability: isPassed ? 0.99 : pReach, overround }),
        priceSelection({ selectionId: `sel_${target}_no`, name: 'No', probability: isPassed ? 0.01 : Math.max(0.0001, 1.0 - pReach), overround }),
      ],
    }));
  };
