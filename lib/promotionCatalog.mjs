...
/** Internal grant vehicles / private campaigns must never appear on the public page. */
export function isPublicCatalogPromotionRow(row = {}) {
  if (row.is_targeted === true || row.is_targeted === 't' || row.is_targeted === 1) return false;
  const code = String(row.code || '').toUpperCase();
  const id = String(row.id || '');
  if (code.startsWith('SPIN_')) return false;
  if (id.startsWith('promo_spin_')) return false;
  if (id.startsWith('promo_tdfb_')) return false;
  if (code.startsWith('TDFB')) return false;
  return true;
}
...
export async function listPublicPromotionCatalog() {
  ...
  const [promosRes, codesRes] = await Promise.all([
    query(`
      SELECT id, name, code, type, max_reward, min_odds, min_stake, wagering_multiplier,
             match_percent, starts_at, expires_at, COALESCE(is_targeted, FALSE) AS is_targeted
      FROM promotions
      WHERE status = 'ACTIVE'
        AND COALESCE(is_targeted, FALSE) = FALSE
        AND code NOT ILIKE 'SPIN\\_%'
        AND id NOT LIKE 'promo_spin_%'
        AND id NOT LIKE 'promo_tdfb_%'
        AND code NOT ILIKE 'TDFB%'
  ...
  return [
    ...promosRes.rows.filter(isPublicCatalogPromotionRow).map(mapDepositPromotionRow),
    ...codesRes.rows.map(mapSignupCodeRow),
  ];
}

/** Targeted deposit free-bet campaigns assigned to this user (authenticated only). */
export async function listTargetedPromotionsForUser(userId) {
  if (!userId) return [];
  const { query } = await import('../db/pg.js');
  const res = await query(
    `
      SELECT p.id, p.name, p.code, p.type, p.max_reward, p.min_odds, p.min_stake, p.wagering_multiplier,
             p.match_percent, p.starts_at, p.expires_at, COALESCE(p.is_targeted, FALSE) AS is_targeted
      FROM promotions p
      INNER JOIN deposit_freebet_campaign_users cu ON cu.promotion_id = p.id
      WHERE cu.user_id = $1
        AND p.status = 'ACTIVE'
        AND COALESCE(p.is_targeted, FALSE) = TRUE
...
/**
 * Public catalog for anonymous visitors.
 * Logged-in users also receive their assigned targeted offers (not other users' codes).
 */
export async function listPromotionCatalogForViewer(userId = null) {
  const publicItems = await listPublicPromotionCatalog();
  if (!userId) return publicItems;
  const targeted = await listTargetedPromotionsForUser(userId);
  const seen = new Set(publicItems.map((p) => String(p.code || '').toUpperCase()));
  const extras = targeted.filter((p) => {
    const code = String(p.code || '').toUpperCase();
    if (!code || seen.has(code)) return false;
    seen.add(code);
    return true;
  });
  return [...extras, ...publicItems];
}
