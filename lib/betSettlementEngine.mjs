...
export class BetSettlementEngine {
  ...
  async settleSingleBet({ betId, matchState, authorization = null }, correlationId = null) {
    ...
    if (authObj) {
      const validation = validateSettlementAuthorization({ authorization: authObj, bet, matchState: matchState || {} });
      if (!validation.valid) {
        logSettlement('SETTLEMENT_UNAUTHORIZED_ATTEMPT', { betId, reason: validation.reason });
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (needsWalletMutation) {
        ...
        const nextLockedBonus = parseFloat((Number(wallet.locked_bonus_winnings || 0) + lockedBonusWinningsCredit).toFixed(2));
    ...
    // Trigger post-settlement hooks for loyalty points and legacy cash promo wagering
