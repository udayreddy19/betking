...
export async function runEventDrivenSettlementForMatch(match) {
  ...
  const liveRes = await settleOpenBetsFromLiveScores({
    limit: 100,
    matchId,
    seedMatches: [withBalls],
  });
...
export function scheduleEventDrivenSettlement(matches = []) {
  ...
  for (const match of matches) {
    ...
    let isFinal = state === 'post' || state === 'completed' || /^(completed|final|finished)$/i.test(String(match?.time || ''));
    if (!isFinal && match?.isCompleted === true) isFinal = true;
...
export async function runSettlementRecoverySweep({ limit = 200 } = {}) {
  ...
  // Prefer OddsYra SRL open bets so micro-markets are not starved behind older real fixtures.
  let srlRecovery = { settled: 0, skipped: 0, errors: 0 };
  try {
    const srlOpen = await query(
      `SELECT DISTINCT match_id FROM bets
       WHERE UPPER(status) IN ('ACCEPTED', 'PENDING', 'OPEN')
         AND match_id LIKE 'srl_%'
       LIMIT 40`,
    );
    for (const row of srlOpen.rows || []) {
      ...
      try {
        ...
        await refreshSrlOperatorSession(mid);
        const fresh = getIplSrlMatchById(mid, Date.now(), { forPublic: false });
        const part = await settleOpenBetsFromLiveScores({
          limit: 80,
          matchId: mid,
          seedMatches: fresh ? [fresh] : [],
        });
        srlRecovery.settled += part.settled || 0;
        srlRecovery.skipped += part.skipped || 0;
        srlRecovery.errors += part.errors || 0;
      } catch (err) {
        srlRecovery.errors += 1;
        console.error('[settlementRecovery] srl', mid, err.message);
      }
    }
  } catch (err) {
    console.error('[settlementRecovery] srl scan', err.message);
  }
  ...
  // Unstick forever-PROCESSING jobs before claiming the queue this cycle.
  let stuckProcessing = { scanned: 0, requeued: 0, failed: 0 };
  try {
    const { recoverStuckProcessingSettlementJobs } = await import('./settlementDeadLetterRecovery.mjs');
    stuckProcessing = await recoverStuckProcessingSettlementJobs({ olderThanHours: 2, limit: 200 });
  } catch (err) {
    console.error('[settlementRecovery] stuck-processing', err.message);
  }
  ...
  let deadLetter = { scanned: 0, settled: 0, requeued: 0, awaiting: 0, stuck: stuckProcessing };
  try {
  ...
  let orphanRecovery = { scanned: 0, settled: 0, awaiting: 0, escalated: 0, skippedLive: 0, errors: 0 };
  try {
    const { recoverOrphanedOpenBets } = await import('./orphanedOpenBetRecovery.mjs');
    orphanRecovery = await recoverOrphanedOpenBets({ limit: Math.min(limit, 80) });
  } catch (err) {
    console.error('[settlementRecovery] orphan', err.message);
  }

  return { recovery: res, queue: queueRes, stuckProcessing, deadLetter, srlRecovery, orphanRecovery };
}
