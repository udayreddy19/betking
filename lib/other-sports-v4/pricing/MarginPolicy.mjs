/**
 * OtherSportsEngineV4 — maximum house-first margin policy (v4.9).
 * Prefer suspending over printing a soft price. Target operator mark: 10.0.
 */

export const OSV4_ENGINE_VERSION = '4.9.0';
...
export const OSV4_MARGIN_CONFIG = Object.freeze({
  ...
  /** Baseball: 7th inning stretch → core only */
  baseballLateLockInning: 7,
  /** Ice hockey: 3rd period late → core only */
  iceHockeyLateLockMinute: 50,
  /** Baseball: looser max selection (more variance per game) */
  baseballMaxSelectionOdds: 2.85,
  baseballMaxOverOdds: 1.28,
  baseballMaxFavoriteOdds: 1.42,
  /** Ice hockey: tighter than soccer (lower-scoring, OT volatile) */
  iceHockeyMaxSelectionOdds: 2.65,
  iceHockeyMaxOverOdds: 1.22,
  iceHockeyMaxFavoriteOdds: 1.38,
});
...
export function marginsForSport(sport, margins = OSV4_MARGIN_CONFIG) {
  ...
  if (s === 'baseball') {
    return {
      ...cfg,
      maxSelectionOdds: cfg.baseballMaxSelectionOdds ?? 2.85,
      maxOverOdds: cfg.baseballMaxOverOdds ?? 1.28,
      maxFavoriteOdds: cfg.baseballMaxFavoriteOdds ?? 1.42,
      matchWinnerOverround: Math.max(cfg.matchWinnerOverround, 0.22),
      totalsOverround: Math.max(cfg.totalsOverround, 0.26),
    };
  }
  if (s === 'ice-hockey') {
    return {
      ...cfg,
      maxSelectionOdds: cfg.iceHockeyMaxSelectionOdds ?? 2.65,
      maxOverOdds: cfg.iceHockeyMaxOverOdds ?? 1.22,
      maxFavoriteOdds: cfg.iceHockeyMaxFavoriteOdds ?? 1.38,
      matchWinnerOverround: Math.max(cfg.matchWinnerOverround, 0.225),
      totalsOverround: Math.max(cfg.totalsOverround, 0.265),
    };
  }
