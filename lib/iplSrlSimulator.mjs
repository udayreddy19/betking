...
function stateAtElapsed(sim, elapsedMs, team1, team2) {
  ...
  if (elapsedMs < firstInningsEndMs) {
    ...
    return {
      ...
      bowler: inningsComplete ? undefined : {
        name: bowler.name, runs: bowler.runs, wickets: sim.first.wickets, overs: bowler.overs,
      },
  ...
  return {
    ...
    bowler: inningsComplete ? undefined : {
      name: bowler.name, runs: bowler.runs, wickets: frame.wickets, overs: bowler.overs,
    },
    currentOverBalls: inningsComplete ? [] : frame.ballDisplays.slice(-6),
    scorecardInnings: [
...
function buildMatchFromSchedule(globalSlot, now) {
  ...
  if (elapsed < 0) {
    return {
      ...
      matchState: 'pre',
      liveDetails: {
        commentary: `Scheduled · ${MAX_OVERS} overs per side · ${formatDuration(sim.timing.totalDurationMs)}`,
      },
    };
  }
