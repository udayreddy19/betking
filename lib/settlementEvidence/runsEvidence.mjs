...
export function generateRunsEvidence({
  ...
  const neverBowled = /never_bowled|never_occurred|match_over/.test(reason);
  ...
  if (neverBowled || (status === 'VOID' && /never_bowled/.test(reason))) {
    return {
      evidenceVersion: settlementEvent?.settlement_version || Number(bet.settlement_version) || 1,
      evidenceStatus: 'VERIFIED',
      evidenceType: 'CRICKET_BALL_BY_BALL',
      source: 'CANONICAL_MATCH_STATE',
      verifiedAt: bet.settled_at || settlementEvent?.created_at || new Date().toISOString(),
      settlementReason: bet.settlement_reason || reason,
      summary: `Over ${overNumber || ''} was voided — the over was not completed in this innings`,
      overNumber,
      innings,
      totalRuns: null,
      line: line > 0 ? line : null,
      timeline: [{
        over: overNumber || null,
        ball: null,
        delivery: overNumber ? `Over ${overNumber}` : 'Over',
        eventType: 'VOID',
        rawLabel: 'VOID',
        runs: null,
        wicket: false,
        outcomeLabel: `Over ${overNumber || ''} never completed / never bowled`,
  ...
  const fromBalls = relevantBalls.length
    ? relevantBalls.reduce((sum, b) => sum + Number(b.runs || 0), 0)
    : null;
  const fromSnapshotTag = String(overSnapshot?.overs_raw || '').match(/\|runs:(\d+)/);
  const totalRuns = fromBalls != null && relevantBalls.length >= 6
    ? fromBalls
    : (fromSnapshotTag ? Number(fromSnapshotTag[1])
      : (overSnapshot?.runs_in_over != null
        ? Number(overSnapshot.runs_in_over)
        : (parsedRuns != null ? parsedRuns : (fromBalls ?? 0))));
