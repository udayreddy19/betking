...
export async function adminOverrideVipTier({
  ...
  const { normalizeVipTier, VIP_TIER_ORDER } = await import('./vipBenefits.mjs');
  ...
  if (String(previous).toUpperCase() === tier) {
    return { success: true, userId, previousTier: previous, newTier: tier, unchanged: true };
  }
  ...
  try {
    ...
    enterpriseAuditEngine.recordEvent?.({
      ...
      referenceId: userId,
      reason: auditReason,
      after: { previousTier: previous, newTier: tier },
    });
  } catch {
    // optional audit sink
  }
