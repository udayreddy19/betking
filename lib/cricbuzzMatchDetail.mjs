...
function parseCommentaryBattersAndBowler(html) {
  ...
  const batterMatches = [...html.matchAll(/([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s+(\d+)\s*\(\s*(\d+)\s*\)/g)];
  if (batterMatches.length >= 1) {
    result.batter1 = {
      name: batterMatches[0][1].trim(),
  ...
  const bowlerMatch = html.match(/([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s+(\d+)[/-](\d+)/);
  if (bowlerMatch) {
    result.bowler = {
