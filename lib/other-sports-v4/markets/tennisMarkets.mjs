...
export function generateTennisMatchWinner(match, team1Name, team2Name, margins = OSV4_MARGIN_CONFIG) {
  ...
  // ESPN linescores include the in-progress set; only completed sets feed Markov set score.
  // Current games: use live score1/score2 when present, else last incomplete set pair.
  let gamesA = score1;
  let gamesB = score2;
...
export function generateTennisExtras(match, team1Name, team2Name, winnerMarket, margins = OSV4_MARGIN_CONFIG) {
  ...
  // Set 1 decided — do not keep a soft certain-outcome book open.
  if (sets1.length > 0 && sets2.length > 0 && isCompletedTennisSet(sets1[0], sets2[0])) {
    return [
      suspendedMarket({
        marketId: 'set1_winner',
        marketType: 'SET_WINNER',
      ...
      // Totals still priced below when match ongoing
      (() => {
