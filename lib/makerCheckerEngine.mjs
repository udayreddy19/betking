  ...
  async approveRequest(requestId, checkerId = 'admin_checker') {
    return await withTransaction(async (client) => {
      ...
      const isDebit = direction === 'debit'
        || req.action_type === 'MANUAL_DEBIT'
        || (req.action_type === 'WALLET_ADJUSTMENT' && Number(payload.amount) < 0);

      // 2. Execute Financial Operation atomically
      if (
        req.action_type === 'MANUAL_CREDIT'
        || req.action_type === 'REFUND'
        || (req.action_type === 'BONUS_ADJUSTMENT' && !isDebit)
        || (req.action_type === 'WALLET_ADJUSTMENT' && !isDebit)
      ) {
        // Lock Wallet
        ...
        const wallet = wRes.rows[0];
        const currentBal = Number(wallet.balance);
        if (currentBal < amount) throw new Error('INSUFFICIENT_BALANCE');
        const newBal = currentBal - amount;
        ...
        await client.query(`
          INSERT INTO transactions (transaction_id, user_id, type, amount, status)
          VALUES ($1, $2, $3, $4, 'COMPLETED');
        `, [txId, userId, req.action_type === 'BONUS_ADJUSTMENT' ? 'BONUS_CLAWBACK' : 'MANUAL_DEBIT', amount]);

        await client.query(`
          INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description)
          VALUES ($1, $2, 'DEBIT', $3, $4, $5);
        `, [wallet.wallet_id, txId, amount, newBal, `Maker-Checker Approved ${req.action_type} debit`]);
      } else if (req.action_type === 'SETTLEMENT_CORRECTION') {
        const betId = req.target_entity_id;
        const bRes = await client.query('SELECT * FROM bets WHERE bet_id = $1 FOR UPDATE', [betId]);
        if (bRes.rows.length === 0) throw new Error('BET_NOT_FOUND');
        const bet = bRes.rows[0];

        const { betSettlementEngine } = await import('./betSettlementEngine.mjs');
        ...
        await betSettlementEngine.settleSingleBet({
          ...
          matchState: {
            ...
            status: 'COMPLETED',
            __forcedOutcome: payload.outcome || payload.requestedOutcome,
            __bypassAuth: true,
            __settlementReason: payload.reason || `Maker-Checker approved settlement by ${checkerId} (requested by ${req.maker_id})`,
          },
        });
      } else if (req.action_type === 'FINANCIAL_REVIEW') {
        // Review queue only — no ledger mutation on approve.
      } else {
        throw new Error(`UNSUPPORTED_MAKER_CHECKER_ACTION: ${req.action_type}`);
      }
