...
/** Elapsed ms on the published / season-jumped sim clock (not wall − startTime). */
function srlSimClockElapsedMs(match, now = Date.now()) {
  if (!match) return 0;
  const simNow = getSrlSimNow(now);
  const startTime = Number(match.startTime || 0);
  const durationMs = Number(match.sim?.totalDuration || match.expectedDurationMs || 0);
  let elapsed = Math.max(0, simNow - startTime);
  if (durationMs > 1) elapsed = Math.min(elapsed, durationMs - 1);
...
export function getIPLSRLControlSnapshot() {
  ...
  return {
    ...
    standings: getIplSrlPointsTable(Date.now(), { forDesk: true }).map((row) => ({
      rank: row.rank,
      teamId: row.key,
      teamName: row.name,
...
export function startIPLSRLControlledMatch(matchId, { admin } = {}) {
  if (isUserFacingSrlMatch(matchId)) {
    ...
    const clockElapsed = srlSimClockElapsedMs(current);
...
export function seekIPLSRLMatch(matchId, { elapsedMs, deltaMs, marker, pause } = {}, admin) {
  ...
  if (!match) throw new Error(`Match not found: ${matchId}`);
  // Chase target / all-out ends the board as post — do not keep advancing overs.
...
export function driveSrlMatchPlayForMarketDeclare(matchId, {
  ...
  if (!op.startedAt && !op.declaredWinnerKey) {
    startSrlOperatorMatch(matchId, now, srlSimClockElapsedMs(match, now));
...
/** Winning selection ids for a toss outcome across the toss-family book. */
export function srlTossWinningSelections(team1Key, team2Key, winnerKey, decision) {
  const decisionNorm = String(decision || 'BAT').toUpperCase() === 'BOWL' ? 'BOWL' : 'BAT';
  const batFirstKey = decisionNorm === 'BAT'
    ? winnerKey
    : (winnerKey === team1Key ? team2Key : team1Key);
  return {
    toss_winner: winnerKey,
    toss_and_bat: decisionNorm === 'BAT' ? 'toss_and_bat_yes' : 'toss_and_bat_no',
    toss_and_bowl: decisionNorm === 'BOWL' ? 'toss_and_bowl_yes' : 'toss_and_bowl_no',
    team_bat_first: batFirstKey,
  };
}

/**
 * House PnL if this toss outcome settles: total open toss stake − payout on winning sels.
 * Higher is better for the house (lowest loss / highest retention).
 */
export function scoreSrlTossHousePnl(stakeRows, winnersByMarket) {
  let totalStake = 0;
  let winPayout = 0;
  for (const row of stakeRows || []) {
    const mid = String(row.market_id || '');
    if (!SRL_TOSS_MARKET_IDS.includes(mid)) continue;
    const stake = Number(row.stake) || 0;
    const payout = Number(row.payout) || 0;
    totalStake += stake;
    const winSel = winnersByMarket?.[mid];
    if (winSel != null && sameSrlSelectionId(row.selection_id, winSel)) {
      winPayout += payout;
    }
  }
  return totalStake - winPayout;
}

/**
 * Pick the toss outcome (winner × BAT/BOWL) with the highest house PnL.
 * Ties: preferred soft-save → BAT → team1.
 */
export function pickLowestLossSrlTossOutcome({
  team1Key,
  team2Key,
  stakeRows = [],
  preferredWinnerKey = null,
  preferredDecision = 'BAT',
} = {}) {
  if (!team1Key || !team2Key) {
    throw new Error('team1Key and team2Key required');
  }
  const prefDec = String(preferredDecision || 'BAT').toUpperCase() === 'BOWL' ? 'BOWL' : 'BAT';
  const candidates = [];
  for (const winnerKey of [team1Key, team2Key]) {
    for (const decision of ['BAT', 'BOWL']) {
      const winners = srlTossWinningSelections(team1Key, team2Key, winnerKey, decision);
      const housePnl = scoreSrlTossHousePnl(stakeRows, winners);
      candidates.push({ winnerKey, decision, housePnl, winners });
    }
  }
  candidates.sort((a, b) => {
    if (b.housePnl !== a.housePnl) return b.housePnl - a.housePnl;
    const aPref = (a.winnerKey === preferredWinnerKey && a.decision === prefDec) ? 1 : 0;
    const bPref = (b.winnerKey === preferredWinnerKey && b.decision === prefDec) ? 1 : 0;
    if (bPref !== aPref) return bPref - aPref;
    if (a.decision !== b.decision) return a.decision === 'BAT' ? -1 : 1;
    if (a.winnerKey === team1Key) return -1;
    if (b.winnerKey === team1Key) return 1;
    return 0;
  });
  return candidates[0];
}
...
/**
 * When admin never locked toss and the public T-25 window is open (or match is live),
 * lock the lowest-house-loss outcome and settle/publish via executeIPLSRLToss.
 */
export async function autoLockLowestLossSrlToss(
  matchId,
  admin = 'SYSTEM',
  role = 'SUPER_ADMIN',
  now = Date.now(),
  ...
  if (session.toss?.locked) {
    return { success: true, alreadyLocked: true, toss: session.toss };
  }

  const team1Key = m.team1?.key;
  const team2Key = m.team2?.key;
  if (!team1Key || !team2Key) {
    return { success: false, reason: 'missing_teams' };
  }

  const stakeRows = await loadOpenStakeRows(matchId);
  const preferredWinnerKey = session.toss?.tossWinnerKey
    || m.toss?.tossWinnerKey
    || team1Key;
  const preferredDecision = session.toss?.decision
    || m.toss?.decision
    || 'BAT';
  ...
  // Guardrail: if auto-pick would pay out more than house keeps by a large margin, hold for desk.
  const maxAutoLoss = Number(process.env.SRL_TOSS_AUTO_MAX_LOSS || '25000');
  const housePnl = Number(pick.housePnl) || 0;
  if (Number.isFinite(maxAutoLoss) && maxAutoLoss > 0 && housePnl < -maxAutoLoss) {
    pushSrlDeskAlert(matchId, {
      level: 'danger',
      code: 'TOSS_AUTO_BLOCKED',
      message: `Auto-toss blocked — worst house PnL ₹${housePnl.toFixed(0)} below −₹${maxAutoLoss}. Desk must lock manually.`,
    });
    try {
      const { raiseOpsAlert } = await import('./opsAlertEngine.mjs');
      await raiseOpsAlert({
        title: 'SRL toss auto-lock blocked (high liability)',
        message: `${matchId}: auto PnL ₹${housePnl.toFixed(0)} (cap −₹${maxAutoLoss})`,
        severity: 'HIGH',
        category: 'SRL',
        source: 'srl_toss_auto',
        dedupeKey: `srl_toss_blocked:${matchId}`,
        entityType: 'match',
        entityId: matchId,
        metadata: { housePnl, maxAutoLoss, pick },
      });
    } catch { /* soft */ }
    return {
      success: false,
      reason: 'liability_cap',
      housePnl,
      maxAutoLoss,
      pick,
    };
  ...
  const result = await executeIPLSRLToss(matchId, {
    winnerTeamId: pick.winnerKey,
    decision: pick.decision,
    lockAndDeclare: true,
    autoPicked: true,
    autoPickHousePnl: pick.housePnl,
    autoPickReason: 'lowest_house_loss',
  }, admin, role, now);

  pushSrlDeskAlert(matchId, {
    level: housePnl < 0 ? 'warn' : 'info',
    code: 'TOSS_AUTO_LOCKED',
    message: `Auto-locked toss (lowest house loss): ${pick.winnerKey} → ${pick.decision} · PnL ${housePnl.toFixed(0)}`,
  });

  try {
    const { raiseOpsAlert } = await import('./opsAlertEngine.mjs');
    await raiseOpsAlert({
      title: 'SRL toss auto-locked',
      message: `${matchId}: ${pick.winnerKey} → ${pick.decision} (PnL ₹${housePnl.toFixed(0)})`,
      severity: 'WARNING',
      category: 'SRL',
      source: 'srl_toss_auto',
      dedupeKey: `srl_toss_auto:${matchId}`,
      entityType: 'match',
      entityId: matchId,
      metadata: { pick, housePnl },
      cooldownSeconds: 300,
    });
  } catch { /* soft */ }
...
export async function executeIPLSRLToss(matchId, {
  ...
  const result = setSrlTossAndLineup(matchId, {
    ...
    autoPicked: autoPicked == null ? undefined : !!autoPicked,
    autoPickHousePnl: autoPickHousePnl == null ? undefined : autoPickHousePnl,
    autoPickReason: autoPickReason == null ? undefined : autoPickReason,
  });
 ...
 * auto-lock unlocked toss at T-25 (lowest house loss), deferred toss publish,
 * and auto-pay match_winner when the board has completed with a known winner.
 */
export async function runSrlDeskOpsSweep() {
  ...
  for (const m of matches) {
    ...
    // Auto-lock toss if admin never locked by T-25 / live: pick lowest house-loss outcome.
    const tossDue = Number.isFinite(startIn)
      ? startIn <= SRL_TOSS_PUBLIC_LEAD_MS
      : (m.controlStatus === 'LIVE' || m.matchState === 'in');
    const notFinished = m.controlStatus !== 'COMPLETED'
      && m.matchState !== 'post'
      && !m.needsSettlement;
    if (!m.toss?.locked && tossDue && notFinished) {
      try {
        const auto = await autoLockLowestLossSrlToss(m.matchId, 'SYSTEM', 'SUPER_ADMIN');
        if (auto?.autoPicked || auto?.lockAndDeclare) {
          actions.push({
            matchId: m.matchId,
            action: 'toss_auto_locked',
            winnerKey: auto?.pick?.winnerKey || auto?.toss?.tossWinnerKey,
            decision: auto?.pick?.decision || auto?.toss?.decision,
            housePnl: auto?.pick?.housePnl,
          });
        }
      } catch (err) {
        actions.push({ matchId: m.matchId, action: 'toss_auto_lock_failed', error: err.message });
      }
    }

    // Auto-publish desk-locked toss once the public T-25 window opens.
    const tossAfter = getSrlOperatorSession(m.matchId).toss || m.toss;
    if (tossAfter?.locked && !tossAfter?.userPublished) {
      try {
