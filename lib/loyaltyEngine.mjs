...
import { earnLoyaltyPoints, getRedeemablePointsFromRow, getVipPointsFromRow, ensureVipPointsSchema } from './loyaltyPointsStore.mjs';
...
export async function addLoyaltyPoints(userId, pointsToAdd = 0) {
  ...
  if (!userId || earned <= 0) {
    return USER_LOYALTY_STORE.get(userId) || {
      userId,
      points: 0,
      vipPoints: 0,
      level: 1,
  ...
  const result = await earnLoyaltyPoints(query, userId, earned);
  const record = {
    userId,
...
export async function redeemLoyaltyPoints(userId, requestedPoints) {
  ...
  return withTransaction(async (client) => {
    await ensureVipPointsSchema(client.query.bind(client));
