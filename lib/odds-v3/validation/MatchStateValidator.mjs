...
export function validateMatchState(state) {
  ...
  if (state.currentInnings === 2 && state.format !== 'TEST') {
    ...
    if (state.ballsRemaining === 0 || battingTeam.wickets >= rules.maxWickets) {
      ...
      // Mirrored first-innings all-out on the chase side (e.g. 172/10 : 172/10 while
      // "need 1 run" and balls still remain) must not finalize as a tie/result.
      const mirroredChaseAllOut = battingTeam.wickets >= rules.maxWickets
        && Number(state.ballsRemaining) > 12
        && bowlingTeam
        && Number(battingTeam.runs) === Number(bowlingTeam.runs);
      if (mirroredChaseAllOut) {
        return { valid: true };
      }
