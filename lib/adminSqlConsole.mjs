...
const DANGEROUS_PG_FUNCTIONS_PATTERN = /\b(pg_read_file|pg_read_binary_file|pg_ls_dir|pg_stat_file|lo_import|lo_export|lo_create|lo_open|dblink|dblink_exec|copy\s+.*program|program\s+['"]|c_create|c_destroy|pg_sleep|pg_terminate_backend|pg_cancel_backend)\b/i;
...
export function validateReadOnlySql(raw) {
  ...
  if (DANGEROUS_PG_FUNCTIONS_PATTERN.test(sql)) {
    const err = new Error('Dangerous PostgreSQL administrative or filesystem functions are blocked');
    err.status = 403;
    err.code = 'SQL_DANGEROUS_BLOCKED';
    throw err;
  }
