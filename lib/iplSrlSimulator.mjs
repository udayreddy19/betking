...
function buildMatchFromSchedule(globalSlot, now) {
  ...
  const completed = (liveSnapshot = null) => {
    ...
    const firstHist = buildOverHistory(sim.first?.timeline || []);
    ...
    const secondHist = liveChaseHist.length
      ...
      : buildOverHistory(sim.second?.timeline || []).map((r) => ({ ...r, inningsId: 2, isCurrent: false }));
    const overHistory = [
  ...
  if (elapsed >= sim.totalDuration) {
    const endSnap = freezeChaseAtTarget(
      applySrlScoreAnchors(stateAtElapsed(sim, sim.totalDuration - 1, batFirst, chase), sim, op.scoreAnchors),
      op.dlsTarget,
    );
    return completed(endSnap);
  }

  let live = applySrlScoreAnchors(stateAtElapsed(sim, elapsed, batFirst, chase), sim, op.scoreAnchors);
  if (op.dlsTarget) {
 ...
 *
 * @param {number} [now]
 * @param {{ publicBoard?: boolean }} [opts]
 *   publicBoard:true — use wall clock (ignore admin season-jump offset) so Sports/SRL
 *   always shows the next real-calendar fixtures. Desk/admin keep sim clock.
 */
export function getIplSrlMatches(now = Date.now(), opts = {}) {
  const publicBoard = opts.publicBoard === true || opts.forPublic === true;
  const wallNow = Number(now) || Date.now();
  now = publicBoard ? wallNow : getSrlSimNow(wallNow);
