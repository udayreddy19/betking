...
export class CashfreeProvider extends PaymentProvider {
   ...
   * With configurable timestamp skew validation for replay attack prevention.
   */
  verifyWebhookSignature({ rawBody, headers, maxSkewSeconds = null }) {
    ...
    // Timestamp skew validation (Priority 21)
    const skewLimit = maxSkewSeconds != null
      ? Number(maxSkewSeconds)
      : (Number(process.env.CASHFREE_WEBHOOK_MAX_SKEW_SEC) || 300);
    ...
    const now = Date.now();
    if (Math.abs(now - tsMs) > skewLimit * 1000) {
      logger.warn('[CashfreeProvider] Webhook rejected: timestamp skew exceeded limit', {
        timestamp,
        now,
        skewSeconds: Math.round(Math.abs(now - tsMs) / 1000),
        skewLimit,
      });
      return false;
    }
    ...
    if (!secret) {
      // Fail closed — never credit deposits on unsigned webhooks.
      return false;
    }
  ...
  async createOrder({
    ...
    const customerPhone = phoneDigits.length === 12 && phoneDigits.startsWith('91')
      ? phoneDigits.slice(2)
      : phoneDigits.slice(0, 10);
    if (!/^[6-9]\d{9}$/.test(customerPhone)) {
      const err = new Error('PHONE_REQUIRED: A valid 10-digit Indian mobile number is required for Cashfree.');
    ...
    try {
      // Do NOT send customer_email to prevent Cashfree automated unbranded customer emails.
      // Cashfree PG API v2023-08-01 requires only customer_id and customer_phone.
