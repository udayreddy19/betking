...
export async function userHasReferralAttribution(userId, exec = query) {
 ...
 */
export async function assertNoReferralPromoConflict(userId, exec = query) {
  const row = await userHasReferralAttribution(userId, exec);
  if (!row) return;
  const err = new Error(
 ...
 * Called after first qualifying deposit (and KYC if configured).
 */
export async function qualifyReferralReward({ referredUserId, depositAmount = null } = {}) {
  const conf = cfg();
  ...
  const refRes = await query(
    `SELECT id, referrer_user_id, referred_user_id, reward_amount, status,
            COALESCE(referred_reward_amount, reward_amount, 500) AS referred_reward_amount,
            COALESCE(referrer_reward_amount, reward_amount, 500) AS referrer_reward_amount,
  ...
  if (depositAmount != null && Number(depositAmount) < conf.minDeposit) {
    return { qualified: false, reason: 'Deposit below minimum' };
  }
  ...
  // Block if user already claimed a signup promo (double benefit)
  ...
  const referredAmt = Number(ref.referred_reward_amount || conf.referredReward);
  const referrerAmt = Number(ref.referrer_reward_amount || conf.referrerReward);

  const result = await withTransaction(async (client) => {
    const locked = await client.query(
      `SELECT id, status, reward_status FROM referrals WHERE id = $1 FOR UPDATE`,
      [ref.id],
    );
