...
import { resolveMatchTimestampTrust, TIMESTAMP_STATUS } from './safeTimestampParser.mjs';
import { inferWallClockMatchFinal, isFeedStillLive } from './wallClockFinality.mjs';
...
const INNINGS_TOTAL_POLICY = { requiredGraceSeconds: 0, requireMatchFinal: false, requireEventFinal: true, allowSnapshotFinality: true };

export function resolveMarketFinalityPolicy(marketTypeOrId = '') {
  const raw = String(marketTypeOrId || '');
  const norm = raw.toUpperCase();
  if (norm.includes('TOSS')) return MARKET_FINALITY_POLICIES.TOSS;
  if (norm.includes('DELIVERY') || norm.includes('NEXT_DELIVERY') || (norm.includes('BALL') && !norm.includes('FOOTBALL'))) {
    return MARKET_FINALITY_POLICIES.NEXT_BALL;
  }
  if (norm.includes('OVER_TOTAL') || norm.includes('NEXT_OVER')) return MARKET_FINALITY_POLICIES.OVER_TOTAL;
  if (norm.includes('WICKET_IN_OVER') || norm.includes('WICKET_IN_NEXT_OVER') || (norm.includes('WICKET') && norm.includes('OVER'))) {
    return MARKET_FINALITY_POLICIES.WICKET_IN_OVER;
  }
  if (norm.includes('DISMISSAL') || norm.includes('FOW') || norm.includes('SCORE_AT_')) {
    return MARKET_FINALITY_POLICIES.DISMISSAL_SCORE;
  }
  if (raw === 'TEAM_TOTAL') {
    return MARKET_FINALITY_POLICIES.TEAM_TOTAL;
  }
  if (
    norm.includes('TEAM_TOTAL')
    || norm.includes('MATCH_TOTAL')
    || norm.includes('TOTAL_RUNS')
    || norm.includes('OVERS_0_')
  ) {
    if (raw === 'team_total' || raw === 'match_total' || raw.startsWith('i1_') || raw.startsWith('i2_')) {
      return INNINGS_TOTAL_POLICY;
    }
    return MARKET_FINALITY_POLICIES.TEAM_TOTAL;
  }
  if (norm.includes('PLAYER') || norm.includes('BATTER')) return MARKET_FINALITY_POLICIES.PLAYER_RUNS;
  if (norm.includes('MATCH_WINNER') || norm.includes('1X2') || norm === 'WINNER' || /(?:^|_)WINNER$/.test(norm)) {
    return MARKET_FINALITY_POLICIES.MATCH_WINNER;
  }
...
export function evaluateSettlementConfidence({
  ...
  // 2. Freshness & Stale Check using Safe Timestamp Trust Model
  const policy = resolveMarketFinalityPolicy(marketContext.marketType || marketId || '');
  const hasImmutableSnapshot = marketContext.hasImmutableSnapshotEvidence !== undefined
    ? Boolean(marketContext.hasImmutableSnapshotEvidence)
    : Boolean(policy.allowSnapshotFinality && evaluatedOutcome != null && marketContext.boundaryReached);
  const isMarketProvenFinal = Boolean(
    (policy.allowSnapshotFinality && hasImmutableSnapshot)
    || (marketContext.hasImmutableSnapshotEvidence && evaluatedOutcome != null)
  );
  ...
  const s1 = Number(match?.live_score1 ?? match?.score1 ?? match?.liveDetails?.score1 ?? match?.liveDetails?.firstRuns ?? match?.liveDetails?.runs ?? match?.team1?.runs ?? 0);
  const s2 = Number(match?.live_score2 ?? match?.score2 ?? match?.liveDetails?.score2 ?? match?.liveDetails?.chaseRuns ?? match?.team2?.runs ?? 0);
  ...
  const updatedAt = match?.updatedAt || match?.updated_at || match?.cachedAt ? new Date(match.updatedAt || match.updated_at || match.cachedAt).getTime() : 0;
  const now = Date.now();
  const isTimeConcluded = inferWallClockMatchFinal(match, { startTime, updatedAt, s1, s2, now });

  const isFinalMatch = Boolean(
    (
      !isFeedStillLive(match)
      && (
        ['COMPLETED', 'FINAL', 'FINISHED', 'CLOSED', 'HISTORICAL', 'SETTLED'].includes(rawStatus)
  ...
  const timestampTrust = resolveMatchTimestampTrust(match, {
    maxAgeSeconds: cfg.settlementDataMaxAgeSeconds,
  });

  const stale = Boolean(!isFinalMatch && !isMarketProvenFinal && timestampTrust.stale);

  const freshness = {
    freshestTimestamp: timestampTrust.freshestTimestamp,
    timestampSource: timestampTrust.timestampSource,
    timestampStatus: timestampTrust.timestampStatus,
    ageSeconds: timestampTrust.ageSeconds,
    maxAgeSeconds: cfg.settlementDataMaxAgeSeconds,
    stale,
  };

  if (stale) {
    const ageDisplay = timestampTrust.ageSeconds != null ? `${Math.round(timestampTrust.ageSeconds)}s` : 'unknown';
    settlementReasonCodes.push(`STALE_DATA: In-play match data age (${ageDisplay}) exceeds max allowed (${cfg.settlementDataMaxAgeSeconds}s)`);
    return {
      eventId,
      marketId,
      settlementAllowed: false,
  ...
  // 5. In-play market boundary completion — never treat "any outcome" alone as final
  // for markets that require match completion (match_winner).
  const outcomeAlone = evaluatedOutcome != null && !policy.requireMatchFinal;
  ...
  if (policy.requireMatchFinal && !isFinalMatch && !isAbandoned) {
    settlementReasonCodes.push('AWAITING_MATCH_FINAL: Match-winner policy requires completed fixture');
    return {
      eventId,
      marketId,
      settlementAllowed: false,
      confidenceState: CONFIDENCE_STATES.PROVISIONAL,
      confidence: CONFIDENCE_STATES.PROVISIONAL,
      finalityState: FINALITY_STATES.FINAL_WAITING,
      finality: FINALITY_STATES.FINAL_WAITING,
      providerConsensus: consensusResult,
      freshness,
      evidenceStatus: 'AWAITING_MATCH_FINAL',
      settlementReasonCodes,
      reasons: settlementReasonCodes,
      evidence: [freshness],
      settlementEligibleAt: null,
  ...
  const settlementAllowed = Boolean(
    ...
    && isSegmentComplete
    && (!policy.requireMatchFinal || isFinalMatch || isAbandoned)
  );
