...
export function getV3WinnerDecimalBook(match = {}, options = {}) {
  const mode = forceV4ForSrl(match, resolveOddsEngineMode());
  const cricket = isCricket(match.sport);
  const useV4 = cricket && (mode === 'v4' || (mode === 'shadow' && isMatchSRL(match)));
  const generate = useV4 ? generateV4 : generateV3;
  ...
  if (!(home > 1) || !(away > 1)) {
    const err = new Error(useV4 ? 'V4_ODDS_UNAVAILABLE' : 'V3_ODDS_UNAVAILABLE');
  ...
  return {
    ...
    engine: snapshot.engine || (useV4 ? 'OddsEngineV4' : 'OddsEngineV3'),
  };
}
