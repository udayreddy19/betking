...
export class WithdrawalEngine {
  ...
  async requestWithdrawal({ userId, amount, bankDetails = {} }, correlationId = null) {
    ...
    const result = await withTransaction(async (client) => {
      ...
      await client.query(
        `UPDATE wallets
         SET reserved_balance = $1, bonus_balance = 0.00, updated_at = NOW()
         WHERE wallet_id = $2`,
  ...
  async reviewWithdrawal({ withdrawalId, adminId, decision, reason = '' }) {
    ...
    if (decision === 'APPROVE') {
      await withTransaction(async (client) => {
        ...
        if (newBalance < 0) {
          throw new Error(`INSUFFICIENT_FUNDS: Wallet balance ₹${currentBalance} cannot support withdrawal debit of ₹${amount}`);
        }

        await client.query(
          `UPDATE wallets SET balance = $1, reserved_balance = $2, updated_at = NOW() WHERE wallet_id = $3`,
          [newBalance, newReserved, wallet.wallet_id]
        );
        ...
        await client.query(
          ...
          [wallet.wallet_id, txId, amount, newBalance]
        );
      });
