...
function ouMarket({ marketId, marketType, name, line, pOver, overround, maxOverOdds, sideBias, category = 'totals' }) {
  ...
  const p = clamp(applySideHouseBias(fairOver, sideBias), 0.02, 0.98);
  // Only soft-crush favorite Overs — leave correctly long Unders/Overs uncapped here.
  const softOverCap = fairOver >= 0.42 ? maxOverOdds : undefined;
...
export function generateV4ExtraMarkets(state, _validation = {}, marginConfig = {}) {
  ...
  const wicketsRemaining = Math.max(0, (rules.maxWickets || 10) - (battingTeam.wickets || 0));
  ...
  if (state.hasBallFeed !== false) {
    ...
    // Odd = 1,3,5… — fours/sixes/doubles/dots are even. Do not treat fours as odd.
    const runMass = dist.dot + dist.single + dist.double + dist.four + dist.six;
    const pOdd = clamp(dist.single / Math.max(0.05, runMass), 0.18, 0.55);
  ...
  if (Number(state.ballsRemaining) >= 18) {
    ...
    if (targetOvers > overNum + 1) {
      ...
      const sd = Math.max(8, Math.sqrt(remBalls) * 1.4);
      const fair = calculateOverUnderProbability(
        mean,
        line,
        1.5,
        Number(battingTeam.runs || 0),
        sd,
      );
      markets.push(ouMarket({
  ...
  // Team total ladder
  {
    ...
    for (const delta of [-15, -10, -5, 5, 10, 15, 25, 35]) {
      ...
      let pOver = calculateOverUnderProbability(
        proj,
        line,
        1.5,
        Number(battingTeam.runs || 0),
  ...
  // Match total ladder
  {
    ...
    for (const delta of [-30, -20, -10, 10, 20, 30, 50]) {
      ...
      let pOver = calculateOverUnderProbability(
        ...
        line,
        1.5,
        Number.isFinite(first) ? first + Number(battingTeam.runs || 0) : 0,
        sd,
      ).pOver;
