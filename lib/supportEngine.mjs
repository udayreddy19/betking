    ...
    try {
      ...
      if (type === 'TRANSACTION' || type === 'DEPOSIT' || type === 'WITHDRAWAL') {
        const res = await safePgQuery(
          `SELECT transaction_id, type, amount, status FROM wallet_transactions WHERE transaction_id = $1 AND user_id = $2`,
          [entityId, userId]
        );
  ...
  async startConversation({
    ...
    if (!bypassDuplicateCheck && supportType === 'TICKET') {
      const activeConvs = Array.from(this.conversations.values()).filter((c) =>
        c.userId === userId &&
        c.supportType === 'TICKET' &&
        ['OPEN', 'ASSIGNED', 'IN_PROGRESS', 'WAITING_FOR_USER', 'PENDING_USER', 'PENDING_INTERNAL', 'ESCALATED', 'REOPENED'].includes(c.status) &&
      ...
      try {
        if (pool && typeof pool.connect === 'function') {
          ...
          if (userCheck.rows.length === 0) {
            const fallbackEmail = String(userId).includes('@') ? userId : `${userId}_${Math.random().toString(36).slice(2, 7)}@oddsyra.local`;
            await client.query(
