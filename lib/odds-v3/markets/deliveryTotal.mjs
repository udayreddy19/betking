...
function ballOutcomeDistribution(state) {
  ...
  const oversDone = ballsCompleted / (rules.ballsPerOver || 6);
  if (oversDone > 1) {
    const currentRPO = (battingTeam.runs || 0) / oversDone;
    const historicalRPO = rules.historicalRunsPerBall * (rules.ballsPerOver || 6);
...
export function generateExtendedDeliveryMarkets(state, validation = {}, marginConfig = {}) {
  ...
  if (currentBalls >= rules.ballsPerInnings) return [];

  // Hard-block when we know there is no ball-by-ball feed (any format, scorecard-only).
  if (state.hasBallFeed === false) return [];

  const slot = nextBallSlot(currentBalls, ballsPerOver);
  const currentOverNum = slot.overNum;
  const nextBallNum = slot.ballNum;

  const inningsNum = (Number(state.currentInnings) || 1) >= 2 ? 2 : 1;
  const dist = ballOutcomeDistribution(state);
  const markets = [];

  // 1. Next Delivery Runs — phase-aware distribution (innings-scoped ids)
  markets.push(createMarketDefinition({
    marketId: `i${inningsNum}_next_delivery_runs_${currentOverNum}_${nextBallNum}`,
  ...
  markets.push(createMarketDefinition({
    marketId: `i${inningsNum}_next_delivery_ou_${currentOverNum}_${nextBallNum}`,
  ...
  markets.push(createMarketDefinition({
    marketId: `i${inningsNum}_next_delivery_boundary_${currentOverNum}_${nextBallNum}`,
  ...
  markets.push(createMarketDefinition({
    marketId: `i${inningsNum}_next_delivery_wicket_${currentOverNum}_${nextBallNum}`,
