...
export async function evaluateWicketInOverMarketBet(bet, match) {
  ...
  if (wicketsInOver == null || !Number.isFinite(wicketsInOver)) return null;
...
function resolveTeamTotalScore(bet, match) {
  ...
  if (market === 'team_total' || market.startsWith('team_total_alt')) {
    if (bat.innings === 2 && leg?.innings !== 1) {
      return Number(ld.firstRuns ?? match.team1?.runs ?? NaN);
    }
    return Number(bat.score ?? NaN);
  }
  return NaN;
}
...
export function evaluateTotalsMarketBet(bet, match) {
  ...
  const inningsScoped = market.match(/^i([12])_team_total(_alt_\d+)?$/);
  if (inningsScoped) {
    const inn = Number(inningsScoped[1]);
    const score = resolveTeamTotalScore(bet, match);
    const teamScore = Number.isFinite(score)
      ? score
      : (inn === 1 ? Number(ld.firstRuns ?? match.team1?.runs ?? NaN) : Number(ld.chaseRuns ?? match.team2?.runs ?? NaN));
    if (!Number.isFinite(teamScore)) return null;
    const firstInningsComplete = inn === 1 && (bat.innings >= 2 || Number(ld.firstWickets) >= 10);
    const chaseInningsComplete = inn === 2 && (isMatchFinal(match) || bat.innings > 2);
    if (inn === 1 && !firstInningsComplete && bat.innings === 1) {
      const liveScore = Number(bat.score) || 0;
      if (isUnderSelection(selectionId, selectionName) && liveScore > line) {
        return { outcome: 'LOST', reason: `team_total_under_crossed score=${liveScore}_line=${line}` };
      }
      if (isOverSelection(selectionId, selectionName) && liveScore > line) {
        return { outcome: 'WON', reason: `team_total_over_hit score=${liveScore}_line=${line}` };
      }
      return null;
    }
    if (!firstInningsComplete && inn === 1 && bat.innings < 2) return null;
    if (inn === 2 && !isMatchFinal(match) && bat.innings === 2) {
      if (isOverSelection(selectionId, selectionName) && teamScore > line) {
        return { outcome: 'WON', reason: `team_total_i${inn}_live=${teamScore}_line=${line}` };
      }
      if (isUnderSelection(selectionId, selectionName) && teamScore > line) {
        return { outcome: 'LOST', reason: `team_total_i${inn}_live=${teamScore}_line=${line}` };
      }
      return null;
    }
    if (!chaseInningsComplete && inn === 2 && !isMatchFinal(match)) return null;
    if (isOverSelection(selectionId, selectionName)) {
      return { outcome: teamScore > line ? 'WON' : 'LOST', reason: `team_total_i${inn}_final=${teamScore}_line=${line}` };
    }
    if (isUnderSelection(selectionId, selectionName)) {
      return { outcome: teamScore < line ? 'WON' : 'LOST', reason: `team_total_i${inn}_final=${teamScore}_line=${line}` };
    }
  ...
  if (market === 'team_total' || market.startsWith('team_total_alt_')) {
    ...
    const boundScore = resolveTeamTotalScore(bet, match);
    const finalScore = Number.isFinite(boundScore) ? boundScore : (
      bat.innings >= 2 ? (Number(ld.firstRuns) || Number(bat.score) || 0) : Number(bat.score) || 0
    );
    if (isOverSelection(selectionId, selectionName)) {
