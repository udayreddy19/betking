...
export async function buildOpsControlTower() {
  ...
  const [
    ...
    query(`SELECT COALESCE(SUM(amount),0) as volume, COUNT(*)::int as count FROM deposits WHERE status = 'COMPLETED' AND created_at >= date_trunc('day', NOW())`).catch(() => ({ rows: [] })),
    query(`SELECT COALESCE(SUM(amount),0) as volume, COUNT(*)::int as count FROM withdrawals WHERE status = 'COMPLETED' AND created_at >= date_trunc('day', NOW())`).catch(() => ({ rows: [] })),
  ...
  return {
    ...
    workers: [
      ...
      { name: 'Financial Reconciliation Worker', status: 'RUNNING', interval: 'Daily Closing 00:00 UTC', failureCount: na(reconDiscrepancies) || 0 },
