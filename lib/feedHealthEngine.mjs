...
export function getFeedHealthSnapshot() {
  const summary = ProviderRegistry.getHealthSummary
    ? ProviderRegistry.getHealthSummary()
    : null;
  const providers = summary?.providers || ProviderRegistry.getAllProviders().map((p) => ({
    id: p.id,
    name: p.name,
    enabled: p.enabled,
  ...
  const unhealthy = providers.filter((p) =>
    ['UNHEALTHY', 'FAILED', 'DISABLED'].includes(p.healthStatus),
  );
  const recentFailures = FEED_EVENTS.filter((e) => e.type === 'HYDRATION_FAIL').slice(0, 10);

  return {
    healthy: unhealthy.length === 0,
    status: summary?.status || (unhealthy.length === 0 ? 'HEALTHY' : 'DEGRADED'),
    activeProvider: summary?.activeProvider || ProviderRegistry.getActiveProvider()?.id || null,
    providers,
    recentFailures,
