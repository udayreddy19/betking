...
/**
 * GET /api/admin/odds-model/v4/shadow
 * OddsEngineV4 dual-run metrics vs V3 / 10cric reference (never publishes ref prices).
 */
router.get('/v4/shadow', async (_req, res) => {
  try {
    const {
      getShadowMetrics,
    ...
    const cutover = evaluateCutoverReadiness({ metrics });
    return res.json({
      success: true,
      data: {
        engineMode: resolveOddsEngineMode(),
        engineStatus: getEngineModeStatus(),
        metrics,
        cutover,
      },
    });
...
/**
 * GET /api/admin/odds-model/v4/cutover
 * Cutover readiness gate for ODDS_ENGINE=v4.
 */
router.get('/v4/cutover', async (_req, res) => {
  try {
    const { evaluateCutoverReadiness, resolveOddsEngineMode, CUTOVER_THRESHOLDS, getEngineModeStatus } = await import('../../../lib/odds-v4/index.mjs');
    return res.json({
      success: true,
      data: {
        ...evaluateCutoverReadiness(),
        currentMode: resolveOddsEngineMode(),
        engineStatus: getEngineModeStatus(),
 ...
 * Active publish engine (V3 | V4 | Shadow). Runtime admin toggle beats env.
 */
router.get('/v4/engine', async (_req, res) => {
  try {
    const { getEngineModeStatus, resolveOddsEngineMode } = await import('../../../lib/odds-v4/index.mjs');
    return res.json({
      success: true,
      data: {
        ...getEngineModeStatus(),
...
/**
 * POST /api/admin/odds-model/v4/engine
 * Body: { mode: 'v3'|'v4'|'shadow' } or { clear: true } to fall back to ODDS_ENGINE env.
 * Enabling v4 stops v3 for cricket customer odds (exclusive).
 */
router.post('/v4/engine', async (req, res) => {
  try {
    ...
    if (req.body?.clear === true || req.body?.mode === 'env') {
      status = await clearRuntimeEngineMode({
        updatedBy,
        reason: req.body?.reason || 'Cleared to env default',
      });
    } else {
