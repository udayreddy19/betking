...
import { inferWallClockMatchFinal, markInferredFinal } from './wallClockFinality.mjs';
...
export async function lookupEventForSettlement({
  ...
  try {
    ...
    if (cachedState && validateMatchIdentity(cachedState)) {
      ...
      const isFinal = ['COMPLETED', 'FINISHED', 'FINAL', 'CLOSED', 'HISTORICAL', 'SETTLED'].includes(String(cachedState.status).toUpperCase())
