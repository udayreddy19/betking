...
export async function addLoyaltyPoints(userId, pointsToAdd = 0) {
  ...
  const previousTier = USER_LOYALTY_STORE.get(userId)?.tier || 'BRONZE';
  record.points += Math.max(0, pointsToAdd);
  record.level = Math.floor(record.points / 1000) + 1;
  record.tier = loyaltyTierFromPoints(record.points);
  ...
  try {
    ...
    `, [userId, record.points, record.tier]);
    await grantCrossedTierRewards(query, userId, previousTier, record.tier);
  } catch (err) {
