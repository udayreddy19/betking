...
function mapState(info = {}) {
  ...
  // Stumps / day-end is still in progress for First Class / Test — never mark Completed.
  if (/\bstumps\b/i.test(combined) && !/\b(won by|have won|has won|match over)\b/i.test(status)) {
    const label = status.trim() || stateTitle.trim() || 'Stumps';
    return { matchState: 'in', isLive: true, time: label };
  }
