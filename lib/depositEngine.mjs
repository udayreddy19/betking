...
export class DepositEngine {
  ...
  async processVerifiedRazorpayPayment({
    ...
    // Concurrency / Idempotency check via idempotencyEngine
    const idemCheck = await idempotencyEngine.checkOrLock(providerPaymentId, 'DEPOSIT_PROCESSOR', '', userId || '');
    if (idemCheck.isDuplicate) {
      return { status: 'PAID', paymentId: providerPaymentId, message: 'Payment already processed' };
    }
    ...
    const result = await withTransaction(async (client) => {
      ...
      // Check if providerPaymentId has already credited another deposit
      const dupCheck = await client.query(
        `SELECT id, deposit_id FROM deposits WHERE payment_id = $1 AND status IN ('PAID', 'CAPTURED')`,
        [providerPaymentId]
      );
      if (dupCheck.rows.length > 0) {
        throw new Error(`DUPLICATE_PAYMENT_ID: Razorpay Payment ID '${providerPaymentId}' has already been credited`);
      }
    ...
    await idempotencyEngine.complete(providerPaymentId, result);
