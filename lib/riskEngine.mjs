/**
 * Enterprise Risk Engine — OddsYra Sportsbook (lib/riskEngine.mjs)
 ...
 * Feeds risk recommendations back to OddsEngineV3 (lib/odds-v3), not the deprecated oddsEngine.mjs.
 */
import { globalLiabilityTracker } from './risk/liabilityTracker.mjs';
import { globalSyndicateDetector } from './risk/syndicateDetector.mjs';
...
export function getUserRiskProfile(userId) {
  ...
  return USER_RISK_PROFILES.get(userId) || null;
}
...
export function validateBetRisk(bet = {}) {
  ...
  // 3–4. Win / match liability caps disabled — accumulators and high-odds multis are allowed.
  ...
  // 6. Arbitrage & Syndicate Detection
  ...
  const syndicateCheck = globalSyndicateDetector.recordAndAnalyze(bet);
  if (syndicateCheck.isSyndicate) {
    flags.push('COORDINATED_SYNDICATE_ACTIVITY');
    if (stake > 15000) {
      isApproved = false;
    }
  }

  // 7. Record to Live Liability Tracker
  const liability = globalLiabilityTracker.recordBet(bet);
  if (liability.isOverLimit) {
    flags.push('MATCH_LIABILITY_CAP_EXCEEDED');
  }
  ...
  return {
    ...
    riskMultiplier,
    flags,
    liabilityReport: liability,
    syndicateDetected: syndicateCheck.isSyndicate,
    recommendationsToOddsEngine: {
      matchId,
      selectionId,
      riskMultiplier,
      shouldShortenOdds: flags.includes('SHARP_BETTOR_DETECTED') || flags.includes('DUPLICATE_BET_DETECTED') || syndicateCheck.isSyndicate,
    },
...
export { globalLiabilityTracker, globalSyndicateDetector };
