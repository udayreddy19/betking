/**
 * Server-Authoritative Multi-Provider Deposit Engine
 * Supports Cashfree and Razorpay with a Single Central Financial Engine.
 * Handles order creation, webhook verification, exactly-once wallet credit idempotency,
 * row locking, and atomic PostgreSQL financial ledger commits.
 *
 * ZERO HARDCODED SECRETS: All credentials read securely from environment variables.
 */

import { query, withTransaction } from '../db/pg.js';
import { addColumnIfMissing, createTableIfMissing, memoizeEnsure } from './schemaGuard.mjs';
import { accountEligibilityEngine } from './accountEligibilityEngine.mjs';
import { idempotencyEngine } from './idempotencyEngine.mjs';
import { responsibleGamingEngine } from './responsibleGaming.mjs';
import { paymentProviderService } from './paymentProviders/paymentProviderService.mjs';
import { logger } from './logger.mjs';
import { isSuccessfulDepositStatus } from './depositStatuses.mjs';
import { assertPaymentTransition, normalizePaymentStatus } from './paymentStateMachine.mjs';

/** Guard deposit row status changes — rejects illegal payment state transitions. */
function guardDepositStatusTransition(fromStatus, toStatus) {
  try {
    return assertPaymentTransition(fromStatus, toStatus);
  } catch (err) {
    // Map SUCCESS↔CREDITED quirks: some rows stay SUCCESS after wallet credit
    const from = normalizePaymentStatus(fromStatus);
    const to = normalizePaymentStatus(toStatus);
    if (from === 'SUCCESS' && to === 'SUCCESS') return { ok: true, noop: true };
    throw err;
  }
}
export const ensureDepositSchema = memoizeEnsure(async () => {
  await createTableIfMissing(
    'payment_webhook_events',
    `
      CREATE TABLE IF NOT EXISTS payment_webhook_events (
        id SERIAL PRIMARY KEY,
        provider VARCHAR(32) NOT NULL DEFAULT 'CASHFREE',
        provider_event_id VARCHAR(128) UNIQUE NOT NULL,
        event_type VARCHAR(64) NOT NULL,
        payload_hash VARCHAR(64),
        status VARCHAR(32) NOT NULL DEFAULT 'PROCESSED',
        raw_payload JSONB,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        processed_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )
    `,
  );
  await addColumnIfMissing('deposits', 'amount_paise', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS amount_paise BIGINT`);
  await addColumnIfMissing('deposits', 'provider', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS provider VARCHAR(32) DEFAULT 'CASHFREE'`);
  await addColumnIfMissing('deposits', 'paid_at', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ`);
  await addColumnIfMissing('deposits', 'payment_session_id', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS payment_session_id VARCHAR(255)`);
  await addColumnIfMissing('deposits', 'cf_order_id', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS cf_order_id VARCHAR(128)`);
  await addColumnIfMissing('deposits', 'razorpay_order_id', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS razorpay_order_id VARCHAR(128)`);
  await addColumnIfMissing('deposits', 'refunded_amount', `ALTER TABLE deposits ADD COLUMN IF NOT EXISTS refunded_amount NUMERIC(14,2) DEFAULT 0.00`);
});

export class DepositEngine {
  /**
   * Verify Razorpay Payment Signature (Delegates to RazorpayProvider)
   */
  verifyPaymentSignature(params) {
    const provider = paymentProviderService.getProvider('RAZORPAY');
    return provider.verifyPaymentSignature(params);
  }

  /**
   * Verify Razorpay Webhook Signature (Delegates to RazorpayProvider)
   */
  verifyWebhookSignature(params) {
    const provider = paymentProviderService.getProvider('RAZORPAY');
    return provider.verifyWebhookSignature(params);
  }

  /**
   * Verify Cashfree Webhook Signature (Delegates to CashfreeProvider)
   */
  verifyCashfreeWebhookSignature(params) {
    const provider = paymentProviderService.getProvider('CASHFREE');
    return provider.verifyWebhookSignature(params);
  }

  /**
   * 1. Create Internal Deposit Transaction & Gateway Order (Cashfree / Razorpay)
   */
  async createOrder({ userId, amount, currency = 'INR', provider = null, returnUrl, notifyUrl }, correlationId = null) {
    await ensureDepositSchema();
    if (!userId) {
      throw new Error('USER_UNAUTHENTICATED: User ID is required');
    }

    await accountEligibilityEngine.verifyEligibility(userId);

    const { assertEmergencyAllows } = await import('./emergencyState.mjs');
    await assertEmergencyAllows('deposit');

    const { assertAdultDob } = await import('./kycAgeGate.mjs');
    await assertAdultDob(userId, { action: 'deposit' });

    const numericAmount = parseFloat(amount);
    if (isNaN(numericAmount) || numericAmount <= 0) {
      throw new Error('INVALID_AMOUNT: Deposit amount must be a positive number');
    }

    const strAmount = String(numericAmount);
    if (strAmount.includes('.') && strAmount.split('.')[1].length > 2) {
      throw new Error('INVALID_AMOUNT: Deposit amount cannot exceed 2 decimal places');
    }
    const { validateDepositAmount } = await import('./walletPromoRules.mjs');
    await validateDepositAmount(numericAmount);

    const rgDeposit = await responsibleGamingEngine.validateDepositAttempt(userId, numericAmount);
    if (!rgDeposit.allowed) {
      throw Object.assign(
        new Error(`${rgDeposit.reason}: ${rgDeposit.message || rgDeposit.reason}`),
        { code: rgDeposit.reason, status: 403 },
      );
    }

    // Fetch User metadata for customer_details
    const userRes = await query(
      `SELECT u.email, u.phone, up.display_name
       FROM users u
       LEFT JOIN user_profiles up ON u.user_id = up.user_id
       WHERE u.user_id = $1
       LIMIT 1`,
      [userId]
    );
    const userProfile = userRes.rows[0] || {};

    const { normalizeIndianPhone } = await import('./userIdentity.mjs');
    const normalizedPhone = normalizeIndianPhone(userProfile.phone);
    if (!normalizedPhone) {
      const err = new Error('PHONE_REQUIRED: Add a valid 10-digit Indian mobile number before depositing.');
      err.code = 'PHONE_REQUIRED';
      err.status = 403;
      throw err;
    }
    userProfile.phone = normalizedPhone;

    const amountPaise = Math.round(numericAmount * 100);
    const targetProvider = await paymentProviderService.resolveTargetProvider(provider);
    const normalizedProvider = String(targetProvider).toUpperCase();
    const providerInstance = paymentProviderService.getProvider(normalizedProvider);

    const depositId = `dep_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

    // Insert deposit row first so webhooks never hit UNKNOWN_ORDER if provider succeeds
    await query(
      `INSERT INTO deposits (
         id, deposit_id, user_id, order_id, amount, amount_paise, currency,
         provider, payment_session_id, cf_order_id, status, created_at, updated_at
       )
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NULL, NULL, 'CREATED', NOW(), NOW())`,
      [
        depositId,
        depositId,
        userId,
        depositId,
        numericAmount,
        amountPaise,
        currency,
        normalizedProvider,
      ]
    );

    let providerOrder;
    try {
      providerOrder = await providerInstance.createOrder({
        userId,
        orderId: depositId,
        amount: numericAmount,
        amountPaise,
        currency,
        customer: {
          email: userProfile.email,
          phone: userProfile.phone,
          displayName: userProfile.display_name,
        },
        returnUrl,
        notifyUrl,
      }, correlationId);
    } catch (err) {
      guardDepositStatusTransition('CREATED', 'FAILED');
      await query(
        `UPDATE deposits SET status = 'FAILED', updated_at = NOW()
         WHERE deposit_id = $1 AND UPPER(COALESCE(status,'')) = 'CREATED'`,
        [depositId],
      ).catch(() => {});
      throw err;
    }

    const effectiveOrderId = providerOrder.orderId || providerOrder.providerOrderId || depositId;
    const paymentSessionId = providerOrder.paymentSessionId || null;
    const cfOrderId = providerOrder.cfOrderId || null;

    await query(
      `UPDATE deposits
       SET order_id = $2,
           payment_session_id = $3,
           cf_order_id = $4,
           updated_at = NOW()
       WHERE deposit_id = $1`,
      [depositId, effectiveOrderId, paymentSessionId, cfOrderId],
    );

    return {
      success: true,
      provider: normalizedProvider,
      depositId,
      orderId: effectiveOrderId,
      providerOrderId: effectiveOrderId,
      amount: numericAmount,
      amountPaise,
      currency,
      paymentSessionId,
      cfOrderId,
      razorpayKeyId: providerOrder.keyId || undefined,
      keyId: providerOrder.keyId || undefined,
      environment: providerOrder.environment || undefined,
    };
  }

  /**
   * 2. Confirm Checkout Payment from Frontend (/api/payments/verify, /api/payments/cashfree/verify, /api/payments/razorpay/verify)
   */
  async confirmCheckoutPayment({
    userId,
    depositId = null,
    provider = null,
    orderId = null,
    paymentId = null,
    // Razorpay specific fields:
    razorpayOrderId = null,
    razorpayPaymentId = null,
    razorpaySignature = null,
    // Cashfree specific fields:
    cfOrderId = null,
    cfPaymentId = null,
  }, correlationId = null) {
    if (!userId) {
      throw Object.assign(new Error('USER_UNAUTHENTICATED: User ID is required'), { status: 401 });
    }

    let normalizedProvider = provider ? String(provider).toUpperCase() : null;
    if (!normalizedProvider) {
      if (razorpayOrderId || razorpaySignature || razorpayPaymentId) {
        normalizedProvider = 'RAZORPAY';
      } else if (cfOrderId || cfPaymentId) {
        normalizedProvider = 'CASHFREE';
      } else {
        normalizedProvider = 'CASHFREE';
      }
    }
    const providerInstance = paymentProviderService.getProvider(normalizedProvider);

    const effectiveOrderId = orderId || razorpayOrderId || cfOrderId;
    const effectivePaymentId = paymentId || razorpayPaymentId || cfPaymentId;

    if (!effectiveOrderId) {
      throw Object.assign(new Error('INVALID_PAYLOAD: order_id is required'), { status: 400 });
    }

    // Razorpay signature check
    if (normalizedProvider === 'RAZORPAY') {
      if (!razorpayOrderId || !razorpayPaymentId || !razorpaySignature) {
        throw Object.assign(new Error('INVALID_PAYLOAD: razorpay_order_id, razorpay_payment_id and razorpay_signature are required'), { status: 400 });
      }
      const isSignatureValid = providerInstance.verifyPaymentSignature({
        razorpayOrderId,
        razorpayPaymentId,
        razorpaySignature,
      });
      if (!isSignatureValid) {
        throw Object.assign(new Error('INVALID_SIGNATURE: Payment signature verification failed'), {
          status: 400,
          code: 'INVALID_SIGNATURE',
        });
      }
    }

    // Verify Deposit Record in DB
    const depositRow = await query(
      `SELECT id, deposit_id, user_id, amount, amount_paise, status, provider
       FROM deposits
       WHERE order_id = $1 OR deposit_id = $2 OR id = $2
       LIMIT 1`,
      [effectiveOrderId, depositId || effectiveOrderId],
    );

    if (depositRow.rows.length === 0) {
      throw Object.assign(new Error(`UNKNOWN_ORDER: No deposit record found for order '${effectiveOrderId}'`), { status: 404 });
    }

    const deposit = depositRow.rows[0];
    if (deposit.user_id !== userId) {
      throw Object.assign(new Error('USER_MISMATCH: Deposit does not belong to this user'), { status: 403 });
    }

    // Fetch authoritative status from payment provider API
    let paymentStatusResult;
    try {
      paymentStatusResult = await providerInstance.fetchPaymentStatus(effectiveOrderId, effectivePaymentId, correlationId);
    } catch (fetchErr) {
      logger.warn(`[DepositEngine] Provider fetchPaymentStatus warning for ${normalizedProvider}:`, { error: fetchErr?.message });
      paymentStatusResult = {
        orderId: effectiveOrderId,
        paymentId: effectivePaymentId,
        status: 'PENDING',
      };
    }

    const resolvedPaymentId = paymentStatusResult.paymentId || effectivePaymentId || `pay_${effectiveOrderId}`;
    const resolvedMethod = paymentStatusResult.method || 'upi';
    const resolvedUtr = paymentStatusResult.utr || resolvedPaymentId;
    const paymentAmountInINR = paymentStatusResult.amountInINR || parseFloat(deposit.amount);

    const isSuccessfulStatus = paymentStatusResult.status === 'SUCCESS' ||
      paymentStatusResult.status === 'PAID' ||
      paymentStatusResult.status === 'captured';

    if (!isSuccessfulStatus) {
      if (paymentStatusResult.status === 'FAILED' || paymentStatusResult.status === 'CANCELLED' || paymentStatusResult.status === 'USER_DROPPED') {
        await query(
          `UPDATE deposits SET status = 'FAILED', updated_at = NOW()
           WHERE id = $1 AND UPPER(COALESCE(status,'')) IN ('CREATED','PENDING')`,
          [deposit.id]
        );
        throw Object.assign(new Error('PAYMENT_FAILED: Payment was not successful with provider'), {
          status: 400,
          code: 'PAYMENT_FAILED',
        });
      }
      throw Object.assign(new Error(`PAYMENT_NOT_COMPLETED: Payment not confirmed by gateway (status: ${paymentStatusResult.status || 'PENDING'})`), {
        status: 400,
        code: 'PAYMENT_NOT_COMPLETED',
      });
    }

    // Delegate to Central Single Financial Processor ONLY when payment status is confirmed SUCCESS
    return this.processVerifiedPayment({
      provider: normalizedProvider,
      depositId: deposit.deposit_id || deposit.id,
      providerOrderId: effectiveOrderId,
      providerPaymentId: resolvedPaymentId,
      amountInINR: paymentAmountInINR,
      userId,
      method: resolvedMethod,
      utr: resolvedUtr,
      rawPayload: { source: 'checkout_verify', provider: normalizedProvider, ...paymentStatusResult.raw },
      source: 'FRONTEND_VERIFY',
      correlationId,
    });
  }

  /**
   * 3. Process Webhook Event (Cashfree or Razorpay)
   */
  async processWebhook({ rawBody, headers = {}, signature = null, payload = null, event = null, provider = null }, correlationId = null) {
    await ensureDepositSchema();
    let normalizedProvider = provider ? String(provider).toUpperCase() : null;
    if (!normalizedProvider) {
      if (signature || headers?.['x-razorpay-signature'] || event?.includes('payment.') || event?.includes('order.')) {
        normalizedProvider = 'RAZORPAY';
      } else if (headers?.['x-webhook-signature'] || headers?.['x-cf-signature']) {
        normalizedProvider = 'CASHFREE';
      } else {
        normalizedProvider = 'CASHFREE';
      }
    }
    const providerInstance = paymentProviderService.getProvider(normalizedProvider);

    // Verify Webhook Signature
    const isSignatureValid = normalizedProvider === 'RAZORPAY'
      ? providerInstance.verifyWebhookSignature({ rawBody, signature: signature || headers['x-razorpay-signature'] })
      : providerInstance.verifyWebhookSignature({ rawBody, headers });

    if (!isSignatureValid) {
      throw new Error(`INVALID_SIGNATURE: ${normalizedProvider} webhook signature verification failed`);
    }

    // Replay skew: Cashfree signs with x-webhook-timestamp; Razorpay embeds created_at on the event.
    if (normalizedProvider === 'RAZORPAY') {
      const skewLimit = Number(process.env.RAZORPAY_WEBHOOK_MAX_SKEW_SECONDS) || 300;
      try {
        const rawParsedPreview = typeof rawBody === 'object' && !Buffer.isBuffer(rawBody)
          ? rawBody
          : (typeof rawBody === 'string' ? JSON.parse(rawBody) : JSON.parse(rawBody.toString('utf8')));
        const createdAt = Number(
          rawParsedPreview?.created_at
          || rawParsedPreview?.payload?.payment?.entity?.created_at
          || rawParsedPreview?.payload?.order?.entity?.created_at
          || 0,
        );
        if (createdAt > 0) {
          const tsMs = createdAt < 1e12 ? createdAt * 1000 : createdAt;
          const skewSec = Math.abs(Date.now() - tsMs) / 1000;
          if (skewSec > skewLimit) {
            throw new Error(`WEBHOOK_REPLAY_REJECTED: Razorpay timestamp skew ${Math.round(skewSec)}s exceeds ${skewLimit}s`);
          }
        }
      } catch (err) {
        if (String(err.message || '').startsWith('WEBHOOK_REPLAY_REJECTED')) throw err;
        // Malformed body will fail later at JSON parse / event parse
      }
    }

    const rawParsed = typeof rawBody === 'object' && !Buffer.isBuffer(rawBody)
      ? rawBody
      : (typeof rawBody === 'string' ? JSON.parse(rawBody) : JSON.parse(rawBody.toString('utf8')));
    const effectiveBody = { ...rawParsed, ...(payload ? { payload } : {}), ...(event ? { event } : {}) };
    const parsedEvent = providerInstance.parseWebhookEvent(effectiveBody, headers);

    const providerEventId = parsedEvent.providerEventId;
    const eventType = parsedEvent.event || event || 'PAYMENT_EVENT';

    // Webhook Idempotency Check — only skip fully PROCESSED events
    const existingWebhook = await query(
      `SELECT id, status FROM payment_webhook_events WHERE provider = $1 AND provider_event_id = $2`,
      [normalizedProvider, providerEventId]
    );
    if (existingWebhook.rows.length > 0) {
      const whStatus = String(existingWebhook.rows[0].status || '').toUpperCase();
      if (whStatus === 'PROCESSED') {
        return { status: 'IGNORED_DUPLICATE', providerEventId, message: 'Webhook event already processed' };
      }
      // PROCESSING / FAILED / other — allow retry by reclaiming the row
      await query(
        `UPDATE payment_webhook_events
         SET status = 'PROCESSING', event_type = $3, raw_payload = $4, processed_at = NULL
         WHERE provider = $1 AND provider_event_id = $2`,
        [normalizedProvider, providerEventId, eventType, JSON.stringify(parsedEvent.raw)],
      );
    } else {
      // Record webhook event receipt
      await query(
        `INSERT INTO payment_webhook_events (provider, provider_event_id, event_type, status, raw_payload, created_at)
         VALUES ($1, $2, $3, 'PROCESSING', $4, NOW())
         ON CONFLICT (provider_event_id) DO NOTHING`,
        [normalizedProvider, providerEventId, eventType, JSON.stringify(parsedEvent.raw)]
      );
    }

    try {
      // Handle Failure and Non-Payment Events
      if (parsedEvent.status !== 'SUCCESS') {
        if (parsedEvent.status === 'FAILED' && parsedEvent.orderId) {
          await query(
            `UPDATE deposits SET status = 'FAILED', updated_at = NOW()
             WHERE order_id = $1 AND UPPER(COALESCE(status,'')) IN ('CREATED','PENDING')`,
            [parsedEvent.orderId]
          );
        }
        await query(
          `UPDATE payment_webhook_events SET status = 'PROCESSED', processed_at = NOW() WHERE provider_event_id = $1`,
          [providerEventId]
        );
        return { status: 'EVENT_ACKNOWLEDGED', event: parsedEvent.event };
      }

      if (!parsedEvent.orderId) {
        throw new Error('INVALID_ORDER: Missing order_id in webhook payment payload');
      }

      // Delegate to Central Single Financial Processor
      const result = await this.processVerifiedPayment({
        provider: normalizedProvider,
        providerOrderId: parsedEvent.orderId,
        providerPaymentId: parsedEvent.paymentId || `wh_${providerEventId}`,
        amountInINR: parsedEvent.amountInINR,
        amountPaise: parsedEvent.amountPaise,
        userId: parsedEvent.userId,
        method: parsedEvent.method || 'upi',
        utr: parsedEvent.utr || parsedEvent.paymentId,
        rawPayload: parsedEvent.raw,
        source: 'WEBHOOK',
        correlationId,
      });

      await query(
        `UPDATE payment_webhook_events SET status = 'PROCESSED', processed_at = NOW() WHERE provider_event_id = $1`,
        [providerEventId]
      );

      return result;
    } catch (err) {
      await query(
        `UPDATE payment_webhook_events
         SET status = 'FAILED', processed_at = NOW()
         WHERE provider_event_id = $1 AND UPPER(COALESCE(status,'')) <> 'PROCESSED'`,
        [providerEventId],
      ).catch(() => {});
      throw err;
    }
  }

  /**
   * 4. CENTRAL PAYMENT PROCESSOR — EXACTLY-ONCE WALLET CREDIT & ATOMIC LEDGER COMMIT
   *
   * The Single Source of Truth for processing verified deposits across ALL gateways:
   *  - Cashfree Webhook & API Verification
   *  - Razorpay Webhook & API Verification
   *  - Admin Finance Authoritative Reconciliation
   */
  async processVerifiedPayment({
    provider = 'CASHFREE',
    depositId = null,
    providerOrderId,
    providerPaymentId,
    amountInINR,
    amountPaise = null,
    userId = null,
    method = 'upi',
    utr = null,
    rawPayload = {},
    source = 'SYSTEM',
    correlationId = null,
  }) {
    if (!providerOrderId || !providerPaymentId) {
      throw new Error('MISSING_PARAMETERS: providerOrderId and providerPaymentId are required');
    }

    await ensureDepositSchema();
    const normalizedProvider = String(provider || 'CASHFREE').toUpperCase();
    const effectivePaise = amountPaise != null
      ? Number(amountPaise)
      : Math.round(Number(amountInINR) * 100);

    const utrFinal = utr || providerPaymentId;

    // Concurrency / Idempotency check scoped by provider, payment, and order
    const idemKey = `${normalizedProvider}_${providerPaymentId}_${providerOrderId}`;
    let idemCheck = await idempotencyEngine.checkOrLock(idemKey, 'DEPOSIT_PROCESSOR', '', userId || '');

    if (idemCheck.isDuplicate) {
      const resolved = await this.#resolveDuplicateDepositCredit({
        idemCheck,
        idemKey,
        normalizedProvider,
        providerOrderId,
        providerPaymentId,
        depositId,
      });
      if (resolved.action === 'return') {
        return resolved.result;
      }
      // Stale/failed lock released — claim a fresh PROCESSING lock
      idemCheck = await idempotencyEngine.checkOrLock(idemKey, 'DEPOSIT_PROCESSOR', '', userId || '');
      if (idemCheck.isDuplicate) {
        const again = await this.#resolveDuplicateDepositCredit({
          idemCheck,
          idemKey,
          normalizedProvider,
          providerOrderId,
          providerPaymentId,
          depositId,
        });
        if (again.action === 'return') {
          return again.result;
        }
        throw Object.assign(
          new Error('DEPOSIT_IN_PROGRESS: Payment is still being credited; retry shortly'),
          { code: 'DEPOSIT_IN_PROGRESS', status: 409 },
        );
      }
    }

    let result;
    try {
      // Atomic Database Transaction
      result = await withTransaction(async (client) => {
      // 1. Lock Deposit Transaction Row (FOR UPDATE)
      const depLock = await client.query(
        `SELECT id, deposit_id, user_id, amount, amount_paise, status, order_id, payment_id, provider
         FROM deposits
         WHERE order_id = $1 ${depositId ? 'OR deposit_id = $2 OR id = $2' : ''}
         FOR UPDATE`,
        depositId ? [providerOrderId, depositId] : [providerOrderId],
      );

      if (depLock.rows.length === 0) {
        throw new Error(`UNKNOWN_ORDER: No deposit record found for order '${providerOrderId}'`);
      }

      const lockedDeposit = depLock.rows[0];
      const targetUserId = lockedDeposit.user_id;

      if (
        lockedDeposit.provider &&
        !['SYSTEM', 'GATEWAY'].includes(lockedDeposit.provider.toUpperCase()) &&
        !['SYSTEM', 'GATEWAY'].includes(normalizedProvider) &&
        lockedDeposit.provider.toUpperCase() !== normalizedProvider
      ) {
        throw new Error(`PROVIDER_MISMATCH: Payment provider '${normalizedProvider}' does not match deposit order provider '${lockedDeposit.provider}'`);
      }

      if (userId && targetUserId !== userId) {
        throw new Error('USER_MISMATCH: Payment userId does not match deposit record');
      }

      // Check if providerPaymentId has already credited another order
      const dupCheck = await client.query(
        `SELECT id, deposit_id, order_id FROM deposits WHERE payment_id = $1 AND status IN ('PAID', 'CAPTURED')`,
        [providerPaymentId]
      );
      if (dupCheck.rows.length > 0 && dupCheck.rows[0].order_id !== providerOrderId) {
        throw new Error(`DUPLICATE_PAYMENT_ID: Payment ID '${providerPaymentId}' has already been credited to another deposit`);
      }

      // Check amount matching (integer paise comparison)
      const expectedPaise = lockedDeposit.amount_paise != null
        ? Number(lockedDeposit.amount_paise)
        : Math.round(Number(lockedDeposit.amount) * 100);

      if (effectivePaise !== expectedPaise) {
        throw new Error(`AMOUNT_MISMATCH: Payment amount (${effectivePaise} paise) does not match deposit order (${expectedPaise} paise)`);
      }

      const currentStatus = String(lockedDeposit.status).toUpperCase();

      // Exactly-Once Check: If already PAID / CAPTURED, return idempotent success
      if (currentStatus === 'PAID' || currentStatus === 'CAPTURED') {
        const wRes = await client.query(`SELECT balance FROM wallets WHERE user_id = $1`, [targetUserId]);
        return {
          success: true,
          status: 'PAID',
          paymentStatus: 'PAID',
          alreadyPaid: true,
          provider: normalizedProvider,
          depositId: lockedDeposit.deposit_id || lockedDeposit.id,
          paymentId: providerPaymentId,
          amount: Number(lockedDeposit.amount),
          amountPaise: expectedPaise,
          userId: targetUserId,
          newBalance: parseFloat(wRes.rows[0]?.balance || 0),
        };
      }

      // 2. Lock User Wallet Row (FOR UPDATE)
      const walletRes = await client.query(
        `SELECT wallet_id, balance FROM wallets WHERE user_id = $1 FOR UPDATE`,
        [targetUserId]
      );

      if (walletRes.rows.length === 0) {
        throw new Error(`WALLET_NOT_FOUND: User wallet does not exist for user '${targetUserId}'`);
      }

      const wallet = walletRes.rows[0];
      const depositRupees = Number(lockedDeposit.amount);

      // 3. Mark Deposit as PAID & Store Payment ID
      await client.query(
        `UPDATE deposits
         SET status = 'PAID',
             payment_id = $1,
             amount_paise = $2,
             provider = $3,
             raw_payload = $4,
             paid_at = NOW(),
             updated_at = NOW()
         WHERE id = $5`,
        [providerPaymentId, effectivePaise, normalizedProvider, JSON.stringify(rawPayload), lockedDeposit.id]
      );

      // 4. Credit Cash Wallet Balance
      const newBalanceRes = await client.query(
        `UPDATE wallets
         SET balance = balance + $1,
             locked_deposit_balance = COALESCE(locked_deposit_balance, 0) + $1,
             updated_at = NOW()
         WHERE wallet_id = $2
         RETURNING balance`,
        [depositRupees, wallet.wallet_id],
      );
      const newBalance = parseFloat(newBalanceRes.rows[0].balance);

      // 5. Record in Transactions Table
      await client.query(
        `INSERT INTO transactions (
           transaction_id, user_id, type, amount, status, utr, method,
           provider_payment_id, provider_order_id, created_at
         )
         VALUES ($1, $2, 'DEPOSIT', $3, 'SUCCESS', $4, $5, $6, $7, NOW())
         ON CONFLICT (transaction_id) DO UPDATE
         SET status = 'SUCCESS', provider_payment_id = $6, utr = $4`,
        [providerPaymentId, targetUserId, depositRupees, utrFinal, method, providerPaymentId, providerOrderId]
      );

      // 6. Record Immutable Double-Entry Ledger Entry
      await client.query(
        `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
         VALUES ($1, $2, 'CREDIT', $3, $4, $5, NOW())`,
        [wallet.wallet_id, providerPaymentId, depositRupees, newBalance, `${normalizedProvider} Deposit Successful`]
      );

      // 7. Insert Branded In-App User Notification
      await client.query(
        `INSERT INTO notifications (
           id, user_id, event_type, category, channel, subject, body, status, is_read, created_at
         )
         VALUES ($1, $2, 'DEPOSIT_SUCCESS', 'PAYMENT', 'IN_APP', 'Deposit Successful', $3, 'DELIVERED', false, NOW())
         ON CONFLICT (id) DO NOTHING`,
        [
          `notif_${providerPaymentId}`,
          targetUserId,
          `₹${depositRupees.toLocaleString('en-IN')} has been successfully added to your wallet.`,
        ]
      );

      // 8. Write Outbox Event for Message Broker / Realtime Telemetry
      await client.query(
        `INSERT INTO outbox_events (id, event_type, aggregate_type, aggregate_id, payload, status, correlation_id, created_at)
         VALUES ($1, 'deposit.completed', 'deposit', $2, $3, 'PENDING', $4, NOW())
         ON CONFLICT (id) DO NOTHING`,
        [
          `evt_${providerPaymentId}`,
          providerPaymentId,
          JSON.stringify({
            userId: targetUserId,
            provider: normalizedProvider,
            amount: depositRupees,
            amountPaise: effectivePaise,
            paymentId: providerPaymentId,
            depositId: lockedDeposit.deposit_id || lockedDeposit.id,
            utr: utrFinal,
            source,
            newBalance,
          }),
          correlationId || null,
        ]
      );

      return {
        success: true,
        status: 'PAID',
        paymentStatus: 'PAID',
        alreadyPaid: false,
        provider: normalizedProvider,
        depositId: lockedDeposit.deposit_id || lockedDeposit.id,
        paymentId: providerPaymentId,
        amount: depositRupees,
        amountPaise: effectivePaise,
        userId: targetUserId,
        newBalance,
        method,
        utr: utrFinal,
      };
    });
    } catch (err) {
      // Only fail the lock when the credit transaction itself failed —
      // never after a successful wallet credit if complete() later errors.
      await idempotencyEngine.fail(idemKey, err.message || String(err)).catch(() => {});
      throw err;
    }

    // Rethrows on DB error so PROCESSING is not left silent after a successful credit.
    await idempotencyEngine.complete(idemKey, result);

    if (!result.alreadyPaid) {
      // Async post-deposit actions (non-blocking)
      void import('./supportNotify.mjs')
        .then(({ emailUserPaymentEvent, emailOpsDepositAlert }) => Promise.all([
          emailUserPaymentEvent('deposit', {
            userId: result.userId,
            amount: result.amount,
            paymentId: result.paymentId,
            newBalance: result.newBalance,
          }),
          emailOpsDepositAlert({
            userId: result.userId,
            amount: result.amount,
            paymentId: result.paymentId,
            provider: result.provider,
            depositId: result.depositId,
            method: result.method,
            utr: result.utr,
          }),
        ]))
        .catch(() => {});

      void import('./referralLoyaltyEngine.mjs')
        .then(({ tryQualifyReferralAfterDeposit }) => tryQualifyReferralAfterDeposit({
          userId: result.userId,
          amount: result.amount,
        }))
        .catch(() => {});

      void import('./depositFreebetEngine.mjs')
        .then(({ tryGrantDepositFreebet }) => tryGrantDepositFreebet({
          userId: result.userId,
          depositId: result.depositId,
          amount: result.amount,
        }))
        .catch(() => {});
    }

    return result;
  }

  /**
   * When an idempotency key already exists, never invent alreadyPaid=true.
   * Prefer stored result / paid deposit row; release stale FAILED locks for retry;
   * surface IN_PROGRESS for live PROCESSING claims.
   */
  async #resolveDuplicateDepositCredit({
    idemCheck,
    idemKey,
    normalizedProvider,
    providerOrderId,
    providerPaymentId,
    depositId,
  }) {
    let stored = idemCheck.result ?? idemCheck.record?.result ?? null;
    if (typeof stored === 'string') {
      try {
        stored = JSON.parse(stored);
      } catch {
        stored = null;
      }
    }
    if (stored && typeof stored === 'object') {
      return { action: 'return', result: { ...stored, alreadyPaid: true } };
    }

    const depRes = await query(
      `SELECT id, deposit_id, user_id, amount, amount_paise, status, payment_id, order_id, provider
       FROM deposits
       WHERE order_id = $1 ${depositId ? 'OR deposit_id = $2 OR id = $2' : ''}
       ORDER BY CASE WHEN UPPER(COALESCE(status,'')) IN ('PAID','CAPTURED') THEN 0 ELSE 1 END, updated_at DESC NULLS LAST
       LIMIT 1`,
      depositId ? [providerOrderId, depositId] : [providerOrderId],
    );
    const dep = depRes.rows[0];
    if (dep && isSuccessfulDepositStatus(dep.status)) {
      const wRes = await query(`SELECT balance FROM wallets WHERE user_id = $1`, [dep.user_id]);
      const expectedPaise = dep.amount_paise != null
        ? Number(dep.amount_paise)
        : Math.round(Number(dep.amount) * 100);
      return {
        action: 'return',
        result: {
          success: true,
          status: 'PAID',
          paymentStatus: 'PAID',
          alreadyPaid: true,
          provider: String(dep.provider || normalizedProvider).toUpperCase(),
          depositId: dep.deposit_id || dep.id,
          paymentId: dep.payment_id || providerPaymentId,
          amount: Number(dep.amount),
          amountPaise: expectedPaise,
          userId: dep.user_id,
          newBalance: parseFloat(wRes.rows[0]?.balance || 0),
          message: 'Payment already processed',
        },
      };
    }

    const idemStatus = String(idemCheck.status || idemCheck.record?.status || '').toUpperCase();
    const createdAt = idemCheck.record?.createdAt || idemCheck.record?.created_at;
    const ageMs = createdAt ? Date.now() - new Date(createdAt).getTime() : 0;
    const staleProcessing = idemStatus === 'PROCESSING' && Number.isFinite(ageMs) && ageMs > 90_000;

    if (idemStatus === 'FAILED' || staleProcessing) {
      await idempotencyEngine.release(idemKey, {
        allowStatuses: staleProcessing ? ['PROCESSING', 'FAILED'] : ['FAILED'],
      });
      logger.warn('deposit_idempotency_lock_released', {
        idemKey,
        idemStatus,
        ageMs,
        providerOrderId,
        providerPaymentId,
      });
      return { action: 'retry' };
    }

    if (idemStatus === 'PROCESSING') {
      for (let attempt = 0; attempt < 8; attempt++) {
        await new Promise((r) => setTimeout(r, 40));
        const retryDepRes = await query(
          `SELECT id, deposit_id, user_id, amount, amount_paise, status, payment_id, order_id, provider
           FROM deposits
           WHERE order_id = $1 ${depositId ? 'OR deposit_id = $2 OR id = $2' : ''}
           ORDER BY CASE WHEN UPPER(COALESCE(status,'')) IN ('PAID','CAPTURED') THEN 0 ELSE 1 END, updated_at DESC NULLS LAST
           LIMIT 1`,
          depositId ? [providerOrderId, depositId] : [providerOrderId],
        );
        const retryDep = retryDepRes.rows[0];
        if (retryDep && isSuccessfulDepositStatus(retryDep.status)) {
          const wRes = await query(`SELECT balance FROM wallets WHERE user_id = $1`, [retryDep.user_id]);
          const expectedPaise = retryDep.amount_paise != null
            ? Number(retryDep.amount_paise)
            : Math.round(Number(retryDep.amount) * 100);
          return {
            action: 'return',
            result: {
              success: true,
              status: 'PAID',
              paymentStatus: 'PAID',
              alreadyPaid: true,
              provider: String(retryDep.provider || normalizedProvider).toUpperCase(),
              depositId: retryDep.deposit_id || retryDep.id,
              paymentId: retryDep.payment_id || providerPaymentId,
              amount: Number(retryDep.amount),
              amountPaise: expectedPaise,
              userId: retryDep.user_id,
              newBalance: parseFloat(wRes.rows[0]?.balance || 0),
              message: 'Payment already processed',
            },
          };
        }
      }
      throw Object.assign(
        new Error('DEPOSIT_IN_PROGRESS: Payment is still being credited; retry shortly'),
        { code: 'DEPOSIT_IN_PROGRESS', status: 409 },
      );
    }

    // COMPLETED/unknown without result and unpaid deposit — do not fake success
    throw Object.assign(
      new Error('DEPOSIT_CREDIT_INCOMPLETE: Idempotency record exists but wallet credit is not confirmed'),
      { code: 'DEPOSIT_CREDIT_INCOMPLETE', status: 409 },
    );
  }

  /**
   * Priority 20 — Age-based reconciliation sweep for pending deposits.
   * Finds deposits stuck in PENDING/CREATED past ageMinutes, queries PSP API (Cashfree/Razorpay),
   * verifies status, credits wallet idempotently if paid, or updates failed/expired.
   */
  async sweepPendingDeposits({ ageMinutes = 5, maxBatch = 50, dryRun = false } = {}) {
    await ensureDepositSchema();
    const minAge = Math.max(1, Number(ageMinutes) || 5);
    const limit = Math.max(1, Math.min(200, Number(maxBatch) || 50));

    const rows = await query(
      `SELECT id, deposit_id, user_id, amount, amount_paise, order_id, payment_id, status, provider, created_at
       FROM deposits
       WHERE UPPER(COALESCE(status, '')) IN ('PENDING', 'CREATED')
         AND created_at <= NOW() - ($1 || ' minutes')::interval
       ORDER BY created_at ASC
       LIMIT $2`,
      [minAge, limit],
    );

    const summary = {
      scanned: rows.rows.length,
      credited: 0,
      failed: 0,
      stillPending: 0,
      errors: [],
    };

    if (dryRun || rows.rows.length === 0) {
      return summary;
    }

    for (const dep of rows.rows) {
      const depProvider = String(dep.provider || 'CASHFREE').toUpperCase();
      const orderId = dep.order_id || dep.deposit_id || dep.id;
      try {
        const providerInstance = paymentProviderService.getProvider(depProvider);
        if (!providerInstance) {
          summary.errors.push({ depositId: dep.id, error: `Unsupported provider: ${depProvider}` });
          continue;
        }

        const statusRes = await providerInstance.fetchPaymentStatus(orderId, dep.payment_id);
        const pStatus = String(statusRes?.status || '').toUpperCase();

        if (pStatus === 'SUCCESS' || pStatus === 'PAID' || pStatus === 'CAPTURED') {
          const resolvedPaymentId = statusRes.paymentId || dep.payment_id || `sweep_${orderId}`;
          const amountInINR = statusRes.amountInINR || parseFloat(dep.amount);

          await this.processVerifiedPayment({
            provider: depProvider,
            depositId: dep.deposit_id || dep.id,
            providerOrderId: orderId,
            providerPaymentId: resolvedPaymentId,
            amountInINR,
            userId: dep.user_id,
            paymentMethod: statusRes.method || 'upi',
            bankReference: statusRes.utr || resolvedPaymentId,
            webhookPayload: { sweepReconciliation: true, statusRes },
          });
          summary.credited += 1;
        } else if (['FAILED', 'CANCELLED', 'USER_DROPPED', 'EXPIRED'].includes(pStatus)) {
          await query(
            `UPDATE deposits
             SET status = 'FAILED',
                 updated_at = NOW()
             WHERE id = $1 AND UPPER(COALESCE(status,'')) IN ('CREATED','PENDING')`,
            [dep.id],
          );
          summary.failed += 1;
        } else {
          // If older than 24 hours and still pending from PSP, expire it
          const ageHours = (Date.now() - new Date(dep.created_at).getTime()) / (1000 * 60 * 60);
          if (ageHours > 24) {
            await query(
              `UPDATE deposits
               SET status = 'EXPIRED',
                   updated_at = NOW()
               WHERE id = $1 AND UPPER(COALESCE(status,'')) IN ('CREATED','PENDING')`,
              [dep.id],
            );
            summary.failed += 1;
          } else {
            summary.stillPending += 1;
          }
        }
      } catch (err) {
        logger.error('[DepositEngine] sweepPendingDeposits error for deposit:', { depositId: dep.id, orderId, error: err.message });
        summary.errors.push({ depositId: dep.id, orderId, error: err.message });
      }
    }

    return summary;
  }

  /**
   * Alias for Razorpay backward compatibility
   */
  async processVerifiedRazorpayPayment(params) {
    return this.processVerifiedPayment({
      ...params,
      provider: 'RAZORPAY',
    });
  }
}

export const depositEngine = new DepositEngine();
