...
import { inferWallClockMatchFinal, isFeedStillLive, inferOffBoardCricketStaleFinal, markInferredFinal } from './wallClockFinality.mjs';
...
export async function lookupEventForSettlement({
  ...
  const identityTokens = (name = '') => {
    const raw = String(name).toLowerCase().replace(/[.,]/g, ' ').replace(/\s+/g, ' ').trim();
    const parts = raw.split(' ').filter(Boolean);
    const tokens = new Set([raw]);
    if (parts.length >= 2) tokens.add(`${parts[parts.length - 1]} ${parts.slice(0, -1).join(' ')}`);
    if (parts.length === 2) tokens.add(`${parts[1]} ${parts[0]}`);
    return tokens;
  };
  const identityOverlap = (a, b) => {
    const as = identityTokens(a);
    const bs = identityTokens(b);
    for (const x of as) {
      for (const y of bs) {
  ...
  const validateMatchIdentity = (candidate) => {
    ...
    if (expectedTeam1 && expectedTeam2 && candidate.team1?.name && candidate.team2?.name) {
      const c1 = candidate.team1.name;
      const c2 = candidate.team2.name;
      const matchDirect = identityOverlap(c1, expectedTeam1) && identityOverlap(c2, expectedTeam2);
      const matchReversed = identityOverlap(c1, expectedTeam2) && identityOverlap(c2, expectedTeam1);
    ...
    for (const alias of aliases) {
      if (byId.has(alias)) {
        ...
        if (validateMatchIdentity(hydrated)) {
          const s1 = Number(hydrated.score1 ?? hydrated.live_score1 ?? hydrated.liveDetails?.score1 ?? hydrated.team1?.runs ?? 0);
          const s2 = Number(hydrated.score2 ?? hydrated.live_score2 ?? hydrated.liveDetails?.score2 ?? hydrated.team2?.runs ?? 0);
          const startTime = hydrated.startTime || hydrated.start_time ? new Date(hydrated.startTime || hydrated.start_time).getTime() : 0;
          const updatedAt = hydrated.updatedAt || hydrated.updated_at || hydrated.cachedAt ? new Date(hydrated.updatedAt || hydrated.updated_at || hydrated.cachedAt).getTime() : 0;
          const now = Date.now();
          const isStaleLive = inferWallClockMatchFinal(hydrated, { startTime, updatedAt, s1, s2, now });
          const isFinal = ['COMPLETED', 'FINISHED', 'FINAL', 'CLOSED', 'HISTORICAL', 'SETTLED'].includes(String(hydrated.status).toUpperCase())
            || String(hydrated.matchState).toLowerCase() === 'post'
            || hydrated.isCompleted
            || isStaleLive;
          if (isStaleLive) {
            markInferredFinal(hydrated);
          }
          return {
            success: true,
            match: hydrated,
            ...
            eventStatus: isFinal ? 'COMPLETED' : (hydrated.status || 'LIVE'),
            errorCode: null,
            errorMessage: null,
            retryable: false,
          };
  ...
  try {
    ...
    if (cachedState && validateMatchIdentity(cachedState)) {
      const s1 = Number(cachedState.score1 ?? cachedState.live_score1 ?? cachedState.liveDetails?.score1 ?? cachedState.team1?.runs ?? 0);
      const s2 = Number(cachedState.score2 ?? cachedState.live_score2 ?? cachedState.liveDetails?.score2 ?? cachedState.team2?.runs ?? 0);
      const startTime = cachedState.startTime || cachedState.start_time ? new Date(cachedState.startTime || cachedState.start_time).getTime() : 0;
      const updatedAt = cachedState.updatedAt || cachedState.updated_at || cachedState.cachedAt ? new Date(cachedState.updatedAt || cachedState.updated_at || cachedState.cachedAt).getTime() : 0;
      const now = Date.now();
      const onLiveBoard = liveById instanceof Map && aliases.some((alias) => liveById.has(alias));
      const offBoardStale = !onLiveBoard && inferOffBoardCricketStaleFinal(cachedState, {
        onLiveBoard: false,
        startTime,
        updatedAt,
        now,
      });
      const isStaleLive = inferWallClockMatchFinal(cachedState, { startTime, updatedAt, s1, s2, now });
      let isFinal = ['COMPLETED', 'FINISHED', 'FINAL', 'CLOSED', 'HISTORICAL', 'SETTLED'].includes(String(cachedState.status).toUpperCase())
        || String(cachedState.matchState).toLowerCase() === 'post'
        || cachedState.isCompleted
        || isStaleLive
        || offBoardStale;

      // Still "LIVE" in Redis but gone from the board and not stale enough yet —
      // fall through to DB/provider instead of locking settlement on a frozen live card.
      if (!isFinal && !onLiveBoard && isFeedStillLive(cachedState)) {
        // continue to Tier 4+
      } else {
        if (isStaleLive || offBoardStale) {
          markInferredFinal(cachedState);
          isFinal = true;
        }
        return {
          success: true,
          ...
          lookupResult: isFinal ? LOOKUP_RESULT_CODES.EVENT_FOUND_COMPLETED : LOOKUP_RESULT_CODES.EVENT_FOUND_LIVE,
          eventStatus: isFinal ? 'COMPLETED' : (cachedState.status || 'LIVE'),
          errorCode: null,
          errorMessage: null,
          retryable: false,
        };
