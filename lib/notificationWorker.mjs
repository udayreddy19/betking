    ...
    try {
      ...
      if (userId && userId !== 'system') {
        ...
        await query(
          ...
          [userId, payload.email || `${userId}@betking.com`]
        );
        ...
        for (const channel of channels) {
          ...
          if (existingNotif.rows.length === 0) {
            await query(
              ...
              [notifId, userId, eventType, category, channel, payload.email || `${userId}@betking.com`, rendered.subject, rendered.body, eventId]
            );
          }
        }
      }
