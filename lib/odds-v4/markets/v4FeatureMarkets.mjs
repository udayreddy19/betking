...
function ou({ marketId, marketType, name, line, pOver, overround, maxOverOdds, category }) {
  const fairOver = Number(pOver);
  const p = clamp(applySideHouseBias(fairOver), 0.03, 0.95);
  return createMarketDefinition({
    ...
    selections: [
      priceSelection({
        ...
        probability: p,
        overround,
        // Soft-crush only when fair Over is a favorite (match extras).
        maxOdds: fairOver < 0.42 ? undefined : maxOverOdds,
      }),
      priceSelection({
...
export function generateV4FeatureMarkets(state, marginConfig = {}, momentum = null, existingIds = null) {
  ...
  const wicketsRaw = Number(batting.wickets) || 0;
  // Prefer batting-side wickets; only use chaseWickets when batting wickets missing/stale (0)
  // and chase feed is a higher non-zero count (never invent lower dismissal markets).
  let wickets = wicketsRaw;
  if (inn >= 2) {
    const chaseW = Number(state.liveDetails?.chaseWickets ?? state.liveDetails?.wickets2);
    if (Number.isFinite(chaseW) && chaseW > wicketsRaw) {
      wickets = chaseW;
    }
  }
  const wicketsRemaining = Math.max(0, (rules.maxWickets || 10) - wickets);
  ...
  if (wickets + 1 < (rules.maxWickets || 10) && Number(state.ballsRemaining) >= 12) {
    ...
    const ballsToWicket = Math.min(ballsRem, clamp(22 + wickets * 2, 14, 40));
    const meanAtWicket = score + rpb * ballsToWicket;
    const line = Math.floor(meanAtWicket) + 0.5;
    if (line > score + 1) {
      const fair = calculateOverUnderProbability(
        meanAtWicket,
        line,
        1.5,
        score,
        Math.max(6, Math.sqrt(ballsToWicket)),
      );
      push(ou({
  ...
  // Forward overs milestones — near horizon only (+2) to avoid noisy far ladders
  if (Number(state.ballsRemaining) >= 18) {
    ...
    for (const add of [2]) {
      ...
      const fair = calculateOverUnderProbability(
        ...
        1.5,
        score,
        Math.max(6, Math.sqrt(remBalls) * 1.3),
      );
      push(ou({
