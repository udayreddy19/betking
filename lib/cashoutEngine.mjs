...
import { cashoutAmountFromPotential } from './vipBenefits.mjs';
  ...
  try {
    const result = await withTransaction(async (client) => {
      ...
      const betRes = await client.query(`
        SELECT bet_id, user_id, stake, odds, potential_payout, status, COALESCE(fund_source, 'cash') AS fund_source
        FROM bets
      ...
      const loyaltyRes = await client.query(`SELECT tier FROM user_loyalty WHERE user_id = $1`, [userId]);
      const calculatedCashout = cashoutAmountFromPotential(
        potentialPayout,
        loyaltyRes.rows[0]?.tier,
      );
      if (requestedCashoutValue && Math.abs(parseFloat(requestedCashoutValue) - calculatedCashout) > 0.05 * calculatedCashout) {
