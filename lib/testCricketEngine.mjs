...
export class TestCricketEngine {
  ...
  startInnings(inningsNum, batTeam, bowlTeam) {
    ...
    this.currentStriker = batSquad[0] || batRoster?.batters?.[0] || 'J. Smith';
    this.currentNonStriker = batSquad[1] || batRoster?.batters?.[1] || 'A. Patel';
    this.currentBowler = bowlSquad[bowlSquad.length - 1] || bowlRoster?.bowlers?.[0] || 'P. Cummins';
