...
export function evaluateV4Calibration(observations = [], options = {}) {
  ...
  const subgroups = evaluateSubgroups(scoredObs, options);
...
/**
 * Evaluates subgroup breakdowns strictly. If a subgroup has insufficient data,
 * marks INSUFFICIENT_DATA and does not extrapolate.
 */
export function evaluateSubgroups(observations = [], options = {}) {
  const minSubgroup = options.minSubgroupSampleSize || 50;

  function evaluateGroup(items) {
    if (items.length < minSubgroup) {
      return {
        status: 'INSUFFICIENT_DATA',
        sampleSize: items.length,
        minRequired: minSubgroup,
        brierScore: null,
        logLoss: null,
        ece: null,
      };
    ...
    return {
      ...
      sampleSize: items.length,
      brierScore: calculateBrierScore(items),
      logLoss: calculateLogLoss(items),
      ece: generateReliabilityBuckets(items, 10).ece,
    };
  }
  ...
  for (const o of observations) {
    const s = o.sport || 'cricket';
    if (!bySport[s]) bySport[s] = [];
    bySport[s].push(o);
  }
  const sportResults = {};
  for (const [k, v] of Object.entries(bySport)) sportResults[k] = evaluateGroup(v);

  // 2. By Market
  const byMarket = {};
  for (const o of observations) {
    const m = o.market || o.marketId || 'unknown';
    if (!byMarket[m]) byMarket[m] = [];
    byMarket[m].push(o);
  }
  const marketResults = {};
  for (const [k, v] of Object.entries(byMarket)) marketResults[k] = evaluateGroup(v);

  // 3. By Probability Bucket (Favorites, Underdogs, Longshots)
  const favorites = observations.filter((o) => (o.calibratedProbability ?? o.rawProbability ?? 0.5) >= 0.60);
  const underdogs = observations.filter((o) => {
    const p = o.calibratedProbability ?? o.rawProbability ?? 0.5;
    return p >= 0.20 && p < 0.60;
  });
  const longshots = observations.filter((o) => (o.calibratedProbability ?? o.rawProbability ?? 0.5) < 0.20);

  return Object.freeze({
    bySport: sportResults,
    byMarket: marketResults,
    byProbabilityBucket: {
      favorites: evaluateGroup(favorites),
      underdogs: evaluateGroup(underdogs),
      longshots: evaluateGroup(longshots),
    },
  });
}
