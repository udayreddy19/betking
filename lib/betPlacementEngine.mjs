    ...
    try {
      ...
      if (fundSource !== 'cash') {
        const oddsToCheck = validatedSelections.length
          ...
          : [acceptedOdds];
        if (oddsToCheck.some((odds) => odds < BONUS_MIN_BET_ODDS)) {
          throw new Error(
            `BONUS_ODDS_GATE: Bonus/Freebet requires odds of ${BONUS_MIN_BET_ODDS.toFixed(2)} or higher on every selection`,
          );
        }
      }
      ...
      const result = await withTransaction(async (client) => {
        ...
        const loyaltyPoints = Math.floor((numericStake / 100) * 5);
        if (loyaltyPoints > 0) {
          ...
          await client.query(
            ...
            [loyaltyTierFromPoints(totalPoints), userId],
          );
        }
