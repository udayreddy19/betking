...
export function generateExtendedWicketMarkets(state, validation = {}, marginConfig = {}) {
  ...
  // 4. Team Score at Next Dismissal — project with balls left + rpb (never fixed +15…25)
  if (currentWkts < maxWkts && (state.ballsRemaining || 0) >= 1) {
    const currentRuns = battingTeam.runs || 0;
    const ballsRem = Math.max(0, Number(state.ballsRemaining) || 0);
    // Typical balls until next wicket shrinks as wickets fall; never exceed balls left.
    const ballsToWicket = Math.min(ballsRem, Math.max(6, 28 - currentWkts * 2));
    const rpb = Math.max(
      0.5,
      (Number(battingTeam.runs) || 0) / Math.max(1, Number(battingTeam.balls) || Number(state.ballsCompleted) || 1),
    );
    const expectedRunsAtDismissal = currentRuns + rpb * ballsToWicket;
    const lineAtDismissal = Math.floor(expectedRunsAtDismissal) + 0.5;
    if (lineAtDismissal > currentRuns + 0.5) {
      const { pOver: pDisOver, pUnder: pDisUnder } = calculateOverUnderProbability(
        expectedRunsAtDismissal,
        lineAtDismissal,
        1.5,
        currentRuns,
        Math.max(4, Math.sqrt(ballsToWicket)),
      );
