...
export async function listPendingWithdrawals({ limit = 100 } = {}) {
  const res = await q(
    `SELECT w.withdrawal_id AS id,
     ...
     WHERE UPPER(w.status) IN ('REQUESTED', 'PENDING', 'PENDING_APPROVAL', 'UNDER_REVIEW', 'HOLD')
...
export async function listAnalyticsReports() {
  ...
  const live = matches.filter((m) => m.isLive || m.matchState === 'in').length;
...
export async function listAuditLogs({ limit = 100 } = {}) {
  const db = await q(
    `SELECT event_id::text AS id,
            ...
            COALESCE(details->>'tenant', 'betking_in') AS tenant
     FROM audit_events
     ORDER BY created_at DESC
     LIMIT $1`,
    [limit],
