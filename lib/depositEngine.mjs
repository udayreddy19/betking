...
export class DepositEngine {
   ...
   */
  async createOrder({ userId, amount, currency = 'INR', provider = 'CASHFREE', returnUrl, notifyUrl }, correlationId = null) {
    if (!userId) {
    ...
    const userRes = await query(
      `SELECT email, phone, display_name, username FROM users WHERE user_id = $1 LIMIT 1`,
      [userId]
    );
  ...
  async confirmCheckoutPayment({
    ...
    const normalizedProvider = String(provider || (razorpayOrderId ? 'RAZORPAY' : 'CASHFREE')).toUpperCase();
    const providerInstance = paymentProviderService.getProvider(normalizedProvider);
   ...
   */
  async processWebhook({ rawBody, headers = {}, signature = null, payload = null, event = null, provider = 'CASHFREE' }, correlationId = null) {
    const normalizedProvider = String(provider || 'CASHFREE').toUpperCase();
    const providerInstance = paymentProviderService.getProvider(normalizedProvider);
