...
const ensureSignupPromoSchema = memoizeEnsure(async () => {
  await addColumnIfMissing(
    'signup_promo_codes',
    'is_invite_only',
    `ALTER TABLE signup_promo_codes ADD COLUMN IF NOT EXISTS is_invite_only BOOLEAN NOT NULL DEFAULT FALSE`,
  );
  await createTableIfMissing(
    'signup_promo_invites',
    `
...
export async function applySignupPromoInTransaction(client, { userId, promoCode }) {
  ...
  const signupMethod = rewardType === 'freebet'
    ? 'SIGNUP_FREEBET'
    : rewardType === 'cash'
      ? 'SIGNUP_CASH'
      : 'SIGNUP_BONUS';
  ...
  await client.query(
    ...
    [txId, userId, signupMethod, amount],
  );
...
export async function sendSignupPromoInvites({
  ...
  for (const email of uniqueEmails) {
    ...
    if (sendResult.success && userId) {
      try {
        const { notifyUserPromoOffer } = await import('./promoUserNotify.mjs');
        await notifyUserPromoOffer({
          userId,
          subject: `Your OddsYra code ${promo.code}`,
          message: promo.name || 'You have a promo code waiting in OddsYra.',
          url: '/promotions',
          eventId: `signup_invite_${promo.code_id}_${userId}`,
        });
      } catch {
