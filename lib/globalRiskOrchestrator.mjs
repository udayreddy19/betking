...
class GlobalRiskOrchestrator {
  ...
  async evaluateBetRequest({
    ...
    // 4. Market Liability & Exposure Check — hard reject (never soft-accept over capacity)
    ...
    if (exposureCheck?.exceedsMaxLiability) {
      return this.recordDecision({
        ...
        maxAllowedStake: Math.max(10, Math.floor(exposureCheck.remainingCapacity / (serverOdds || clientOdds || 1))),
        userId, matchId, marketId, selectionId, stake, clientOdds, serverOdds, timestamp,
      });
    }
