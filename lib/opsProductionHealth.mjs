...
export async function buildProductionHealth() {
  ...
  return {
    ...
    observabilityClassification: 'PROCESS_LOCAL',
    observability: {
      httpCounters: 'PROCESS_LOCAL',
      securityCounters: 'PROCESS_LOCAL',
      outboxCounts: 'DISTRIBUTED',
      redisHealth: 'DISTRIBUTED',
      p95Latency: 'NOT_VERIFIED',
    },
  };
}

/**
 * Drill-down for Production Health KPI tiles — shared admin KPI drilldown engine.
 */
export async function getProductionHealthDrilldown(metricKey, opts = {}) {
  const { getAdminKpiDrilldown } = await import('./adminKpiDrilldown.mjs');
  return getAdminKpiDrilldown(metricKey, opts);
}
