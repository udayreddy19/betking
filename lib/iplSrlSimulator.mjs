...
export function applySrlScoreAnchors(live, sim, anchors = []) {
  ...
  for (const anchor of anchors) {
    ...
    if (innings === 1) {
      ...
      // Apply once the innings has completed the anchored delivery (ballIndex is 0-based).
      const pastAnchor = currentBalls >= (ballIndex + 1);
...
function buildMatchFromSchedule(globalSlot, now) {
  ...
  const preMatch = {
    ...
    liveDetails: {
      ...
      ...(toss ? { toss, tossWinner: toss.wonToss, tossDecision: toss.decision } : {}),
    },
  };
  ...
  if (op.customCommentary?.text) {
    live.commentary = `📢 [${op.customCommentary.eventTag || 'LIVE'}] ${op.customCommentary.text} · ${live.commentary}`;
  }
...
export function getIplSrlMatches(now = Date.now()) {
  ...
  return enrichIplSrlMatchList(sorted);
}
