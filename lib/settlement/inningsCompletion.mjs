...
export function isInningsComplete(match, settleInnings, bat = null, ld = null) {
  ...
  if (inn === 2) {
    ...
    if (batting.innings > 2) return true;
    if (resolveInningsWickets(match, 2) >= 10) return true;
    if (/all\s*out|target\s*(chased|reached)|won by/i.test(commentary)) return true;
    const maxOvers = getMatchMaxOvers(match);
    const oversStr = getInningsOversString(match, 2) ?? batting.oversStr;
    const parts = parseOversParts(oversStr);
    if (maxOvers && parts && parts.completed >= maxOvers && batting.innings === 2) return true;
    return false;
  }
