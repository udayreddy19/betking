...
export async function processSettlementQueue({ limit = 50, matchLookup = null } = {}) {
  ...
  for (const job of claimed) {
    try {
      ...
      // Worker may lag API desk seeks — refresh SRL operator clock before grading.
      if (String(bet.match_id || '').startsWith('srl_')) {
        try {
          ...
          await refreshSrlOperatorSession(bet.match_id);
          const fresh = getIplSrlMatchById(bet.match_id, Date.now(), { forPublic: false });
          if (fresh && matchLookup) {
            // Prefer fresh board when lookup is a Map-like function wrapping seeded matches.
            const existing = matchLookup(bet.match_id);
            if (!existing || existing.source === 'srl' || String(existing.id || '').startsWith('srl_')) {
              // Attach onto a mutable cache if the lookup closes over byId — best-effort via evaluate with fresh seed below.
              bet.__srlFreshMatch = fresh;
            }
          } else if (fresh) {
            bet.__srlFreshMatch = fresh;
          }
      ...
      let evaluated = null;
      if (matchLookup || bet.__srlFreshMatch) {
        const { evaluateBetForSettlement } = await import('../liveMatchSettlement.mjs');
        const lookup = (id) => {
          if (bet.__srlFreshMatch && String(id) === String(bet.match_id)) return bet.__srlFreshMatch;
          return matchLookup?.(id) || null;
        };
        evaluated = await evaluateBetForSettlement(bet, lookup);
      }

      if (!evaluated || evaluated.awaitingEvidence || !evaluated.outcome) {
        await markJob(job.job_id, 'AWAITING_EVIDENCE', evaluated?.reason || 'not_eligible_yet', (job.attempts || 0) + 1, job.max_attempts);
        skipped += 1;
        continue;
      }

      const targetMatch = bet.__srlFreshMatch || matchLookup?.(job.match_id) || { id: job.match_id };
      const isSnapshotBacked = Boolean(
        evaluated?.hasImmutableSnapshotEvidence
        || (evaluated?.reason && /^(?:dismissal_|wicket_in_over_|next_over_|delivery_|team_total_|overs_|over_)/.test(evaluated.reason)),
      );
