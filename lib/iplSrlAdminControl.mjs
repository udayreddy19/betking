...
export function triggerDelivery(matchId, { admin, incident: directIncident } = {}) {
  if (isUserFacingSrlMatch(matchId)) {
    ...
    // Capture pre-seek totals so we can replace the natural ball with the injected outcome.
    ...
    if (incident) {
      ...
      const ballIndex = Math.max(0, ballCount - 1);
      ...
      if (addRuns != null) {
        // Replace the natural ball contribution with the scripted outcome.
    ...
    // Re-read after anchors/commentary so replay + response reflect the board users see.
    const mFinal = getIplSrlMatchById(matchId);
    const ldFinal = mFinal?.liveDetails || {};
    const oversDoneFinal = String(ldFinal.firstOvers ?? ldFinal.overs ?? '0.0');
    ...
    recordSrlReplayDelivery(matchId, {
      ...
      innings: finalInn,
      bowler: typeof ldFinal.bowler === 'object' ? (ldFinal.bowler?.name || 'Bowler') : (ldFinal.bowler || 'Bowler'),
      batsman: typeof ldFinal.batsman === 'object'
        ? (ldFinal.batsman?.name || 'Striker')
        : (ldFinal.batsman || ldFinal.batter1?.name || 'Striker'),
      outcome,
      runs,
      wicket: isWicket,
      wicketType: incident?.subType || null,
      commentary,
      score: { runs: boardRuns, overs: oversDoneFinal },
      timestamp: Date.now(),
    });
...
export function injectIPLSRLIncident(matchId, incident, admin) {
  ...
  if (incident?.instant) {
    ...
    return triggerDelivery(matchId, { admin, incident: payload });
  }
