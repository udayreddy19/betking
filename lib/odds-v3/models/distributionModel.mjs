 ...
 */
export function calculateOverUnderProbability(expectedMean, line, varianceRatio = 1.5) {
  if (expectedMean <= 0) {
    return { pOver: 0.01, pUnder: 0.99 };
  }

  const stdDev = Math.sqrt(expectedMean * varianceRatio);
  if (stdDev <= 0) {
  ...
  const pUnder = Math.max(0.01, Math.min(0.99, pUnderRaw));
