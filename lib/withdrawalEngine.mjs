    ...
    const result = await withTransaction(async (client) => {
      const walletRes = await client.query(
        `SELECT wallet_id, balance, COALESCE(reserved_balance, 0.00) as reserved_balance,
                ...
                COALESCE(winnings_balance, 0.00) as winnings_balance
      ...
      const winningsBalance = parseFloat(wallet.winnings_balance);
      const availableBalance = parseFloat(Math.max(0, winningsBalance - reservedBalance).toFixed(2));

      if (availableBalance < numericAmount) {
        throw new Error(`INSUFFICIENT_FUNDS: Withdrawable winnings ₹${availableBalance} is less than requested withdrawal ₹${numericAmount}`);
      }
      if (currentBalance < numericAmount) {
        throw new Error(`INSUFFICIENT_FUNDS: Cash balance ₹${currentBalance} is less than requested withdrawal ₹${numericAmount}`);
      }

      const newReserved = parseFloat((reservedBalance + numericAmount).toFixed(2));
      const newWinnings = parseFloat(Math.max(0, winningsBalance - numericAmount).toFixed(2));
      ...
      await client.query(
        `UPDATE wallets
         SET reserved_balance = $1,
             ...
             updated_at = NOW()
         WHERE wallet_id = $4`,
        [newReserved, newWinnings, newBalance, wallet.wallet_id]
      );
  ...
  async reviewWithdrawal({ withdrawalId, adminId, decision, reason = '' }) {
    ...
    if (decision === 'APPROVE') {
      await withTransaction(async (client) => {
        ...
        const txId = `tx_app_${withdrawalId}`;
        await client.query(
          `INSERT INTO transactions (transaction_id, user_id, type, amount, status, created_at)
           VALUES ($1, $2, 'WITHDRAWAL', $3, 'SUCCESS', NOW())`,
          [txId, userId, amount]
        );

        await client.query(
          `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
           VALUES ($1, $2, 'DEBIT', $3, $4, 'Withdrawal Approved & Executed', NOW())`,
          [wallet.wallet_id, txId, amount, currentBalance],
        );
      });
    ...
    if (decision === 'REJECT') {
      ...
      await withTransaction(async (client) => {
        const walletRes = await client.query(
          `SELECT wallet_id, balance, reserved_balance, COALESCE(winnings_balance, 0) AS winnings_balance
           FROM wallets WHERE user_id = $1 FOR UPDATE`,
          [userId],
        );
        const wallet = walletRes.rows[0];
        const currentBalance = parseFloat(wallet.balance);
        const currentWinnings = parseFloat(wallet.winnings_balance);
        const newReserved = Math.max(0, parseFloat(wallet.reserved_balance) - amount);
        const newBalance = parseFloat((currentBalance + amount).toFixed(2));
        const newWinnings = parseFloat((currentWinnings + amount).toFixed(2));

        await client.query(
          `UPDATE wallets
           SET reserved_balance = $1, balance = $2, winnings_balance = $3, updated_at = NOW()
           WHERE wallet_id = $4`,
          [newReserved, newBalance, newWinnings, wallet.wallet_id],
        );
