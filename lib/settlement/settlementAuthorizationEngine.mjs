...
export function authorizeSettlement({
  ...
  if (!confidence.settlementAllowed) {
    settlementMetrics.settlement_blocked_total += 1;
    const isMissingData = confidence.confidenceState === 'MISSING_DATA' && confidence.settlementReasonCodes?.length > 0;
    return {
      success: false,
      error: isMissingData
        ? confidence.settlementReasonCodes[0]
        : `SETTLEMENT_BLOCKED: Confidence is ${confidence.confidenceState} (${confidence.settlementReasonCodes.join(', ')})`,
      authorization: null,
      confidence,
    };
  }
  ...
  const authId = `auth_${now}_${crypto.randomBytes(4).toString('hex')}`;
