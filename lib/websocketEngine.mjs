...
const WS_REDIS_FANOUT = process.env.WS_REDIS_FANOUT_CHANNEL || 'oddsyra:ws:fanout';
const WS_ORIGIN_PID = process.pid;
let wssInstance = null;
let fanoutSubscriber = null;
...
export async function canSubscribeToChannel(session, channel) {
  ...
  if (channel.startsWith('user:')) {
    ...
    if (!session.userId || session.userId !== targetUserId) return false;
    return true;
  }

  // Ops Control Tower live alerts (non-PII operational events)
  if (channel === 'admin:ops') {
    const role = String(session.role || '').toLowerCase();
    return ADMIN_WS_ROLES.has(role) || role === 'super_admin' || role.includes('admin');
  }
...
export function sendToUser(userId, eventType, payload = {}) {
  if (!userId) return { sent: 0, broadcastedCount: 0 };
  const channelId = `user:${userId}`;
  const event = {
    eventId: payload.eventId || `evt_${eventType}_${payload.betId || userId}_${Date.now()}`,
    eventType,
    payload,
    timestamp: Date.now(),
  };
  const res = sendJsonToChannel(channelId, event);
  // Cross-process fanout: worker outbox → Redis → API sockets (skip own echo).
  void publishWsFanout(channelId, event);
  return { sent: res.broadcastedCount || 0, ...res };
}

async function publishWsFanout(channelId, event) {
  try {
    if (redis.status !== 'ready' && redis.status !== 'connecting' && redis.status !== 'connect') {
      try { await redis.connect(); } catch { /* ignore */ }
    }
    await redis.publish(WS_REDIS_FANOUT, JSON.stringify({
      originPid: WS_ORIGIN_PID,
      channelId,
      event,
    }));
  } catch (err) {
    if (process.env.DEBUG_REDIS) {
      logger.warn('ws_fanout_publish_failed', { error: err.message });
    }
  }
}

export async function startWsFanoutSubscriber() {
  if (fanoutSubscriber) return fanoutSubscriber;
  try {
    fanoutSubscriber = redis.duplicate();
    fanoutSubscriber.on('error', (err) => {
      if (process.env.DEBUG_REDIS) logger.warn('ws_fanout_sub_error', { error: err.message });
    });
    await fanoutSubscriber.connect?.().catch(() => {});
    await fanoutSubscriber.subscribe(WS_REDIS_FANOUT);
    fanoutSubscriber.on('message', (channel, raw) => {
      if (channel !== WS_REDIS_FANOUT) return;
      try {
        const msg = JSON.parse(raw);
        if (!msg?.channelId || !msg?.event) return;
        if (Number(msg.originPid) === WS_ORIGIN_PID) return; // already delivered locally
        sendJsonToChannel(msg.channelId, msg.event);
      } catch {
        /* ignore malformed */
      }
    });
    logger.info('ws_fanout_subscriber_ready', { channel: WS_REDIS_FANOUT });
  } catch (err) {
    logger.warn('ws_fanout_subscriber_failed', { error: err.message });
  }
...
export function broadcastWsMessage(eventType, payload = {}) {
  ...
  // NEVER global-broadcast sensitive payloads (support chat text, PII).
  // Deliver only to authorized channel subscribers.
  ...
  // Ops / admin alert fan-out (non-PII operational signals)
  if (String(eventType || '').startsWith('admin.')) {
    const opsEvent = {
      ...event,
      channel: 'admin:ops',
    };
    const res = sendJsonToChannel('admin:ops', opsEvent);
    activeCount += res.broadcastedCount || 0;
    void publishWsFanout('admin:ops', opsEvent);
  }
