...
export function shouldEnforceKycAge() {
  if (process.env.NODE_ENV === 'test' && process.env.ENFORCE_KYC_AGE !== '1') return false;
  return true;
}
...
/** Real-money deposit/cash bet requires VERIFIED KYC and date of birth proving 18+. */
export async function assertRealMoneyKycAge(userId) {
  ...
  if (!row || String(row.kyc_status || '').toUpperCase() !== 'VERIFIED') {
    throw kycAgeGateError('Verify your identity (KYC) before depositing or placing cash bets.');
  }

  const age = ageFromDob(row.date_of_birth, new Date());
  if (age == null) {
    throw kycAgeGateError('Add a verified date of birth before depositing or placing cash bets.');
  }
  if (age < MIN_LEGAL_AGE) {
    throw kycAgeGateError('You must be 18 or older to use real money.');
  }
