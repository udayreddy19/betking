...
export async function dispatchTargetedDepositFreebetEmails({
  ...
  for (const row of users.rows) {
    ...
    // Never use admin "email_subject" on the wire — stored copy is usually promo blast wording
    // ("100% Deposit Free Bet Offer…") which Gmail always files under Promotions.
    const baseSubject = 'Your OddsYra wallet has an update';
    const subject = `${baseSubject} · ${String(promotionId).slice(-8)}`;
    try {
