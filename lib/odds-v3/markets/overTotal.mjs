...
/**
 * Remaining-window scoring vs innings-average rate.
 * Linear innings-fraction underpriced powerplay (T20 0–5 sat at ~40.5 / 8.1 rpo).
 */
const MILESTONE_WINDOW_RATE = Object.freeze({
  5: 1.26,
  10: 1.16,
  15: 1.09,
  20: 1.04,
...
/** Extra runs on the projection so the posted line sits above fair. */
const MILESTONE_HOUSE_PAD = 3;
 ...
 * Projects remaining balls in the window at a phase-weighted rate, not a
 * linear slice of full-innings par (that priced T20 0–5 at ~8.1 rpo).
 */
export function estimateTotalAtOvers(state, targetOvers) {
  ...
  const phaseMult = MILESTONE_WINDOW_RATE[targetOvers] ?? 1.04;
  const pad = MILESTONE_HOUSE_PAD * (ballsLeftInWindow / targetBalls);
  let expectedAtTarget = currentScore + (ballsLeftInWindow * inningsAvgRpb * phaseMult) + pad;
...
export function generateExtendedOverMarkets(state, validation = {}, marginConfig = {}) {
  ...
  const milestones = [5, 10, 15, 20].filter((overs) => {
    const targetBalls = overs * ballsPerOver;
    if (targetBalls > rules.ballsPerInnings) return false;
    // Full-innings range is the same event as team_total — do not print a second book.
    if (targetBalls >= rules.ballsPerInnings) return false;
    return true;
  });
