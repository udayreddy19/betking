...
export function parseMarketInstance(marketId) {
  ...
  const playerMilestone = id.match(/^player_(25|50|75|100)_(.+)$/i);
  if (playerMilestone) {
    return {
      type: 'PLAYER_MILESTONE',
      marketType: `PLAYER_SCORE_${playerMilestone[1]}`,
      target: Number(playerMilestone[1]),
      playerSlug: playerMilestone[2].toLowerCase(),
      instanceKey: `PLAYER_${playerMilestone[1]}:${playerMilestone[2].toLowerCase()}`,
    };
  }
  const playerAlt = id.match(/^player_alt_(.+)$/i);
  if (playerAlt) {
    return {
      type: 'PLAYER_RUNS_ALT',
      marketType: 'PLAYER_RUNS_ALT',
      playerSlug: playerAlt[1].toLowerCase(),
      instanceKey: `PLAYER_ALT:${playerAlt[1].toLowerCase()}`,
    };
  }
...
export function buildLegPlacementContext(sel) {
  ...
  const inningsRaw = sel.innings ?? sel.currentInnings ?? sel.inningsAtPlacement ?? null;
  const innings = inningsRaw != null && Number.isFinite(Number(inningsRaw)) ? Number(inningsRaw) : null;
  ...
  return {
    ...
    line,
    innings,
    scoreAtPlacement: sel.scoreAtPlacement || sel.score_at_placement || null,
    stateKey: sel.stateKey || sel.state_key || null,
    teamId: sel.teamId || sel.team_id || null,
...
export function buildPlacementSnapshot({
  ...
  return {
    ...
    quoteTimestamp: quoteTimestamp || null,
    betType,
    ...
    modelVersion: modelVersion || engineVersion || null,
    riskDecision: riskDecision || 'ACCEPT',
    fairProbability: fairProbability != null ? Number(fairProbability) : null,
    provider: provider || null,
    providerTimestamp: providerTimestamp || null,
    providerStateVersion: providerStateVersion ?? stateVersion,
    canonicalStateVersion: canonicalStateVersion ?? stateVersion,
    odds: odds != null ? Number(odds) : (first.odds ?? null),
    market: market || first.marketId || null,
    selection: selection || first.selectionId || null,
    legs,
  };
}
