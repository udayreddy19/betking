...
import { resolveSrlV4Margins } from './odds-v4/v4HouseProtect.mjs';
import { generatePublicMatchOddsSnapshotAsync } from './odds-v4/engineDispatch.mjs';
...
function resolveMatchObject(matchId) {
  ...
  const isSrlId = id.startsWith('srl_') || id.includes('srl');

  // SRL: prefer the live simulator clock over a stale aggregator/DB "completed" row.
  // Otherwise Sports keeps provisional Match Winner while Tracker still looks live.
  if (isSrlId) {
    try {
      const srl = getIplSrlMatchById(matchId, Date.now(), { forPublic: true })
        || findMatchInList(getIplSrlMatches(Date.now(), { publicBoard: true }) || [], matchId);
      if (srl) {
        const state = String(srl.matchState || '').toLowerCase();
        const live = srl.isLive === true || state === 'in' || state === 'pre';
        if (live) return srl;
      }
    } catch {
  ...
  if (isSrlId) {
    const srl = findMatchInList(getIplSrlMatches(Date.now(), { publicBoard: true }) || [], matchId)
      || getIplSrlMatchById(matchId, Date.now(), { forPublic: true });
    if (srl) return srl;
  }
...
export async function buildMatchOddsPayload({
  ...
  const run = (async () => {
    ...
    const matchState = String(matchObj.matchState || '').toLowerCase();
    const statusUpper = String(matchObj.status || '').toUpperCase();
    const isNonLive = Boolean(
      matchObj.isCompleted
      || matchState === 'post'
      || ['COMPLETED', 'FINISHED', 'FINAL', 'CLOSED', 'HISTORICAL'].includes(statusUpper)
      // Prematch (`pre`) must stay odds-open. Only treat as closed when we are neither
      // in-play nor upcoming (e.g. abandoned / unknown idle rows).
      || (matchObj.isLive === false && matchState !== 'in' && matchState !== 'pre')
    );
    ...
    const v3Config = {
      debug: false,
      ...(isMatchSRL(matchObj) ? { margins: resolveSrlV4Margins(SRL_MARGIN_CONFIG) } : {}),
    };
    const { publicSnapshot } = await generatePublicMatchOddsSnapshotAsync(matchObj, v3Config);
    try {
