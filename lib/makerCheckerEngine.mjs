  ...
  async approveRequest(requestId, checkerId = 'admin_checker') {
    return await withTransaction(async (client) => {
      ...
      if (req.action_type === 'MANUAL_CREDIT' || req.action_type === 'REFUND' || req.action_type === 'BONUS_ADJUSTMENT') {
        ...
        const { authorizeSettlement } = await import('./settlement/settlementAuthorizationEngine.mjs');

        const matchState = {
          matchId: bet.match_id,
          status: 'COMPLETED',
          __forcedOutcome: payload.outcome,
        ...
        const authRes = authorizeSettlement({
          ...
          evaluatedOutcome: payload.outcome,
          authorizedBy: `MakerChecker:${checkerId}`,
        });

        if (!authRes.success) {
          throw new Error(`MAKER_CHECKER_SETTLEMENT_AUTH_FAILED: ${authRes.error}`);
        }
