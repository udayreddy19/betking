...
export function findQuotedSelection(snapshot, marketId, selectionId) {
  ...
  const market = markets.find((m) => (
    m.marketId === marketId || m.marketType === marketId || m.key === marketId
  ));
  ...
  const sel = (market.selections || market.options || []).find((s) => (
    s.selectionId === selectionId || s.selection === selectionId || s.name === selectionId
  ));
  if (!sel) return null;
