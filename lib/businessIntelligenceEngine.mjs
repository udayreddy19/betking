...
export async function getFinancialPnlLedger(opts = {}) {
  const { period = '30d' } = opts;
  let intervalClause = "INTERVAL '30 days'";
  if (period === 'today') intervalClause = "INTERVAL '1 day'";
  else if (period === 'yesterday') intervalClause = "INTERVAL '2 days'";
  else if (period === '7d') intervalClause = "INTERVAL '7 days'";

  try {
    const [pnlRes, sportRes, cashflowRes] = await Promise.all([
      query(`
        SELECT
          COUNT(*) AS total_bets,
          COALESCE(SUM(CASE WHEN status IN ('ACCEPTED', 'SETTLED', 'WON', 'LOST') THEN stake ELSE 0 END), 0) AS turnover,
      ...
      query(`
        SELECT
          ...
          COALESCE(SUM(CASE WHEN b.status IN ('SETTLED', 'WON', 'LOST') THEN b.stake ELSE 0 END), 0) -
          COALESCE(SUM(CASE WHEN b.status = 'WON' THEN b.potential_payout ELSE 0 END), 0) AS ggr
        FROM bets b
        LEFT JOIN matches m ON m.match_id = b.match_id
        LEFT JOIN competitions c ON c.competition_id = m.competition_id
        WHERE b.created_at >= NOW() - ${intervalClause}
        GROUP BY COALESCE(c.sport_id, 'cricket')
        ORDER BY turnover DESC;
      `),
      query(`
        SELECT
          COALESCE(SUM(CASE WHEN type = 'DEPOSIT' AND status = 'COMPLETED' THEN amount ELSE 0 END), 0) AS total_deposits,
          COALESCE(SUM(CASE WHEN type = 'DEPOSIT' AND status = 'COMPLETED' THEN 1 ELSE 0 END), 0) AS deposit_count,
          COALESCE(SUM(CASE WHEN type = 'WITHDRAWAL' AND status = 'COMPLETED' THEN amount ELSE 0 END), 0) AS total_withdrawals,
          COALESCE(SUM(CASE WHEN type = 'WITHDRAWAL' AND status = 'COMPLETED' THEN 1 ELSE 0 END), 0) AS withdrawal_count
        FROM transactions
        WHERE created_at >= NOW() - ${intervalClause};
      `),
    ]);

    const p = pnlRes.rows[0] || {};
    const c = cashflowRes.rows[0] || {};
    const turnover = parseFloat(p.turnover || 0);
    const settledStake = parseFloat(p.settled_stake || 0);
    const settledPayout = parseFloat(p.settled_payout || 0);
    const ggr = parseFloat((settledStake - settledPayout).toFixed(2));
    const holdPct = settledStake > 0 ? parseFloat(((ggr / settledStake) * 100).toFixed(2)) : 0.0;
    const totalSettled = parseInt(p.won_bets || 0, 10) + parseInt(p.lost_bets || 0, 10);
    const playerWinRatePct = totalSettled > 0 ? parseFloat(((parseInt(p.won_bets || 0, 10) / totalSettled) * 100).toFixed(1)) : 0.0;

    const totalDeposits = parseFloat(c.total_deposits || 0);
    const totalWithdrawals = parseFloat(c.total_withdrawals || 0);
    const netCashflow = parseFloat((totalDeposits - totalWithdrawals).toFixed(2));

    const sportBreakdown = sportRes.rows.map((row) => {
      const spTurnover = parseFloat(row.turnover || 0);
      const spGgr = parseFloat(row.ggr || 0);
      return {
        sport: row.sport,
        betsCount: parseInt(row.bets_count || 0, 10),
        turnover: spTurnover,
        ggr: spGgr,
        holdPct: spTurnover > 0 ? parseFloat(((spGgr / spTurnover) * 100).toFixed(2)) : 0.0,
      };
    });
    ...
    return {
      ...
      holdPct,
      playerWinRatePct,
      activeBettors: parseInt(p.active_bettors || 0, 10),
      totalBets: parseInt(p.total_bets || 0, 10),
      cashflow: {
        totalDeposits,
        depositCount: parseInt(c.deposit_count || 0, 10),
        totalWithdrawals,
        withdrawalCount: parseInt(c.withdrawal_count || 0, 10),
        netCashflow,
      },
      sportBreakdown,
      timestamp: new Date().toISOString(),
    ...
    // Return gracefully computed snapshot
    return {
      success: true,
      period,
      turnover: 0,
      ...
      activeBettors: 0,
      totalBets: 0,
      cashflow: { totalDeposits: 0, depositCount: 0, totalWithdrawals: 0, withdrawalCount: 0, netCashflow: 0 },
      sportBreakdown: [
        { sport: 'cricket', betsCount: 0, turnover: 0, ggr: 0, holdPct: 0 },
        { sport: 'football', betsCount: 0, turnover: 0, ggr: 0, holdPct: 0 },
      ],
      timestamp: new Date().toISOString(),
    };
  }
