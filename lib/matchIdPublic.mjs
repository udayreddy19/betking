...
export function matchIdAliases(matchId = '') {
  ...
  if (!raw) return [];
  // OddsYra SRL slots are unique by full id — do not alias onto oy_/cb_/bare forms.
  if (/^srl_ipl_\d+$/i.test(raw) || /^srl_/i.test(raw)) {
    return [raw];
  }
...
export function matchIdsEqual(a, b) {
  ...
  const sa = String(a);
  const sb = String(b);
  // SRL ids only equal the exact same slot (never oy_ipl_N / bare ipl_N).
  if (/^srl_/i.test(sa) || /^srl_/i.test(sb)) {
