...
const CACHE_TTL_LIVE_MS = 0;

function isCacheValid() {
  if (!cachedResponse) return false;
  const hasLive = (cachedResponse.matches || []).some((m) => m.isLive);
  const ttl = hasLive ? CACHE_TTL_LIVE_MS : CACHE_TTL_MS;
...
export async function aggregateLiveScores(opts = {}) {
  ...
  // Enrich all live API-backed matches with real-time detail
  const liveEnrichable = matches.filter(
    (m) => m.isLive && (
      (m.sport === 'cricket' && m.cricbuzzMatchId)
      || m.source === 'espn'
      || m.id?.startsWith('api_')
    ),
  );

  if (liveEnrichable.length > 0) {
    await Promise.allSettled(
      liveEnrichable.map(async (m) => {
        try {
          const detail = await fetchMatchDetail(m, { fast: true });
          if (!detail?.liveDetails) return;
          const idx = matches.indexOf(m);
          if (idx < 0) return;
          matches[idx] = {
            ...m,
            isLive: detail.isLive ?? m.isLive,
            matchState: detail.matchState ?? m.matchState,
            time: detail.time ?? m.time,
            liveDetails: { ...m.liveDetails, ...detail.liveDetails },
          };
        } catch (err) {
          console.warn(`[Aggregator] Detail enrich failed for ${m.id}:`, err.message);
        }
      }),
    );
  }
