...
export function extractMatchWinnerOdds(snapshot, match = {}) {
  ...
  const drawSel = sels.find((s) => (
    s.selectionId === 'X'
    || s.selection === 'X'
    || /^(draw|tie)$/i.test(String(s.name || ''))
  ));
  const s1 = findSel(t1Name, 0);
  const s2 = findSel(t2Name, drawSel ? 2 : 1);
  ...
  const o2 = s2?.odds != null && Number.isFinite(Number(s2.odds)) && Number(s2.odds) > 1
    ...
    : null;
  const oDraw = drawSel?.odds != null && Number.isFinite(Number(drawSel.odds)) && Number(drawSel.odds) > 1
    ? Number(Number(drawSel.odds).toFixed(2))
    : null;
