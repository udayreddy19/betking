...
export async function buildOpsControlTower() {
  ...
  const [
      ...
      `SELECT COUNT(*)::int AS c FROM reconciliation_cases
       WHERE UPPER(status)='OPEN'
         AND (UPPER(COALESCE(severity,'')) IN ('HIGH','CRITICAL')
              OR UPPER(COALESCE(reconciliation_type,'')) LIKE '%DISCREP%')`,
    ),
    ...
    count(
      `SELECT COUNT(*)::int AS c FROM deposit_freebet_grants
       WHERE created_at >= ${SQL_IST_DAY_START}`,
    ),
    count(
      `SELECT COUNT(*)::int AS c FROM deposit_freebet_grants
       WHERE UPPER(COALESCE(status,'')) IN ('USED','CLAIMED','REDEEMED')
         AND COALESCE(updated_at, created_at) >= ${SQL_IST_DAY_START}`,
    ),
    ...
    query(`SELECT EXTRACT(EPOCH FROM (NOW() - MIN(updated_at)))::int as age_sec FROM user_profiles WHERE UPPER(kyc_status) IN ('PENDING','SUBMITTED','IN_REVIEW')`).catch(() => ({ rows: [] })),
  ...
  // Primary counts already query deposit_freebet_grants; keep nulls honest if those fail.
  const freebetsIssuedFinal = freebetsIssued;
  const freebetsClaimedFinal = freebetsClaimed;
  ...
  return {
    ...
    financial: {
      ...
      depositsToday: Number(depositsTodayVolRes.rows?.[0]?.count || depositsToday || 0),
      ...
      pendingWithdrawals: na(pendingWithdrawals) || 0,
    ...
    promotions: {
      // Ops KPI key is crmActiveCampaigns (Growth Overview uses promotions.activeCampaigns).
      crmActiveCampaigns: na(campaignsActive),
