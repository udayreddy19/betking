...
export function toCanonicalSport(raw = {}) {
  return {
    sportId: valOrNull(raw.id || raw.sportId),
    sportName: valOrNull(raw.name || raw.sportName),
    slug: valOrNull(raw.slug || raw.name?.toLowerCase().replace(/\s+/g, '-')),
...
export function toCanonicalMatch(raw = {}, providerName = 'gateway') {
  ...
  return {
    ...
    sport: toCanonicalSport({ name: raw.sport || 'cricket' }),
    league: toCanonicalLeague({ name: raw.league || raw.seriesName || raw.competition || 'International League' }),
    tournament: toCanonicalTournament({ name: raw.tournament || raw.seriesName || raw.league }),
