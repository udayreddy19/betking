...
export async function processSettlementQueue({ limit = 50, matchLookup = null } = {}) {
  ...
  for (const job of claimed) {
    try {
      ...
      if (!evaluated) {
        await markJob(job.job_id, 'RETRY', 'not_eligible_yet', job.attempts + 1, job.max_attempts);
        skipped += 1;
        continue;
      }

      logSettlement('SETTLEMENT_STARTED', { betId: job.bet_id, outcome: evaluated.outcome });
...
async function markJob(jobId, status, error, attempts = 0, maxAttempts = 5) {
  if (status === 'RETRY' && attempts >= maxAttempts) {
    status = 'DEAD_LETTER';
    await query(
  ...
  await query(
    `UPDATE settlement_jobs
     SET status = $1, last_error = $2, completed_at = CASE WHEN $1 IN ('COMPLETED', 'FAILED', 'DEAD_LETTER') THEN NOW() ELSE completed_at END
     WHERE job_id = $3`,
    [status, error, jobId],
  );
}
