...
export class BetPlacementEngine {
  ...
  async placeBet(params = {}, correlationId = null) {
    ...
    // 1. Idempotency Check — hash payload so same key + different body is rejected
    if (idempotencyKey) {
      ...
      const requestHash = createHash('sha256')
        .update(JSON.stringify({
          ...
          selections: Array.isArray(selections)
            ? selections.map((s) => ({
              ...
              selectionId: s.selectionId || s.selection_id,
              odds: s.odds ?? s.clientOdds,
              stake: s.stake,
            }))
            : null,
          fundSource,
        ...
        .digest('hex');
      const idemCheck = await idempotencyEngine.checkOrLock(idempotencyKey, 'BET_PLACE', requestHash, userId);
    ...
    try {
      ...
      await assertAdultDob(userId, { action: 'place a bet' });
      ...
      const result = await withTransaction(async (client) => {
        ...
        const cashBalance = fromPaise(toPaise(wallet.balance || 0));
        const bonusBalance = fromPaise(toPaise(wallet.bonus_balance || 0));
        const freebetBalance = fromPaise(toPaise(wallet.freebet_balance || 0));
        const stakePaise = toPaise(effectiveStake);
        ...
        if (rewardId) {
          ...
          if (toPaise(bonusBalance) <= 0 || toPaise(bonusBalance) < stakePaise) {
            throw new Error(
          ...
          newBonus = fromPaise(toPaise(bonusBalance) - stakePaise);
          ledgerAfter = newBonus;
          await client.query(
          ...
          if (toPaise(freebetBalance) <= 0) {
            throw new Error(
              `INSUFFICIENT_BALANCE: Insufficient freebet balance. Required: ₹${effectiveStake}, Available: ₹${freebetBalance}`,
            );
          }
          const { validatePromoBetStake, getWalletPromoRules } = await import('./walletPromoRules.mjs');
          await validatePromoBetStake({ fundSource: 'freebet', requestedStake: effectiveStake, availableBalance: freebetBalance, userId });
          const promoRules = await getWalletPromoRules().catch(() => ({ allowPartialFreeBet: false }));
          const allowPartial = Boolean(promoRules?.allowPartialFreeBet);
          newFreebet = allowPartial
            ? fromPaise(Math.max(0, toPaise(freebetBalance) - stakePaise))
            : 0.00;
          ledgerAfter = newFreebet;
          await client.query(
          ...
          const fromSpin = await consumeSpinGrants(client, userId, 'freebet', effectiveStake);
          const leftover = fromPaise(stakePaise - toPaise(fromSpin || 0));
          if (leftover > 0) {
            await consumeDepositFreebetGrants(client, userId, leftover);
          }
          const { consumeMatchingDiscreteRewardsForWalletSpend } = await import('./discreteRewardEngine.mjs');
          ...
          if (toPaise(availableCash) < stakePaise) {
            throw new Error(
          ...
          stakeFromLocked = fromPaise(toPaise(allocation.fromLocked));
          stakeFromWinnings = fromPaise(toPaise(allocation.fromWinnings));
          stakeFromCash = fromPaise(toPaise(allocation.fromNonWinnings));
          newCash = fromPaise(toPaise(cashBalance) - stakePaise);
          const newLocked = fromPaise(toPaise(wallet.locked_deposit_balance || 0) - toPaise(stakeFromLocked));
          ledgerAfter = newCash;
          await client.query(
            `UPDATE wallets
             SET balance = $1,
        ...
        const placementSnapshot = buildPlacementSnapshot({
          ...
          engine: quote?.engine || validatedSelections[0]?.engine || null,
          engineVersion: quote?.engineVersion || placementOddsVersion || null,
          modelVersion: quote?.modelVersion || quote?.engineVersion || null,
          riskDecision: 'ACCEPT',
          fairProbability: quote?.fairProbability ?? validatedSelections[0]?.fairProbability ?? null,
          providerStateVersion: quote?.stateVersion ?? placementStateVersion,
        });
        ...
        // Loyalty/VIP XP only from cash stakes — freebet/bonus must not farm progression.
        if (fundSource === 'cash') {
      ...
      try {
        const { recordAcceptedBetPrediction } = await import('./odds-v4/calibration/calibrationBridge.mjs');
        recordAcceptedBetPrediction({
          betId: result.betId,
          matchId: matchId || validatedSelections[0]?.matchId,
          marketId: marketId || validatedSelections[0]?.marketId,
          selectionId: selectionId || validatedSelections[0]?.selectionId,
          acceptedOdds: result.acceptedOdds || result.odds,
          fairProbability: quote?.fairProbability ?? validatedSelections[0]?.fairProbability,
          fairOdds: quote?.fairOdds ?? validatedSelections[0]?.fairOdds,
          sport: quote?.sport || validatedSelections[0]?.sport || 'cricket',
          engineVersion: quote?.engineVersion || placementOddsVersion,
          matchState: { status: quote?.matchStatus || 'live' },
        });
      } catch {
        // Non-fatal calibration capture
      }
      ...
      try {
        await recordBetRiskLiability(validatedSelections, result.stake, { userId });
      } catch {
