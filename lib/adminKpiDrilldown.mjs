...
export async function getAdminKpiDrilldown(metricKey, { limit = 50 } = {}) {
  ...
  if ([
    ...
    if (/fail/i.test(key)) {
      ...
      where += ` AND UPPER(status) IN ('COMPLETED','SUCCESS','CAPTURED')`;
  ...
  if ([
    ...
    if (/vip/i.test(key)) {
      const res = await safeQuery(
        `SELECT user_id AS id, vip_tier, vip_points, updated_at
         FROM user_loyalty
         WHERE UPPER(COALESCE(vip_tier,'')) NOT IN ('','NONE','BRONZE')
            OR COALESCE(vip_points,0) > 0
         ORDER BY COALESCE(vip_points,0) DESC LIMIT $1`,
        [lim],
      );
      return tableResult({
        metric: key,
