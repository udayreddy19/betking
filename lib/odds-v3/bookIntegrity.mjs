...
export function assertBettableQuote(serverOdds, clientOdds) {
  ...
  const maxDrift = Number(process.env.MAX_ODDS_DRIFT_PCT ?? 0.02);
  if (maxDrift > 0 && Math.abs(server - client) / server > maxDrift) {
    const err = new Error(
      `STALE_ODDS: Requested odds ${client} differ from server ${server} beyond ${maxDrift * 100}%`,
    );
    err.code = 'STALE_ODDS';
    throw err;
  }
...
export function oddsQuoteChanged(serverOdds, clientOdds, threshold = 0.02) {
  const server = Number(serverOdds);
  const client = Number(clientOdds);
  if (!Number.isFinite(server) || !Number.isFinite(client) || server <= 0) return false;
  return Math.abs(server - client) / server > threshold;
}
