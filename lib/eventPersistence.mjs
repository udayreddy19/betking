...
export async function upsertPersistentMatch(match) {
  ...
  const matchId = String(match.id || match.matchId || '').trim().slice(0, 64);
  if (!matchId) return null;

  const competition = String(match.competition || match.league || match.seriesName || 'Default League').slice(0, 64);
  const team1Name = String(match.team1?.name || match.team1 || match.homeTeam || 'Team 1').slice(0, 64);
  const team2Name = String(match.team2?.name || match.team2 || match.awayTeam || 'Team 2').slice(0, 64);
  ...
  const sport = String(match.sport || inferPersistedSport(competition, team1Name, team2Name) || '')
    .toLowerCase()
    .trim()
    .slice(0, 32) || null;
  const winnerSide = ['1', '2', 'X'].includes(String(match.winnerSide || '').trim())
    ? String(match.winnerSide).trim()
    : null;

  const sql = `
    INSERT INTO matches (
      match_id, competition_id, team1_id, team2_id, start_time, status,
      live_score1, live_score2, sport, winner_side, updated_at
    ...
    ON CONFLICT (match_id) DO UPDATE SET
      ...
      status = CASE
        WHEN matches.status IN ('COMPLETED', 'FINISHED', 'FINAL', 'SETTLED')
          AND EXCLUDED.status IN ('LIVE', 'SCHEDULED', 'IN')
          THEN matches.status
        ELSE EXCLUDED.status
      END,
      live_score1 = CASE
        WHEN EXCLUDED.live_score1 <> '' THEN EXCLUDED.live_score1
        ELSE matches.live_score1
      END,
      live_score2 = CASE
        WHEN EXCLUDED.live_score2 <> '' THEN EXCLUDED.live_score2
        ELSE matches.live_score2
      END,
      sport = COALESCE(NULLIF(EXCLUDED.sport, ''), matches.sport),
      winner_side = CASE
        WHEN EXCLUDED.winner_side IS NOT NULL THEN EXCLUDED.winner_side
        ELSE matches.winner_side
      END,
      updated_at = NOW()
    RETURNING *;
  `;
  ...
  try {
    ...
    // Non-blocking in case of concurrent writes / pre-migration deploy
    return null;
  }
}
...
export async function backfillMatchesFromPlacedBets() {
  try {
    ...
    for (const row of betMatches.rows) {
      ...
      const sport = String(leg.sport || snap?.sport || inferPersistedSport(league, team1Name, team2Name) || 'cricket');

      const sql = `
        INSERT INTO matches (
          match_id, competition_id, team1_id, team2_id, start_time, status, sport, updated_at
        ) VALUES ($1, $2, $3, $4, $5, 'COMPLETED', $6, NOW())
        ON CONFLICT (match_id) DO NOTHING;
      `;
      await query(sql, [matchId, league, team1Name, team2Name, startTime, sport]);
      backfilled++;
    }
    return { backfilled };
  } catch (err) {
...
export function inferPersistedSport(competition = '', team1 = '', team2 = '') {
  ...
  if (/\btennis\b|\batp\b|\bwta\b|\bwimbledon\b|\bus open\b/.test(text)) return 'tennis';
  if (/handball|andebol|handboll|håndbold|handebol/.test(text)) return 'handball';
  // Portuguese / Iberian club heuristics when league name is generic ("1a Divisao")
  if (/\b(aguas?\s*santas|belenenses|benfica|porto|sporting\s*cp|abc\s*braga|madeira\s*sad|ávila|avila)\b/.test(text)
    && !/\bcricket|ipl|t20|odi\b/.test(text)) {
    return 'handball';
  }
  if (/soccer|premier league|la liga|serie a|bundesliga|ligue|champions league|mls\b|world cup/.test(text)) return 'soccer';
  if (/nba|wnba|basket|euroleague/.test(text)) return 'basketball';
  if (/\bhockey\b|nhl|khl/.test(text)) return 'hockey';
  return 'cricket';
}
  ...
  try {
    ...
    if (mRes.rows.length > 0) {
      ...
      const sport = String(row.sport || '').trim()
        || inferPersistedSport(row.competition_id, row.team1_id, row.team2_id);
      const probe = {
        sport,
      ...
      // In-memory finality only; score-sport path needs sport on the probe (V4.4).
      ...
      let winnerSide = row.winner_side || null;
      if (isFinal && !winnerSide && (s1 > 0 || s2 > 0)) {
        if (s1 > s2) winnerSide = '1';
        else if (s2 > s1) winnerSide = '2';
        else if (/soccer|football|esoccer|handball|hockey/.test(sport)) winnerSide = 'X';
      }
    ...
    if (betRes.rows.length > 0) {
      ...
      const league = leg.league || snap?.league || 'Cricket League';
      const sport = leg.sport || snap?.sport || inferPersistedSport(league, team1, team2);
      ...
      const hasScoreEvidence = Boolean(latestOver && latestOver.score_at_end != null);
      const betOpen = /^(ACCEPTED|PENDING|OPEN)$/i.test(String(betRes.rows[0].status || ''));
      // Never force COMPLETED from a placement snapshot alone — that VOIDs open in-play bets.
      const treatFinal = hasScoreEvidence && !betOpen;
      const matchState = {
        id: matchId,
        matchId,
        sport,
        ...
        status: treatFinal ? 'HISTORICAL' : (betOpen ? 'LIVE' : 'SCHEDULED'),
        matchState: treatFinal ? 'post' : (betOpen ? 'in' : 'pre'),
        isLive: betOpen && !treatFinal,
        isCompleted: treatFinal,
        ...
        liveDetails: latestOver ? {
          ...
          commentary: 'Match hydrated from placed bet snapshot (not forced final).',
        },
        source: 'POSTGRESQL_PLACED_BET_SNAPSHOT',
      };

      // Persist only when we have a real final signal — avoid sticky false COMPLETED.
      if (treatFinal) {
