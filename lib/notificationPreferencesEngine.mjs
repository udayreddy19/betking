...
export async function getUserPreferences(userId) {
  ...
  if (res.rows.length === 0) {
    // Match DB column defaults (marketing_* DEFAULT true). Missing row ≠ opt-out.
    return {
      userId,
      marketingEmail: true,
      marketingSms: true,
...
export function isChannelAllowedForUser(preferences, category = 'BETTING', channel = 'IN_APP') {
  ...
  // Non-mandatory SUPPORT email respects transactional_email opt-out
  // (SECURITY / ACCOUNT / other MANDATORY_CATEGORIES already returned true above)
  if (chan === 'EMAIL' && cat === 'SUPPORT') {
