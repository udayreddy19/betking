...
const FALLBACK_MAILBOXES = [
  { id: 'no-reply', email: 'no-reply@oddsyra.com', label: 'No-reply', description: 'Transactional, invites, and campaigns (inbox)' },
  { id: 'promos', email: 'promos@oddsyra.com', label: 'Promotions (legacy)', description: 'Avoid — Gmail files these in Promotions. Prefer No-reply.' },
...
function CommunicationsPanels({ subModule = 'dispatch-logs' }) {
  ...
  useEffect(() => {
    if (subModule === 'broadcast' || subModule === 'compose' || subModule === 'social') return undefined;
    let cancelled = false;
    const load = async () => {
      try {
