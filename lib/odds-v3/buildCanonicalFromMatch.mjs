...
import { resolveLabeledTeamSide } from '../../src/utils/cricketMatchNormalizer.js';
...
function whichTeam(label, team1, team2, scoreHints = {}) {
  if (!label) return null;
  const side = resolveLabeledTeamSide(label, team1, team2, {
  ...
  // Keep exact short/prefix fallback for codes like ESS/NOTTS when score hints are empty
  ...
  const prefixHit = (short) => {
    const s = String(short || '').toLowerCase();
    if (!s || s.length < 3 || l.length < 3) return false;
    return l.startsWith(s) || s.startsWith(l);
  };
  const p1 = prefixHit(t1Short);
  const p2 = prefixHit(t2Short);
  if (p1 && !p2) return 1;
  if (p2 && !p1) return 2;
...
/** Cricbuzz often sums wickets across Test innings (17, 20). Map onto 0..max. */
export function normalizeInningsWickets(n, maxWickets = 10) {
  const w = Number(n);
  if (!Number.isFinite(w) || w <= 0) return 0;
  const max = Number(maxWickets) > 0 ? Number(maxWickets) : 10;
  if (w <= max) return Math.floor(w);
  const r = Math.floor(w) % max;
  return r === 0 ? max : r;
}

function isTestFourthInnings(ld = {}) {
  const inns = Array.isArray(ld.testInnings) ? ld.testInnings : [];
  if (inns.length >= 4) return true;
  if (Number(ld.inningsId) >= 4) return true;
  return /\b(fourth\s+innings|4th\s+innings)\b/i.test(String(ld.commentary || ''));
}

function testInningsTeamLabel(inn) {
  return inn?.batTeam || inn?.team || inn?.teamName || inn?.battingTeam || inn?.batting || null;
}
...
function teamId(team, fallback) {
  return String(team?.providerTeamId || team?.teamId || team?.id || team?.shortName || fallback);
}
...
export function buildCanonicalFromMatch(match, opts = {}) {
  ...
  let chaseRuns = Number(
    ld.chaseRuns != null
      ? ld.chaseRuns
      : (isSecond ? (ld.score2 ?? 0) : 0),
  );
  let chaseWickets = Number(
    ld.chaseWickets != null
      ? ld.chaseWickets
      : (isSecond ? (ld.wickets2 ?? 0) : 0),
  );
  const maxWktsForMirror = rules.maxWickets || 10;
  const looksMirroredFirst = (runs, wkts) => {
    const r = Number(runs);
    const w = Number(wkts);
    if (!(Number(firstRuns) > 0) || r !== Number(firstRuns)) return false;
    if (Number.isFinite(w) && w === Number(firstWickets)) return true;
    // Provider often stamps first-innings all-out (172/10) onto both sides at innings break.
    return Number(firstWickets) >= maxWktsForMirror && w >= maxWktsForMirror;
  };

  // score2 is often 0 / cumulative-wickets junk while liveDetails.runs is the chase.
  // Never promote ld.runs when it is still the first-innings total (mirrored 172/10 : 172/10).
  if (isSecond && chaseRuns <= 0 && Number(ld.runs) > 0) {
    const candidateRuns = Number(ld.runs);
    const candidateWkts = ld.wickets != null ? Number(ld.wickets) : chaseWickets;
    if (!looksMirroredFirst(candidateRuns, candidateWkts)) {
      chaseRuns = candidateRuns;
      if (ld.wickets != null && Number(ld.wickets) >= 0) {
        chaseWickets = Number(ld.wickets);
      }
    }
  }

  // If chase still mirrors first all-out, clear it — then try score1 when it looks like real chase progress
  // (some feeds park chase totals on score1/overs1 while score2 stays empty).
  if (isSecond && looksMirroredFirst(chaseRuns, chaseWickets)) {
    chaseRuns = 0;
    chaseWickets = 0;
  }
  if (
    ...
    && chaseRuns <= 0
    && Number(ld.score1) > 0
    && Number(ld.score1) !== Number(firstRuns)
  ) {
    const oversAligned = isNonZeroOvers(ld.chaseOvers)
      && ld.overs1 != null
      && String(ld.overs1).trim() === String(ld.chaseOvers).trim();
    const chaseOversOnly = isNonZeroOvers(ld.chaseOvers) || isNonZeroOvers(ld.overs2);
    if (oversAligned || chaseOversOnly || Number(ld.inningsId) >= 2) {
      chaseRuns = Number(ld.score1);
      if (ld.wickets1 != null && Number(ld.wickets1) >= 0 && Number(ld.wickets1) < maxWktsForMirror) {
        chaseWickets = Number(ld.wickets1);
      }
    }
  }
  ...
  const scoreHints = {
    team1Runs: Number(match.team1?.runs ?? ld.score1 ?? firstRuns ?? 0),
    team2Runs: Number(match.team2?.runs ?? ld.score2 ?? chaseRuns ?? 0),
  };
  const firstSide = whichTeam(ld.firstTeamName, match.team1 || t1Name, match.team2 || t2Name, scoreHints);
  const chaseSide = whichTeam(ld.chaseTeamName, match.team1 || t1Name, match.team2 || t2Name, scoreHints);
  ...
  // Do NOT lift with the other innings' all-out total (chase remapped onto team1).
  const maxWkts = rules.maxWickets || 10;
  ...
  if (!hasInningsLabels) {
    ...
    if (cardT1Wkts > team1Wickets && cardT1Wkts < maxWkts) team1Wickets = cardT1Wkts;
    if (cardT2Wkts > team2Wickets && cardT2Wkts < maxWkts) team2Wickets = cardT2Wkts;
  }
  team1Wickets = normalizeInningsWickets(team1Wickets, maxWkts);
  team2Wickets = normalizeInningsWickets(team2Wickets, maxWkts);

  const isTest = format === 'TEST';
  const testInns = Array.isArray(ld.testInnings) ? ld.testInnings : [];
  if (isTest && testInns.length >= 2) {
    let acc1 = 0;
    let acc2 = 0;
    const lastByTeam = { 1: null, 2: null };
    for (const inn of testInns) {
      const side = whichTeam(testInningsTeamLabel(inn), match.team1 || t1Name, match.team2 || t2Name);
      const r = Number(inn.runs) || 0;
      if (side === 1) {
        acc1 += r;
        lastByTeam[1] = inn;
      } else if (side === 2) {
    ...
    if (lastByTeam[1]?.wickets != null) {
      team1Wickets = normalizeInningsWickets(lastByTeam[1].wickets ?? lastByTeam[1].wkts, maxWkts);
    }
    if (lastByTeam[2]?.wickets != null) {
      team2Wickets = normalizeInningsWickets(lastByTeam[2].wickets ?? lastByTeam[2].wkts, maxWkts);
    }
    if (acc1 > 0 || acc2 > 0) {
      team1Runs = acc1;
      team2Runs = acc2;
    }
    const lastTestInn = testInns[testInns.length - 1];
    const lastSide = whichTeam(testInningsTeamLabel(lastTestInn), match.team1 || t1Name, match.team2 || t2Name);
    const lastWkts = lastTestInn.wickets ?? lastTestInn.wkts;
    const lastOvers = lastTestInn.overs;
    if (lastSide === 1) {
      if (lastWkts != null) team1Wickets = normalizeInningsWickets(lastWkts, maxWkts);
      if (lastOvers != null) team1Balls = parseOversToBalls(lastOvers, match, ballsPerOver);
      battingTeamId = t1Id;
      bowlingTeamId = t2Id;
    } else if (lastSide === 2) {
      if (lastWkts != null) team2Wickets = normalizeInningsWickets(lastWkts, maxWkts);
      if (lastOvers != null) team2Balls = parseOversToBalls(lastOvers, match, ballsPerOver);
      battingTeamId = t2Id;
      bowlingTeamId = t1Id;
    }
  }
  ...
  // Test innings have no fixed ball budget — do not invent remaining balls from a 2700 ceiling.
  ...
  // Limited-overs chase, or Test 4th innings vs the opposition's match total.
  const useTestChaseTarget = isTest && isTestFourthInnings(ld) && firstInningsRuns > 0;
  const target = (useTestChaseTarget || (!isTest && currentInnings === 2 && firstInningsRuns > 0))
    ? firstInningsRuns + 1
    : null;
  ...
  const isCompleted = isCricketMatchCompleted(match)
    ...
    // Trust matchState=post only when not a multi-day board paused at stumps/day-end.
    || (match.matchState === 'post' && !/\bstumps\b/i.test([
      match.time,
      match.liveStatus,
      match.status,
  ...
  const tossRaw = match.toss || ld.toss || null;
  const toss = tossRaw && typeof tossRaw === 'object'
    ? {
      wonToss: tossRaw.wonToss || tossRaw.winner || tossRaw.tossWinnerName || tossRaw.winnerName || ld.tossWinner || null,
      decision: tossRaw.decision || tossRaw.choice || ld.tossDecision || null,
      batFirst: tossRaw.batFirst || null,
      fieldFirst: tossRaw.fieldFirst || null,
    }
    : (ld.tossWinner
      ? { wonToss: ld.tossWinner, decision: ld.tossDecision || null }
      : null);

  const pickBoundary = (teamObj, slot, key) => {
    const ldSlot = ld[slot] || {};
    const v = teamObj?.[key] ?? teamObj?.[`total${key[0].toUpperCase()}${key.slice(1)}`] ?? ldSlot[key];
  ...
  return createCanonicalMatchState({
    ...
    team1: {
      ...
      fours: pickBoundary(match.team1, 'team1', 'fours'),
      sixes: pickBoundary(match.team1, 'team1', 'sixes'),
    },
    team2: {
      id: t2Id,
      name: String(t2Name),
      ...
      fours: pickBoundary(match.team2, 'team2', 'fours'),
      sixes: pickBoundary(match.team2, 'team2', 'sixes'),
    },
    currentInnings,
    battingTeamId,
    bowlingTeamId,
    ...
    liveDetails: {
      ...
      toss: toss || ld.toss || undefined,
      tossWinner: toss?.wonToss || ld.tossWinner || undefined,
    },
    toss,
    odds: providerOdds,
    hasBallFeed,
    providerTimestamp: match.providerTimestamp
      ...
      || (opts.frozenClock != null ? opts.frozenClock : (opts.deterministicFallback ? 0 : Date.now())),
    providerTimestampFallback: !(match.providerTimestamp || match.timestamp || match.lastUpdated || ld.timestamp || ld.lastUpdated || match.eventTimestamp),
    providerSequence: match.providerSequence ?? match.sequence ?? ld.sequence ?? null,
    eventTimestamp: match.eventTimestamp ?? ld.eventTimestamp ?? null,
