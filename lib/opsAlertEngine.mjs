  ...
  try {
    ...
    for (const rule of rules.rules || []) {
      ...
      if (key === 'RECONCILIATION_DISCREPANCY') {
        ...
        count = Number(r.rows[0]?.c || 0);
      } else if (key === 'DEPOSIT_FAILURE_SPIKE') {
        const win = Number(rule.window_seconds || 300);
        const r = await query(
          `SELECT COUNT(*)::int AS c FROM deposits
           WHERE UPPER(COALESCE(status,'')) IN ('FAILED','CANCELLED','EXPIRED')
             AND COALESCE(updated_at, created_at) > NOW() - ($1::int * INTERVAL '1 second')`,
          [win],
        ).catch(() => ({ rows: [{ c: 0 }] }));
        count = Number(r.rows[0]?.c || 0);
      } else if (key === 'REFERRAL_PLAY_SPIKE') {
        const win = Number(rule.window_seconds || 900);
        const r = await query(
          `SELECT COUNT(*)::int AS c FROM referral_reward_events
           WHERE reward_type IN ('REFERRER_PLAY_FREEBET','REFERRER_PLAY_BONUS')
             AND status = 'GRANTED'
             AND created_at > NOW() - ($1::int * INTERVAL '1 second')`,
          [win],
        ).catch(() => ({ rows: [{ c: 0 }] }));
        count = Number(r.rows[0]?.c || 0);
      } else if (key === 'WALLET_LEDGER_MISMATCH') {
        const r = await query(
          `SELECT COUNT(*)::int AS c FROM financial_discrepancies
           WHERE UPPER(COALESCE(status,'')) IN ('OPEN','UNRESOLVED','PENDING')`,
        ).catch(() => ({ rows: [{ c: 0 }] }));
        count = Number(r.rows[0]?.c || 0);
      } else if (key === 'SRL_TOSS_AUTO_LOCK') {
        // Raised directly from auto-lock path; threshold evaluator is a no-op heartbeat.
        continue;
