 ...
 *   betting:settlement-engine→ open/pending/accepted bets (Settlement desk / pendingOnly=1)
...
export async function buildAdminNavAttention() {
  const [
    ...
    // Bets → Settlement: same filter as listBets({ pendingOnly: true })
    count(
    ...
    count(
      `SELECT COUNT(*)::int AS c FROM outbox_events
       WHERE UPPER(COALESCE(status, '')) IN ('FAILED', 'DEAD_LETTER')
         AND (event_type ~* 'mail|email|comm|notify|webhook|alert|digest|sms|whatsapp')
         AND COALESCE(updated_at, created_at) > NOW() - INTERVAL '7 days'`,
    ),
    count(
      `SELECT COUNT(*)::int AS c FROM notifications
       WHERE UPPER(COALESCE(status, '')) IN ('FAILED', 'DEAD_LETTER', 'ERROR')
         AND COALESCE(created_at, NOW()) > NOW() - INTERVAL '7 days'`,
    ),
  ]);

  const failedDeliveries = failedWebhookDeliveries + failedOutbox + failedNotifications;
