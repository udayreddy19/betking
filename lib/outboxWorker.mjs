...
subscribeToEvent('BET_SETTLED', async (payload) => {
  ...
  try {
    const { dispatchNotificationEvent } = await import('./notificationEngine.mjs');
    const status = payload.outcome || payload.status || 'SETTLED';
    await dispatchNotificationEvent({
      eventId: `settle_${payload.betId}_v${payload.settlementVersion || 1}`,
      eventType: 'BET_SETTLED',
      userId: payload.userId,
      category: 'SETTLEMENT',
      channel: 'IN_APP',
      data: {
        subject: `Bet ${String(status).toLowerCase()}`,
        body: `Your bet ${payload.betId} settled as ${status}. Payout ₹${Number(payload.payout || 0).toFixed(2)}.`,
        betId: payload.betId,
        matchId: payload.matchId,
      },
    });
    const { recordSettlementObservation } = await import('./oddsCalibrationObservations.mjs');
    ...
    console.error('[Outbox BET_SETTLED notify]', err.message);
  }
});
...
subscribeToEvent('BET_CASHED_OUT', async (payload) => {
  ...
  try {
    const { dispatchNotificationEvent } = await import('./notificationEngine.mjs');
    await dispatchNotificationEvent({
      eventId: `cashout_${payload.betId}`,
      ...
      data: {
        ...
        body: `Cashed out ₹${Number(payload.cashoutAmount || 0).toFixed(2)} on bet ${payload.betId}.`,
        betId: payload.betId,
      },
    });
  } catch (err) {
    console.error('[Outbox BET_CASHED_OUT notify]', err.message);
  }
});
 ...
 */
export async function processPendingOutboxEvents(batchSize = 20, filter = null) {
  try {
    const events = await withTransaction(async (client) => {
      ...
      if (filter && typeof filter === 'object') {
        ...
        if (filter.aggregateId) {
          queryParams.push(filter.aggregateId);
          queryText += ` AND aggregate_id = $${queryParams.length}`;
        }
        if (filter.userId) {
          queryParams.push(`%${filter.userId}%`);
          queryText += ` AND payload::text LIKE $${queryParams.length}`;
        }
      }
      queryText += `
      ...
      `;
      const claimRes = await client.query(queryText, queryParams);
