...
export function getFormatRules(format) {
  ...
  if (norm.includes('ODI') || norm.includes('ONE-DAY') || norm.includes('ONE DAY') || norm.includes('LIST A') || norm.includes('50 OVER') || norm.includes('CWC LEAGUE') || norm.includes('50-OVER') || norm.includes('D50') || /\bD[\s-]?50\b/.test(norm)) {
  ...
  if (
    norm.includes('T20')
    || norm.includes('TWENTY20')
    || norm.includes('20-OVER')
    || norm.includes('SRL')
    || norm.includes('IPL')
    || norm.includes('BBL')
    || norm.includes('PSL')
...
/** Pricing fallback: unknown formats use T20, never The Hundred. */
export function formatRulesOrDefault(format) {
  return getFormatRules(format) || FORMAT_RULES.T20;
}
