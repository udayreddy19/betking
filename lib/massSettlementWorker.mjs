      ...
      for (const b of batch) {
        try {
          const authResult = authorizeSettlement({
            match: matchState,
            bet: b,
            marketContext: { marketId: b.market_id },
            providerObservations: matchState.providerObservations || [],
            authorizedBy: 'MassSettlementWorker',
          });

          if (!authResult.success) {
            console.warn(`[MassSettlement Blocked] Bet ${b.bet_id}: ${authResult.error}`);
            continue;
          }
