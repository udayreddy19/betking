...
export function isMarketEligible(marketKey, state) {
  ...
  // Toss is pre-match; keep eligible while SCHEDULED, and while LIVE so SETTLED can publish.
  if (keyUpper.includes('TOSS')) {
    return true;
  }
  ...
  // Past / far-future dismissal markets must not stay bettable — only next or next+1.
  // Prefer chase-slot wickets in innings 2 (card/first-innings wickets must not invent "10th" while 4 down).
  const liveWkts = (() => {
    const batting = state.battingTeamId === state.team1?.id ? state.team1 : state.team2;
    let wkts = Number(batting?.wickets) || 0;
    if ((Number(state.currentInnings) || 1) >= 2) {
      const chaseW = Number(state.liveDetails?.chaseWickets ?? state.liveDetails?.wickets2);
      if (Number.isFinite(chaseW) && chaseW >= 0 && chaseW < 10) wkts = chaseW;
    }
    return wkts;
  })();
  const dismissalHit = keyUpper.match(/TEAM_SCORE_AT_(\d+)_DISMISSAL/);
  if (dismissalHit) {
    const n = Number(dismissalHit[1]);
    if (liveWkts >= n) return false;
    if (n > liveWkts + 2) return false; // next or next+1 only
  }
  const methodHit = keyUpper.match(/METHOD_OF_NEXT_WICKET_(\d+)/);
  if (methodHit) {
    const n = Number(methodHit[1]);
    if (liveWkts >= n) return false;
    if (n > liveWkts + 1) return false; // method: true next only
  }
