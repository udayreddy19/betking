...
export class KycEngine {
  ...
  async submitKycVerification({ userId, documentType = 'PAN', documentNumber = '', documentUrls = [], dateOfBirth = null }) {
    ...
    const { encryptKycField, decryptKycField, generateBlindIndex } = await import('./kycEncryption.mjs');
    const { addColumnIfMissing } = await import('./schemaGuard.mjs');
    await addColumnIfMissing('kyc_cases', 'pan_hash', `ALTER TABLE kyc_cases ADD COLUMN IF NOT EXISTS pan_hash VARCHAR(64)`);
    await addColumnIfMissing('kyc_cases', 'aadhaar_hash', `ALTER TABLE kyc_cases ADD COLUMN IF NOT EXISTS aadhaar_hash VARCHAR(64)`);
    await query(`ALTER TABLE kyc_cases ALTER COLUMN pan_number TYPE TEXT, ALTER COLUMN aadhaar_number TYPE TEXT`).catch(() => {});

    const existingRes = await query(
      `SELECT case_id, pan_number, aadhaar_number, status, pan_hash, aadhaar_hash
       FROM kyc_cases
       WHERE user_id = $1
    ...
    const existingPanDecrypted = existing?.pan_number ? normalizePan(decryptKycField(existing.pan_number)) : '';
    const existingAadhaarDecrypted = existing?.aadhaar_number ? normalizeAadhaar(decryptKycField(existing.aadhaar_number)) : '';

    if (isPan) {
      const ownPan = existingPanDecrypted;
    ...
    if (isAadhaar) {
      const ownAadhaar = existingAadhaarDecrypted;
    ...
    const keepVerified = existing?.status === 'VERIFIED'
      && ((isPan && existingPanDecrypted === panNum)
        || (isAadhaar && existingAadhaarDecrypted === aadhaarNum));
    ...
    const encPan = panNum ? encryptKycField(panNum) : null;
    const encAadhaar = aadhaarNum ? encryptKycField(aadhaarNum) : null;
    const panHash = panNum ? generateBlindIndex(panNum) : null;
    const aadhaarHash = aadhaarNum ? generateBlindIndex(aadhaarNum) : null;
        ...
        await query(
          `UPDATE kyc_cases
           SET status = $2,
               ...
               aadhaar_hash = COALESCE($6, aadhaar_hash),
               updated_at = NOW()
           WHERE case_id = $1`,
          [caseId, nextStatus, encPan, encAadhaar, panHash, aadhaarHash],
        );
      } else {
        await query(
          `INSERT INTO kyc_cases (case_id, user_id, status, pan_number, aadhaar_number, pan_hash, aadhaar_hash, document_urls, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())`,
          [caseId, userId, nextStatus, encPan, encAadhaar, panHash, aadhaarHash, documentUrls],
        );
      }
    } catch (err) {
      if (err.code === '23505') {
    ...
    if (nextStatus === 'VERIFIED') {
      try {
        ...
        console.error('[KYC] bindPromoIdentityOnKycVerify failed', { userId, error: err?.message || err });
        try {
          const { publishOutboxEvent } = await import('./outboxEngine.mjs');
          await publishOutboxEvent({
            eventType: 'PROMO_IDENTITY_BIND_RETRY',
            aggregateType: 'user',
            aggregateId: userId,
            payload: { userId, reason: 'kyc_verified' },
          });
        } catch (outboxErr) {
          console.error('[KYC] outbox enqueue for promo identity failed', outboxErr?.message || outboxErr);
        }
      }
      try {
        ...
        console.error('[KYC] tryQualifyReferralAfterVerification failed', { userId, error: err?.message || err });
      }
    }
  ...
  async verifyKycCase({ caseId, decision, reviewerId = 'admin', notes = '' }) {
    ...
    if (decision === 'VERIFIED') {
      try {
        ...
        try {
          ...
          await publishOutboxEvent({
            ...
            aggregateType: 'user',
            aggregateId: userId,
            payload: { userId, reason: 'kyc_admin_verified' },
          });
        } catch (outboxErr) {
          console.error('[KYC] outbox enqueue for promo identity failed', outboxErr?.message || outboxErr);
        }
      ...
      try {
        const uRes = await query(`SELECT email FROM users WHERE user_id = $1`, [userId]);
        const email = uRes.rows[0]?.email;
        if (email) {
          const { sendKycApprovedEmail } = await import('../server/auth/emailService.js');
          await sendKycApprovedEmail({ email, name: userId });
        }
      } catch (err) {
        console.error('[KYC] sendKycApprovedEmail failed', err?.message || err);
      }
      ...
      try {
        ...
        const email = uRes.rows[0]?.email;
        if (email) {
          const { sendKycRejectedEmail } = await import('../server/auth/emailService.js');
          await sendKycRejectedEmail({ email, name: userId, reason: notes });
        }
      } catch (ignored) {}
    }
  ...
  async getUserKycStatus(userId) {
    ...
    const { decryptKycField } = await import('./kycEncryption.mjs');
    const row = caseRes.rows[0];
    const panDecrypted = row.pan_number ? decryptKycField(row.pan_number) : null;
    const aadhaarDecrypted = row.aadhaar_number ? decryptKycField(row.aadhaar_number) : null;
    return {
      caseId: row.case_id,
      userId: row.user_id,
      status: row.status,
      panMasked: panDecrypted ? maskPan(panDecrypted) : null,
      aadhaarMasked: aadhaarDecrypted ? maskAadhaar(aadhaarDecrypted) : null,
