  ...
  for (const evt of eventsRes.rows) {
    ...
    try {
      ...
      if (userId && userId !== 'system') {
        // Ensure user row exists in users table to satisfy notifications_user_id_fkey
        await query(
          `INSERT INTO users (user_id, email, password_hash) VALUES ($1, $2, 'hash') ON CONFLICT (user_id) DO NOTHING`,
          [userId, payload.email || `${userId}@oddsyra.com`]
        );
        ...
        for (const channel of channels) {
          ...
          // Defer promotional emails during quiet hours
          ...
          if (existingNotif.rows.length === 0) {
            await query(
              ...
              [notifId, userId, eventType, category, channel, payload.email || `${userId}@oddsyra.com`, rendered.subject, rendered.body, eventId]
            );
          }
        }
      }
