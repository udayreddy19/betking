...
export async function processQueuedNotifications(opts = {}) {
  ...
  for (const notif of queuedRes.rows) {
    ...
    try {
      ...
      const isSuccess = await simulateProviderDispatch(notif.channel, notif.recipient, notif.body);
...
async function simulateProviderDispatch(channel, recipient, body) {
  // Provider adapters for IN_APP, ADMIN, EMAIL, SMS, WEB_PUSH
