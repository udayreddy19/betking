...
function findMatchInList(matches, matchId) {
  ...
  return matches.find((m) => m.id === matchId || m.matchId === matchId) || null;
}
...
export async function buildMatchOddsPayload({
  ...
  const run = (async () => {
    ...
    if (!matchObj) {
      const err = new Error(`Match '${matchId}' not found or not available`);
      err.statusCode = 404;
    ...
    const sport = String(matchObj.sport || 'cricket').toLowerCase();
