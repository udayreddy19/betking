...
/** BONUS_CLAIM.method → human label (covers freebet / XP so history does not say "Bonus"). */
const BONUS_CLAIM_METHOD_LABELS = Object.freeze({
  LOYALTY_REDEEM: 'Loyalty Redemption',
  SIGNUP_FREEBET: 'Signup Free Bet',
  SIGNUP_BONUS: 'Signup Bonus',
  SIGNUP_CASH: 'Signup Cash',
  SIGNUP_PROMO: 'Signup Promo',
  DAILY_SPIN_FREEBET: 'Daily Spin Free Bet',
  DAILY_SPIN_BONUS: 'Daily Spin Bonus',
  DAILY_SPIN_XP: 'Daily Spin XP',
  DAILY_SPIN: 'Daily Spin',
  DEPOSIT_FREEBET: 'Deposit Free Bet',
  REFERRAL: 'Referral Reward',
  ADMIN_REWARD: 'Admin Reward',
});

const FREEBET_CLAIM_METHODS = new Set([
  'SIGNUP_FREEBET',
  'DAILY_SPIN_FREEBET',
  'DEPOSIT_FREEBET',
]);
...
export function mapDbTypeToUiType(dbType, method = '') {
  ...
  switch (t) {
    ...
    case 'BONUS':
    case 'PROMO_BONUS':
      if (FREEBET_CLAIM_METHODS.has(m) || m.includes('FREEBET')) return 'freebet';
      if (m === 'LOYALTY_REDEEM') return 'loyalty_redeem';
      if (m === 'DAILY_SPIN_XP' || m.endsWith('_XP')) return 'xp';
      return 'bonus';
    case 'BONUS_FORFEIT':
...
function formatLabel(row, uiType) {
  ...
  if (type === 'BONUS_CLAIM' && BONUS_CLAIM_METHOD_LABELS[methodKey]) {
    return `${BONUS_CLAIM_METHOD_LABELS[methodKey]}${utr}`.trim();
  }
  ...
  const showMethod = Boolean(method) && (
    ...
    || (SOURCE_TAG_TYPES.has(type) && methodKey !== 'UPI')
  );
...
export function mapTransactionRow(row) {
  const uiType = mapDbTypeToUiType(row.type, row.method);
