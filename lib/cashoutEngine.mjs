...
export async function quoteBetCashout({ betId, userId }) {
  ...
  const legsRes = await query(
    `SELECT match_id, market_id, selection_id, odds FROM bet_selections WHERE bet_id = $1`,
    [betId],
  );
...
export async function executeBetCashout({
  ...
  try {
    ...
    const calculatedCashout = quote.cashoutValue;
    if (requestedCashoutValue != null && Math.abs(parseFloat(requestedCashoutValue) - calculatedCashout) > 0.05 * Math.max(calculatedCashout, 1)) {
      throw new Error('STALE_PRICE: Cashout value changed. Please accept updated value.');
    }
    const finalCashoutAmount = Number(calculatedCashout.toFixed(2));
