...
const COMPLETED_TIME_HINTS = ['completed', 'full time', 'match over', 'result', 'won by', 'finished', 'concluded', 'beat', 'defeated'];
...
export function getMatchState(match) {
  ...
  if (/innings\s*break/i.test(combined) || /innings\s*break/i.test(String(match?.matchState || ''))) {
    return 'in';
  }

  // Day-end / session stumps is a pause in multi-day cricket, not a finished match.
  // Providers often label "2nd Day - Stumps" — that must stay in-play for settlement.
  if (/\bstumps\b/.test(combined) && !/\b(won by|have won|has won|match over|match completed)\b/.test(combined)) {
    return 'in';
  }

  if (isCricketMatchCompleted(match)) return 'post';

  if (time === 'ft' || combined.includes('full time')) return 'post';
...
export function isInPlayMatch(match) {
