...
export async function buildTradingExposures({ limit = 40 } = {}) {
  ...
  try {
    ...
    openByMatch = new Map(openRes.rows.map((r) => [r.match_id, r]));
  } catch {
    openByMatch = new Map();
  }
  ...
  const exposures = live.map((m) => {
    ...
    const open = openByMatch.get(matchId);
    const openStake = open ? Number(open.open_stake) : null;
    const openPayout = open ? Number(open.open_payout) : null;
    return {
      matchId,
      ...
      exposure: openStake,
      liability: openPayout != null && openStake != null
        ? Number(Math.max(0, openPayout - openStake).toFixed(2))
        : null,
  ...
  return {
    ...
    note: 'Open stake/liability from accepted bets when present; odds and risk band are live.',
