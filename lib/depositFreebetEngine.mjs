...
async function ensureTargetedSchema() {
  await query(`ALTER TABLE promotions ADD COLUMN IF NOT EXISTS is_targeted BOOLEAN NOT NULL DEFAULT FALSE`).catch(() => null);
  await query(`ALTER TABLE promotions ADD COLUMN IF NOT EXISTS description TEXT`).catch(() => null);
  await query(`ALTER TABLE promotions ADD COLUMN IF NOT EXISTS email_subject VARCHAR(255)`).catch(() => null);
  await query(`ALTER TABLE promotions ADD COLUMN IF NOT EXISTS email_body TEXT`).catch(() => null);
  await query(`ALTER TABLE promotions ADD COLUMN IF NOT EXISTS audience_segment_id VARCHAR(64)`).catch(() => null);
  await query(`ALTER TABLE promotions ADD COLUMN IF NOT EXISTS audience_vip_tiers JSONB NOT NULL DEFAULT '[]'::jsonb`).catch(() => null);
...
export async function createTargetedDepositFreebetCampaign(raw = {}, { adminId } = {}) {
  ...
  const res = await query(
    `INSERT INTO promotions (
       ...
       audience_exclude_segment_ids, updated_at
     ) VALUES (
...
export async function dispatchTargetedDepositFreebetEmails({
  ...
  for (const row of users.rows) {
    ...
    // Respect promotional email opt-out (transactional mail unaffected)
    try {
      const { canSendPromotionalEmail } = await import('./notificationPreferencesEngine.mjs');
      const allowed = await canSendPromotionalEmail(row.user_id);
      if (!allowed) {
        await query(
          `UPDATE deposit_freebet_campaign_users
    ...
    } catch { /* fail-open only on preference lookup errors is intentional for deliverability of admin-forced? No — prefer skip on false; on error allow */ }
