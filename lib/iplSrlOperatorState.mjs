...
export async function hydrateSrlOperatorSessions({ force = false } = {}) {
...
/** Force-reload one match session from Postgres (worker must see API desk seeks). */
export async function refreshSrlOperatorSession(matchId) {
  ...
  try {
    const res = await query(`SELECT * FROM srl_operator_sessions WHERE match_id = $1 LIMIT 1`, [id]);
...
// Auto-hydrate sessions on module initialization so operator state survives server/worker reboots
if (process.env.NODE_ENV !== 'test') {
  void hydrateSrlOperatorSessions();
}
...
export function setSrlTossAndLineup(matchId, {
  ...
  s.toss = {
    ...
    autoPicked: autoPicked == null ? !!s.toss?.autoPicked : !!autoPicked,
    autoPickHousePnl: autoPickHousePnl != null
      ? Number(autoPickHousePnl)
      : (s.toss?.autoPickHousePnl ?? null),
    autoPickReason: autoPickReason != null
      ? String(autoPickReason)
      : (s.toss?.autoPickReason ?? null),
  };
