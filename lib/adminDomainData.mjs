...
const OPEN_BET_STATUSES = ['PENDING', 'OPEN', 'ACCEPTED'];
const COMPLETED_BET_STATUSES = ['WON', 'LOST', 'VOID', 'REFUNDED', 'SETTLED', 'CASHED_OUT', 'SETTLED_WON', 'SETTLED_LOST', 'SETTLED_VOID'];

const SETTLEMENT_COUNTS_TTL_MS = 15_000;
const settlementCountsCache = { lite: null, full: null };

/** Markets that typically settle while the fixture is still live. */
const LIVE_SETTLE_MARKET_RE = /(?:^|_)(?:next_over_|overs_0_|wicket_in_over_|ball_|player_\d+|innings_runs|runs_in_over)/i;

function isLiveFeedMatch(match) {
...
export function collectLiveMatchIdAliases(matches = []) {
  const ids = new Set();
  for (const match of matches) {
    if (!isLiveFeedMatch(match)) continue;
    const rawIds = [
      match.id,
      match.matchId,
      match.legacyId,
    ...
    for (const id of rawIds) {
      for (const alias of matchIdAliases(id)) ids.add(alias);
    }
  }
  return [...ids];
}

export function isLiveSettleMarketId(marketId = '') {
  return LIVE_SETTLE_MARKET_RE.test(String(marketId || ''));
}
...
async function liveSnapshot() {
  ...
  if (process.env.NODE_ENV === 'test' || Boolean(process.env.VITEST)) {
    return { matches: [], sources: {}, fetchedAt: Date.now() };
  }
...
export async function listBets({
  ...
  const params = [];
  let idx = 1;
  const settlement = String(settlementFilter || '').trim().toLowerCase();

  if (settlement === 'pending' || pendingOnly) {
    clauses.push(`b.status = ANY($${idx}::text[])`);
    params.push(OPEN_BET_STATUSES);
    idx += 1;
  } else if (settlement === 'completed') {
    clauses.push(`(
      b.status = ANY($${idx}::text[])
      OR b.settled_at IS NOT NULL
    )`);
    params.push(COMPLETED_BET_STATUSES);
    idx += 1;
  } else if (settlement === 'live') {
    clauses.push(`b.status = ANY($${idx}::text[])`);
    params.push(OPEN_BET_STATUSES);
    idx += 1;

    const snap = await liveSnapshot();
    const liveIds = collectLiveMatchIdAliases(snap?.matches || []);
    if (liveIds.length) {
      clauses.push(`(
        b.match_id = ANY($${idx}::text[])
        OR EXISTS (
          SELECT 1 FROM bet_selections bs
          WHERE bs.bet_id = b.bet_id
            AND bs.match_id = ANY($${idx}::text[])
        )
        OR COALESCE(b.market_id, '') ~* $${idx + 1}
      )`);
      params.push(liveIds);
      params.push(LIVE_SETTLE_MARKET_RE.source);
      idx += 2;
    } else {
      // No live board hits — still surface open in-play market bets
      clauses.push(`COALESCE(b.market_id, '') ~* $${idx}`);
      params.push(LIVE_SETTLE_MARKET_RE.source);
      idx += 1;
    }
  } else if (status) {
  ...
  const res = await q(
    `SELECT b.bet_id AS id,
            ...
            COALESCE(legs_agg.legs, '[]'::json) AS legs
     FROM bets b
  ...
  return {
    ...
    source: res.error ? 'error' : 'database',
    note: schemaNote(res.error),
    settlementFilter: settlement || null,
  };
}

export async function countBetsBySettlement({ includeLive = false } = {}) {
  const now = Date.now();
  const cacheKey = includeLive ? 'full' : 'lite';
  const hit = settlementCountsCache[cacheKey];
  if (hit && (now - hit.at) < SETTLEMENT_COUNTS_TTL_MS) {
    return hit.value;
  }

  // Pending/completed are what the Settlements UI needs. Avoid live aggregator
  // hydration and full-table scans on every desk open.
  const [pendingRes, completedRes] = await Promise.all([
    q(
      `SELECT COUNT(*)::int AS c FROM bets WHERE status = ANY($1::text[])`,
      [OPEN_BET_STATUSES],
    ),
    q(
      `SELECT COUNT(*)::int AS c FROM bets
       WHERE status = ANY($1::text[]) OR settled_at IS NOT NULL`,
  ...
  if (includeLive) {
    ...
    const liveIds = collectLiveMatchIdAliases(snap?.matches || []);
    const marketRe = LIVE_SETTLE_MARKET_RE.source;
        ...
        ? q(
          `SELECT COUNT(*)::int AS c FROM bets b
           WHERE b.status = ANY($1::text[])
             AND (
               b.match_id = ANY($2::text[])
               OR EXISTS (
                 SELECT 1 FROM bet_selections bs
                 WHERE bs.bet_id = b.bet_id AND bs.match_id = ANY($2::text[])
               )
               OR COALESCE(b.market_id, '') ~* $3
             )`,
          [OPEN_BET_STATUSES, liveIds, marketRe],
        )
        : q(
          `SELECT COUNT(*)::int AS c FROM bets b
           WHERE b.status = ANY($1::text[])
             AND COALESCE(b.market_id, '') ~* $2`,
          [OPEN_BET_STATUSES, marketRe],
        ),
      q(`SELECT COUNT(*)::int AS c FROM bets`),
    ]);
    live = Number(liveRes.rows?.[0]?.c || 0);
    all = Number(allRes.rows?.[0]?.c || 0);
    liveError = liveRes.error;
    allError = allRes.error;
  }
  ...
  const value = {
    ...
    live,
    source: (pendingRes.error || completedRes.error || liveError || allError) ? 'error' : 'database',
    note: schemaNote(pendingRes.error || completedRes.error || liveError || allError),
  };
  settlementCountsCache[cacheKey] = { at: now, value };
  return value;
}
...
export async function enrichControlTowerFinancials(base = {}) {
  // Run KPI queries in parallel. Skip all-time SUM(stake) / full settled GGR scans —
  // those full-table walks were dominating every AdminShell 30s poll.
  const [
    users,
    openBets,
    pendingWd,
    ...
    q(`SELECT COUNT(*)::int AS c FROM bets WHERE status = ANY($1::text[])`, [OPEN_BET_STATUSES]),
    q(
    ...
    q(
      `SELECT COUNT(*)::int AS c FROM support_conversations
       WHERE COALESCE(status,'OPEN') NOT IN ('RESOLVED','CLOSED','RESOLVED_CLOSED')`,
    ),
    q(
      `SELECT COUNT(*)::int AS c
       FROM user_profiles p
       WHERE COALESCE(p.kyc_status, 'NOT_STARTED') NOT IN ('VERIFIED', 'APPROVED')`,
    ),
    q(
      `SELECT COUNT(*)::int AS c FROM markets WHERE COALESCE(status,'') = 'SUSPENDED'`,
    ),
    q(
    ...
    q(
      `SELECT COALESCE(SUM(
          CASE
            WHEN status IN ('LOST') THEN stake
            WHEN status IN ('WON') THEN stake - COALESCE(potential_payout, 0)
            ELSE 0
          END
        ), 0)::float AS c
       FROM bets
       WHERE status IN ('WON','LOST')
         AND COALESCE(settled_at, created_at) >= date_trunc('day', NOW())`,
    ),
  ]);

  const registeredUsers = users.rows?.[0]?.c ?? null;
  const todayTurnover = turnoverToday.rows?.[0]?.c ?? null;
  ...
  return {
    ...
    todayTurnover,
    stakeTurnoverAllTime: null,
    ggr: ggrToday.error ? (base.ggr ?? null) : (ggrToday.rows?.[0]?.c ?? base.ggr ?? null),
    ggrNote: ggrToday.error ? undefined : 'Today settled stake − payouts (approx)',
    turnoverScope: 'today',
