...
export function normalizeMatch(rawPayload = {}, sourceProviderId = 'UNKNOWN') {
  ...
  const rawFormat = String(
    `${rawPayload.league || ''} ${rawPayload.seriesName || ''} ${rawPayload.format || ''} ${rawPayload.matchType || ''} ${rawPayload.matchFormat || ''}`
  ).toLowerCase();
  const format = rawFormat.includes('hundred') ? 'THE_HUNDRED'
    : /t10|ten10|t-10|european cricket series|\becs\b|abu dhabi t10|max60|german super league|quantum cricket|\bqcl\b|10[\s-]?overs?/.test(rawFormat) ? 'T10'
