...
function getPairKey(m) {
  const t1 = (m.team1?.name || m.homeTeam?.teamName || m.homeTeam?.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const t2 = (m.team2?.name || m.awayTeam?.teamName || m.awayTeam?.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
