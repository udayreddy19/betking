...
import { isLiveMatch, readLiveScoreState, isLiveClockKnown } from '../state/readMatchState.mjs';
...
export function isYesishName(name = '') {
  const n = String(name).toLowerCase();
  return n === 'yes' || n.startsWith('yes ') || /\bor draw\b/.test(n);
}
...
export function liveMarginBump(match, margins = OSV4_MARGIN_CONFIG) {
  ...
  if (sport === 'soccer' || sport === 'esoccer') {
    ...
    progress = Math.min(1, Math.max(0, inning / 9));
  } else {
    progress = 0.35;
  }
 ...
 * Unknown clocks fail closed (suspend extras) so ESPN/FanCode gaps cannot leave soft books open.
 */
export function applyOsV4LateLock(markets = [], match, margins = OSV4_MARGIN_CONFIG) {
  if (!isLiveMatch(match)) return markets;
  const sport = normalizeSportKey(match?.sport);
  const { minute, liveDetails } = readLiveScoreState(match);
  const clockKnown = isLiveClockKnown(liveDetails, { sport });
  ...
  if (sport === 'soccer' || sport === 'esoccer') {
    ...
    threshold = Number(margins.iceHockeyLateLockMinute) || 50;
  } else if (sport === 'baseball') {
    const inning = Number(match?.liveDetails?.inning) || 1;
    const lockInning = Number(margins.baseballLateLockInning) || 7;
    if (inning >= lockInning) threshold = 0; // force lock
    else return markets; // not late enough
  } else if (sport === 'tennis') {
    // No clock — skip time lock (set markets already guarded separately).
    return markets;
  }

  const shouldLock = !clockKnown || (threshold != null && minute >= threshold);
  if (!shouldLock) return markets;
  ...
  return (markets || []).map((m) => {
    ...
    return {
      ...
      status: 'SUSPENDED',
      selections: [],
      suspensionReason: clockKnown ? 'v48_late_lock' : 'v486_clock_unknown_lock',
    };
  });
}
