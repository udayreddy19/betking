...
function mapSrlDeskMatch(match) {
  ...
  const row = {
    ...
    injectHistory: getSrlInjectHistory(match.id, { limit: 20 }),
    injectHistoryCount: (session.injectHistory || []).length,
    integrityHold: session.integrityHold || null,
    scoreDrift: null,
    deskAlerts: [],
  };
  row.scoreDrift = computeScoreDriftForMatch(match, session);
  row.deskAlerts = buildSrlMatchAlerts(row, session);
  return row;
}
...
export async function enrichSnapshotWithStakes(payload) {
  ...
  const matches = applySrlStakeRows(snap.matches, rows).map((m) => {
    ...
    const withAlerts = {
      ...m,
      deskAlerts: buildSrlMatchAlerts(m, session),
    };
    maybeAutoPauseOnLiability(withAlerts, session);
    return {
      ...withAlerts,
      deskAlerts: buildSrlMatchAlerts(withAlerts, getSrlOperatorSession(m.matchId)),
    };
  });
  const next = { ...snap, matches, reasonCodes: SRL_REASON_CODES };
  if (payload?.matches) return next;
  return { ...payload, snapshot: next };
}

function maybeAutoPauseOnLiability(match, session) {
  ...
  if (!(liability >= threshold && threshold > 0)) return;
  ...
  try {
    ...
    pushSrlDeskAlert(match.matchId, {
      level: 'danger',
      code: 'AUTO_PAUSE',
      message: `Auto-paused: liability ₹${Math.round(liability).toLocaleString('en-IN')}`,
    });
...
export function declareIPLSRLWinner(matchId, teamId, admin, role = 'SUPER_ADMIN') {
  assertSrlDangerousAction('declare', admin, role);
  if (!isUserFacingSrlMatch(matchId)) {
...
export function clearIPLSRLScoreAnchors(matchId, admin = 'admin', role = 'SUPER_ADMIN') {
  assertSrlDangerousAction('clear_anchors', admin, role);
...
export async function declareIPLSRLMarketOutcome(matchId, {
  ...
  if (voidMarket) assertSrlDangerousAction('void_market', admin, role);
  if (!isUserFacingSrlMatch(matchId)) {
...
export function toggleIPLSRLEmergencyKillSwitch(matchId, active, admin = 'admin', role = 'SUPER_ADMIN') {
  if (!matchId) throw new Error('matchId required');
  if (active) assertSrlDangerousAction('kill_switch', admin, role);
...
export function engageIPLSRLIntegrityHold(matchId, { note, release = false } = {}, admin = 'admin', role = 'SUPER_ADMIN') {
  assertSrlDangerousAction('integrity_hold', admin, role);
...
export async function runIPLSRLSettlementWizard(matchId, {
  ...
  assertSrlDangerousAction('settlement_wizard', admin, role);
  ...
  if (settleToss) {
    const tossRes = await bulkSettleIPLSRLMarkets(matchId, 'toss', admin);
    steps.push({ id: 'toss_settled', ok: true, markets: tossRes.markets || tossRes.settled || [] });
  }
  if (declareTeamId) {
    declareIPLSRLWinner(matchId, declareTeamId, admin, role);
    steps.push({ id: 'winner_declared', ok: true, teamId: declareTeamId });
  }
  if (settleMatchMarkets) {
    try {
      const matchRes = await bulkSettleIPLSRLMarkets(matchId, 'innings1', admin);
      steps.push({ id: 'match_markets_settled', ok: true, markets: matchRes.markets || matchRes.settled || [] });
    } catch (err) {
  ...
  pushAudit('Settlement Wizard', `${matchId} → ${steps.map((s) => s.id).join(' → ')}`, { admin });
  return {
    success: true,
    matchId,
    steps,
...
export { PLAYER_ROLES, BALL_OUTCOMES, OVER_BLUEPRINT_PRESETS, DIRECTOR_MODES, PLAYER_BUFF_TYPES, PITCH_WEAR_TYPES, SRL_REASON_CODES };
