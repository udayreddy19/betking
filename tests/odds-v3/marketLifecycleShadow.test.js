...
describe('OddsEngineV3 — Phase 7 Market Lifecycle Shadow Tests', () => {
  ...
  it('marks match winner as SETTLED when target is reached (no bettable lock odds)', () => {
    ...
    const loserSel = winnerMarket.selections.find(s => s.name === 'Oval Invincibles');
    expect(winnerSel.won).toBe(true);
    expect(winnerSel.bettable).toBe(false);
    expect(winnerSel.odds).toBeNull();
    expect(loserSel.won).toBe(false);
    expect(loserSel.odds).toBeNull();
    expect(Number.isFinite(loserSel.odds) || loserSel.odds === null).toBe(true);
  });
  ...
  it('marks match winner as SETTLED when balls are exhausted', () => {
    ...
    expect(ovalSel.won).toBe(true);
    expect(ovalSel.bettable).toBe(false);
    expect(ovalSel.odds).toBeNull();
  });
});
