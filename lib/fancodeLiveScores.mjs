...
const FANCODE_HEADERS = {
  'User-Agent': 'Mozilla/5.0 (compatible; OddsYra/1.0; +https://oddsyra.app)',
...
function parseCricketSquads(squads = []) {
  ...
  const firstCompleted = String(innings1.status || '').toUpperCase() === 'COMPLETED'
    || Number(innings1.wickets) >= 10;
  const secondInPlay = String(innings2.status || '').toUpperCase() === 'IN_PROGRESS'
    || (!!batting && batting === team2)
    || (firstCompleted && (
      Number(innings2.runs) > 0
      || Number(innings2.wickets) > 0
      || (innings2.overs != null && String(innings2.overs) !== '0' && String(innings2.overs) !== '0.0')
    ));
  const liveDetails = {
    ...
    firstRuns: innings1.runs ?? 0,
    firstWickets: innings1.wickets ?? 0,
    firstOvers: String(innings1.overs ?? '0.0'),
    firstTeamName: team1.name,
  };
  if (secondInPlay || firstCompleted) {
    liveDetails.inningsId = 2;
    liveDetails.chaseRuns = innings2.runs ?? 0;
    liveDetails.chaseWickets = innings2.wickets ?? 0;
    liveDetails.chaseOvers = String(innings2.overs ?? '0.0');
    liveDetails.chaseTeamName = team2.name;
    liveDetails.runs = innings2.runs ?? 0;
    liveDetails.wickets = innings2.wickets ?? 0;
    liveDetails.overs = String(innings2.overs ?? '0.0');
  } else {
...
export async function fetchFanCodeLiveScores() {
  // Phase 1: Fetch home page + static tour pages
  const staticUrls = [...CRICKET_TOUR_URLS, ...SOCCER_TOUR_URLS];
  const homeHtml = await fetchHtml(FANCODE_HOME).catch(() => null);
  const homeState = homeHtml ? extractInitState(homeHtml) : null;
  ...
  if (homeState) {
    pagesFetched += 1;
    parsedMatches.push(...extractLiveNowMatches(homeState), ...extractTourMatches(homeState));
  }

  // Phase 2: Discover tour URLs from LiveNowSegment edges
  const discoveredTourUrls = new Set();
  if (homeState) {
    const key = Object.keys(homeState).find(
      (k) => k.includes('LiveNowSegment') && homeState[k]?.liveNowSegment,
    );
    if (key) {
      const edges = homeState[key].liveNowSegment.edges || [];
      for (const edge of edges) {
        const tourSlug = edge.tour?.collectionSlug;
        const tourId = edge.tour?.collectionId;
        const sport = edge.match?.sport?.slug || '';
        if (tourSlug && tourId) {
          const sportPath = sport === 'football' ? 'football' : 'cricket';
          const tourUrl = `${FANCODE_HOME}/${sportPath}/tour/${tourSlug}-${tourId}/matches`;
          if (!staticUrls.includes(tourUrl)) {
            discoveredTourUrls.add(tourUrl);
          }
        }
      }
    }
  ...
  // Phase 3: Fetch all tour pages (static + discovered)
  const allTourUrls = [...new Set([...staticUrls, ...discoveredTourUrls])];
  const tourResults = await Promise.allSettled(allTourUrls.map((url) => fetchHtml(url)));
