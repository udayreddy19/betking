...
async function countCapturedDeposits(client, userId) {
  const res = await client.query(
    `SELECT COUNT(*)::int AS n FROM deposits
     WHERE user_id = $1 AND UPPER(COALESCE(status, '')) IN ('PAID','CAPTURED','SUCCESS','COMPLETED')`,
    [userId],
  );
...
/**
 * Dispatch assigned-user offer emails as account mail (no-reply@), not promos@.
...
export async function dispatchTargetedDepositFreebetEmails({
  ...
  for (const row of users.rows) {
    ...
    const matchPct = Number(promo.match_percent);
    // Account-style subject — avoid "promotion/offer/free bet" blast wording that Gmail tabs.
    const baseSubject = promo.email_subject
      || (Number.isFinite(matchPct)
        ? `Your ${matchPct}% deposit match is ready`
        : 'Your OddsYra deposit match is ready');
    // Unique per dispatch so Gmail does not collapse re-sends; keep tone transactional.
    const subject = `${baseSubject} · ref ${String(promo.code || promotionId).slice(0, 12)}`;
    try {
      result = await sendTargetedDepositOfferEmail({
        ...
        splitParts: promo.split_parts,
        splitEach: promo.split_each,
      });
    } catch (err) {
    ...
    if (result.success) {
      sent += 1;
      try {
        const { notifyUserPromoOffer } = await import('./promoUserNotify.mjs');
        await notifyUserPromoOffer({
          userId: row.user_id,
          subject: baseSubject,
          message: promo.email_body || promo.name || 'Your deposit match is ready on OddsYra.',
          url: '/wallet',
          eventId: `promo_email_${promotionId}_${row.user_id}_${sendStamp}`,
        });
      } catch {
