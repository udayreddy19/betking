...
export async function assertPanAvailable(pan, excludeUserId, exec) {
  ...
  if (!value) return;
  const { generateBlindIndex } = await import('./kycEncryption.mjs');
  const hash = generateBlindIndex(value);
  const run = runner(exec);
  const res = await run(
    `SELECT user_id FROM kyc_cases
     WHERE ((pan_hash IS NOT NULL AND pan_hash = $1) OR (pan_number IS NOT NULL AND upper(btrim(pan_number)) = $2))
       AND status IN ${ACTIVE_KYC_STATUSES}
       AND ($3::text IS NULL OR user_id <> $3)
     LIMIT 1`,
    [hash, value, excludeUserId],
  );
...
export async function assertAadhaarAvailable(aadhaar, excludeUserId, exec) {
  ...
  const res = await run(
    `SELECT user_id FROM kyc_cases
     WHERE ((aadhaar_hash IS NOT NULL AND aadhaar_hash = $1) OR (aadhaar_number IS NOT NULL AND regexp_replace(aadhaar_number, '[^0-9]', '', 'g') = $2))
       AND status IN ${ACTIVE_KYC_STATUSES}
       AND ($3::text IS NULL OR user_id <> $3)
     LIMIT 1`,
    [hash, value, excludeUserId],
