/**
 * Indian Premier League SRL — continuous schedule, odds, and live T20 simulation.
 */
...
const MATCH_GAP_MS = 30_000;
const MS_PER_BALL = 2500;
const INNINGS_BREAK_MS = 35_000;
const MAX_BALLS = 120;
const VISIBLE_MATCHES = 8;

const TEAMS = {
  csk: { key: 'csk', name: 'Chennai Super Kings SRL', shortName: 'CSK', rating: 82 },
  dc: { key: 'dc', name: 'Delhi Capitals SRL', shortName: 'DC', rating: 78 },
  gt: { key: 'gt', name: 'Gujarat Titans SRL', shortName: 'GT', rating: 85 },
  kkr: { key: 'kkr', name: 'Kolkata Knight Riders SRL', shortName: 'KKR', rating: 83 },
  lsg: { key: 'lsg', name: 'Lucknow Super Giants SRL', shortName: 'LSG', rating: 80 },
  mi: { key: 'mi', name: 'Mumbai Indians SRL', shortName: 'MI', rating: 84 },
  pbks: { key: 'pbks', name: 'Punjab Kings SRL', shortName: 'PBKS', rating: 76 },
  rcb: { key: 'rcb', name: 'Royal Challengers Bengaluru SRL', shortName: 'RCB', rating: 81 },
  rr: { key: 'rr', name: 'Rajasthan Royals SRL', shortName: 'RR', rating: 79 },
  srh: { key: 'srh', name: 'Sunrisers Hyderabad SRL', shortName: 'SRH', rating: 77 },
};
...
function simulateInnings(rng, battingRating, rosterNames, target = null) {
  ...
  let nextBatterIdx = 2;
  const bowlerName = rosterNames[rosterNames.length - 1] || 'Bowler 1';
  const bowler = { name: bowlerName, overs: 0, balls: 0, runs: 0, wickets: 0 };
  const ballDisplays = [];
  ...
  for (let ball = 0; ball < MAX_BALLS && wickets < 10; ball += 1) {
    ...
    timeline.push({
      ...
      bowler: { ...bowler, overs: ballsToOvers(bowler.balls) },
    });
  }
...
function simulateFullMatch(seed, team1Key, team2Key) {
  ...
  const firstDuration = first.ballCount * MS_PER_BALL;
  const secondDuration = second.ballCount * MS_PER_BALL;
  const totalDuration = firstDuration + INNINGS_BREAK_MS + secondDuration;
  ...
  return { first, second, target, totalDuration, winner, team1Key, team2Key };
}
...
function findAnchorGlobalSlot(now) {
  const { globalSlot } = resolveSchedulePosition(now);
  const current = buildMatchFromSchedule(globalSlot, now);
  if (current.matchState === 'post') return globalSlot + 1;
  return globalSlot;
}
...
function stateAtElapsed(sim, elapsedMs, team1, team2) {
  const { first, second, target } = sim;
  const firstEnd = first.ballCount * MS_PER_BALL;
  const breakEnd = firstEnd + INNINGS_BREAK_MS;

  if (elapsedMs < firstEnd) {
    const ballIdx = Math.min(first.ballCount - 1, Math.floor(elapsedMs / MS_PER_BALL));
    ...
    return {
      ...
      wickets2: 0,
      overs2: '0.0',
      commentary: `${team1.name} batting`,
      batter1: { name: striker.name, runs: striker.runs, balls: striker.balls, fours: striker.fours, sixes: striker.sixes, isStriker: true },
      batter2: { name: nonStriker.name, runs: nonStriker.runs, balls: nonStriker.balls, fours: nonStriker.fours, sixes: nonStriker.sixes, isStriker: false },
      bowler: { name: bowler.name, runs: bowler.runs, wickets: sim.first.wickets, overs: bowler.overs },
      currentOverBalls: frame.ballDisplays.slice(-6),
      scorecardInnings: [buildScorecardInnings(team1.name, { ...first, batters: frame.batters, runs: frame.runs, wickets: frame.wickets, overs: frame.overs }, 1)],
      overHistory: buildOverHistory(first.timeline.slice(0, ballIdx + 1)),
    };
  }

  if (elapsedMs < breakEnd) {
    return {
      phase: 'break',
      inningsId: 1,
      matchState: 'in',
  ...
  const chaseElapsed = elapsedMs - breakEnd;
  const ballIdx = Math.min(second.ballCount - 1, Math.floor(chaseElapsed / MS_PER_BALL));
  ...
  return {
    ...
    commentary: `${team2.name} need ${runsNeeded} runs in ${ballsLeft} balls`,
    batter1: { name: striker.name, runs: striker.runs, balls: striker.balls, fours: striker.fours, sixes: striker.sixes, isStriker: true },
    batter2: { name: nonStriker.name, runs: nonStriker.runs, balls: nonStriker.balls, fours: nonStriker.fours, sixes: nonStriker.sixes, isStriker: false },
    bowler: { name: bowler.name, runs: bowler.runs, wickets: frame.wickets, overs: bowler.overs },
    currentOverBalls: frame.ballDisplays.slice(-6),
    scorecardInnings: [
      buildScorecardInnings(team1.name, first, 1),
      buildScorecardInnings(team2.name, { ...second, batters: frame.batters, runs: frame.runs, wickets: frame.wickets, overs: frame.overs }, 2),
    ],
...
function buildMatchFromSchedule(globalSlot, now) {
  ...
  if (elapsed >= sim.totalDuration) {
    const winnerTeam = sim.winner === template.team1Key ? team1.name : team2.name;
    return {
      ...base,
      time: 'Completed',
      isLive: false,
      matchState: 'post',
      liveDetails: {
        commentary: `${winnerTeam} won`,
        firstTeamName: team1.name,
        chaseTeamName: team2.name,
        firstRuns: sim.first.runs,
        firstWickets: sim.first.wickets,
        firstOvers: sim.first.overs,
        chaseRuns: sim.second.runs,
        chaseWickets: sim.second.wickets,
        chaseOvers: sim.second.overs,
        runs: sim.first.runs,
        wickets: sim.first.wickets,
        overs: sim.first.overs,
        score2: sim.second.runs,
        wickets2: sim.second.wickets,
        overs2: sim.second.overs,
      },
      scorecardInnings: [
        buildScorecardInnings(team1.name, sim.first, 1),
        buildScorecardInnings(team2.name, sim.second, 2),
      ],
    };
  }
