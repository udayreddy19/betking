 ...
 * or the innings finishing earlier (all out / chase complete) before over N.
 */
export function isMilestoneBoundaryReached(match, innings, targetOver) {
  ...
  if (isOverBoundaryComplete(oversStr, targetOver, match)) return true;
  // Early innings end before over N: treat as boundary for settlement at final score
  if (innings != null && getCurrentInningsNumber(match) > innings) return true;
  return false;
}
