...
export async function buildAdminNavAttention() {
  const [
    ...
    // Help → SLA: open tickets past sla_due_at
    count(
      `SELECT COUNT(*)::int AS c FROM support_conversations
  ...
  // Hub tabs: Withdrawals (cash-money) + Approvals (pending-approvals)
  ...
  for (const [key, val] of Object.entries(subModules)) {
    ...
    // Avoid double-counting hub aliases that share the same underlying queue
    if (key === 'communications:dlq-retry') continue;
