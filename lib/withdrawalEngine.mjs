...
import { evaluateWithdrawalRisk, assertApprovalAllowedByRisk, requiresWithdrawalDualControl, assertWithdrawalHardBlocks } from './withdrawalRiskEngine.mjs';
import { toPaise, fromPaise } from './money.mjs';
...
export class WithdrawalEngine {
  ...
  async requestWithdrawal({ userId, amount, bankDetails = {} }, correlationId = null) {
    ...
    // UPI penny-drop / simulated name enquiry before payout hold
    const method = String(normalizedBankDetails.method || bankDetails.method || '').toUpperCase();
    const vpa = normalizedBankDetails.upiId || normalizedBankDetails.vpa || bankDetails.upiId || bankDetails.vpa;
    if ((method === 'UPI' || vpa) && vpa) {
      const { verifyUpiBeneficiary } = await import('./upiBeneficiaryVerify.mjs');
      const verified = await verifyUpiBeneficiary({
        userId,
        vpa,
        declaredName: normalizedBankDetails.accountHolderName || bankDetails.accountHolderName || null,
      });
      if (!verified.ok) {
        const err = new Error(`UPI_VERIFICATION_FAILED: ${verified.reason || 'Could not verify UPI ID'}`);
        err.code = 'UPI_VERIFICATION_FAILED';
        err.status = 400;
        throw err;
      }
      normalizedBankDetails.upiVerified = true;
      normalizedBankDetails.upiVerificationStatus = verified.status;
      if (verified.beneficiaryName) {
        normalizedBankDetails.verifiedBeneficiaryName = verified.beneficiaryName;
      }
    }
    ...
    const numericAmount = fromPaise(toPaise(amount));
    if (isNaN(Number(amount)) || !Number.isFinite(Number(amount)) || !(toPaise(numericAmount) > 0)) {
    ...
    if (Array.isArray(risk.hardBlocks) && risk.hardBlocks.length > 0) {
      const block = risk.hardBlocks[0];
      throw Object.assign(new Error(block.message || 'Withdrawal blocked by risk policy'), {
        code: block.code || 'WITHDRAWAL_HARD_BLOCK',
        status: 400,
    ...
    // Defense in depth — re-check deposits / open bets at request time
    await assertWithdrawalHardBlocks({ userId });
    ...
    const result = await withTransaction(async (client) => {
      const walletRes = await client.query(
        `SELECT wallet_id, balance, COALESCE(reserved_balance, 0.00) as reserved_balance,
                ...
                COALESCE(locked_bonus_winnings, 0.00) as locked_bonus_winnings,
      ...
      const currentBalance = fromPaise(toPaise(wallet.balance));
      const reservedBalance = fromPaise(toPaise(wallet.reserved_balance));
      const bonusBalance = fromPaise(toPaise(wallet.bonus_balance));
      const lockedBonusWinnings = fromPaise(toPaise(wallet.locked_bonus_winnings || 0));
      const lockedDeposit = fromPaise(toPaise(wallet.locked_deposit_balance || 0));
      const availableBalance = fromPaise(Math.max(0, toPaise(currentBalance) - toPaise(lockedDeposit)));

      if (toPaise(availableBalance) < toPaise(numericAmount)) {
        throw new Error(`INSUFFICIENT_FUNDS: Withdrawable balance ₹${availableBalance} is less than requested withdrawal ₹${numericAmount}`);
      }

      const newReserved = fromPaise(toPaise(reservedBalance) + toPaise(numericAmount));
      const newBalance = fromPaise(toPaise(currentBalance) - toPaise(numericAmount));

      // Check active bonuses or unreleased bonus funds
      const activeBonusesRes = await client.query(
        `SELECT id, bonus_amount, COALESCE(locked_winnings, 0) AS locked_winnings
         FROM user_bonuses
         WHERE user_id = $1 AND status IN ('ACTIVE', 'COMPLETED')
         FOR UPDATE`,
        [userId]
      );
      const hasActiveBonuses = activeBonusesRes.rows.length > 0;
      const shouldForfeitBonus = bonusBalance > 0 || lockedBonusWinnings > 0 || hasActiveBonuses;

      if (shouldForfeitBonus) {
        // 1. Forfeit both bonus principal AND all locked bonus winnings
        await client.query(
          `UPDATE wallets
        ...
        // 2. Mark active user_bonuses as FORFEITED and zero out locked_winnings
        await client.query(
          `UPDATE user_bonuses
        ...
        // 3. Cancel any open accepted bonus bets
        await client.query(
          `UPDATE bets
           SET status = 'CANCELLED',
               settled_at = NOW(),
               settlement_reason = 'FORFEITED_ON_WITHDRAWAL'
           WHERE user_id = $1 AND fund_source = 'bonus' AND status = 'ACCEPTED'`,
          [userId]
        );

        // 4. Audit ledger entry for bonus principal forfeit
        // balance_after = post-hold cash (newBalance); bonus buckets are separate from cash.
        if (bonusBalance > 0) {
          const forfeitTxId = `tx_forfeit_bn_${withdrawalId}`;
          await client.query(
          ...
          await client.query(
            `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
             VALUES ($1, $2, 'DEBIT', $3, $4, 'Bonus principal forfeited on withdrawal', NOW())`,
            [wallet.wallet_id, forfeitTxId, bonusBalance, newBalance]
          );
        }

        // 5. Audit ledger entry for locked bonus winnings forfeit
        if (lockedBonusWinnings > 0) {
          const forfeitWinTxId = `tx_forfeit_win_${withdrawalId}`;
          await client.query(
            `INSERT INTO transactions (transaction_id, user_id, type, amount, status, created_at)
             VALUES ($1, $2, 'BONUS_WINNINGS_FORFEIT', $3, 'SUCCESS', NOW())
             ON CONFLICT (transaction_id) DO NOTHING`,
            [forfeitWinTxId, userId, lockedBonusWinnings]
          );
          await client.query(
            `INSERT INTO ledger_entries (wallet_id, transaction_id, type, amount, balance_after, description, created_at)
             VALUES ($1, $2, 'DEBIT', $3, $4, 'Bonus locked winnings forfeited on withdrawal', NOW())`,
            [wallet.wallet_id, forfeitWinTxId, lockedBonusWinnings, newBalance]
          );
        }
      } else {
        await client.query(
      ...
      return {
        ...
        forfeitedLockedWinnings: lockedBonusWinnings,
        risk,
      };
    });
  ...
  async reviewWithdrawal({
    ...
    if (dec === 'APPROVE') {
      ...
      await withTransaction(async (client) => {
        ...
        // Hard blocks cannot be force-approved (no-deposit / open bets)
        await assertWithdrawalHardBlocks({ userId: withdrawal.user_id, exec: run });
        ...
        const amount = fromPaise(toPaise(withdrawal.amount));
        ...
        const wallet = walletRes.rows[0];
        const currentReserved = fromPaise(toPaise(wallet.reserved_balance));
        const newReserved = fromPaise(Math.max(0, toPaise(currentReserved) - toPaise(amount)));

        if (toPaise(currentReserved) < toPaise(amount)) {
      ...
      if (outcome?.status === 'APPROVED' && !outcome.idempotent) {
        try {
          ...
          const row = wr.rows[0];
          if (row) {
            const { emailUserPaymentEvent, emailOpsWithdrawalAlert } = await import('./supportNotify.mjs');
            ...
            void emailOpsWithdrawalAlert({
              userId: row.user_id,
              amount: parseFloat(row.amount),
              status: 'APPROVED',
              withdrawalId,
              utr: outcome.payoutRef || null,
              payoutRef: outcome.payoutRef || null,
            });
          }
        } catch (err) {
    ...
    await withTransaction(async (client) => {
      ...
      const newReserved = fromPaise(Math.max(0, toPaise(currentReserved) - toPaise(amount)));
      const newBalance = fromPaise(toPaise(currentBalance) + toPaise(amount));
  ...
  async holdWithdrawal({ withdrawalId, adminId, reason = '' }) {
    ...
    try {
      ...
      const row = wr.rows[0];
      if (row) {
        const { emailOpsWithdrawalAlert } = await import('./supportNotify.mjs');
        void emailOpsWithdrawalAlert({
          userId: row.user_id,
          amount: parseFloat(row.amount),
          status: 'HOLD',
      ...
      console.error('[withdrawalEngine] hold ops email', err.message);
    }
    return {
      success: true,
      withdrawalId,
