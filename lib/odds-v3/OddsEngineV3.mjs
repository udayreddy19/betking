...
import { auditSnapshotQuality } from './monitoring/oddsQualityMonitor.mjs';
...
export function generate(matchState, config = {}) {
  ...
  if (!isCricketSport(matchState?.sport)) {
    return generateOtherSportsSnapshot(matchState, {
      ...config,
      allowModelOnly: config.allowModelOnly ?? false,
    });
  }
  ...
  if (config.winnerOnly) {
    ...
    const protectedMarkets = applyBookIntegrity(winner?.marketId ? [winner] : [], matchState);
    return createOddsSnapshot({
      matchId: matchState.matchId,
      stateVersion: matchState.stateVersion,
  ...
  const validMarkets = rawMarkets.filter((m) => {
    ...
    if (!compat.compatible) {
      if (debug) console.warn(`[OddsEngineV3] Suppressed orphan market ${m.marketId}:`, compat.reason);
      return false;
    }
    ...
    const totalsOverround = (marginConfig.liveTeamTotalOverround ?? 0.10)
      + (marginConfig.liveTotalsOverExtraOverround ?? 0);
    const effectiveOverround = Number.isFinite(m.overround)
      ? m.overround
      : (totalsBook ? totalsOverround : (marginConfig.liveMatchWinnerOverround || 0.05));
  ...
  const snapshot = createOddsSnapshot({
    matchId: matchState.matchId,
    stateVersion: matchState.stateVersion,
  ...
  // Phase 13: Odds Quality Anomaly Audit
  const qualityAudit = auditSnapshotQuality(snapshot);
  if (!qualityAudit.healthy && debug) {
    console.warn('[OddsEngineV3] Quality anomalies detected:', qualityAudit.anomalies);
  }
