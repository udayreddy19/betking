import {
  generateAdminToken,
  generateAdminMfaPendingToken,
  verifyAdminToken,
  ADMIN_ROLES,
} from '../server/middleware/adminAuth.js';
import { isAdminEligibleUser, adminJwtRoleForUser } from './adminAccess.mjs';
import {
  confirmAdminMfaEnrollment,
  getAdminMfaRow,
  isAdminMfaEnforced,
  startAdminMfaEnrollment,
  verifyAdminMfaCode,
} from './adminMfa.mjs';

export function isProductionRuntime() {
  return process.env.NODE_ENV === 'production';
}

/** Passwordless admin bootstrap is never allowed in production, even if ADMIN_DEV_LOGIN=1. */
export function isAdminDevBootstrapAllowed() {
  if (isProductionRuntime()) return false;
  return process.env.ADMIN_DEV_LOGIN === '1';
}

export function issueAdminSessionJson(adminId, role, opts = {}) {
  return {
    success: true,
    token: generateAdminToken(adminId, role, 'oddsyra_in', { sessionId: opts.sessionId }),
    role,
    adminId,
    sessionId: opts.sessionId || null,
    expiresInHours: 8,
  };
}

async function mfaGate(account, role) {
  const userId = account.userId || account.user_id;
  const email = account.email;
  const enforced = isAdminMfaEnforced();

  // Authenticator disabled — password + admin eligibility only.
  if (!enforced) {
    return issueAdminSessionJson(userId, role);
  }

  const row = await getAdminMfaRow(userId);

  if (row?.enabled) {
    return {
      success: false,
      code: 'MFA_REQUIRED',
      error: 'Enter the authenticator code for this admin account.',
      mfaToken: generateAdminMfaPendingToken(userId, role),
    };
  }

  const enroll = await startAdminMfaEnrollment(userId, email || userId);
  return {
    success: false,
    code: 'MFA_SETUP_REQUIRED',
    error: 'Set up authenticator app (TOTP) before opening admin.',
    mfaToken: generateAdminMfaPendingToken(userId, role),
    secret: enroll.secret,
    otpauthUrl: enroll.otpauthUrl,
  };
}

export async function completeAdminPasswordLogin(account) {
  if (!isAdminEligibleUser(account)) {
    const err = new Error('This account is not allowed to open the admin console.');
    err.code = 'ADMIN_NOT_ALLOWED';
    err.status = 403;
    throw err;
  }
  const role = adminJwtRoleForUser(account);
  return mfaGate(account, role);
}

export async function completeAdminBearerUpgrade(user) {
  const account = {
    userId: user.user_id || user.userId,
    email: user.email,
    role: user.role,
  };
  return completeAdminPasswordLogin(account);
}

export async function completeAdminMfa(mfaToken, code, { enroll = false } = {}) {
  const decoded = verifyAdminToken(mfaToken);
  if (!decoded || decoded.type !== 'admin_mfa_pending' || !decoded.sub) {
    const err = new Error('MFA session expired. Sign in again.');
    err.code = 'MFA_EXPIRED';
    err.status = 401;
    throw err;
  }

  const {
    isAdminMfaPendingConsumed,
    tryConsumeAdminMfaPendingToken,
  } = await import('./adminMfaPendingOnce.mjs');
  try {
    if (await isAdminMfaPendingConsumed(mfaToken)) {
      await auditMfaEvent({
        actorId: decoded.sub,
        action: 'ADMIN_MFA_REPLAY_DENIED',
        details: { enroll, code: 'MFA_REPLAY' },
        riskLevel: 'HIGH',
      });
      const err = new Error('MFA challenge already used. Sign in again.');
      err.code = 'MFA_REPLAY';
      err.status = 401;
      throw err;
    }
  } catch (err) {
    if (err.code === 'MFA_INFRA_UNAVAILABLE') {
      await auditMfaEvent({
        actorId: decoded.sub,
        action: 'ADMIN_MFA_INFRA_UNAVAILABLE',
        details: { enroll: Boolean(enroll), phase: 'precheck' },
        riskLevel: 'HIGH',
      });
      throw err;
    }
    throw err;
  }

  const userId = decoded.sub;
  const role = Object.values(ADMIN_ROLES).includes(decoded.role)
    ? decoded.role
    : ADMIN_ROLES.SUPER_ADMIN;

  try {
    if (enroll) {
      await confirmAdminMfaEnrollment(userId, code);
    } else {
      await verifyAdminMfaCode(userId, code);
    }
  } catch (err) {
    await auditMfaEvent({
      actorId: userId,
      action: enroll ? 'ADMIN_MFA_ENROLL_FAILED' : 'ADMIN_MFA_VERIFY_FAILED',
      details: { code: err.code || 'MFA_INVALID', enroll: Boolean(enroll) },
      riskLevel: 'MEDIUM',
    });
    throw err;
  }

  let consumed;
  try {
    consumed = await tryConsumeAdminMfaPendingToken(mfaToken);
  } catch (err) {
    if (err.code === 'MFA_INFRA_UNAVAILABLE') {
      await auditMfaEvent({
        actorId: userId,
        action: 'ADMIN_MFA_INFRA_UNAVAILABLE',
        details: { enroll: Boolean(enroll) },
        riskLevel: 'HIGH',
      });
      throw err;
    }
    throw err;
  }
  if (!consumed) {
    await auditMfaEvent({
      actorId: userId,
      action: 'ADMIN_MFA_REPLAY_DENIED',
      details: { enroll, code: 'MFA_REPLAY', race: true },
      riskLevel: 'HIGH',
    });
    const err = new Error('MFA challenge already used. Sign in again.');
    err.code = 'MFA_REPLAY';
    err.status = 401;
    throw err;
  }

  await auditMfaEvent({
    actorId: userId,
    action: enroll ? 'ADMIN_MFA_ENROLLED' : 'ADMIN_MFA_VERIFIED',
    details: { role, enroll: Boolean(enroll) },
    riskLevel: 'LOW',
  });
  return issueAdminSessionJson(userId, role);
}

async function auditMfaEvent({ actorId, action, details, riskLevel }) {
  try {
    const { logAdminAction } = await import('../server/middleware/auditLogger.js');
    await logAdminAction({
      actorId: String(actorId || 'unknown'),
      targetId: String(actorId || 'unknown'),
      action,
      details,
      riskLevel,
    });
  } catch {
    /* never block auth on audit write */
  }
}
