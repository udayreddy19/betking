...
function rowToSession(row) {
  ...
  return {
    ...
    marginDefense: meta.marginDefense || { marginBump: 0, spreadBias: 0, autoFreezeThreshold: 50000 },
...
function emptySession(matchId) {
  return {
    ...
    marginDefense: { marginBump: 0, spreadBias: 0, autoFreezeThreshold: 50000 },
    targetScore: null,
  };
}
  ...
  void (async () => {
    ...
    try {
      ...
      session.marketControls._meta = {
        ...
        marginDefense: session.marginDefense || { marginBump: 0, spreadBias: 0, autoFreezeThreshold: 50000 },
...
export function setSrlMarginDefense(matchId, { marginBump = 0, spreadBias = 0, autoFreezeThreshold = 50000 } = {}) {
  const s = getSrlOperatorSession(matchId);
  s.marginDefense = {
    marginBump: Math.max(0, Math.min(0.20, Number(marginBump) || 0)),
    spreadBias: Math.max(-0.20, Math.min(0.20, Number(spreadBias) || 0)),
    autoFreezeThreshold: Math.max(1000, Number(autoFreezeThreshold) || 50000),
    updatedAt: Date.now(),
  };
  queuePersist(s);
