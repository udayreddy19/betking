...
export function walletViewFromRow(wallet) {
  return {
    ...
    lockedBonusWinnings: Number(wallet.locked_bonus_winnings || 0),
...
export function splitSettlementWinCredits(bet, payout) {
  ...
  // If Free Bet: profit only is credited to cash (freebet stake is not returned)
  ...
  return splitBetWinPayout({
    ...
    stake,
    fundSource,
    cashStake: fundSource === 'cash' ? stake : (Number(bet.cash_stake || bet.cashStake) || 0),
    bonusStake: fundSource === 'bonus' ? stake : (Number(bet.bonus_stake || bet.bonusStake) || 0),
    freebetStake: fundSource === 'freebet' ? stake : (Number(bet.freebet_stake || bet.freebetStake) || 0),
  });
}
...
export function voidRefundCredits(bet) {
  ...
  // Discrete-reward bets still debit the promo bucket (mirrored wallet). Restore the
  // bucket here; settlement separately reactivates user_rewards via reverseRewardForVoidedBet.
  if (bet.reward_id) {
    if (fundSource === 'bonus') {
    ...
    return { balanceCredit: 0, bonusCredit: 0, freebetCredit: 0, lockedCredit: 0, winningsCredit: 0 };
  }
