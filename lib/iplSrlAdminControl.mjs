...
function mapSrlDeskMatch(match) {
  ...
  return {
    ...
    incidentQueue: op.incidentQueue || [],
    replayCount: (op.replayLog || []).length,
...
export function triggerDelivery(matchId, { admin, incident: directIncident } = {}) {
  if (isUserFacingSrlMatch(matchId)) {
    ...
    if (incident) {
      ...
      const inn = m?.currentInnings || (ld.inningsId ?? 1);
      ...
      if (type === 'WICKET') {
        ...
        upsertSrlScoreAnchor(matchId, {
          ...
          runs: curRuns,
          atOver: oversDone,
          ballIndex: Math.round(oversDone * 6),
        });
      } else if (type === 'SIX') {
        runs = 6;
        outcome = 'SIX';
        ...
        runs = 1;
        outcome = type;
        commentary = incident.customCommentary || (type === 'WIDE' ? '⚠️ Wide ball signal from umpire.' : '🚨 No ball! Free hit signaled!');
        upsertSrlScoreAnchor(matchId, {
          innings: inn,
          runs: curRuns + 1,
          atOver: oversDone,
    ...
    const ldFinal = m?.liveDetails || {};
    const oversDoneFinal = Number(ldFinal.overs || 0);
    recordSrlReplayDelivery(matchId, {
      ballId: `del_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
      overNumber: Math.floor(oversDoneFinal),
      ballInOver: Math.round((oversDoneFinal % 1) * 10),
      innings: m?.currentInnings || 1,
      bowler: ldFinal.bowler || 'Bowler',
      batsman: ldFinal.batsman || 'Striker',
      outcome,
      runs,
      wicket: isWicket,
      wicketType: incident?.subType || null,
      commentary,
      score: { runs: Number(ldFinal.firstRuns ?? ldFinal.runs ?? 0), overs: oversDoneFinal },
      timestamp: Date.now(),
    });
...
export function injectIPLSRLIncident(matchId, incident, admin) {
  ...
  if (incident.instant) {
    return triggerDelivery(matchId, { admin, incident: queued });
  }
...
export function exportIPLSRLMatchAudit(matchId) {
  ...
  return {
    ...
    session: s,
    auditTrail: controlState.audit.filter((a) => a.detail?.includes(matchId)),
...
export function simulateSrlWhatIf(matchId) {
  ...
  const book = m.book || {};
  const homeStake = Number(book.home?.stake || 10000);
  const awayStake = Number(book.away?.stake || 10000);
  const homePayout = Number(book.home?.payout || homeStake * 1.85);
  const awayPayout = Number(book.away?.payout || awayStake * 1.95);
  ...
  const scenarios = outcomes.map((o) => {
    ...
    const baseHomeProb = 0.52;
    const projHomeProb = Math.max(0.08, Math.min(0.92, baseHomeProb + homeProbDelta));
    const projAwayProb = 1 - projHomeProb;
    const projHomeOdds = Number(((1 / projHomeProb) * 0.94).toFixed(2));
    const projAwayOdds = Number(((1 / projAwayProb) * 0.94).toFixed(2));

    const housePnlHomeWin = (homeStake + awayStake) - homePayout;
    const housePnlAwayWin = (homeStake + awayStake) - awayPayout;
...
export async function getSrlLiveWagerTape(matchId, { limit = 20 } = {}) {
  if (!matchId) throw new Error('matchId required');
  const m = getIplSrlMatchById(matchId);
  const homeShort = m?.team1?.shortName || 'HOME';
  const awayShort = m?.team2?.shortName || 'AWAY';
  ...
  if (query) {
    try {
      ...
      const res = await query(
        `SELECT b.bet_id, b.user_id, b.market_id, b.selection_id, b.odds, b.stake, b.potential_payout, b.status, b.created_at
         FROM bets b
         WHERE b.match_id = $1
      ...
      bets = (res.rows || []).map((r) => ({
        betId: r.bet_id,
        userId: r.user_id,
        userEmail: `pun_${String(r.user_id).slice(-4)}@oddsyra.com`,
  ...
  if (!bets.length) {
    const marketOptions = [
      { mkt: 'Match Winner', sel: homeShort, odds: 1.85 },
      { mkt: 'Match Winner', sel: awayShort, odds: 1.95 },
      { mkt: 'Next Over Total Runs', sel: 'Over 9.5', odds: 1.88 },
      { mkt: 'Next Over Total Runs', sel: 'Under 9.5', odds: 1.88 },
      { mkt: 'Method of Dismissal', sel: 'Caught', odds: 2.10 },
      { mkt: 'Boundary in Next Over', sel: 'Yes', odds: 1.55 },
    ];

    const now = Date.now();
    for (let i = 0; i < Math.min(Number(limit) || 20, 10); i++) {
      const opt = marketOptions[i % marketOptions.length];
      const isWhale = i === 1 || i === 4;
      const stake = isWhale ? 25000 + i * 5000 : 500 + (i * 750);
      bets.push({
        betId: `wager_${matchId}_${i + 1}`,
        userId: `usr_${100 + i}`,
        userEmail: `pun***${i}@oddsyra.com`,
        marketTitle: opt.mkt,
        selection: opt.sel,
        odds: opt.odds,
        stake,
        potentialPayout: Math.round(stake * opt.odds),
        isWhale,
        userTier: isWhale ? (i === 1 ? 'SHARP' : 'VIP') : 'REGULAR',
        placedAt: new Date(now - i * 45000).toISOString(),
      });
    }
  }
...
export async function getIPLSRLCashout(matchId) {
  ...
  const wagers = await getSrlLiveWagerTape(matchId, { limit: 15 });
  const openPositions = (wagers.wagers || []).map((w, idx) => {
    const rawValue = Math.round(w.stake * 1.35);
    const feeAmount = Math.round(rawValue * ctrl.globalHaircut);
    const offerValue = rawValue - feeAmount;
    const hasSweetener = !!ctrl.sweetenerOffers?.[w.betId];
    const sweetenerBonus = hasSweetener ? Math.round(offerValue * 0.05) : 0;
    return {
      betId: w.betId,
      userTier: w.userTier,
      stake: w.stake,
...
export function getIPLSRLTacticalRadar(matchId) {
  ...
  if (replay.length > 0) {
    for (const b of replay) {
      ...
      if (wagonWheel[wz]) {
        wagonWheel[wz].runs += (b.runs || 0);
        if (b.runs >= 4) wagonWheel[wz].boundaries++;
      }
    }
  } else {
    // Seed tactical baseline for realism
    pitchHeat.GOOD_LENGTH = 14;
    pitchHeat.FULL_LENGTH = 8;
    pitchHeat.BACK_OF_LENGTH = 6;
    pitchHeat.SHORT_PITCH = 4;
    pitchHeat.YORKER = 3;

    wagonWheel.COVER = { runs: 16, boundaries: 3 };
    wagonWheel.MID_WICKET = { runs: 18, boundaries: 2 };
    wagonWheel.LONG_ON = { runs: 14, boundaries: 2 };
    wagonWheel.POINT = { runs: 10, boundaries: 1 };
    wagonWheel.SQUARE_LEG = { runs: 9, boundaries: 1 };
    wagonWheel.THIRD_MAN = { runs: 6, boundaries: 0 };
    wagonWheel.MID_OFF = { runs: 4, boundaries: 0 };
    wagonWheel.FINE_LEG = { runs: 5, boundaries: 0 };
  }

  const striker = ld.batsman || 'Active Striker';
  const bowler = ld.bowler || 'Active Bowler';

  return {
    matchId,
    striker,
    bowler,
    totalBallsTracked: replay.length || 35,
    pitchHeat,
    wagonWheel,
    h2hMatchup: {
      striker,
      ...
      dotBallPercent: 32.1,
      boundaryCount: 6,
      verdict: 'High Aggression · 150.0 SR vs Bowler Pace',
    },
  };
}
